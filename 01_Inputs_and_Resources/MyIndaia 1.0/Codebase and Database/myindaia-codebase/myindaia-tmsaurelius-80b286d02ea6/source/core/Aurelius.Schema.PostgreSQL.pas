...
procedure TPostgreSQLSchemaRetriever.GetDBIndexes;
begin
  RetrieveDBIndexes(
    'select table_name, table_schema, index_name, column_name              ' +
    'from (                                                                ' +
    '    select                                                            ' +
...
begin
  RetrieveForeignKeys(
    'SELECT '+
    '  TC.constraint_name, '+
    '  TC.table_name as FK_TABLE_NAME, TC.table_schema as FK_TABLE_SCHEMA, '+
    '  KP.table_name as PK_TABLE_NAME, KP.table_schema as PK_TABLE_SCHEMA, '+
...
begin
  RetrievePrimaryKeys(
    'SELECT C.CONSTRAINT_NAME, C.TABLE_NAME, C.TABLE_SCHEMA, K.COLUMN_NAME, K.ORDINAL_POSITION '+
    ...
    'ORDER BY C.TABLE_SCHEMA, C.TABLE_NAME, C.CONSTRAINT_NAME, K.ORDINAL_POSITION'
  );
end;
