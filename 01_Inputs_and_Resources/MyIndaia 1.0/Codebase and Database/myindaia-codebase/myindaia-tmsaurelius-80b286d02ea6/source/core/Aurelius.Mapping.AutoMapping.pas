...
begin
  ...
  IsManyValued := Explorer.IsObjectList(RealType.AsInstance.MetaclassType);
...
begin
  ...
  begin
    ...
    Explorer.ResolveColumnType(Col);
  ...
  begin
    ...
    if Explorer.IsObjectList(Metaclass) then
    begin
      ...
      begin
        ...
        Result[I].ReferencedColumnIsId := true;
...
begin
  Result := nil;
  Member := TRttiUtils.GetInstance.FindMember(AClass, 'FID');
  if Member <> nil then
  begin
    Result := TMetaId.Create;
    Result.Columns := Explorer.GetColumnsFromMember(Member);
    Result.Members.Add(Explorer.GetOptimization(Member));
    Result.GeneratorClass := TNoneGenerator;
    if Length(Result.Columns) = 1 then
    begin
      case Result.Columns[0].FieldType of
        ftGuid:
