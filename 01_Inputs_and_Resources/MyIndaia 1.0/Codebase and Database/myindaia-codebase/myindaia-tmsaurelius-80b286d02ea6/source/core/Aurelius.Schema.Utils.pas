...
type
  ...
  public
    ...
    class function FindDBIndex(Table: TTableMetadata; const DBIndexName: string): TDBIndexMetadata;
  end;
...
class function TSchemaUtils.FindDBIndex(Table: TTableMetadata;
  const DBIndexName: string): TDBIndexMetadata;
var
  DBIndex: TDBIndexMetadata;
begin
  for DBIndex in Table.DBIndexes do
    if SameText(DBIndex.Name, DBIndexName) then
      Exit(DBIndex);
  Result := nil;
end;
...
begin
  for Table in Database.Tables do
    if SameText(Table.Name, TableName) and
      (SameText(Table.Schema, TableSchema) or (Table.Schema = '') or (TableSchema = '')) then
      Exit(Table);
  Result := nil;
end;
...
begin
  ...
  try
    for Table in Database.Tables do
      for ForeignKey in Table.ForeignKeys do
      begin
        if SameText(ForeignKey.ToTable.Name, ReferencedTable.Name) and (
          SameText(ForeignKey.ToTable.Schema, ReferencedTable.Schema)
          or (ForeignKey.ToTable.Schema = '')
          or (ReferencedTable.Schema = '')
          ) then
