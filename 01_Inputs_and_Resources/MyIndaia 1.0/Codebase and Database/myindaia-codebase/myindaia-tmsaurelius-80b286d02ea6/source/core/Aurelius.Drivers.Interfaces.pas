...
function TDBParam.ToString: string;
var
  ValueAsString: string;
begin
  if ParamValue = Variants.Null then
    ValueAsString := 'NULL'
  else
  if ParamType = ftDateTime then
    ValueAsString := '"' + DateTimeToStr(ParamValue) + '"'
  else
  if ParamType = ftDate then
    ValueAsString := '"' + DateToStr(ParamValue) + '"'
  else
    ValueAsString := '"' + VarToStr(ParamValue) + '"';

  Result := ParamName + ' = ' + ValueAsString + ' (' +
    GetEnumName(TypeInfo(TFieldType), Ord(ParamType)) + ')';
end;
