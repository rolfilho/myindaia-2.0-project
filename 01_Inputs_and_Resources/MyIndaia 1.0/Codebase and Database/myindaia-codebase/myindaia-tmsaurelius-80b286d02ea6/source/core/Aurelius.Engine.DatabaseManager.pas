...
type
  ...
  strict private
    ...
    FExplorers: TArray<TMappingExplorer>;
...
//    procedure ActionDBIndexChanged(OldDBIndex, NewDBIndex: TDBIndexMetadata);
  ...
  public
    ...
    constructor Create(Connection: IDBConnection; AExplorers: TArray<TMappingExplorer>); overload;
    destructor Destroy; override;
  public
    class procedure Update(Connection: IDBConnection); overload;
    class procedure Update(Connection: IDBConnection; AExplorer: TMappingExplorer); overload;
    ...
    property IgnoreConstraintName: Boolean read FIgnoreConstraintName write FIgnoreConstraintName;
    property AllowDestructiveCommands: Boolean read FAllowDestructiveCommands write FAllowDestructiveCommands;
  end;
...
uses
  ...
  Aurelius.Commands.DBIndexCreator,
...
procedure TDatabaseManager.ActionDBIndexAdded(DBIndex: TDBIndexMetadata);
begin
  FCommands.Add(TCreateDBIndexCommand.Create(DBIndex));
  FActions.Add(TDBIndexCreatedAction.Create(DBIndex));
end;

procedure TDatabaseManager.ActionDBIndexRemoved(DBIndex: TDBIndexMetadata);
begin
  // For now, just ignore if there are existing indexes in the database
  // that are not declared in Aurelius mapping. Otherwise, we would require
  // users to map all existing tables to avoid warnings/errors
  // (MySQL creates indexes for every foreign key, for example. We shouldn't
  // require that from user

//  FCommands.Add(TDropDBIndexCommand.Create(DBIndex));
//  FActions.Add(TDBIndexRemovedAction.Create(DBIndex));
end;
...
var
  C: TClass;
  E: TMappingExplorer;
  Pair: TPair<TClass, TMappingExplorer>;
  TableCreator: TTableCreator;
  FKCreator: TForeignKeyCreator;
  SequenceCreator: TSequenceCreator;
  DBIndexCreator: TDBIndexCreator;
  Classes: TDictionary<TClass, TMappingExplorer>;
begin
  Database.Clear;
  Classes := TDictionary<TClass, TMappingExplorer>.Create;
  for E in FExplorers do
    for C in E.Hierarchy.Classes do
      if not Classes.ContainsKey(C) then
        Classes.Add(C, E);
  ...
  try
    ...
    begin
      C := Pair.Key;
      E := Pair.Value;
      if E.HasTable(C) then
      begin
        TableCreator := CommandFactory.GetCommand<TTableCreator>(C, E);
        try
    ...
    begin
      ...
      if E.HasTable(C) then
      begin
        FKCreator := CommandFactory.GetCommand<TForeignKeyCreator>(C, E);
        try
    ...
    begin
      ...
      if E.HasTable(C) then
      begin
        DBIndexCreator := CommandFactory.GetCommand<TDBIndexCreator>(C, E);
        try
          DBIndexCreator.CreateDBIndexes(Database);
        finally
          DBIndexCreator.Free;
        end;
      end;
    ...
    begin
      C := Pair.Key;
      E := Pair.Value;
      if E.HasSequence(C, False) then
      begin
        SequenceCreator := CommandFactory.GetCommand<TSequenceCreator>(C, E);
        try
...
constructor TDatabaseManager.Create(Connection: IDBConnection;
  AExplorers: TArray<TMappingExplorer>);
begin
  if Length(AExplorers) = 0 then
    raise EOPFInternalError.Create('Cannot create database manager without a least one mapping explorer');
  Create(Connection, AExplorers[0]);
  FExplorers := AExplorers;
end;
...
begin
  ...
  FExplorers := TArray<TMappingExplorer>.Create(Self.Explorer);
...
begin
  SQLPerformer := CommandFactory.GetCommand<TSQLPerformer>(TObject);
  try
...
procedure TDatabaseManager.GetUpdateDatabaseCommands(OldDB, NewDB: TDatabaseMetadata);
  ...
  function AreDBIndexesEqual(OldDBIndex, DBIndex: TDBIndexMetadata): boolean;
  begin
    Result := AreColumnsEqual(OldDBIndex.Columns, DBIndex.Columns);
  end;

  function FindDBIndex(Table: TTableMetadata; DBIndex: TDBIndexMetadata): TDBIndexMetadata;
  begin
    Result := TSchemaUtils.FindDBIndex(Table, DBIndex.Name)
  end;
  ...
  procedure CheckDBIndexes(OldTable, NewTable: TTableMetadata);
  var
    DBIndex: TDBIndexMetadata;
    OldDBIndex: TDBIndexMetadata;
  begin
    // Compare table DBIndexs
    for DBIndex in OldTable.DBIndexes do
      if FindDBIndex(NewTable, DBIndex) = nil then
        ActionDBIndexRemoved(DBIndex);

    for DBIndex in NewTable.DBIndexes do
    begin
      OldDBIndex := FindDBIndex(OldTable, DBIndex);
      if OldDBIndex = nil then
        ActionDBIndexAdded(DBIndex)
      else
      begin
        // Compare existing db indexes
        if not AreDBIndexesEqual(OldDBIndex, DBIndex) then
        begin
          ActionDBIndexRemoved(OldDBIndex);
          ActionDBIndexAdded(DBIndex);
        end;
      end;
    end;
  ...
  function SameTable(T1, T2: TTableMetadata): Boolean;
  begin
    Result := SameText(T1.Name, T2.Name)
      and (SameText(T1.Schema, T2.Schema) or (T1.Schema = '') or (T2.Schema = ''));
  end;

  function AreForeignKeysEqual(OldForeignKey, ForeignKey: TForeignKeyMetadata): boolean;
  begin
    Result :=
      SameTable(OldForeignKey.FromTable, ForeignKey.FromTable)
      and SameTable(OldForeignKey.ToTable, ForeignKey.ToTable)
  ...
  begin
    ...
    CheckDBIndexes(OldTable, NewTable);
  end;
...
class procedure TDatabaseManager.Update(Connection: IDBConnection);
begin
  Update(Connection, TMappingExplorer.Default);
end;
...
begin
  DatabaseManager := TDatabaseManager.Create(Connection, AExplorer);
  try
