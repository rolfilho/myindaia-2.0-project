...
type
  ...
  public
    ...
    function LoadUnsavedValue(RttiType: TRttiType): TValue; override;
    ...
    function LoadDBIndexes(RttiType: TRttiType): TArray<TDBIndex>; override;
    function LoadForeignKeyName(Member: TRttiMember): string; override;
    procedure LoadOrderBy(A: TAssociation); override;
  end;

implementation
uses
  DB, SysUtils, Classes, StrUtils, Types,
  Bcl.Rtti.Utils,
  Aurelius.Id.Guid,
...
begin
  ...
  if not RealType.IsInstance or not FExplorer.IsObjectList(RealType.AsInstance.MetaclassType) then
  ...
  begin
    ...
    begin
      ...
      Col.ReferencedColumnIsId := (I < Length(IdRefCols)) and (RefCol = IdRefCols[I]);
    end;
...
begin
  ...
  try
    ...
    Result.IsLazy := (TColumnProp.Lazy in Col.Properties) and FExplorer.IsBlob(RealType.Handle);
    Result.Length := Col.Length;
    Result.Precision := Col.Precision;
    Result.Scale := Col.Scale;
    FExplorer.ResolveColumnType(Result);
  except
    Result.Free;
    raise;
  end;
...
begin
  ...
  for A in Member.GetAttributes do
    ...
    begin
      ...
      Result.Optimization := FExplorer.GetOptimization(Member);
      if IsManyValued then
...
function TMappingAttributeStrategy.LoadDBIndexes(
  RttiType: TRttiType): TArray<TDBIndex>;
var
  A: TCustomAttribute;
  DBIdx: TDBIndex;
  List: TList<TDBIndex>;
  I: Integer;
begin
  List := TObjectList<TDBIndex>.Create;
  try
    for A in RttiType.GetAttributes do
      if A is DBIndex then
      begin
        DBIdx := TDBIndex.Create;
        List.Add(DBIdx);
        DBIdx.Name := DBIndex(A).Name;
        DBIdx.FieldNames.AddRange(DBIndex(A).Columns);
      end;
...
function TMappingAttributeStrategy.LoadForeignKeyName(Member: TRttiMember): string;
var
  A: TCustomAttribute;
begin
  Result := '';
  for A in Member.GetAttributes do
    if A is ForeignKey then
      Exit(ForeignKey(A).Name);
end;
    ...
    for A in RttiType.GetAttributes do
      ...
      begin
        ...
        begin
          ...
          if Id(A).GeneratorClass <> nil then
            Result.GeneratorClass := Id(A).GeneratorClass
          else
            case Id(A).Generator of
              TIdGenerator.IdentityOrSequence:
                Result.GeneratorClass := TIdentityOrSequenceGenerator;
              TIdGenerator.Guid:
                Result.GeneratorClass := TGuidGenerator;
              TIdGenerator.Uuid38:
                Result.GeneratorClass := TUuid38Generator;
              TIdGenerator.Uuid36:
                Result.GeneratorClass := TUuid36Generator;
              TIdGenerator.Uuid32:
                Result.GeneratorClass := TUuid32Generator;
              TIdGenerator.SmartGuid:
                Result.GeneratorClass := TSmartGuidGenerator;
            else
              //TIdGenerator.None:
              Result.GeneratorClass := TNoneGenerator;
            end;
        end else
          // Composite Id
          Result.GeneratorClass := TNoneGenerator;

        RttiInfo := FExplorer.GetOptimization(TRttiUtils.GetInstance.GetMember(EntityClass, Id(A).MemberName));
        Result.Members.Add(RttiInfo);

        TempCols := FExplorer.GetColumnsFromMember(RttiInfo.MemberRef);
...
procedure TMappingAttributeStrategy.LoadOrderBy(A: TAssociation);
var
  O: TCustomAttribute;
  OrderMembers: TStringDynArray;
  Len, SubLen: integer;
  I, J: integer;
  MemberName: string;
  SubMembers: TStringDynArray;
  P: integer;
  DirText: string;
  Info: TOrderByInfo;
  CurrentAssociation: TAssociation;
  Associations: TArray<TAssociation>;
begin
  // Get orderby specification and separate items delimited by comma
  for O in A.Optimization.MemberRef.GetAttributes do
    if O is OrderBy then
    begin
      OrderMembers := SplitString(OrderBy(O).MemberNames, ',');
      break;
    end;

  // parse format "MemberName1 ASC, MemberName2 DESC"
  Len := Length(OrderMembers);
  for I := 0 to Len - 1 do
  begin
    // split <membername> ASC|DESC
    MemberName := Trim(OrderMembers[I]);
    P := Pos(' ', MemberName);
    if P > 0 then
    begin
      DirText := Trim(Copy(MemberName, P + 1, MaxInt));
      MemberName := Trim(Copy(MemberName, 1, P - 1));
    end;

    // Create OrderByInfo object to hold information
    Info := TOrderByInfo.Create;
    A.OrderByList.Add(Info);

    // Now split member names using association names
    // (Association1.Association2.MemberName)
    SubMembers := SplitString(MemberName, '.');

    CurrentAssociation := A;
    // navigate through associations
    SubLen := Length(SubMembers);
    SetLength(Associations, SubLen - 1);
    for J := 0 to Length(Associations) - 1 do
    begin
      CurrentAssociation := FExplorer.FindAssociationByPropertyName(CurrentAssociation.Target, SubMembers[J]);    
      if CurrentAssociation = nil then
        raise EInvalidOrderByMemberName.Create(SubMembers[J], A.Optimization.ParentClass.ClassName);
      Associations[J] := CurrentAssociation;      
    end;
    Info.Associations := Associations;
    
    // find columns of the member
    MemberName := SubMembers[SubLen - 1];
    Info.Column := FExplorer.FindColumnByPropertyName(CurrentAssociation.Target, MemberName);
    if Info.Column = nil then
      raise EInvalidOrderByMemberName.Create(MemberName, A.Optimization.ParentClass.ClassName);

    // set direction
    if SameText(DirText, 'DESC') then
      Info.Direction := TOrderByDirection.Descending
    else
      Info.Direction := TOrderByDirection.Ascending;
  end;
end;
...
function TMappingAttributeStrategy.LoadUnsavedValue(
  RttiType: TRttiType): TValue;
var
  A: TCustomAttribute;
begin
  Result := TValue.Empty;
  for A in RttiType.GetAttributes do
    if A is IdUnsavedValue then
      Exit(IdUnsavedValue(A).Value);
end;
