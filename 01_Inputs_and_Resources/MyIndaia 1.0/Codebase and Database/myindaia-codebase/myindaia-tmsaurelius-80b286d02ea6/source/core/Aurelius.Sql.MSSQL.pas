...
function TMSSQLSQLGenerator.CloseQuote: string;
begin
  Result := ']';
end;

constructor TMSSQLSQLGenerator.Create;
begin
  inherited Create;
  ...
  RegisterFunction('year', TFormatSQLFunction.Create('datepart(year, %0:s)', TypeInfo(integer)));
  RegisterFunction('month', TFormatSQLFunction.Create('datepart(month, %0:s)', TypeInfo(integer)));
  RegisterFunction('day', TFormatSQLFunction.Create('datepart(day, %0:s)', TypeInfo(integer)));
  RegisterFunction('hour', TFormatSQLFunction.Create('datepart(hour, %0:s)', TypeInfo(integer)));
  RegisterFunction('minute', TFormatSQLFunction.Create('datepart(minute, %0:s)', TypeInfo(integer)));
  RegisterFunction('second', TFormatSQLFunction.Create('datepart(second, %0:s)', TypeInfo(integer)));

  RegisterFunction('length', TSimpleSQLFunction.Create('len', TypeInfo(integer)));
  RegisterFunction('substring', TSimpleSQLFunction.Create('substring', TypeInfo(string)));
  RegisterFunction('position', TSimpleSQLFunction.Create('charindex', TypeInfo(integer)));
  RegisterFunction('bytelength', TSimpleSQLFunction.Create('datalength', TypeInfo(integer)));
end;
...
function TMSSQLSQLGenerator.GenerateInsert(Command: TInsertCommand): string;
const
  OutputTableVar = '@AureliusOutputTbl';
begin
  Result := inherited GenerateInsert(Command);
  if Command.OutputColumn <> '' then
  begin
    Insert(
      Format('OUTPUT INSERTED.%s INTO %s' + sqlLineBreak, [Command.OutputColumn, OutputTableVar]),
      Result,
      Pos(' VALUES (', Result)
    );

    Result :=
      'SET NOCOUNT ON; ' + sqlLineBreak
      + Format('DECLARE %s TABLE (ID INT); ', [OutputTableVar]) + sqlLineBreak
      + Result + ';' + sqlLineBreak
      + 'SET NOCOUNT OFF;' + sqlLineBreak
      + Format('SELECT ID From %s;', [OutputTableVar]);
  end;
end;
...
function TMSSQLSQLGenerator.GenerateLimitedSelect(SelectSql: TSelectSql;
  ...
  begin
    ...
    Result := 'SELECT * FROM ( ' +
      SubSql + ') as tempset WHERE ' + RowCondition;
  end;
...
function TMSSQLSQLGenerator.GetOptimalGuidMode: TSequentialGuidMode;
begin
  Result := TSequentialGuidMode.SequentialAtEnd;
end;
...
function TMSSQLSQLGenerator.OpenQuote: string;
begin
  Result := '[';
end;
