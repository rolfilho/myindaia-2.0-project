...
{$ENDIF}
  ...
  protected
    ...
    property Converters: TDictionary<PTypeInfo, TJsonTypeConverter> read FConverters;
  public
...
begin
  ...
  DateConverter :=
    ...
    begin
        Result := Serializer.DateToJson(Value.AsType<TDateTime>);
    end;
  ...
  FConverters.Add(TypeInfo(TBlob),
    ...
    begin
      ...
      begin
        // blob with proxy
        IntExplorer := TInternalExplorer(Serializer.FExplorer);
        Result := Serializer.BlobInfoToJson(IntExplorer.GetBlobInfo(MemberInfo, Value));
      end;
    end
  );
end;
...
begin
  if (Obj <> nil) and FExplorer.IsObjectList(Obj.ClassType) then
    Result := ListToJson(FExplorer.AsObjectList(Obj))
  else
