...
type
  ...
  protected
    function IdName(const AName: string): string; virtual; abstract;
    function QuoteId(const Name: string): string; virtual;
    function IsQuoted(const Name: string): Boolean; virtual;
    function Unquote(const Name: string): string; virtual;
    function OpenQuote: string; virtual;
    function CloseQuote: string; virtual;
    ...
    function GenerateCreateDBIndex(DBIndex: TDBIndexMetadata): string; virtual; abstract;
    function GenerateDropDBIndex(DBIndex: TDBIndexMetadata): string; virtual; abstract;
    ...
    function GetOptimalGuidMode: TSequentialGuidMode; virtual; abstract;

    function FindSQLFunction(const AFunctionName: string): ISQLFunction; virtual; abstract;
  end;
...
function TAbstractSQLGenerator.CloseQuote: string;
begin
  Result := '"';
end;
...
begin
  Result := Field.Table.Alias + '_' + Unquote(Field.Field);
...
function TAbstractSQLGenerator.QuoteId(const Name: string): string;
begin
  if not IsQuoted(Name) then
    Result := OpenQuote + Name + CloseQuote;
end;

function TAbstractSQLGenerator.Unquote(const Name: string): string;
begin
  if IsQuoted(Name) then
...
begin
  Result := Field.Table.Alias + '_' + StringReplace(Unquote(Field.Field), ' ', '', [rfReplaceAll]);
end;

function TAbstractSQLGenerator.IsQuoted(const Name: string): Boolean;
var
  L: Integer;
begin
  L := Length(Name);
  Result := (L >= 2) and (Name[1] = OpenQuote) and (Name[L] = CloseQuote);
end;

function TAbstractSQLGenerator.OpenQuote: string;
begin
  Result := '"';
end;
