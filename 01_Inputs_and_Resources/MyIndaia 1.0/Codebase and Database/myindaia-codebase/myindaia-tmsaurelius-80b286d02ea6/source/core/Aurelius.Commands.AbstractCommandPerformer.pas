...
type
  ...
  strict private
    type
      ...
      public
        ...
        function ExecuteQuery(SQL: string; Params: TList<TDBParam>): IDBResultSet;
        function Execute(SQL: string; Params: TList<TDBParam>): Integer;
      end;
  strict private
    FClazz: TClass;
  ...
  strict protected
    ...
    function BuildStatement(SQL: string; Params: TList<TDBParam>): IDBStatement;
  ...
  public
    ...
    property EntityManager: TObject read FEntityManager;
  ...
  public
    ...
    property Columns: TList<TColumn> read FColumns;
    property PKColumns: TArray<TColumn> read FPKColumns;
...
function TAbstractSQLPerformer.Execute(SQL: string; Params: TList<TDBParam>): Integer;
var
  Statement: IDBStatement;
begin
  Statement := BuildStatement(SQL, Params);
...
function TAbstractSQLPerformer.ExecuteQuery(SQL: string; Params: TList<TDBParam>): IDBResultSet;
var
  Statement: IDBStatement;
begin
  Statement := BuildStatement(SQL, Params);
...
function TAbstractSQLPerformer.BuildStatement(SQL: string; Params: TList<TDBParam>): IDBStatement;
var
  Statement: IDBStatement;
  Listener: ICommandExecutionListener;
  P: TDBParam;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Convert blob value to memo if needed
      if (P.ParamType in [ftMemo, ftWideMemo]) and (VarIsArray(P.ParamValue)) then
        P.ParamValue := TEncoding.Unicode.GetString(TUtils.VariantToBytes(P.ParamValue));
    end;

    Statement.SetParams(Params);
  end;

  // Notify SQL Executing event
  Events := TInternalObjectManager(FEntityManager).Events;
  if Events.OnSQLExecuting.HasListeners then
  begin
    Args := TSQLExecutingArgs.Create(FEntityManager, SQL, Params);
    try
      for SQLListener in Events.OnSQLExecuting.Listeners do
        SQLListener(Args);
    finally
      Args.Free;
    end;
...
begin
  ...
  begin
    // Workaround for drivers that return GUID values as array of bytes
    // (For now, only SQL-Direct is known to do that
    if (ClassFieldType = ftGuid) and VarIsArray(Result) then
      Result := GuidToString(TUtils.VariantToGuid(Result));

    // Replace equivalent field type
...
function TAbstractSQLPerformer.TSQLPerformerWrapper.Execute(SQL: string; Params: TList<TDBParam>): Integer;
begin
  Result := FCommand.Execute(SQL, Params);
end;

function TAbstractSQLPerformer.TSQLPerformerWrapper.ExecuteQuery(SQL: string;
  Params: TList<TDBParam>): IDBResultSet;
begin
  Result := FCommand.ExecuteQuery(SQL, Params);
end;
...
function TAbstractCommandPerformer.TCommandPerformerWrapper.Execute(SQL: string;
  Params: TList<TDBParam>): Integer;
begin
  Result := FCommand.Execute(SQL, Params);
end;

function TAbstractCommandPerformer.TCommandPerformerWrapper.ExecuteQuery(SQL: string;
  Params: TList<TDBParam>): IDBResultSet;
begin
  Result := FCommand.ExecuteQuery(SQL, Params);
end;
