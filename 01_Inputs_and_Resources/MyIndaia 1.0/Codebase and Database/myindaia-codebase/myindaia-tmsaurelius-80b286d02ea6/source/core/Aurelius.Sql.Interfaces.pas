...
uses
  TypInfo, Generics.Collections, DB,
...
type
  ...
  ['{3D7707C4-D463-45A0-97DB-2598E814B8DF}']
  ...
  ISQLGenerator = interface
    ...
    function GenerateCreateDBIndex(DBIndex: TDBIndexMetadata): string;
    function GenerateDropDBIndex(DBIndex: TDBIndexMetadata): string;
    ...
    function GetFullTableName(const ATable, ASchema: string): string;

    // Function to be called right before each command is executed
    procedure ProcessCommand(var SQL: string);

    function GetOptimalGuidMode: TSequentialGuidMode;

    function FindSQLFunction(const AFunctionName: string): ISQLFunction;
  end;
