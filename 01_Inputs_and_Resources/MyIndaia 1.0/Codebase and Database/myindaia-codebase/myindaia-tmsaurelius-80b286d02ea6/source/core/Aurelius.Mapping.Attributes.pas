...
type
  ...
  public
    constructor Create(AMemberName: string; AGenerator: TIdGenerator); overload;
    constructor Create(AMemberName: string; AGeneratorClass: TGeneratorClass); overload;
    property MemberName: string read FMemberName;
    property Generator: TIdGenerator read FGenerator;
    property GeneratorClass: TGeneratorClass read FGeneratorClass;
  end;

  IdUnsavedValue = class(TCustomAttribute)
  private
    FValue: TValue;
  public
  ...
  DBIndex = class(TCustomAttribute)
  private
    FName: string;
    FColumns: TList<string>;
  public
    constructor Create(const AName, AColumns: string); // Specify with comma-separated values
    destructor Destroy; override;
    property Name: string read FName write FName;
    property Columns: TList<string> read FColumns write FColumns;
  end;
  ...
  strict private
    FMemberNames: string;
  public
    constructor Create(const AMemberNames: string);
    property MemberNames: string read FMemberNames;
  end;
...
const
  ...
  CascadeTypeAllRemoveOrphan = Aurelius.Mapping.Metadata.CascadeTypeAllRemoveOrphan;
  CascadeTypeAllButRemove = Aurelius.Mapping.Metadata.CascadeTypeAllButRemove;
...
type
  ...
  DBTypeMemo = class(TCustomAttribute)
  end;

  DBTypeWideMemo = class(TCustomAttribute)
  end;
...
begin
  ...
  begin
    {$IFDEF NEXTGEN}
    if (Value.Length <> 1) or not Value.Chars[0].IsUpper then
    {$ELSE}
...
constructor Id.Create(AMemberName: string; AGeneratorClass: TGeneratorClass);
begin
  FMemberName := AMemberName;
  FGeneratorClass := AGeneratorClass;
end;
...
constructor ForeignKey.Create(AName: string);
begin
  FName := AName;
end;
...
constructor OrderBy.Create(const AMemberNames: string);
begin
  FMemberNames := AMemberNames;
end;
...
constructor IdUnsavedValue.Create(const AValue: Integer);
begin
  FValue := AValue;
end;

constructor IdUnsavedValue.Create(const AValue: string);
begin
  FValue := AValue;
end;
...
constructor DBIndex.Create(const AName, AColumns: string);
var
  List: TStringList;
  I: Integer;
begin
  ...
  List := TStringList.Create;
  try
    ExtractStrings([',', ';'], [' '], PChar(AColumns), List);
    for I := 0 to List.Count - 1 do
      FColumns.Add(List[I]);
  finally
    List.Free;
