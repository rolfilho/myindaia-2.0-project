...
uses
  Generics.Defaults, Generics.Collections, DB, SysUtils,
  Bcl.Collections,
...
type
  ...
  private
    FSQLFunctions: TDictionary<string, ISQLFunction>;
    FOnGetIdName: TFunc<string, string>;
  ...
  protected
    function IdName(const AName: string): string; override;
    ...
    function GenerateCreateDBIndex(DBIndex: TDBIndexMetadata): string; override;
    ...
    function GetOptimalGuidMode: TSequentialGuidMode; override;

    function FindSQLFunction(const AFunctionName: string): ISQLFunction; override;
    procedure RegisterFunction(const AFunctionName: string; ASQLFunction: ISQLFunction);
    procedure UnregisterFunction(const AFunctionName: string);

    function AllDBFeatures: TDBFeatures;
  public
    constructor Create;
    destructor Destroy; override;
    property OnGetIdName: TFunc<string, string> read FOnGetIdName write FOnGetIdName;
...
begin
  ...
  Result := Result + GetFullTablename(Column.Table.Name, Column.Table.Schema) + sqlLineBreak + '  ADD ' +
    IdName(Column.Name) + ' ' + GetColumnSQLType(Column);

  Result := Result + GetNullableConstraint(Column.NotNull);
end;

function TAnsiSQLGenerator.GenerateCreateDBIndex(
  DBIndex: TDBIndexMetadata): string;
begin
  Result := 'CREATE INDEX ' + DBIndex.Name + ' ON ';

  Result := Result + GetFullTablename(DBIndex.Table.Name, DBIndex.Table.Schema)
    + sqlLineBreak + '  (' + ConcatColumnNames(DBIndex.Columns, ', ') + ')';
end;

function TAnsiSQLGenerator.GenerateCreateForeignKey(ForeignKey: TForeignKeyMetadata): string;
begin
  Result := 'ALTER TABLE ';
  Result := Result + GetFullTableName(ForeignKey.FromTable.Name, ForeignKey.FromTable.Schema) + ' ADD CONSTRAINT ' + sqlLineBreak + '  ' +
...
begin
  ...
  Result := Result + GetFullTableName(Table.Name, Table.Schema) + ' (' + sqlLineBreak + '  ';
  ...
  begin
    ...
    PkName := IdName(Copy(PkName, 1, GetMaxConstraintNameLength));
    Result := Result + ',' + sqlLineBreak + '  CONSTRAINT ' + GeneratePrimaryKey(PkName, Table.IdColumns);
  end;

  // Foreign Keys
  if CreateForeignKeys then
    for ForeignKey in Table.ForeignKeys do
      Result := Result + ',' + sqlLineBreak + '  CONSTRAINT ' + GenerateForeignKeyDefinition(ForeignKey);

  // Unique Constraints
  for I := 0 to Table.UniqueKeys.Count - 1 do
  begin
    Result := Result + ',' + sqlLineBreak + '  CONSTRAINT ' + GenerateUniqueKeyDefinition(Table.UniqueKeys[I]);
  end;
...
begin
  ...
  Result := Result + sqlLineBreak + GenerateWhereClause(Command.WhereFields, False, Command.WhereStatement);
end;

function TAnsiSQLGenerator.GenerateDropDBIndex(
  DBIndex: TDBIndexMetadata): string;
begin
  Result := 'DROP INDEX ' +
...
begin
  ...
  Result := Result + GetFullTableName(ForeignKey.FromTable.Name, ForeignKey.FromTable.Schema) + sqlLineBreak + '  DROP CONSTRAINT ' +
...
begin
  ...
  Result := Result + GetFullTableName(UniqueKey.Table.Name, UniqueKey.Table.Schema) + sqlLineBreak + '  DROP CONSTRAINT ' +
...
begin
  Result := IdName(Field.Name) + ' ' + GetColumnSQLType(Field);
...
//  Result := Result + Command.Table.Name + ' AS ' + Command.Table.Alias;
  ...
  begin
    ...
    case Command.Joins[I].JoinType of
      TJoinType.Inner: Result := Result + sqlLineBreak + '  INNER JOIN ';
      TJoinType.Left:  Result := Result + sqlLineBreak + '  LEFT JOIN ';
    end;
...
//      ' AS ' + Command.Joins[I].Segments[0].PKField.Table.Alias + ' ON (';
    ...
    begin
      ...
      Result := Result +
        ...
        IdName(Command.Joins[I].Segments[J].PKField.Field) + ' = ' +
        Command.Joins[I].Segments[J].FKField.Table.Alias + '.' +
        IdName(Command.Joins[I].Segments[J].FKField.Field);
    end;
    Result := Result + ')';
  end;
...
begin
  ...
  Result := IdName(Copy(Result, 1, GetMaxConstraintNameLength));
end;

function TAnsiSQLGenerator.GetFullTableName(const ATable, ASchema: string): string;
begin
  Result := '';
  if (ASchema <> '') and (TDBFeature.Schemas in GetSupportedFeatures) then
    Result := Result + IdName(ASchema) + '.';
  Result := Result + IdName(ATable);
end;
...
begin
  ...
  begin
    if groupByClause <> '' then
      groupByClause := groupByClause + ', ' + sqlLineBreak;
...
begin
  ...
  try
    ...
    // watch out for the "VALUES (" string. MSSQL generator looks for it to tune the SQL
    // if you change this, change there!
    Result := Result + GetFullTableName(Command.Table.Name, Command.Table.Schema) + ' (' + sqlLineBreak + '  ' +
      TUtils.ConcatStrings(FieldNames) + ')' + sqlLineBreak + ' VALUES (' + sqlLineBreak + '  ';
...
begin
  ...
  begin
    ...
    begin
      ...
      orderByClause := orderByClause + Fields[I].Table.Alias + '.' + IdName(Fields[I].Field);
  ...
  begin
    if orderByClause <> '' then
      orderByClause := orderByClause + ', ' + sqlLineBreak;
...
begin
  ...
  begin
    if I > 0 then
      Result := Result + ', ';
    Result := Result + IdName(AColumns[I].Name);
  end;
...
begin
  Result := SelectSql.SelectClause +
    sqlLineBreak + SelectSql.FromClause;
  if SelectSql.WhereClause <> '' then
    Result := Result + sqlLineBreak + SelectSql.WhereClause;
  if SelectSql.GroupClause <> '' then
    Result := Result + sqlLineBreak + SelectSql.GroupClause;
  if SelectSql.HavingClause <> '' then
    Result := Result + sqlLineBreak + SelectSql.HavingClause;
  if SelectSql.OrderClause <> '' then
    Result := Result + sqlLineBreak + SelectSql.OrderClause;
end;
...
begin
  Result := IdName(UniqueKey.Name) + ' UNIQUE (' +
...
begin
  ...
  Result := Result + GetFullTableName(Command.Table.Name, Command.Table.Schema) + ' SET ' + sqlLineBreak + '  ';
  ...
  begin
    ...
    Result := Result + IdName(Command.UpdateFields[I].Field) + ' = :' +
...
begin
  ...
  begin
    ...
    if I > 0 then
      whereClause := whereClause + sqlLineBreak + '  AND ';

    leftOperand := '';
    if UseAliases then
      leftOperand := leftOperand + Fields[I].Table.Alias + '.';
    leftOperand := leftOperand + IdName(Fields[I].Field);
...
//        woLessOrEqual, woLike, woIsNull, woIsNotNull:
  ...
  begin
    if whereClause <> '' then
      Result := Result + sqlLineBreak + ' AND';
...
begin
  ...
  begin
    for I := 0 to Fields.Count - 1 do
      Result.Add(Fields[I].Table.Alias + '.' + IdName(Fields[I].Field));
  end
  else
  begin
    for I := 0 to Fields.Count - 1 do
      Result.Add(IdName(Fields[I].Field));
  end;
end;
...
function TAnsiSQLGenerator.GetOptimalGuidMode: TSequentialGuidMode;
begin
  Result := TSequentialGuidMode.SequentialAsString;
end;
...
begin
  ...
  begin
    Result := Result + AggregateFunctionToSQL(Field.AggregateFuntion) +
      '(' + Field.Table.Alias + '.' + IdName(Field.Field) + ')';
  end
  else
  begin
    Result := Result + Field.Table.Alias + '.' + IdName(Field.Field);
  end;
end;
...
begin
  ...
  begin
    ...
    Result := Result + QualifiedField + ' AS ' + IdName(GetDefaultColumnName(Fields[I]));
  end;
...
function TAnsiSQLGenerator.IdName(const AName: string): string;
begin
  if Assigned(FOnGetIdName) then
    Result := FOnGetIdName(AName)
  else
  begin
    Result := AName;
    // later we could use IsValidIdent. For now, let's just check spaces
    if QuoteIdentifiers or (Pos(' ', AName) > 0) then
      Result := QuoteId(AName);
  end;
end;
...
procedure TAnsiSQLGenerator.RegisterFunction(const AFunctionName: string;
  ASQLFunction: ISQLFunction);
begin
  FSQLFunctions.AddOrSetValue(AFunctionName, ASQLFunction);
end;

procedure TAnsiSQLGenerator.UnregisterFunction(const AFunctionName: string);
begin
  FSQLFunctions.Remove(AFunctionName);
end;
...
begin
  Result := [Low(TDBFeature)..High(TDBFeature)];
//  [TDBFeature.Sequences, TDBFeature.AutoGenerated, TDBFeature.ForeignKeys, TDBFeature.DropUniqueKey,
...
begin
  ...
  begin
    ...
    Result := Result + IdName(Columns[I].Name);
...
begin
  ...
  begin
    Result := (UpperCase(Value) = 'T') or (UpperCase(Value) = 'TRUE') or (Value = '1');
  end
  else
...
constructor TAnsiSQLGenerator.Create;
var
  Comparer: IEqualityComparer<string>;
begin
  inherited Create;
  ...
  {$ENDIF}
  FSQLFunctions := TDictionary<string, ISQLFunction>.Create(Comparer);
  RegisterFunction('add', TArithmeticSQLFunction.Create('+'));
  RegisterFunction('subtract', TArithmeticSQLFunction.Create('-'));
  RegisterFunction('multiply', TArithmeticSQLFunction.Create('*'));
  RegisterFunction('divide', TFormatSQLFunction.Create('(1.0E0 * (%s) / %s)', TypeInfo(double)));

  RegisterFunction('upper', TSimpleSQLFunction.Create('upper'));
  RegisterFunction('lower', TSimpleSQLFunction.Create('lower'));
  RegisterFunction('concat', TFormatSQLFunction.Create('(%0:s || %1:s)', TypeInfo(string)));
  RegisterFunction('contains', TFormatSQLFunction.Create('(%0:s) LIKE (''%%'' || (%1:s) || ''%%'')', TypeInfo(boolean)));
  RegisterFunction('startswith', TFormatSQLFunction.Create('(%0:s) LIKE ((%1:s) || ''%%'')', TypeInfo(boolean)));
  RegisterFunction('endswith', TFormatSQLFunction.Create('(%0:s) LIKE (''%%'' || (%1:s))', TypeInfo(boolean)));

  RegisterFunction('year', TFormatSQLFunction.Create('extract(year from %0:s)', TypeInfo(integer)));
  RegisterFunction('month', TFormatSQLFunction.Create('extract(month from %0:s)', TypeInfo(integer)));
  RegisterFunction('day', TFormatSQLFunction.Create('extract(day from %0:s)', TypeInfo(integer)));
  RegisterFunction('hour', TFormatSQLFunction.Create('extract(hour from %0:s)', TypeInfo(integer)));
  RegisterFunction('minute', TFormatSQLFunction.Create('extract(minute from %0:s)', TypeInfo(integer)));
  RegisterFunction('second', TFormatSQLFunction.Create('extract(second from %0:s)', TypeInfo(integer)));

  RegisterFunction('length', TSimpleSQLFunction.Create('char_length', TypeInfo(integer)));
  RegisterFunction('substring', TStandardSubstringFunction.Create);
  RegisterFunction('position', TFormatSQLFunction.Create('position(%0:s in %1:s)', TypeInfo(integer)));
  RegisterFunction('bytelength', TSimpleSQLFunction.Create('octet_length', TypeInfo(integer)));
end;
...
destructor TAnsiSQLGenerator.Destroy;
begin
  FSQLFunctions.Free;
  inherited;
end;

function TAnsiSQLGenerator.FindSQLFunction(
  const AFunctionName: string): ISQLFunction;
begin
  if not FSQLFunctions.TryGetValue(AFunctionName, Result) then
    Result := nil;
end;
