...
constructor TAbsoluteDBSQLGenerator.Create;
begin
  inherited Create;
...
begin
  ...
  Result := Result + IdName(Column.Table.Name) + sqlLineBreak + '  ADD (' +
...
begin
  ...
  Result := Result + IdName(Table.Name) + ' (' + sqlLineBreak + '  ';
  ...
  begin
    ...
    Result := Result + ',' + sqlLineBreak + '  ' + GeneratePrimaryKey(PkName, Table.IdColumns);
  end;

  // Unique Constraints
  for I := 0 to Table.UniqueKeys.Count - 1 do
  begin
    Result := Result + ',' + sqlLineBreak + '  ' + GenerateUniqueKeyDefinition(Table.UniqueKeys[I]);
  end;
...
begin
  Result := 'DROP INDEX ' + IdName(UniqueKey.Table.Name) + '.' + IdName(UniqueKey.Name) + ';';
end;
...
begin
  Result := 'SELECT LastAutoinc(' + IdName(SQLField.Table.Name) + ', ' + IdName(SQLField.Field) + ') FROM ' + IdName(SQLField.Table.Name);
end;
...
begin
  Result := 'PRIMARY KEY ' + IdName(APkName) + ' (';
...
begin
  Result := 'UNIQUE INDEX ' + IdName(UniqueKey.Name) + ' (' +
