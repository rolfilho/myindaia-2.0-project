...
type
  ...
  protected
    ...
    function DBIndexName(DBIndex: TDBIndexMetadata): string;
  public
  ...
  TDBIndexCreatedAction = class(TSchemaAction)
  public
    constructor Create(DBIndex: TDBIndexMetadata);
  end;

  TDBIndexRemovedAction = class(TSchemaAction)
  public
    constructor Create(DBIndex: TDBIndexMetadata);
  end;
  {$ENDREGION}
...
function TSchemaMessage.DBIndexName(DBIndex: TDBIndexMetadata): string;
begin
  Result := Format('%s.%s', [TableName(DBIndex.Table), DBIndex.Name]);
end;
...
{ TDBIndexRemovedAction }

constructor TDBIndexRemovedAction.Create(DBIndex: TDBIndexMetadata);
begin
  Text := Format('Database Index: %s - Removed.', [DBIndexName(DBIndex)]);
end;

{ TDBIndexCreatedAction }

constructor TDBIndexCreatedAction.Create(DBIndex: TDBIndexMetadata);
begin
  Text := Format('Database Index: %s - Created.', [DBIndexName(DBIndex)]);
end;
