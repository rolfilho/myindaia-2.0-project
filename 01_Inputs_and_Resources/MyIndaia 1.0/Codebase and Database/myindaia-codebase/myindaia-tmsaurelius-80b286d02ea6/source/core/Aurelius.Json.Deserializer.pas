...
//    FOnLoadProxy: TLoadProxyEvent;
    ...
    function GetEntities: TEnumerable<TObject>;
...
//    function FromJson<T>(Value: IJsonValue): T; overload;
    property Reverters: TDictionary<PTypeInfo, TJsonTypeReverter> read FReverters;
  ...
  public
    ...
    property Entities: TEnumerable<TObject> read GetEntities;
...
procedure JsonError(const Msg: string);
begin
  raise EIncompatibleJsonValueType.Create(Msg);
end;
...
{ Leave this as regular procedure to avoid inadvertently capturing Self in anonymous methods,
  causing memory leak on ARC compilers. }
procedure AddReverters(AReverters: TDictionary<PTypeInfo, TJsonTypeReverter>);
var
  DateReverter: TJsonTypeReverter;
begin
  AReverters.Add(TypeInfo(boolean),
    ...
    begin
      case Value.ValueType of
        ...
        JsonError('(Boolean): Not a JSON boolean');
      end;
      Result := MemberValue;
    end
  );
  ...
  DateReverter :=
    ...
    begin
      ...
      case Value.ValueType of
        ...
        JsonError('(TDateTime): Not a JSON String');
      end;
      TValue.Make(@DateValue, MemberInfo.RealType.Handle, MemberValue);
      Result := MemberValue;
    end;
  AReverters.Add(TypeInfo(TDateTime), DateReverter);
  AReverters.Add(TypeInfo(TDate), DateReverter);
  AReverters.Add(TypeInfo(TTime), DateReverter);

  AReverters.Add(TypeInfo(TGUID),
    function(Deserializer: TCustomJsonDeserializer;
      Entity: TObject; MemberInfo: TRttiOptimization; Value: IJsonValue; var Handled: boolean): TValue
    var
      MemberValue: TValue;
    begin
      case Value.ValueType of
        ...
        JsonError('(TGUID): Not a JSON String or NULL');;
      end;
      Result := MemberValue;
    end
  );

  AReverters.Add(TypeInfo(TBlob),
    function(Deserializer: TCustomJsonDeserializer;
      Entity: TObject; MemberInfo: TRttiOptimization; Value: IJsonValue; var Handled: boolean): TValue
    var
      Blob: TBlob;
    ...
    begin
      case Value.ValueType of
        ...
        TJsonType.JObject:
          begin
            BlobInfo := Deserializer.JsonValueToBlobInfo(Value);
            Deserializer.Explorer.SetBlobController(Entity, MemberInfo,
              TCustomJsonDeserializer.TJsonBlobController.Create(Deserializer.BlobLoader, BlobInfo));
            Handled := true;
          end;
      else
        JsonError('(TBlob): Not JSON String nor Null');
      end;
...
function TCustomJsonDeserializer.GetEntities: TEnumerable<TObject>;
begin
  Result := FEntities;
end;
...
begin
  ...
  Entity := FExplorer.ObjectFactory.CreateInstance(ObjectClass);
  try
    ...
    begin
      ...
      MemberInfo := FExplorer.FindOptimization(ObjectClass, PropName);
...
begin
  case Value.ValueType of
    ...
    JsonError('(BlobInfo): Not a JSON object');
  end;
end;
...
begin
  if List = nil then
    List := FExplorer.ObjectFactory.CreateInstance(ListClass);
  ListIntf := FExplorer.AsObjectList(List);
...
begin
  if FExplorer.IsObjectList(ObjectClass) then
  begin
    case JsonValue.ValueType of
      ...
      JsonError('(List): Not a JSON array');
    end;
  end
  else
  begin
    case JsonValue.ValueType of
      ...
      JsonError('(Instance): Not a JSON Object nor null');
    end;
  end;
end;
...
begin
  case Value.ValueType of
    ...
    JsonError('(Proxy): Expected JSON Object, Array or Null');
  end;
end;
  ...
  case Value.ValueType of
    TJsonType.JObject:
      begin
        ...
        TypeValue := JObj.FindProp(FSettings.ProxyProperty);
        if TypeValue = nil then
          JsonError('(ProxyInfo): Proxy property not found')
        else
        begin
          ClassValue := JObj.FindProp(FSettings.ProxyClassProperty);
          if ClassValue = nil then
            JsonError('(ProxyInfo): ProxyClass property not found');
          MemberValue := JObj.FindProp(FSettings.ProxyMemberProperty);
          if MemberValue = nil then
            JsonError('(ProxyInfo): ProxyMember property not found');
    ...
    JsonError('(ProxyInfo): Expected JSON Object');
  end;
end;
  ...
  case MemberInfo.RealType.TypeKind of
    tkInteger:
      case Value.ValueType of
        ...
        JsonError('(Integer): Expected JSON String or Integer');
      end;
    tkInt64:
      case Value.ValueType of
        ...
        WriteLn('ERRO Value: ' + Value.StringValue);
        JsonError('(Int64): Expected JSON String or Integer. Actual: ' + JsonTypeToStr(Value.ValueType));
      end;
    tkChar:
      case Value.ValueType of
        TJsonType.JString:
          if Length(Value.StringValue) >= 1 then
            MemberValue := TValue.FromOrdinal(MemberInfo.RealType.Handle, Ord(Value.StringValue[1]))
          else
            MemberValue := TValue.FromOrdinal(MemberInfo.RealType.Handle, 0);
        TJsonType.JInteger:
          MemberValue := TValue.FromOrdinal(MemberInfo.RealType.Handle, Value.IntValue);
      else
        JsonError('(Char): Expected JSON String or Integer');
      end;
    tkFloat:
      case Value.ValueType of
        TJsonType.JString:
        ...
        JsonError('(Float): Expected JSON String, Integer or Float');
      end;
    ...
    tkString, tkLString, tkUString:
      case Value.ValueType of
        ...
        JsonError('(String): Expected JSON String, Integer, Float or Boolean');
      end;
    ...
    tkWString:
      begin
        case Value.ValueType of
          ...
          JsonError('(WString): Expected JSON String, Integer, Float or Boolean');
        end;
    ...
    tkWChar:
      case Value.ValueType of
        ...
        JsonError('(WChar): Expected JSON String or Integer');
      end;

    tkEnumeration:
      case Value.ValueType of
        TJsonType.JString:
          MemberValue := TValue.FromOrdinal(MemberInfo.RealType.Handle, GetEnumValue(MemberInfo.RealType.Handle, Value.StringValue));
        TJsonType.JInteger:
          MemberValue := TValue.FromOrdinal(MemberInfo.RealType.Handle, Value.IntValue);
      else
        JsonError('(Enumeration): Expected JSON String or Integer');
      end;
    ...
    tkDynArray:
      begin
        ...
        case Value.ValueType of
          ...
          JsonError('(DynArray): Expected JSON String or Null');
        end;
...
procedure TCustomJsonDeserializer.TJsonProxyController.AfterLoad;
begin
end;
...
procedure TCustomJsonDeserializer.TJsonBlobController.AfterLoad;
begin
end;
