...
type
  ...
  public
    ...
    function GetCommand<T: constructor, TAbstractSQLPerformer>(Clazz: TClass): T; overload;
    function GetCommand<T: constructor, TAbstractSQLPerformer>(Clazz: TClass; AExplorer: TMappingExplorer): T; overload;
  end;
...
function TCommandPerformerFactory.GetCommand<T>(Clazz: TClass;
  AExplorer: TMappingExplorer): T;
var
  L: ICommandExecutionListener;
begin
  {$IFDEF TRIAL}
  if Now > EncodeDate(2017, 11, 1) then
    raise Exception.Create('TMS Aurelius trial period expired. Please purchase a license for commercial usage.');
  {$ENDIF}
  Result := T.Create;
  try
...
begin
  Result := GetCommand<T>(Clazz, FExplorer);
end;
