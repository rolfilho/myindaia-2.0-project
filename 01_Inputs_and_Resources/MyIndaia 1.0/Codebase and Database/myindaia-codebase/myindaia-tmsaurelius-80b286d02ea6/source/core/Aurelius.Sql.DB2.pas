...
constructor TDB2SQLGenerator.Create;
begin
  inherited Create;
//  RegisterFunction('length', TSimpleSQLFunction.Create('length', TypeInfo(integer)));
  RegisterFunction('length', TFormatSQLFunction.Create('character_length(%0:s, codeunits32)', TypeInfo(integer)));
  RegisterFunction('substring', TSimpleSQLFunction.Create('substr', TypeInfo(string)));
  RegisterFunction('position', TSimpleSQLFunction.Create('locate', TypeInfo(integer)));
end;
...
begin
  Result := Format('SELECT INTEGER(NEXT VALUE FOR %s) FROM SYSIBM.SYSDUMMY1;', [IdName(Command.SequenceName)]);
end;
...
begin
  Result := AllDBFeatures - [TDBFeature.RetrieveIdOnInsert];
end;
