...
type
  TSequentialGuidMode = (SequentialAtEnd, SequentialAsString, SequentialAsBinary);

  TUtils = class
  private
    class procedure ReverseBytes(var V; const Len: integer);
    class function VariantArrayToString(Value: Variant): string;
  public
    ...
    class function StringToVariantArray(const Value: string): Variant;
    ...
    class function NewCombGuid: TGuid; static;
    class function NewSequentialGuid(Mode: TSequentialGuidMode): TGuid; static;
...
uses
  Classes, Types, Variants, DateUtils, DB, Math, SqlTimSt, StrUtils, FmtBCD;
...
begin
  ...
  if (StrLen > 0) and (S[StrLen] = '=') then
  begin
    DataLen := DataLen - 1;
    if (StrLen > 1) and (S[StrLen - 1] = '=') then
      DataLen := DataLen - 1;
  end;
...
class function TUtils.NewCombGuid: TGuid;
const
  DaysLen = SizeOf(integer);
  MSecsLen = SizeOf(Int64);
  GuidLen = SizeOf(TGuid);
var
  days: integer;
  msecs: Int64;
  D: TDateTime;
  daysArray: array[0..DaysLen - 1] of byte;
  msecsArray: array[0..MSecsLen - 1] of byte;
  P: PByte;
begin
  D := SysUtils.Now;
  days := Trunc(D);
  msecs := DateTimeToTimeStamp(D).Time;
  System.Move(days, daysArray[0], DaysLen);
  System.Move(msecs, msecsArray[0], MsecsLen);
  ReverseBytes(DaysArray[0], DaysLen);
  ReverseBytes(MSecsArray[0], MSecsLen);

  Result := NewGuid;
  P := PByte(@Result.D1);
  Inc(P, 10); // copy to GuidAray[GuidLen - 6]
  System.Move(DaysArray[DaysLen - 2], P^, 2);
  Inc(P, 2);  // copy to GuidAray[GuidLen - 4]
  System.Move(MSecsArray[MSecsLen - 4], P^, 4);
end;

// FastGuid is based on the algorithm proposed at
// http://www.codeproject.com/Articles/388157/GUIDs-as-fast-primary-keys-under-multiple-database
class function TUtils.NewSequentialGuid(Mode: TSequentialGuidMode): TGuid;
const
  TimestampLen = SizeOf(Int64);
  GuidLen = SizeOf(TGuid);
var
  TS: TTimeStamp;
  Timestamp: Int64;
  TimestampBytes: array[0..TimestampLen - 1] of byte;
  P: PByte;
begin
  TS := DateTimeToTimeStamp(SysUtils.Now);

  Timestamp := TS.Date;
  Timestamp := (Timestamp * MSecsPerDay) + TS.Time;
  System.Move(Timestamp, TimestampBytes[0], TimestampLen);
  // if IsLittleEndian then
  ReverseBytes(TimestampBytes, TimestampLen);

  Result := NewGuid;
  P := PByte(@Result.D1);
  case Mode of
    TSequentialGuidMode.SequentialAsBinary:
      System.Move(TimestampBytes[2], P^, 6);
    TSequentialGuidMode.SequentialAsString:
      begin
        System.Move(TimestampBytes[2], P^, 6);
        ReverseBytes(P^, 4);
        Inc(P, 4);
    ...
    TSequentialGuidMode.SequentialAtEnd:
      begin
        Inc(P, 10);
        System.Move(TimestampBytes[2], P^, 6);
      end;
...
class procedure TUtils.ReverseBytes(var V; const Len: integer);
var
  P: PByte;
  P2: PByte;
  I: integer;
...
class function TUtils.StringToVariantArray(const Value: string): Variant;
var
  SL: TStringList;
  I: Integer;
begin
...
class function TUtils.VariantArrayToString(Value: Variant): string;
var
  I: Integer;
  SL: TStringList;
begin
  ...
  try
    for I := 0 to VarArrayHighBound(Value, 1) do
      SL.Add(VariantToString(Value[I]));
    Result := SL.CommaText;
  finally
    SL.Free;
  end;
...
begin
  ...
  begin
    case VarType(V) of
      varString: result := TEncoding.Unicode.GetBytes(string(V));
      varUString: result := TEncoding.Unicode.GetBytes(string(V));
      varOleStr: result := TEncoding.Unicode.GetBytes(UnicodeString(V));
    else
...
begin
  if VarIsArray(Value) then
    Result := VariantArrayToString(Value)
  else
