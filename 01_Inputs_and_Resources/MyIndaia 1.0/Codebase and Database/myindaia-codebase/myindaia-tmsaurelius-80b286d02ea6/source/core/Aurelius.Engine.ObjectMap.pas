...
type
  ...
  public
    constructor Create(AExplorer: TMappingExplorer);
    destructor Destroy; override;
    procedure UpdatePropertyState(const MemberName: string);
    procedure UpdateObjectState;
    procedure UpdateState(AEntity: TObject; AState: TObjectState);
    property Entity: TObject read FEntity write FEntity;
    property OldState: TObjectState read FOldState;
  end;
  ...
  public
    ...
    procedure UpdatePropertyOldState(Entity: TObject; const MemberName: string);
...
begin
  Result := TMapEntry.Create(FExplorer);
...
begin
  ...
  if (AListObj <> nil) and (FExplorer.IsObjectList(AListObj.ClassType)) then
    List := FExplorer.AsObjectList(AListObj)
  else
    List := nil;
...
begin
  ...
  Result := Result + ':' + TUtils.VariantToString(Id); // If you change this, revise TObjectManager.TEngineCursor class
end;
...
function TObjectMap.IsAttached(Entity: TObject): Boolean;
var
  Entry: TMapEntry;
begin
  // Is Attached doesn't check for ids or anything. Just the object pointer
  // This is useful in cases where Entity has been already destroyed. So we can safely
  // return is Entity is here or not just by checking the pointer itself, not the content
  for Entry in FMap.Values do
...
procedure TObjectMap.UpdatePropertyOldState(Entity: TObject; const MemberName: string);
var
  Entry: TMapEntry;
begin
  Assert(IsMapped(Entity));
  Entry := FMap[GenerateKey(Entity)];
  Entry.UpdatePropertyState(MemberName);
end;

{ TObjectState }

constructor TObjectState.Create;
begin
...
constructor TMapEntry.Create(AExplorer: TMappingExplorer);
begin
  FExplorer := AExplorer;
...
procedure TMapEntry.UpdateObjectState;
begin
  if FOldState = nil then
    FOldState := TObjectState.Create;

  UpdateState(Entity, OldState);
end;

procedure TMapEntry.UpdatePropertyState(const MemberName: string);
begin
  if FOldState = nil then
    FOldState := TObjectState.Create;

  FExplorer.GetColumnValues(Entity, OldState.Values, MemberName);
end;

procedure TMapEntry.UpdateState(AEntity: TObject; AState: TObjectState);
begin
  // Get the current state of column fields. Use Values instead of FValues
  AState.Values.Clear;
  FExplorer.GetColumnValues(AEntity, AState.Values, '');
end;
