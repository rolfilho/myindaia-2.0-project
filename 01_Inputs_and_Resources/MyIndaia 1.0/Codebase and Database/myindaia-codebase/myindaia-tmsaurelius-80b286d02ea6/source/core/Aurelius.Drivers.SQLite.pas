...
uses
  Classes, DB, Variants, Generics.Collections, SysUtils,
...
constructor EFieldNotFound.Create(const FieldName: string);
begin
  inherited CreateFmt('Field "%s" not found', [FieldName]);
end;
...
function TSQLiteNativeStatementAdapter.Execute: Integer;
var
...
begin
  ...
  if FieldIndex = -1 then
    raise EFieldNotFound.Create(FieldName);
