...
uses
  Classes, DB, Variants, Generics.Collections, ADODB, ADOInt,
...
function TDbGoStatementAdapter.Execute: Integer;
begin
...
begin
  ...
  try
    ...
    ResultSet.CursorType := TCursorType.ctOpenForwardOnly;
...
begin
  ...
  for P in Params do
  begin
    // Let's keep the previous behavior unless param doesn't have a name
    // (only happens with RemoteDB + Dataset Provider)
    // We need it to keep backward compatibility and because otherwise DB2
    // raises a "no enough storage error" sometimes
    if not Parsed and (P.ParamName <> '') then
    begin
    ...
    if (P.ParamName = '') then
    begin
      while (I >= FADOQuery.Parameters.Count) do
        FADOQuery.Parameters.AddParameter.Direction := pdInput;
      Parameter := FADOQuery.Parameters[I];
    end
    else
...
begin
  ...
  try
    ...
    Statement.CursorType := TCursorType.ctOpenForwardOnly;
  except
    Statement.Free;
    raise;
  end;
