...
uses
  ...
  pFIBQuery, pFIBDatabase, pFIBProps, SqlTxtRtns,
...
function TFIBPlusStatementAdapter.Execute: Integer;
begin
  Result := -1;
  ...
  begin
    FFIBQuery.ExecQuery;
    if FFIBQuery.SQLKind in
      [skUpdate, skInsert, skDelete, skExecuteProc, skExecuteBlock, skUpdateOrInsert] then
      Result := FFIBQuery.RowsAffected;
  end
  else
  begin
...
begin
  ...
  for P in Params do
  begin
    if (P.ParamName = '') and (I < FFIBQuery.Params.Count) then
      Parameter := FFIBQuery.Params[I]
    else
...
var
  Field: TFIBXSQLVAR;
begin
  Field := FDataset.Fields[FieldIndex];
  if Field.IsNull then
    Result := Variants.Null
  else
  begin
    Result := Field.Value;
    if Field.IsBlob and (Field.ServerSQLSubType = 0) then // raw blob
      Result := TUtils.BytesToVariant(BytesOf(string(Result)));
  end;
end;
