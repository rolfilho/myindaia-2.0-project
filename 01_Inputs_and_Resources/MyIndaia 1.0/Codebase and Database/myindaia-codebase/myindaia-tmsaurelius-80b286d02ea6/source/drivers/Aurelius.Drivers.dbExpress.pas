...
function TDBExpressStatementAdapter.Execute: Integer;
begin
...
begin
  ...
  try
    ...
    if FSQLQuery.Params.Count <> ResultSet.Params.Count then
      ResultSet.Params.Assign(FSQLQuery.Params);
...
begin
  ...
  begin
    if (P.ParamName = '') then
    begin
      while (I >= FSQLQuery.Params.Count) do
        {$IFDEF DELPHIXE2_LVL}
        FSQLQuery.Params.AddParameter;
        {$ELSE}
        FSQLQuery.Params.Add;
        {$ENDIF}
      Parameter := FSQLQuery.Params[I];
    end
    else
...
//        Parameter.AsWideString := P.ParamValue;
    ...
    begin
      if VarIsNull(P.ParamValue) or VarIsEmpty(P.ParamValue) then
      begin
        Parameter.DataType := ftGuid;
