...
begin
  {$IFDEF DELPHIXE6_LVL}
  Result := TJsonValueWrapper.Create(JsonArray.Items[I]);
  {$ELSE}
...
begin
  {$IFDEF DELPHIXE6_LVL}
  Result := JsonObject.Pairs[I].JsonString.Value;
  {$ELSE}
...
begin
  {$IFDEF DELPHIXE6_LVL}
  Result := TJsonValueWrapper.Create(JsonObject.Pairs[I].JsonValue);
  {$ELSE}
