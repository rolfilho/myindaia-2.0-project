...
uses
  ComObj, ActiveX, AspTlb, StdVcl, Windows, SysUtils, TlHelp32, PsAPI, ADODB, IniFiles, IdSMTP, IdMessage;
...
begin
  ...
  try
    ...
    Result := MySendMail(pwRemetente, pwDestinatarios, '', '', pwAssunto, pwHtml, '');
    {
    try
      SMTP.Connect;
      SMTP.Send(Msg);
...
begin
  Result := 'D:\sites\myindaia\engine\MyComLog.txt';//GetLocalPath + FormatDateTime('yyyy-mm-dd', Now)+ '.log';
end;
...
Initialization
  ...
  with TIniFile.Create('D:\sites\myindaia\engine\config_myCOM.ini') do
  try
