...
type
  Tfrm_proc_aplic = class(TForm)
    ...
    DBText1: TDBText;
    DBText2: TDBText;
    dbchkbox_aplicavel: TDBCheckBox;
    btn_obs: TButton;
    pnl_selecao: TPanel;
...
var
  frm_proc_aplic: Tfrm_proc_aplic;

implementation

uses GSMLIB,  PGSM039, PGGA002, PGSM018, PGSM010, PGGP015,
     PGGP017, PGGA004, PGSM136;

{$R *.DFM}

procedure Tfrm_proc_aplic.btn_sairClick(Sender: TObject);
begin
  Close;
end;

procedure Tfrm_proc_aplic.FormShow(Sender: TObject);
begin
...
procedure Tfrm_proc_aplic.FormCreate(Sender: TObject);
begin
  Application.CreateForm(Tdatm_proc_aplic, datm_proc_aplic );
  ...
  datm_proc_aplic.ds_tfollowup.AutoEdit := st_modificar;
end;

procedure Tfrm_proc_aplic.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Grava;
  datm_proc_aplic.Free;
  Action:= cafree;
end;

procedure Tfrm_proc_aplic.btn_obsClick(Sender: TObject);
begin
...
procedure Tfrm_proc_aplic.msk_nr_processoExit(Sender: TObject);
begin
  btn_obs.Enabled             := st_modificar;
  dbchkbox_aplicavel.ReadOnly := Not st_modificar;
  with datm_proc_aplic do
  begin
    Grava;   
    qry_processo_.Close;
    ...
    begin
        ...
        begin
          ...
          if qry_tfollowup_.RecordCount = 0 then
             dbchkbox_aplicavel.ReadOnly   := True
          else dbchkbox_aplicavel.ReadOnly := False;
...
procedure Tfrm_proc_aplic.msk_cd_unid_negExit(Sender: TObject);
begin
  Grava;
...
procedure Tfrm_proc_aplic.msk_cd_produtoExit(Sender: TObject);
begin
  Grava;
...
procedure Tfrm_proc_aplic.msk_cd_unid_negEnter(Sender: TObject);
begin
  st_unid_neg := msk_cd_unid_neg.Text;
end;

procedure Tfrm_proc_aplic.msk_cd_produtoEnter(Sender: TObject);
begin
  st_produto := msk_cd_produto.Text;
end;

function Tfrm_proc_aplic.Habilitacao : Boolean;
begin
  Habilitacao := True;
...
procedure Tfrm_proc_aplic.btn_cd_unid_negClick(Sender: TObject);
begin
  vStr_cd_modulo := '23';
  str_cd_rotina := '2301';
...
procedure Tfrm_proc_aplic.btn_cd_produtoClick(Sender: TObject);
begin
  vStr_cd_modulo := '23';
  str_cd_rotina := '2302';
  ...
  begin
    ...
    frm_proc_aplic.msk_cd_produtoExit(nil);
  end;
  vStr_cd_modulo := st_cd_modulo;
  str_cd_rotina := st_cd_rotina;
...
procedure Tfrm_proc_aplic.btn_processoClick(Sender: TObject);
begin
...
procedure Tfrm_proc_aplic.Grava;
begin
  if datm_Proc_aplic.qry_tfollowup_.State = dsEdit then
     datm_Proc_aplic.qry_tfollowup_.Post;
end;

procedure Tfrm_proc_aplic.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #27 then Close;
end;

procedure Tfrm_proc_aplic.msk_cd_unid_negKeyDown(Sender: TObject;
