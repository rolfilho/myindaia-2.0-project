object frm_proc_inspec: Tfrm_proc_inspec
  Left = 53
  Top = 28
  Width = 640
  Height = 480
  BorderIcons = [biSystemMenu]
  Caption = 'Inspeção por Processo'
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  Menu = menu_manut
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object bvl_nm_usuario: TBevel
    Left = 232
    Top = 0
    Width = 393
    Height = 41
  end
  object lbl_nm_usuario: TLabel
    Left = 240
    Top = 14
    Width = 39
    Height = 13
    Caption = 'Usuário:'
  end
  object pnl_entrada_de_dados: TPanel
    Left = 0
    Top = 128
    Width = 625
    Height = 305
    BevelOuter = bvLowered
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    object bvl_cabecalho_grid: TBevel
      Left = 8
      Top = 8
      Width = 608
      Height = 25
      Style = bsRaised
    end
    object lbl_cd_evento: TLabel
      Left = 9
      Top = 9
      Width = 56
      Height = 21
      AutoSize = False
      Caption = 'Código'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object lbl_Evento: TLabel
      Left = 64
      Top = 9
      Width = 256
      Height = 21
      AutoSize = False
      Caption = 'Evento'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object lbl_Aplicavel: TLabel
      Left = 321
      Top = 9
      Width = 112
      Height = 21
      AutoSize = False
      Caption = 'Data de Inspeção'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label1: TLabel
      Left = 434
      Top = 9
      Width = 87
      Height = 21
      AutoSize = False
      Caption = 'Resultado'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label2: TLabel
      Left = 522
      Top = 9
      Width = 87
      Height = 21
      AutoSize = False
      Caption = 'RNC'
      Color = clBlack
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object dbg_evento_aplicavel: TDBCtrlGrid
      Left = 8
      Top = 33
      Width = 609
      Height = 250
      ColCount = 1
      DataSource = datm_proc_inspec.ds_tfollowup
      PanelHeight = 25
      PanelWidth = 593
      TabOrder = 0
      RowCount = 10
      SelectedColor = clTeal
      object DBText1: TDBText
        Left = 3
        Top = 4
        Width = 54
        Height = 17
        DataField = 'CD_EVENTO'
        DataSource = datm_proc_inspec.ds_tfollowup
      end
      object DBText2: TDBText
        Left = 56
        Top = 4
        Width = 257
        Height = 17
        DataField = 'Look_Evento'
        DataSource = datm_proc_inspec.ds_tfollowup
      end
      object dbedt_inspecao: TDBEdit
        Left = 312
        Top = 2
        Width = 97
        Height = 21
        DataField = 'DT_INSPECAO'
        DataSource = datm_proc_inspec.ds_tfollowup
        TabOrder = 0
        OnChange = dbedt_inspecaoChange
        OnDblClick = dbedt_inspecaoDblClick
      end
      object dbckbx_cd_result_inspec: TDBCheckBox
        Left = 428
        Top = 4
        Width = 77
        Height = 17
        Caption = 'Conforme'
        DataField = 'CD_RESULT_INSPECAO'
        DataSource = datm_proc_inspec.ds_tfollowup
        TabOrder = 1
        ValueChecked = 'C'
        ValueUnchecked = 'N'
        OnClick = dbckbx_cd_result_inspecClick
      end
      object dbedt_rnc: TDBEdit
        Left = 515
        Top = 2
        Width = 70
        Height = 21
        DataField = 'NR_RNC'
        DataSource = datm_proc_inspec.ds_tfollowup
        TabOrder = 2
      end
    end
  end
  object pnl_manut_cadastro: TPanel
    Left = 0
    Top = 0
    Width = 225
    Height = 41
    BevelOuter = bvLowered
    TabOrder = 1
    object btn_sair: TSpeedButton
      Left = 8
      Top = 8
      Width = 25
      Height = 25
      Flat = True
      Glyph.Data = {
        
        0333337F777FFFFF7F3333000000000003333377777777777333}
      NumGlyphs = 2
      OnClick = btn_sairClick
    end
    object btn_obs: TButton
      Left = 134
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Observações'
      Enabled = False
      TabOrder = 0
      OnClick = btn_obsClick
    end
  end
  object pnl_selecao: TPanel
    Left = 0
    Top = 44
    Width = 625
    Height = 81
    BevelOuter = bvLowered
    TabOrder = 2
    object lbl_cd_cliente: TLabel
      Left = 330
      Top = 3
      Width = 40
      Height = 13
      Caption = 'Cliente'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbl_nr_processo: TLabel
      Left = 202
      Top = 41
      Width = 53
      Height = 13
      Caption = 'Processo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object btn_processo: TSpeedButton
      Left = 298
      Top = 54
      Width = 25
      Height = 21
      Glyph.Data = {
        
        5555555555555555555555555555555555555555555555555555}
      NumGlyphs = 2
      OnClick = btn_processoClick
    end
    object Label3: TLabel
      Left = 14
      Top = 41
      Width = 45
      Height = 13
      Caption = 'Produto'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object btn_cd_produto: TSpeedButton
      Left = 167
      Top = 53
      Width = 25
      Height = 22
      Glyph.Data = {
        
        5555555555555555555555555555555555555555555555555555}
      NumGlyphs = 2
      OnClick = btn_cd_produtoClick
    end
    object lbl_cd_unid_neg: TLabel
      Left = 14
      Top = 3
      Width = 48
      Height = 13
      Caption = 'Unidade'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object btn_cd_unid_neg: TSpeedButton
      Left = 167
      Top = 15
      Width = 25
      Height = 22
      Glyph.Data = {
        
        5555555555555555555555555555555555555555555555555555}
      NumGlyphs = 2
      OnClick = btn_cd_unid_negClick
    end
    object lbl_cd_servico: TLabel
      Left = 330
      Top = 41
      Width = 44
      Height = 13
      Caption = 'Serviço'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object msk_nr_processo: TMaskEdit
      Left = 200
      Top = 54
      Width = 97
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 18
      ParentFont = False
      TabOrder = 2
      OnExit = msk_nr_processoExit
      OnKeyDown = msk_cd_unid_negKeyDown
    end
    object msk_cd_produto: TMaskEdit
      Left = 14
      Top = 54
      Width = 21
      Height = 21
      EditMask = '99;0; '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 2
      ParentFont = False
      TabOrder = 1
      OnEnter = msk_cd_produtoEnter
      OnExit = msk_cd_produtoExit
      OnKeyDown = msk_cd_unid_negKeyDown
    end
    object edt_nm_produto: TEdit
      Left = 38
      Top = 54
      Width = 125
      Height = 21
      TabStop = False
      Color = clMenu
      ReadOnly = True
      TabOrder = 3
      Text = 'edt_nm_produto'
    end
    object msk_cd_unid_neg: TMaskEdit
      Left = 14
      Top = 16
      Width = 21
      Height = 21
      EditMask = '99;0; '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 2
      ParentFont = False
      TabOrder = 0
      OnEnter = msk_cd_unid_negEnter
      OnExit = msk_cd_unid_negExit
      OnKeyDown = msk_cd_unid_negKeyDown
    end
    object edt_nm_unid_neg: TEdit
      Left = 38
      Top = 16
      Width = 125
      Height = 21
      TabStop = False
      Color = clMenu
      ReadOnly = True
      TabOrder = 4
      Text = 'edt_nm_unid_neg'
    end
    object dbedt_cd_cliente: TDBEdit
      Left = 330
      Top = 15
      Width = 39
      Height = 21
      TabStop = False
      Color = clMenu
      DataField = 'CD_CLIENTE'
      DataSource = datm_proc_inspec.ds_processo
      ReadOnly = True
      TabOrder = 5
    end
    object dbedt_cd_servico: TDBEdit
      Left = 330
      Top = 54
      Width = 39
      Height = 21
      TabStop = False
      Color = clMenu
      DataField = 'CD_SERVICO'
      DataSource = datm_proc_inspec.ds_processo
      ReadOnly = True
      TabOrder = 6
    end
    object dbedt_nm_cliente: TDBEdit
      Left = 374
      Top = 15
      Width = 231
      Height = 21
      TabStop = False
      Color = clMenu
      DataField = 'AP_EMPRESA'
      DataSource = datm_proc_inspec.ds_processo
      ReadOnly = True
      TabOrder = 7
    end
    object dbedt_nm_servico: TDBEdit
      Left = 374
      Top = 54
      Width = 231
      Height = 21
      TabStop = False
      Color = clMenu
      DataField = 'NM_SERVICO'
      DataSource = datm_proc_inspec.ds_processo
      ReadOnly = True
      TabOrder = 8
    end
  end
  object edt_nm_usuario: TEdit
    Left = 286
    Top = 8
    Width = 211
    Height = 21
    TabStop = False
    Color = clMenu
    ReadOnly = True
    TabOrder = 3
    Text = 'edt_nm_usuario'
  end
  object menu_manut: TMainMenu
    Left = 456
    Top = 296
    object mi_sair: TMenuItem
      Caption = '&Sair'
      OnClick = btn_sairClick
    end
  end
end
