unit uVisualizaArquivos;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, OleCtnrs, ExtCtrls, StdCtrls, Buttons, ShellCtrls, ShellAPI, ShlObj,
  ActiveX, ClipBrd, sSkinManager, sSkinProvider, sPanel, sLabel, sMemo,
  sComboBox, sCheckBox, sSpeedButton, uListaArquivos, DBTables, DB,
  DelphiTwain, JPEG, sGauge, CommDlg, DateUtils;

type
 TTipoForm = (tfExibir, tfEditar, tfVincular);

type
  TfrmVisualizaArquivos = class(TForm)
    ScrollBox1: TScrollBox;
    Panel1: TPanel;
    sSkinProvider1: TsSkinProvider;
    sSkinManager1: TsSkinManager;
    btn_sair: TSpeedButton;
    btn_salvar: TSpeedButton;
    btnScanner: TSpeedButton;
    dtScanner: TDelphiTwain;
    pnlAndamento: TPanel;
    ggAndamentoGeral: TsGauge;
    lblAndamento: TsLabel;
    chkMiniaturas: TCheckBox;
    btn_novo: TSpeedButton;
    OpenDialog1: TOpenDialog;
    scnArquivos: TShellChangeNotifier;
    procedure FormResize(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure scnArquivosChange;
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure btn_sairClick(Sender: TObject);
    procedure btn_salvarClick(Sender: TObject);
    procedure btnScannerClick(Sender: TObject);
    procedure dtScannerAcquireError(Sender: TObject; const Index: Integer;
      ErrorCode, Additional: Integer);
    procedure dtScannerTwainAcquire(Sender: TObject; const Index: Integer;
      Image: TBitmap; var Cancel: Boolean);
    procedure FormActivate(Sender: TObject);
    procedure FormKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure chkMiniaturasClick(Sender: TObject);
    procedure btn_novoClick(Sender: TObject);
  private
    Arquivos1, Arquivos2: TStrings;
    FWindowProc : TWndMethod;
    FTipoForm: TTipoForm;
    procedure DropFiles(var Msg: TWMDROPFILES);
    procedure ShellWindowProc(var Msg: TMessage);
    procedure AdicionaArquivo(AArquivo, AArquivoPai, ATipo, ADescricao: String;
       ANumeroArquivos: Integer);
    procedure ExcluiArquivo(AArquivo: String);
    procedure AdicionaPasta(APasta: String);
    procedure Alteracao(Sender: TObject);
    procedure IniciaAndamento(Max: Integer);
    procedure FinalizaAndamento;
    procedure ChangeImages(Sender: TObject; MaxProgress, CurrentProgress: Int64);
  public
    procedure Exibir;
    procedure Editar;
    procedure Vincular(AArquivo: TFileName; ATipoArquivo: String);
  end;

var
  frmVisualizaArquivos: TfrmVisualizaArquivos;
  ParentFileName: TFileName;

implementation

uses
  dDigitalizacao, PGGP017, Funcoes, ConsOnlineEx, uDigitalizacao, StrUtils;

var
  Lista: TListaArquivos;

{$R *.dfm}

// procedimento criado por Michel em 27/05/2009 para tirar atributo Somente Leitura antes de renomear
function RenomeiaArquivo(ANomeVelho, ANomeNovo: String): Boolean;
begin                                  
  SetFileAttributes(PAnsiChar(ANomeVelho), 0);
  Result := RenameFile(PAnsiChar(ANomeVelho), PAnsiChar(ANomeNovo));
  if Result then
    SetFileAttributes(PAnsiChar(ANomeNovo), SysUtils.faReadOnly);
end;

function CopiaArquivo(AArquivoExistente, ANovoArquivo: String; AFalharSeExiste: Boolean): Boolean;
begin
  Result := CopyFile(PAnsiChar(AArquivoExistente), PAnsiChar(ANovoArquivo), AFalharSeExiste);
  if Result then
    SetFileAttributes(PAnsiChar(ANovoArquivo), SysUtils.faReadOnly);
end;

procedure TfrmVisualizaArquivos.FormResize(Sender: TObject);
begin
  Lista.SortWindow;
end;

procedure TfrmVisualizaArquivos.FormCreate(Sender: TObject);
begin
  Lista                  := TListaArquivos.Create(ScrollBox1);
  Lista.OnProgress       := ChangeImages;

  SetCurrentDir(datmDigitalizacao.DocPath);
  scnArquivos.Root       := datmDigitalizacao.DocPath;
  OpenDialog1.InitialDir := datmDigitalizacao.DocPath;
  //'OpenDialog1.FileName   := OpenDialog1.InitialDir;
  Arquivos1              := ListaDiretorio(datmDigitalizacao.DocPath);
                                           
  datmDigitalizacao.qry_TipoArquivo.First;
  while not datmDigitalizacao.qry_TipoArquivo.Eof do
  begin
    if (datmDigitalizacao.qry_TipoArquivoIN_ALTERACAO.AsString = '1') then
      Lista.DefaultTypeItems.Add(datmDigitalizacao.qry_TipoArquivoNM_TIPO_DOCTO.AsString);
    datmDigitalizacao.qry_TipoArquivo.Next;
  end;
end;

procedure TfrmVisualizaArquivos.scnArquivosChange;
var
  I: Integer;
  MS: TMemoryStream;
begin
  MS := TMemoryStream.Create;
  Arquivos2 := ListaDiretorio(datmDigitalizacao.DocPath);
  // Procura pelos arquivos inseridos
  for I := 0 to Arquivos2.Count -1 do
    if GetFileAttributes(PAnsiChar(datmDigitalizacao.DocPath + Arquivos2[I])) and faDirectory <> faDirectory then
    begin
      if Arquivos1.IndexOf(Arquivos2[I]) < 0 then
      begin
        if not chkMiniaturas.Checked then
          AdicionaArquivo(datmDigitalizacao.DocPath + Arquivos2[I], '', '', ChangeFileExt(Arquivos2[I], ''), 1)
        else
          try
            MS.LoadFromFile(datmDigitalizacao.DocPath + Arquivos2[I]);
            Arquivos1.Add(Arquivos2[I]);
            if FTipoForm = tfEditar then
            begin
              if MessageDlg('Foi incluído um novo arquivo em ''Meus Documentos''.'#13 +
                 'Deseja utilizá-lo para substituir o arquivo em edição?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
                Lista.Items[0].FileName := datmDigitalizacao.DocPath + Arquivos2[I];
            end
            else
              AdicionaArquivo(datmDigitalizacao.DocPath + Arquivos2[I], '', '', ChangeFileExt(Arquivos2[I], ''), 1);
          except
          end;
      end;
    end;
  Lista.DeleteMissingFiles;
  MS.Free;
end;

procedure TfrmVisualizaArquivos.DropFiles(var Msg: TWMDROPFILES);
var
  NumFiles, I, Count: Integer;
  Buffer: array[0..MAX_PATH] of Char;
begin
  NumFiles := DragQueryFile(Msg.Drop, $FFFFFFFF, nil, 0);
  Count := NumFiles;
  for I := 0 to NumFiles -1 do
  begin
    DragQueryFile(Msg.Drop, I, @Buffer, Sizeof(Buffer));
    if GetFileAttributes(Buffer) and faDirectory = faDirectory then
      Count := Count + ListaDiretorio(Buffer).Count;
  end;
  IniciaAndamento(Count);
  for I := 0 to NumFiles -1 do
  begin
    DragQueryFile(Msg.Drop, I, @Buffer, Sizeof(Buffer));
    // Se é pasta então lista a pasta e insere os arquivos, se não apenas insere o arquivo
    if GetFileAttributes(Buffer) and faDirectory = faDirectory then
      AdicionaPasta(Buffer)
    else
      AdicionaArquivo(Buffer, '', '', ExtractFileName(ChangeFileExt(Buffer, '')), 1);
  end;
  FinalizaAndamento;
end;

procedure TfrmVisualizaArquivos.ShellWindowProc(var Msg: TMessage);
begin
  if Msg.Msg = WM_DROPFILES then
    DropFiles(TWMDROPFILES(Msg))
  else
    FWindowProc(Msg);
end;

procedure TfrmVisualizaArquivos.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  if btn_salvar.Enabled then
    case MessageDlg('As alteração não foram salvas.'#13'Deseja salvá-las antes de sair?', mtConfirmation, [mbYes, mbNo, mbCancel], 0) of
      mrYes: btn_salvarClick(btn_salvar);
      mrNo: {Apenas sai sem salvar};
      mrCancel: Abort;
    end;
  datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO.ReadOnly := True;
  sSkinManager1.Active := False;
  try
    Lista.Destroy;
  except
  end;
end;

procedure TfrmVisualizaArquivos.AdicionaArquivo(AArquivo, AArquivoPai, ATipo, ADescricao: String;
   ANumeroArquivos: Integer);
var
  AItemType: TItemType;
begin
  lblAndamento.Caption := 'Adicionado arquivos...';
  ADescricao := Trim(ADescricao);
  lblAndamento.Update;
  ggAndamentoGeral.Progress := ggAndamentoGeral.Progress +1;
  if Lista.Items.FileNameExists(AArquivo) then
  begin
    if Self.Showing and Self.Visible and Lista.Items.GetItemByFileName(AArquivo).Visible then
      Lista.Items.GetItemByFileName(AArquivo).SetFocus;
    Exit;
  end;
  case FTipoForm of
    tfEditar: AItemType := itEdit;
    tfExibir: if Pos('\', AArquivo) > 0 then
                AItemType := itNew
              else
                AItemType := itShow;
    tfVincular: AItemType := itNew;
  end;
  if Pos('\', AArquivo) > 0 then
    AArquivo := AArquivo
  else
    AArquivo := datmDigitalizacao.DigPath + AArquivo;
  with Lista.Items.AddParented(AArquivo, Lista.Items.GetItemByFileName(AArquivoPai)) do
  begin
    Lista.SortWindow;
    FileCount := ANumeroArquivos;
    {if Pos('\', AArquivo) > 0 then
      FileName := AArquivo
    else
      FileName := datmDigitalizacao.DigPath + AArquivo;} // Comentado e utilizando acima do WITH, pois agora passa o nome do arquivo na criação do item - Michel - 25/10/2010
    TypeDoc := ATipo;
    Description.Text := ADescricao;
    ItemType := AItemType;
    if (ItemType = itNew) and (ADescricao <> '') then
      DescriptionFontColor := clRed;
    Application.ProcessMessages;
  end;
end;

procedure TfrmVisualizaArquivos.AdicionaPasta(APasta: String);
var
  I: Integer;
  L1: TStrings;
begin
  if APasta[Length(APasta)] <> '\' then
    APasta := APasta + '\';
  L1 := ListaDiretorio(APasta);
  for I := 0 to L1.Count -1 do
    AdicionaArquivo(APasta + L1[I], '', '', '', 1);
end;

procedure TfrmVisualizaArquivos.btn_sairClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmVisualizaArquivos.Editar;
begin
  FTipoForm := tfEditar;
  ShowModal;
end;

procedure TfrmVisualizaArquivos.Exibir;
begin
  FTipoForm := tfExibir;
  ShowModal;
end;

procedure TfrmVisualizaArquivos.Alteracao(Sender: TObject);
begin
  btn_salvar.Enabled := (Lista.Items.Count > 0);
end;

procedure TfrmVisualizaArquivos.btn_salvarClick(Sender: TObject);
var
  vNomeArquivo: String;
  I: Integer;
  B: Boolean;

  function NomeArquivoPara(APrefixo, AExt: String): String;
  var
    A: Integer;
  begin
    // Pegar o índice para o nome do arquivo
    with TQuery.Create(Application) do
    begin
      DatabaseName := 'DBBROKER';
      SQL.Clear;
      SQL.Add('SELECT TOP 1 NM_ARQUIVO');
      SQL.Add('FROM TPROCESSO_DOCTOS');
      SQL.Add('WHERE NR_PROCESSO = :NR_PROCESSO');
      SQL.Add('  AND NM_ARQUIVO LIKE :PREFIXO + ''%''');
      SQL.Add('  AND ISNUMERIC(SUBSTRING(NM_ARQUIVO, LEN(:PREFIXO) + 1, 2)) = 1');
      SQL.Add('ORDER BY NM_ARQUIVO DESC');
      ParamByName('NR_PROCESSO').AsString := datmDigitalizacao.qry_ProcessoNR_PROCESSO.AsString;
      ParamByName('PREFIXO').AsString  := APrefixo;
      Open;
      if RecordCount = 0 then
        A := 0
      else
        A := StrToInt(Copy(Fields[0].AsString, Length(APrefixo) +1, 2));
      Close;
      Free;
    end;
    repeat
      Inc(A);
      Result := APrefixo + FormatFloat('00', A);
    until not FileExists(datmDigitalizacao.DigPath + Result + '.*');
    Result := AnsiUpperCase(Result + AExt);
  end;
begin
  lblAndamento.Caption := 'Salvando arquivos...';
  lblAndamento.Update;
  try
    if FTipoForm = tfEditar then
    begin
      datmDigitalizacao.qry_ListaArquivos.Edit;
      if Lista.Items[0].ReplaceFile then
      begin
        datmDigitalizacao.qry_ListaArquivosIN_ATIVO.AsString := '0';
        datmDigitalizacao.qry_ListaArquivosNM_MOTIVO_ALTERACAO.AsString := 'Substituído em edição';
        datmDigitalizacao.qry_ListaArquivos.Post;

        ggAndamentoGeral.Progress := ggAndamentoGeral.Progress +1;

        datmDigitalizacao.qry_ListaArquivos.Insert;
        //datmDigitalizacao.qry_ListaArquivosNR_PROCESSO.AsString        := datmDigitalizacao.qry_ProcessoNR_PROCESSO.AsString;
        datmDigitalizacao.qry_TipoArquivo.Locate('NM_TIPO_DOCTO', Lista.Items[0].TypeDoc, []);
        datmDigitalizacao.qry_ListaArquivosCD_TIPO_DOCTO.AsString      := datmDigitalizacao.qry_TipoArquivoCD_TIPO_DOCTO.AsString;
        datmDigitalizacao.qry_ListaArquivosNM_DESCRICAO.AsString       := Lista.Items[0].Description.Text;
        datmDigitalizacao.qry_ListaArquivosIN_ATIVO.AsString           := '1';

        // Nome Arquivo
        vNomeArquivo := NomeArquivoPara(datmDigitalizacao.qry_TipoArquivoNM_PREFIXO_ARQ.AsString, ExtractFileExt(Lista.Items[0].FileName));
        datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO.AsString := vNomeArquivo;
        if not CopiaArquivo(Lista.Items[0].FileName, datmDigitalizacao.DigPath + vNomeArquivo, False) then
        begin
          MessageDlg('Erro ao copiar o arquivo ''' + Lista.Items[0].FileName + '''.'#13'A operação não foi concluída.', mtError, [mbOk], 0);
          datmDigitalizacao.qry_ListaArquivos.CancelUpdates;
          Exit;
        end;
        datmDigitalizacao.qry_ListaArquivos.Post;
        Lista.Items[0].ReplaceFile := False;
        ggAndamentoGeral.Progress := ggAndamentoGeral.Progress +1;
      end
      else
      begin
        IniciaAndamento(2);
        ggAndamentoGeral.Progress := ggAndamentoGeral.Progress +1;
        datmDigitalizacao.qry_TipoArquivo.Locate('CD_TIPO_DOCTO', datmDigitalizacao.qry_ListaArquivosCD_TIPO_DOCTO.AsString, []);
        if Lista.Items[0].TypeDoc <> datmDigitalizacao.qry_TipoArquivoNM_TIPO_DOCTO.AsString then
        begin
          datmDigitalizacao.qry_TipoArquivo.Locate('NM_TIPO_DOCTO', Lista.Items[0].TypeDoc, []);
          datmDigitalizacao.qry_ListaArquivosCD_TIPO_DOCTO.AsString := datmDigitalizacao.qry_TipoArquivoCD_TIPO_DOCTO.AsString;
        end;
        datmDigitalizacao.qry_ListaArquivosNM_DESCRICAO.AsString  := Lista.Items[0].Description.Text;
        datmDigitalizacao.qry_ListaArquivos.Post;
        ggAndamentoGeral.Progress := ggAndamentoGeral.Progress +1;
        Lista.Items[0].CaptionFileName := ExtractFileName(vNomeArquivo);
      end;
      datmDigitalizacao.qry_ListaArquivos.ApplyUpdates;
      datmDigitalizacao.qry_ListaArquivos.Close;
      datmDigitalizacao.qry_ListaArquivos.Open;
      datmDigitalizacao.qry_ListaArquivos.Locate('NM_ARQUIVO', vNomeArquivo, []);
      btn_salvar.Enabled := False;
      Close;
    end
    else
    begin
      IniciaAndamento(2*Lista.Items.Count);
      // Roda primeiro os visiveis que poderão ter filhos e depois os não visiveis que são filhos
      // Tem que ser nesta ordem para não dar erro ao nomear o pai do filho no banco de dados
      for B := False to True do
      begin
        for I := 0 to Lista.Items.Count -1 do
        begin
          ggAndamentoGeral.Progress := ggAndamentoGeral.Progress +1;
          if Assigned(Lista.Items[I].ParentItem) = B then
          begin
            if (Lista.Items[I].ItemType = itNew) and (Lista.Items[I].Checked) then
            begin
              datmDigitalizacao.qry_ListaArquivos.Insert;
              //datmDigitalizacao.qry_ListaArquivosNR_PROCESSO.AsString        := datmDigitalizacao.qry_ProcessoNR_PROCESSO.AsString;
              datmDigitalizacao.qry_TipoArquivo.Locate('NM_TIPO_DOCTO', Lista.Items[I].TypeDoc, []);
              datmDigitalizacao.qry_ListaArquivosCD_TIPO_DOCTO.AsString      := datmDigitalizacao.qry_TipoArquivoCD_TIPO_DOCTO.AsString;
              datmDigitalizacao.qry_ListaArquivosIN_VISUALIZACAO.AsString    := '1';
              datmDigitalizacao.qry_ListaArquivosIN_ALTERACAO.AsString       := '1';
              datmDigitalizacao.qry_ListaArquivosNM_DESCRICAO.AsString       := Lista.Items[I].Description.Text;
              datmDigitalizacao.qry_ListaArquivosIN_ATIVO.AsString           := '1';
              if Assigned(Lista.Items[I].ParentItem) then
                datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO_PAI.AsString   := Lista.Items[I].ParentItem.CaptionFileName
              else
                datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO_PAI.AsVariant  := Null;

              vNomeArquivo := NomeArquivoPara(datmDigitalizacao.qry_TipoArquivoNM_PREFIXO_ARQ.AsString, ExtractFileExt(Lista.Items[I].FileName));
              datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO.AsString := vNomeArquivo;
              if not CopiaArquivo(Lista.Items[I].FileName, datmDigitalizacao.DigPath + vNomeArquivo, False) then
              begin
                MessageDlg('Erro ao copiar o arquivo ''' + Lista.Items[I].FileName + '''.'#13'A operação não foi concluída.', mtError, [mbOk], 0);
                datmDigitalizacao.qry_ListaArquivos.CancelUpdates;
                Exit;
              end;
              Lista.Items[I].CaptionFileName := AnsiUpperCase(vNomeArquivo);
              Application.ProcessMessages;
            end
            else if (Lista.Items[I].ItemType in [itShow, itEdit]) and (Lista.Items[I].Checked) then
            begin
              if datmDigitalizacao.qry_ListaArquivos.Locate('NM_ARQUIVO', Lista.Items[I].CaptionFileName, []) then
              begin
                datmDigitalizacao.qry_ListaArquivos.Edit;
                datmDigitalizacao.qry_ListaArquivosNM_DESCRICAO.AsString := Lista.Items[I].Description.Text;

                datmDigitalizacao.qry_TipoArquivo.Locate('CD_TIPO_DOCTO', datmDigitalizacao.qry_ListaArquivosCD_TIPO_DOCTO.AsString, []);
                if Lista.Items[I].TypeDoc <> datmDigitalizacao.qry_TipoArquivoNM_TIPO_DOCTO.AsString then
                begin
                  datmDigitalizacao.qry_TipoArquivo.Locate('NM_TIPO_DOCTO', Lista.Items[I].TypeDoc, []);
                  datmDigitalizacao.qry_ListaArquivosCD_TIPO_DOCTO.AsString := datmDigitalizacao.qry_TipoArquivoCD_TIPO_DOCTO.AsString;
                end;
              end;
            end;
            if (Lista.Items[I].Checked) and Assigned(Lista.Items[I].ParentItem) and (Trim(datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO_PAI.AsString) <> Trim(Lista.Items[I].ParentItem.CaptionFileName)) then
            begin
              if (datmDigitalizacao.qry_ListaArquivos.State in [dsEdit, dsInsert]) or
                 ((not (datmDigitalizacao.qry_ListaArquivos.State in [dsEdit, dsInsert])) and
                    datmDigitalizacao.qry_ListaArquivos.Locate('NM_ARQUIVO', Lista.Items[I].CaptionFileName, [])) then
              begin
                if (datmDigitalizacao.qry_ListaArquivos.State in [dsEdit, dsInsert]) then
                  datmDigitalizacao.qry_ListaArquivos.Edit;
                datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO_PAI.AsString := Lista.Items[I].ParentItem.CaptionFileName;
              end; // If Locate
            end; // Se o item tem pai e é diferente do que já está salvo
          end; // If Visibilidade
          if datmDigitalizacao.qry_ListaArquivos.State in [dsEdit, dsInsert] then
          begin
            datmDigitalizacao.qry_ListaArquivos.Post;
            datmDigitalizacao.qry_ListaArquivos.ApplyUpdates;
            Lista.Items[I].ItemType := itShow;
          end;
          Application.ProcessMessages;
        end; // Loop para inserir e editar
      end; // Loop de visibilidade
    end;
  except
    on E: Exception do
    begin
      MessageDlg('Erro efetuando a operação.'#13'Alguns ítens podem não ter sido salvos.'#13 +
         'Mensagem original: ' + E.Message, mtError, [mbOk], 0);
      FinalizaAndamento;
      Exit;
    end;
  end;
  btn_salvar.Enabled := False;
  FinalizaAndamento;
end;

procedure TfrmVisualizaArquivos.btnScannerClick(Sender: TObject);
var
  SelectedSource: Integer;
begin
  ParentFileName := '';
  if dtScanner.LibraryLoaded then
    dtScanner.UnloadLibrary;
  if dtScanner.LoadLibrary then
  begin
    {Carregar o gerenciador do Scanner}
    dtScanner.SourceManagerLoaded := True;
    {Permitir que o usuário selecione o Scanner}
    SelectedSource := dtScanner.SelectSource;
    if SelectedSource <> -1 then
    begin
      dtScanner.Source[SelectedSource].Loaded       := True;
      dtScanner.Source[SelectedSource].TransferMode := ttmMemory;
      dtScanner.Source[SelectedSource].Enabled      := True;
    end; {if SelectedSource <> -1}
  end
  else
    ShowMessage('Falta componente para que possa utilizar esta opção.'#13 +
      'Verifique se há um Scanner conectado ao computador e se está ligado corretamente.'#13 +
      'Caso esteja tudo correto e o erro persista, informe ao TI.');
end;

procedure TfrmVisualizaArquivos.dtScannerAcquireError(Sender: TObject;
  const Index: Integer; ErrorCode, Additional: Integer);
begin
  MessageDlg('Erro ao digitalizar o documento.'#13'Tente novamente ou informe ao TI.', mtError, [mbOk], 0);
end;

procedure TfrmVisualizaArquivos.dtScannerTwainAcquire(Sender: TObject;
  const Index: Integer; Image: TBitmap; var Cancel: Boolean);
var
  I: Integer;
  JPG: TJPEGImage;
  ScanFileName: TFileName;
begin
  JPG := TJPEGImage.Create;
  JPG.CompressionQuality := 50;
  I := 0;
  repeat
    Inc(I);
    ScanFileName := datmDigitalizacao.ScanPath + 'Imagem' + IntToStr(I) + '.jpg';
  until not FileExists(ScanFileName);

  JPG.Assign(Image);
  JPG.SaveToFile(ScanFileName);

  with Lista.Items.AddParented(ScanFileName, Lista.Items.GetItemByFileName(ParentFileName)) do
  begin
    //FileName := ScanFileName;
    //ParentItem := Lista.Items.GetItemByFileName(ParentFileName);
    ParentFileName := IfThen(ParentFileName = '', ScanFileName, ParentFileName);
  end;

  JPG.Free;
end;

procedure TfrmVisualizaArquivos.IniciaAndamento(Max: Integer);
begin
  ggAndamentoGeral.MaxValue := Max;
  ggAndamentoGeral.Progress := 0;
  pnlAndamento.Show;
end;

procedure TfrmVisualizaArquivos.FormActivate(Sender: TObject);
begin
  Application.ProcessMessages;
  datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO.ReadOnly := False;
  if FTipoForm = tfEditar then
  begin
    AdicionaArquivo(datmDigitalizacao.qry_ListaArquivosNM_ARQUIVO.AsString, '',
       datmDigitalizacao.qry_ListaArquivosNM_TIPO_DOCTO.AsString, datmDigitalizacao.qry_ListaArquivosNM_DESCRICAO.AsString,
       datmDigitalizacao.qry_ListaArquivosARQUIVOS.AsInteger);
    IniciaAndamento(1);
  end
  else if FTipoForm = tfExibir then
  begin                                                      
    FWindowProc           := ScrollBox1.WindowProc;
    ScrollBox1.WindowProc := ShellWindowProc;
    DragAcceptFiles(ScrollBox1.Handle, True);

    with TQuery.Create(Self) do
    begin
      // Seleciona a quantidade total de arquivos para a barra de progresso
      DatabaseName := 'DBBROKER';
      SQL.Add('SELECT COUNT(*)');
      SQL.Add('FROM TPROCESSO_DOCTOS PD INNER JOIN TCONTROLE_ACESSO_DIGITALIZACAO CAD ON CAD.ID_TABELA     = PD.ID_TABELA');
      SQL.Add('                                                                      AND CAD.CD_TIPO_DOCTO = PD.CD_TIPO_DOCTO');
      SQL.Add('WHERE PD.NR_PROCESSO                  = :NR_PROCESSO');
      SQL.Add('  AND PD.ID_TABELA                    = :ID_TABELA');
      SQL.Add('  AND CAD.CD_CARGO                    = :CD_CARGO');
      SQL.Add('  AND PD.IN_ATIVO                     = ''1''');
      SQL.Add('  AND ISNULL(CAD.IN_ALTERACAO, ''0'') = ''1''');
      ParamByName('NR_PROCESSO').AsString := datmDigitalizacao.qry_ListaArquivosNR_PROCESSO.AsString;
      ParamByName('CD_CARGO').AsString    := str_cd_cargo;
      ParamByName('ID_TABELA').AsInteger  := datmDigitalizacao.qry_ListaArquivosID_TABELA.AsInteger;
      Open;
      IniciaAndamento(Fields[0].AsInteger);
      Close;

      // Adiciona os Arquivos Pai
      SQL.Clear;
      SQL.Add('SELECT PD.*, TDD.NM_TIPO_DOCTO,');
      SQL.Add('       (SELECT COUNT(*)');
      SQL.Add('        FROM TPROCESSO_DOCTOS PD1');
      SQL.Add('        WHERE PD1.NR_PROCESSO = PD.NR_PROCESSO');
      SQL.Add('          AND PD1.NM_ARQUIVO_PAI = PD.NM_ARQUIVO)+1 AS PAGINAS');
      SQL.Add('FROM TPROCESSO_DOCTOS PD INNER JOIN TTP_DOCTO_DIGITALIZADO TDD ON TDD.CD_TIPO_DOCTO = PD.CD_TIPO_DOCTO');
      SQL.Add('                                                              AND TDD.ID_TABELA     = PD.ID_TABELA');
      SQL.Add('   INNER JOIN TCONTROLE_ACESSO_DIGITALIZACAO CAD ON CAD.ID_TABELA     = PD.ID_TABELA');
      SQL.Add('                                                AND CAD.CD_TIPO_DOCTO = PD.CD_TIPO_DOCTO');
      SQL.Add('WHERE NR_PROCESSO                     = :NR_PROCESSO');
      SQL.Add('  AND PD.ID_TABELA                    = :ID_TABELA');
      SQL.Add('  AND CAD.CD_CARGO                    = :CD_CARGO');
      SQL.Add('  AND PD.IN_ATIVO                     = ''1''');
      SQL.Add('  AND ISNULL(CAD.IN_ALTERACAO, ''0'') = ''1''');
      SQL.Add('  AND ISNULL(NM_ARQUIVO_PAI, '''')    = ''''');
      SQL.Add('ORDER BY TDD.NM_TIPO_DOCTO');
      ParamByName('NR_PROCESSO').AsString := datmDigitalizacao.qry_ListaArquivosNR_PROCESSO.AsString;
      ParamByName('CD_CARGO').AsString    := str_cd_cargo;
      ParamByName('ID_TABELA').AsInteger  := datmDigitalizacao.qry_ListaArquivosID_TABELA.AsInteger;
      Open;
      First;
      while not Eof do
      begin
        // Ve primeiro se encontra o tipo de documento na lista principal, pois lá está com descrição de envio para a web - Michel - 06/01/2010
        if datmDigitalizacao.qry_TipoArquivo.Locate('CD_TIPO_DOCTO', FieldByName('CD_TIPO_DOCTO').AsString, []) then
          AdicionaArquivo(FieldByName('NM_ARQUIVO').AsString, '',
             datmDigitalizacao.qry_TipoArquivoNM_TIPO_DOCTO.AsString, FieldByName('NM_DESCRICAO').AsString, 1)
        else
          AdicionaArquivo(FieldByName('NM_ARQUIVO').AsString, '',
             FieldByName('NM_TIPO_DOCTO').AsString, FieldByName('NM_DESCRICAO').AsString, 1);
        Next;
        Application.ProcessMessages;
      end;
      Close;

      // Adiciona os Arquivos Filho
      SQL.Clear;
      SQL.Add('SELECT PD.*, TDD.NM_TIPO_DOCTO,');
      SQL.Add('       (SELECT COUNT(*)');
      SQL.Add('        FROM TPROCESSO_DOCTOS PD1');
      SQL.Add('        WHERE PD1.NR_PROCESSO = PD.NR_PROCESSO');
      SQL.Add('          AND PD1.NM_ARQUIVO_PAI = PD.NM_ARQUIVO)+1 AS PAGINAS');
      SQL.Add('FROM TPROCESSO_DOCTOS PD INNER JOIN TTP_DOCTO_DIGITALIZADO TDD ON TDD.CD_TIPO_DOCTO = PD.CD_TIPO_DOCTO');
      SQL.Add('                                                              AND TDD.ID_TABELA     = PD.ID_TABELA');
      SQL.Add('   INNER JOIN TCONTROLE_ACESSO_DIGITALIZACAO CAD ON CAD.ID_TABELA     = PD.ID_TABELA');
      SQL.Add('                                                AND CAD.CD_TIPO_DOCTO = PD.CD_TIPO_DOCTO');
      SQL.Add('WHERE NR_PROCESSO                     = :NR_PROCESSO');
      SQL.Add('  AND PD.ID_TABELA                    = :ID_TABELA');
      SQL.Add('  AND CAD.CD_CARGO                    = :CD_CARGO');
      SQL.Add('  AND PD.IN_ATIVO                     = ''1''');
      SQL.Add('  AND ISNULL(CAD.IN_ALTERACAO, ''0'') = ''1''');
      SQL.Add('  AND ISNULL(NM_ARQUIVO_PAI, '''')   <> ''''');
      SQL.Add('ORDER BY TDD.NM_TIPO_DOCTO');
      ParamByName('NR_PROCESSO').AsString := datmDigitalizacao.qry_ListaArquivosNR_PROCESSO.AsString;
      ParamByName('CD_CARGO').AsString    := str_cd_cargo;
      ParamByName('ID_TABELA').AsInteger  := datmDigitalizacao.qry_ListaArquivosID_TABELA.AsInteger;
      Open;
      First;
      while not Eof do
      begin
        // Ve primeiro se encontra o tipo de documento na lista principal, pois lá está com descrição de envio para a web - Michel - 06/01/2010
        if datmDigitalizacao.qry_TipoArquivo.Locate('CD_TIPO_DOCTO', FieldByName('CD_TIPO_DOCTO').AsString, []) then
          AdicionaArquivo(FieldByName('NM_ARQUIVO').AsString, FieldByName('NM_ARQUIVO_PAI').AsString,
             datmDigitalizacao.qry_TipoArquivoNM_TIPO_DOCTO.AsString, FieldByName('NM_DESCRICAO').AsString, 1)
        else
          AdicionaArquivo(FieldByName('NM_ARQUIVO').AsString, FieldByName('NM_ARQUIVO_PAI').AsString,
             FieldByName('NM_TIPO_DOCTO').AsString, FieldByName('NM_DESCRICAO').AsString, 1);
        Next;
        Application.ProcessMessages;
      end;
      Close;
    end; // TQuery
  end;
  Lista.OnChange := Alteracao;
  FinalizaAndamento;
end;

procedure TfrmVisualizaArquivos.FinalizaAndamento;
begin
  pnlAndamento.Hide;
end;

procedure TfrmVisualizaArquivos.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_ESCAPE then
    Close;
end;

procedure TfrmVisualizaArquivos.Vincular(AArquivo: TFileName; ATipoArquivo: String);
begin
  FTipoForm := tfVincular;
  AdicionaArquivo(AArquivo, '', ATipoArquivo, ExtractFileName(ChangeFileExt(AArquivo, '')), 1);
  btn_salvar.Enabled := True;
  if not Showing then
    ShowModal;
end;

procedure TfrmVisualizaArquivos.chkMiniaturasClick(Sender: TObject);
begin
  IniciaAndamento(Lista.Items.Count);
  lblAndamento.Caption := 'Atualizando imagens...';
  pnlAndamento.Update;
  Application.ProcessMessages;
  if chkMiniaturas.Checked then
    Lista.Items.ImageStyle := isShow
  else
    Lista.Items.ImageStyle := isNone;
  FinalizaAndamento;
end;

procedure TfrmVisualizaArquivos.ChangeImages(Sender: TObject; MaxProgress,
  CurrentProgress: Int64);
begin
  ggAndamentoGeral.MaxValue := MaxProgress;
  ggAndamentoGeral.Progress := CurrentProgress;
  pnlAndamento.Update;
  Application.ProcessMessages;
end;

procedure TfrmVisualizaArquivos.btn_novoClick(Sender: TObject);
begin
  if Lista.DefaultTypeItems.Count = 0 then
    MessageDlg('Você não tem habilitação para inserir arquivos.'#13'Solicite ao TI.', mtInformation, [mbOk], 0)
  else if OpenDialog1.Execute then
    AdicionaArquivo(OpenDialog1.FileName, '', '', ExtractFileName(ChangeFileExt(OpenDialog1.FileName, '')), 1);
end;

procedure TfrmVisualizaArquivos.ExcluiArquivo(AArquivo: String);
begin
  Lista.Items.DeleteByFileName(AArquivo);
end;

end.
