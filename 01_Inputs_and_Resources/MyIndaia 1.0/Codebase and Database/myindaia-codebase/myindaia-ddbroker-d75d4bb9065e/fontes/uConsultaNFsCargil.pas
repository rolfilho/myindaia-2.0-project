...
uses
  ...
  cxGridCustomPopupMenu, cxGridPopupMenu, uConsultaNFsCargilCtx, dxPSGlbl,
...
type
  TfmConsultaNFsCargil = class(TForm)
    Panel1: TPanel;
    ...
    cxGridDBTableViewDUE: TcxGridDBColumn;
    cxGridDBTableViewNFNumero: TcxGridDBColumn;
    cxGridDBTableViewNFItem: TcxGridDBColumn;
    cxGridDBTableViewNFCodMercadoria: TcxGridDBColumn;
    cxGridDBTableViewNFDescrio: TcxGridDBColumn;
    cxGridDBTableViewNFPesoLiquido: TcxGridDBColumn;
    cxGridDBTableViewNFNCM: TcxGridDBColumn;
    cxGridDBTableViewNFQtdeComercializada: TcxGridDBColumn;
    cxGridDBTableViewNFUnidadeComercializada: TcxGridDBColumn;
    cxGridDBTableViewNFReferenciadaItem: TcxGridDBColumn;
    cxGridDBTableViewNFReferenciadaChavedeAces: TcxGridDBColumn;
    cxGridDBTableViewNFReferenciadaQtdeTributav: TcxGridDBColumn;
    qryMainNrProcesso: TStringField;
    qryMainNrPedido: TStringField;
    qryMainDUE: TStringField;
    qryMainNFNumero: TStringField;
    qryMainNFItem: TStringField;
    qryMainNFCodMercadoria: TStringField;
    qryMainNFDescrio: TMemoField;
    qryMainNFPesoLiquido: TFloatField;
    qryMainNFNCM: TStringField;
    qryMainNFQtdeComercializada: TFloatField;
    qryMainNFUnidadeComercializada: TStringField;
    qryMainNFReferenciadaItem: TIntegerField;
    qryMainNFReferenciadaChavedeAces: TMemoField;
    qryMainNFReferenciadaQtdeTributav: TFloatField;
...
{ TfmConsultaNFsCargil }

class procedure TfmConsultaNFsCargil.Execute;
var
  Form: TfmConsultaNFsCargil;
begin
  Form := TfmConsultaNFsCargil.Create(nil);
  try
    Form.ShowModal;
  finally
    Form.Free;
...
function TfmConsultaNFsCargil.GetSQL: string;
begin
  Result :=
            ' SELECT ' +
...
procedure TfmConsultaNFsCargil.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfmConsultaNFsCargil.QueryContext;
var
  Filtros: string;
  CursorBk: TCursor;
begin
  if TfmConsultaNFsCargilCtx.Executa(Opcoes) then
  begin
    CursorBk := Cursor;
    Cursor := crSQLWait;
    Caption := 'Consulta NFs Cargil - Por favor aguarde...';
    try
      qryMain.Close;
...
//
      ...
      Caption := 'Consulta NFs Cargil';
      Cursor := CursorBk;
    end;
  end;
end;

procedure TfmConsultaNFsCargil.FormCreate(Sender: TObject);
begin
  Opcoes := TOpcoesCtx.Create;
end;

procedure TfmConsultaNFsCargil.FormDestroy(Sender: TObject);
begin
  Opcoes.Free;
end;

procedure TfmConsultaNFsCargil.BitBtn2Click(Sender: TObject);
begin
  QueryContext;
end;

procedure TfmConsultaNFsCargil.FormActivate(Sender: TObject);
begin
  QueryContext;
end;

procedure TfmConsultaNFsCargil.BitBtn1Click(Sender: TObject);
begin
  Close;
end;

procedure TfmConsultaNFsCargil.BitBtn3Click(Sender: TObject);
var
  FileExt: String;
begin
...
procedure TfmConsultaNFsCargil.BitBtn4Click(Sender: TObject);
begin
...
procedure TfmConsultaNFsCargil.qryMainPL_MERCADORIAGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
...
procedure TfmConsultaNFsCargil.qryMainIN_UTILIZADOGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
  Text := IntFldSimNao(Sender);
end;

procedure TfmConsultaNFsCargil.qryMainIN_SUBSTITUIDAGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
  Text := IntFldSimNao(Sender);
end;

procedure TfmConsultaNFsCargil.SalvarLeiaute1Click(Sender: TObject);
var
  Nome: string;
  TituloRel: string;
  Ini: TMemIniFile;
...
procedure TfmConsultaNFsCargil.CarregarLeiaute1Click(Sender: TObject);
begin
...
procedure TfmConsultaNFsCargil.dxComponentPrinterLink1MeasureReportLinkTitle(
  Sender: TBasedxReportLink; var AHeight: Integer);
begin
  AHeight := 2 * PenWidth + FontHeight;
end;

procedure TfmConsultaNFsCargil.dxComponentPrinterLink1CustomDrawPageHeader(
...
procedure TfmConsultaNFsCargil.qryMainIN_CANCELADOGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
