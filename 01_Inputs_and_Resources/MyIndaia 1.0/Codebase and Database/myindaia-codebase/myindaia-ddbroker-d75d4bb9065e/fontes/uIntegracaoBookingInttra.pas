...
type
  ...
  TfmIntegracaoBookingInttra = class(TForm)
...
{ TfmIntegracaoBookingInttra }


procedure TfmIntegracaoBookingInttra.BitBtn1Click(Sender: TObject);
var
  MyBrowserPath, MyBrowser: string;
  NrProcesso: string;
begin
  MyBrowserPath := ExtractFilePath(paramStr(0)) + 'MiniBroker\MiniBrowser\';
  MyBrowser := MyBrowserPath + 'MiniBrowser.exe';
  ...
  StartApp('Open', PChar(MyBrowser), PChar('BOOKING ' + msk_cd_unid_neg.Text + '02' + msk_nr_processo.Text), PChar(MyBrowserPath), SW_SHOW);

end;

procedure TfmIntegracaoBookingInttra.BitBtn2Click(Sender: TObject);
begin
  Close;
  ModalResult := mrCancel;
end;

procedure TfmIntegracaoBookingInttra.btn_co_unid_negClick(Sender: TObject);
begin
...
procedure TfmIntegracaoBookingInttra.FormActivate(Sender: TObject);
begin
  msk_cd_unid_neg.Text := str_cd_unid_neg;
  btn_co_unid_negClick(Sender);
end;

procedure TfmIntegracaoBookingInttra.msk_cd_unid_negExit(Sender: TObject);
begin
  ValCodEdt( msk_cd_unid_neg );
end;

procedure TfmIntegracaoBookingInttra.btn_nr_processoClick(Sender: TObject);
var
