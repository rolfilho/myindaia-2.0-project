object datm_sel_unid_prod: Tdatm_sel_unid_prod
  OldCreateOrder = True
  Left = 190
  Top = 48
  Height = 780
  Width = 1076
  object qry_unid_neg_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CD_UNID_NEG, NM_UNID_NEG'
      'FROM   TUNID_NEG '
      'WHERE  CD_UNID_NEG = :CD_UNID_NEG')
    Left = 40
    Top = 584
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_UNID_NEG'
        ParamType = ptUnknown
      end>
    object qry_unid_neg_CD_UNID_NEG: TStringField
      FieldName = 'CD_UNID_NEG'
      Origin = 'TUNID_NEG.CD_UNID_NEG'
      Size = 2
    end
    object qry_unid_neg_NM_UNID_NEG: TStringField
      FieldName = 'NM_UNID_NEG'
      Origin = 'TUNID_NEG.NM_UNID_NEG'
      Size = 50
    end
  end
  object qry_produto_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CD_PRODUTO, NM_PRODUTO'
      'FROM   TPRODUTO'
      'WHERE  CD_PRODUTO = :CD_PRODUTO')
    Left = 138
    Top = 584
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_PRODUTO'
        ParamType = ptUnknown
      end>
    object qry_produto_CD_PRODUTO: TStringField
      FieldName = 'CD_PRODUTO'
      Origin = 'TPRODUTO.CD_PRODUTO'
      Size = 2
    end
    object qry_produto_NM_PRODUTO: TStringField
      FieldName = 'NM_PRODUTO'
      Origin = 'TPRODUTO.NM_PRODUTO'
      Size = 40
    end
  end
  object qry_cliente_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CD_EMPRESA, NM_EMPRESA, AP_EMPRESA'
      'FROM TEMPRESA_NAC '
      'WHERE CD_EMPRESA = :CD_CLIENTE ')
    Left = 375
    Top = 586
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_CLIENTE'
        ParamType = ptUnknown
      end>
    object qry_cliente_CD_EMPRESA: TStringField
      FieldName = 'CD_EMPRESA'
      Origin = 'TEMPRESA_NAC.CD_EMPRESA'
      Size = 5
    end
    object qry_cliente_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      Origin = 'TEMPRESA_NAC.NM_EMPRESA'
      Size = 50
    end
    object qry_cliente_AP_EMPRESA: TStringField
      FieldName = 'AP_EMPRESA'
      Origin = 'TEMPRESA_NAC.AP_EMPRESA'
      Size = 10
    end
  end
  object qry_param_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CD_ITEM_COMISSAO, CD_ITEM_EXPED'
      'FROM TPARAMETROS')
    Left = 375
    Top = 643
    object qry_param_CD_ITEM_COMISSAO: TStringField
      FieldName = 'CD_ITEM_COMISSAO'
      Origin = 'TPARAMETROS.CD_ITEM_COMISSAO'
      Size = 3
    end
    object qry_param_CD_ITEM_EXPED: TStringField
      FieldName = 'CD_ITEM_EXPED'
      Origin = 'TPARAMETROS.CD_ITEM_EXPED'
      Size = 3
    end
  end
  object qry_flp_plan_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_ord_ref_cli_
    SQL.Strings = (
      'SELECT'
      '    P.NR_PROCESSO, P.CD_UNID_NEG, P.CD_PRODUTO,'
      '    P.CD_CLIENTE, P.DT_ABERTURA, P.TX_MERCADORIA,'
      '    P.TX_OBSERVACOES, P.NR_VOO, P.NR_CONHECIMENTO,'
      '    E.CD_EMPRESA, E.NM_EMPRESA,'
      '    B.NM_EMBARCACAO,'
      '    PD.DT_REALIZACAO DT_PREV_DESEMB,'
      '    PC.DT_REALIZACAO DT_PREV_CHEGADA,'
      '    ETD.DT_REALIZACAO DT_ETD,'
      '    D.DT_REALIZACAO DT_DESEMB,'
      '    C.DT_REALIZACAO DT_CHEGADA,'
      '    R.DT_REALIZACAO DT_REG_DI'
      'FROM'
      '    TPROCESSO P,'
      '    TEMPRESA_NAC E,'
      '    TEMBARCACAO B,'
      '    VW_DT_PREV_DESEMB PD,'
      '    VW_DT_PREV_CHEGADA PC,'
      '    VW_DT_ETD ETD,'
      '    VW_DT_DESEMB D,'
      '    VW_DT_CHEGADA C,'
      '    VW_DT_REG_DI R'
      'WHERE'
      '    P.CD_CLIENTE = E.CD_EMPRESA AND'
      '    P.CD_EMBARCACAO *= B.CD_EMBARCACAO AND'
      '    P.NR_PROCESSO *= D.NR_PROCESSO AND'
      '    P.NR_PROCESSO *= C.NR_PROCESSO AND'
      '    P.NR_PROCESSO *= PD.NR_PROCESSO AND'
      '    P.NR_PROCESSO *= PC.NR_PROCESSO AND'
      '    P.NR_PROCESSO *= R.NR_PROCESSO AND'
      '    P.NR_PROCESSO *= ETD.NR_PROCESSO AND'
      '    P.NR_PROCESSO = :NR_PROCESSO')
    Left = 264
    Top = 336
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_flp_plan_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Size = 18
    end
    object qry_flp_plan_CD_UNID_NEG: TStringField
      FieldName = 'CD_UNID_NEG'
      Size = 2
    end
    object qry_flp_plan_CD_PRODUTO: TStringField
      FieldName = 'CD_PRODUTO'
      Size = 2
    end
    object qry_flp_plan_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      Size = 5
    end
    object qry_flp_plan_DT_ABERTURA: TDateTimeField
      FieldName = 'DT_ABERTURA'
    end
    object qry_flp_plan_TX_MERCADORIA: TMemoField
      FieldName = 'TX_MERCADORIA'
      BlobType = ftMemo
    end
    object qry_flp_plan_TX_OBSERVACOES: TMemoField
      FieldName = 'TX_OBSERVACOES'
      BlobType = ftMemo
    end
    object qry_flp_plan_NR_VOO: TStringField
      FieldName = 'NR_VOO'
      Size = 18
    end
    object qry_flp_plan_CD_EMPRESA: TStringField
      FieldName = 'CD_EMPRESA'
      Size = 5
    end
    object qry_flp_plan_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      Size = 50
    end
    object qry_flp_plan_NM_EMBARCACAO: TStringField
      FieldName = 'NM_EMBARCACAO'
      Size = 40
    end
    object qry_flp_plan_DT_PREV_DESEMB: TDateTimeField
      FieldName = 'DT_PREV_DESEMB'
    end
    object qry_flp_plan_DT_PREV_CHEGADA: TDateTimeField
      FieldName = 'DT_PREV_CHEGADA'
    end
    object qry_flp_plan_DT_ETD: TDateTimeField
      FieldName = 'DT_ETD'
    end
    object qry_flp_plan_DT_DESEMB: TDateTimeField
      FieldName = 'DT_DESEMB'
    end
    object qry_flp_plan_DT_CHEGADA: TDateTimeField
      FieldName = 'DT_CHEGADA'
    end
    object qry_flp_plan_DT_REG_DI: TDateTimeField
      FieldName = 'DT_REG_DI'
    end
    object qry_flp_plan_NR_CONHECIMENTO: TStringField
      FieldName = 'NR_CONHECIMENTO'
      Size = 30
    end
  end
  object qry_ref_cli_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_flp_plan
    SQL.Strings = (
      'SELECT CD_REFERENCIA '
      'FROM TREF_CLIENTE'
      'WHERE NR_PROCESSO = :NR_PROCESSO'
      'ORDER BY CD_REFERENCIA')
    Left = 144
    Top = 384
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_ref_cli_CD_REFERENCIA: TStringField
      FieldName = 'CD_REFERENCIA'
      Origin = 'TREF_CLIENTE.CD_REFERENCIA'
      Size = 15
    end
  end
  object ds_flp_plan: TDataSource
    DataSet = qry_flp_plan_
    Left = 144
    Top = 288
  end
  object qry_exp_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_flp_plan
    SQL.Strings = (
      'SELECT NM_FORN_ESTR'
      'FROM TADICAO_DE_IMPORTACAO'
      'WHERE NR_PROCESSO = :NR_PROCESSO AND'
      'NR_ADICAO = '#39'001'#39)
    Left = 376
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_exp_NM_FORN_ESTR: TStringField
      FieldName = 'NM_FORN_ESTR'
      Origin = 'TADICAO_DE_IMPORTACAO.NM_FORN_ESTR'
      Size = 60
    end
  end
  object qry_embalagem_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_flp_plan
    SQL.Strings = (
      'SELECT*'
      'FROM TPROCESSO_EMBALAGEM P, TTP_EMBALAGEM E'
      'WHERE P.NR_PROCESSO = :NR_PROCESSO AND'
      'P.CD_EMBALAGEM = E.CODIGO'
      '')
    Left = 504
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_embalagem_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TPROCESSO_EMBALAGEM.NR_PROCESSO'
      Size = 18
    end
    object qry_embalagem_CD_EMBALAGEM: TStringField
      FieldName = 'CD_EMBALAGEM'
      Origin = 'TPROCESSO_EMBALAGEM.CD_EMBALAGEM'
      Size = 2
    end
    object qry_embalagem_PB_EMBALAGEM: TFloatField
      FieldName = 'PB_EMBALAGEM'
      Origin = 'TPROCESSO_EMBALAGEM.PB_EMBALAGEM'
    end
    object qry_embalagem_QT_EMBALAGEM: TIntegerField
      FieldName = 'QT_EMBALAGEM'
      Origin = 'TPROCESSO_EMBALAGEM.QT_EMBALAGEM'
    end
    object qry_embalagem_Codigo: TStringField
      FieldName = 'Codigo'
      Origin = 'TPROCESSO_EMBALAGEM.NR_PROCESSO'
      Size = 2
    end
    object qry_embalagem_Descricao: TStringField
      FieldName = 'Descricao'
      Origin = 'TPROCESSO_EMBALAGEM.CD_EMBALAGEM'
      Size = 120
    end
  end
  object qry_li_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_flp_plan
    SQL.Strings = (
      'SELECT COUNT(NR_OPER_TRAT_PREV) TOT_LI'
      'FROM TADICAO_DE_IMPORTACAO'
      'WHERE NR_PROCESSO = :NR_PROCESSO AND'
      'ISNULL(NR_OPER_TRAT_PREV, '#39#39') <> '#39#39)
    Left = 40
    Top = 384
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_li_TOT_LI: TIntegerField
      FieldName = 'TOT_LI'
      Origin = 'TADICAO_DE_IMPORTACAO.NR_OPER_TRAT_PREV'
    end
  end
  object qry_ord_ref_cli_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT P.NR_PROCESSO,'
      '  (SELECT MIN(CD_REFERENCIA)'
      '     FROM TREF_CLIENTE'
      '     WHERE NR_PROCESSO = P.NR_PROCESSO) MENOR'
      'FROM TPROCESSO P,'
      '     TEMPRESA_NAC E,'
      '     VW_DT_DESEMB D,'
      '     TSERVICO S'
      'WHERE P.NR_PROCESSO = D.NR_PROCESSO AND'
      'P.CD_CLIENTE = E.CD_EMPRESA AND'
      'P.IN_CANCELADO = '#39'0'#39' AND'
      'P.IN_PRODUCAO = '#39'1'#39' AND'
      '( D.DT_REALIZACAO = NULL OR'
      '  D.DT_REALIZACAO > DATEADD(DAY, -7, GETDATE()) ) AND'
      'S.CD_SERVICO = P.CD_SERVICO AND'
      'S.IN_FOLLOWUP = '#39'1'#39' AND'
      '( P.CD_CLIENTE  = :CD_CLIENTE ) AND'
      '( ( E.CD_GRUPO = :CD_GRUPO ) OR ( :CD_GRUPO = '#39#39' ) ) AND'
      
        '( ( P.CD_UNID_NEG = :CD_UNID_NEG ) OR ( :CD_UNID_NEG = '#39#39' ) ) AN' +
        'D'
      '( ( P.CD_PRODUTO = :CD_PRODUTO) OR ( :CD_PRODUTO = '#39#39' ) )'
      'ORDER BY (SELECT MIN(CD_REFERENCIA)'
      '            FROM TREF_CLIENTE'
      '            WHERE NR_PROCESSO = P.NR_PROCESSO)')
    Left = 264
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_CLIENTE'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_GRUPO'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_GRUPO'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_UNID_NEG'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_UNID_NEG'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_PRODUTO'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_PRODUTO'
        ParamType = ptUnknown
      end>
    object qry_ord_ref_cli_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TREF_CLIENTE.NR_PROCESSO'
      Size = 18
    end
    object qry_ord_ref_cli_MENOR: TStringField
      FieldName = 'MENOR'
      Origin = 'TREF_CLIENTE.CD_REFERENCIA'
      Size = 15
    end
  end
  object ds_ord_ref_cli_: TDataSource
    DataSet = qry_ord_ref_cli_
    Left = 264
    Top = 48
  end
  object sp_tot_serv_cli_periodo: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_tot_serv_cli_periodo'
    Left = 144
    Top = 96
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_ini'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_area'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@legenda'
        ParamType = ptOutput
      end>
  end
  object qry_grupo_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT *'
      'FROM TGRUPO'
      'WHERE CD_GRUPO = :CD_GRUPO ')
    Left = 264
    Top = 585
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_GRUPO'
        ParamType = ptUnknown
      end>
    object qry_grupo_CD_GRUPO: TStringField
      FieldName = 'CD_GRUPO'
      Origin = 'TGRUPO.CD_GRUPO'
      Size = 3
    end
    object qry_grupo_NM_GRUPO: TStringField
      FieldName = 'NM_GRUPO'
      Origin = 'TGRUPO.NM_GRUPO'
      Size = 50
    end
    object qry_grupo_DT_INCLUSAO: TDateTimeField
      FieldName = 'DT_INCLUSAO'
      Origin = 'TGRUPO.DT_INCLUSAO'
    end
    object qry_grupo_IN_ATIVO: TStringField
      FieldName = 'IN_ATIVO'
      Origin = 'TGRUPO.IN_ATIVO'
      Size = 1
    end
  end
  object sp_rel_desp_pagto: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_desp_pagto'
    Left = 264
    Top = 96
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object sp_andamento_: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_aux_andamento'
    Left = 144
    Top = 336
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_unidade'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object sp_rel_num_nao_rec: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_num_nao_rec'
    Left = 264
    Top = 384
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_ini'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object sp_rel_ctrl_demurrage: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_ctrl_demurrage'
    Left = 616
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object sp_rel_req_pagto: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_req_pagtos'
    Left = 40
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@nr_docto'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_pagto'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object sp_rel_fat_pend_boleto: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_fat_pend_boleto'
    Left = 264
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object sp_rel_est_mercadoria: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_est_mercadoria'
    Left = 144
    Top = 192
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 2
      end>
  end
  object sp_rel_fat_pago_cli: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_fat_pago_cli'
    Left = 40
    Top = 192
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object qry_rel_est_mercadoria_imp_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      
        'SELECT R.NR_IDENTIFICADOR, R.CD_UNID_NEG, U.NM_UNID_NEG, R.CD_PR' +
        'ODUTO, R.CD_CLIENTE, E.NM_EMPRESA AS NM_CLIENTE,'
      
        '       R.CD_NCM_SH, R.CD_MERCADORIA, M.AP_MERCADORIA, R.NR_DI, R' +
        '.NR_ADICAO, R.NR_PEDIDO,'
      
        '       R.NR_ITEM_PEDIDO, R.NR_ATO_DRAWBACK, R.VL_QTDE, R.VL_FOB,' +
        ' R.VL_FRETE, R.VL_SEGURO,'
      
        '       R.VL_PESO_LIQUIDO, R.CD_REG_TRIB_II, T.DESCRICAO AS NM_RE' +
        'G_TRIB_II, R.VL_ALIQ_II, R.VL_II,'
      '       R.CD_REG_TRIB_IPI,'
      '       CASE R.CD_REG_TRIB_IPI'
      '          WHEN '#39'4'#39' THEN '#39'Recolhimento Integral'#39
      '          WHEN '#39'5'#39' THEN '#39'Suspens'#227'o'#39
      '          WHEN '#39'2'#39' THEN '#39'Redu'#231#227'o'#39
      '          WHEN '#39'1'#39' THEN '#39'Isen'#231#227'o'#39
      '          WHEN '#39'3'#39' THEN '#39'N'#227'o Tribut'#225'vel'#39
      
        '       END AS NM_REG_TRIB_IPI, R.VL_ALIQ_IPI, R.VL_IPI, R.CD_REG' +
        '_TRIB_ICMS,'
      
        '       TI.NM_TRIBUTACAO_ICMS AS NM_REG_TRIB_ICMS, R.VL_ALIQ_ICMS' +
        ', R.VL_ICMS,'
      
        '       R.CD_MOEDA_MLE, M_MLE.AP_MOEDA AS AP_MOEDA_MLE, R.TX_CAMB' +
        'IO_MLE,'
      
        '       R.CD_MOEDA_FRETE, M_FRETE.AP_MOEDA AS AP_MOEDA_FRETE, R.T' +
        'X_CAMBIO_FRETE,'
      
        '       R.CD_MOEDA_SEGURO, M_SEGURO.AP_MOEDA AS AP_MOEDA_SEGURO, ' +
        'R.TX_CAMBIO_SEGURO,'
      '       R.TX_CAMBIO_DOLAR, R.DT_REGISTRO_DI,'
      '       R.CD_EXPORTADOR, EX.NM_EMPRESA AS NM_EXPORTADOR,'
      '       R.CD_PAIS_ORIGEM, PO.NM_PAIS AS NM_PAIS_ORIGEM,'
      '       R.CD_AREA, A.NM_AREA'
      
        'FROM   TREL_EST_MERCADORIA R, TEMPRESA_NAC E, TMERCADORIA M, TRE' +
        'G_TRIBUTARIO T,'
      
        '       TTRIBUTACAO_ICMS TI, TEMPRESA_EST EX, TPAIS_BROKER PO, TU' +
        'NID_NEG U,'
      
        '       TMOEDA_BROKER M_MLE, TMOEDA_BROKER M_FRETE, TMOEDA_BROKER' +
        ' M_SEGURO, TAREA A'
      'WHERE  R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       R.CD_UNID_NEG = U.CD_UNID_NEG AND'
      '       R.CD_CLIENTE = E.CD_EMPRESA AND'
      '       R.CD_MERCADORIA *= M.CD_MERCADORIA AND'
      '       R.CD_REG_TRIB_II *= T.CODIGO AND'
      '       R.CD_REG_TRIB_ICMS *= TI.CD_TRIBUTACAO_ICMS AND'
      '       R.CD_EXPORTADOR *= EX.CD_EMPRESA AND'
      '       R.CD_PAIS_ORIGEM *= PO.CD_PAIS AND'
      '       R.CD_MOEDA_MLE *= M_MLE.CD_MOEDA AND'
      '       R.CD_MOEDA_FRETE *= M_FRETE.CD_MOEDA AND'
      '       R.CD_MOEDA_FRETE *= M_SEGURO.CD_MOEDA AND'
      '       R.CD_AREA *= A.CD_AREA'
      
        'ORDER BY R.NR_IDENTIFICADOR, R.CD_UNID_NEG, R.CD_PRODUTO, R.CD_C' +
        'LIENTE,'
      
        '         R.NR_DI, R.NR_ADICAO, R.NR_PEDIDO, R.NR_ITEM_PEDIDO, R.' +
        'CD_NCM_SH,'
      '         R.CD_MERCADORIA'
      '')
    Left = 376
    Top = 48
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_est_mercadoria_imp_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
    end
    object qry_rel_est_mercadoria_imp_CD_UNID_NEG: TStringField
      FieldName = 'CD_UNID_NEG'
      Size = 2
    end
    object qry_rel_est_mercadoria_imp_NM_UNID_NEG: TStringField
      FieldName = 'NM_UNID_NEG'
      Size = 50
    end
    object qry_rel_est_mercadoria_imp_CD_PRODUTO: TStringField
      FieldName = 'CD_PRODUTO'
      Size = 2
    end
    object qry_rel_est_mercadoria_imp_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      Size = 5
    end
    object qry_rel_est_mercadoria_imp_NM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Size = 50
    end
    object qry_rel_est_mercadoria_imp_CD_NCM_SH: TStringField
      FieldName = 'CD_NCM_SH'
      Size = 11
    end
    object qry_rel_est_mercadoria_imp_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Size = 30
    end
    object qry_rel_est_mercadoria_imp_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Size = 25
    end
    object qry_rel_est_mercadoria_imp_NR_DI: TStringField
      FieldName = 'NR_DI'
      Size = 10
    end
    object qry_rel_est_mercadoria_imp_NR_ADICAO: TStringField
      FieldName = 'NR_ADICAO'
      Size = 3
    end
    object qry_rel_est_mercadoria_imp_NR_PEDIDO: TStringField
      FieldName = 'NR_PEDIDO'
      Size = 18
    end
    object qry_rel_est_mercadoria_imp_NR_ITEM_PEDIDO: TStringField
      FieldName = 'NR_ITEM_PEDIDO'
      Size = 3
    end
    object qry_rel_est_mercadoria_imp_NR_ATO_DRAWBACK: TStringField
      FieldName = 'NR_ATO_DRAWBACK'
      Size = 13
    end
    object qry_rel_est_mercadoria_imp_VL_QTDE: TFloatField
      FieldName = 'VL_QTDE'
    end
    object qry_rel_est_mercadoria_imp_VL_FOB: TFloatField
      FieldName = 'VL_FOB'
    end
    object qry_rel_est_mercadoria_imp_VL_FRETE: TFloatField
      FieldName = 'VL_FRETE'
    end
    object qry_rel_est_mercadoria_imp_VL_SEGURO: TFloatField
      FieldName = 'VL_SEGURO'
    end
    object qry_rel_est_mercadoria_imp_VL_PESO_LIQUIDO: TFloatField
      FieldName = 'VL_PESO_LIQUIDO'
    end
    object qry_rel_est_mercadoria_imp_CD_REG_TRIB_II: TStringField
      FieldName = 'CD_REG_TRIB_II'
      Size = 1
    end
    object qry_rel_est_mercadoria_imp_NM_REG_TRIB_II: TStringField
      FieldName = 'NM_REG_TRIB_II'
      Size = 120
    end
    object qry_rel_est_mercadoria_imp_VL_ALIQ_II: TFloatField
      FieldName = 'VL_ALIQ_II'
    end
    object qry_rel_est_mercadoria_imp_VL_II: TFloatField
      FieldName = 'VL_II'
    end
    object qry_rel_est_mercadoria_imp_CD_REG_TRIB_IPI: TStringField
      FieldName = 'CD_REG_TRIB_IPI'
      Size = 1
    end
    object qry_rel_est_mercadoria_imp_NM_REG_TRIB_IPI: TStringField
      FieldName = 'NM_REG_TRIB_IPI'
      Size = 21
    end
    object qry_rel_est_mercadoria_imp_VL_ALIQ_IPI: TFloatField
      FieldName = 'VL_ALIQ_IPI'
    end
    object qry_rel_est_mercadoria_imp_VL_IPI: TFloatField
      FieldName = 'VL_IPI'
    end
    object qry_rel_est_mercadoria_imp_CD_REG_TRIB_ICMS: TStringField
      FieldName = 'CD_REG_TRIB_ICMS'
      Size = 1
    end
    object qry_rel_est_mercadoria_imp_NM_REG_TRIB_ICMS: TStringField
      FieldName = 'NM_REG_TRIB_ICMS'
    end
    object qry_rel_est_mercadoria_imp_VL_ALIQ_ICMS: TFloatField
      FieldName = 'VL_ALIQ_ICMS'
    end
    object qry_rel_est_mercadoria_imp_VL_ICMS: TFloatField
      FieldName = 'VL_ICMS'
    end
    object qry_rel_est_mercadoria_imp_CD_MOEDA_MLE: TStringField
      FieldName = 'CD_MOEDA_MLE'
      Size = 3
    end
    object qry_rel_est_mercadoria_imp_AP_MOEDA_MLE: TStringField
      FieldName = 'AP_MOEDA_MLE'
      Size = 6
    end
    object qry_rel_est_mercadoria_imp_TX_CAMBIO_MLE: TFloatField
      FieldName = 'TX_CAMBIO_MLE'
    end
    object qry_rel_est_mercadoria_imp_CD_MOEDA_FRETE: TStringField
      FieldName = 'CD_MOEDA_FRETE'
      Size = 3
    end
    object qry_rel_est_mercadoria_imp_AP_MOEDA_FRETE: TStringField
      FieldName = 'AP_MOEDA_FRETE'
      Size = 6
    end
    object qry_rel_est_mercadoria_imp_TX_CAMBIO_FRETE: TFloatField
      FieldName = 'TX_CAMBIO_FRETE'
    end
    object qry_rel_est_mercadoria_imp_CD_MOEDA_SEGURO: TStringField
      FieldName = 'CD_MOEDA_SEGURO'
      Size = 3
    end
    object qry_rel_est_mercadoria_imp_AP_MOEDA_SEGURO: TStringField
      FieldName = 'AP_MOEDA_SEGURO'
      Size = 6
    end
    object qry_rel_est_mercadoria_imp_TX_CAMBIO_SEGURO: TFloatField
      FieldName = 'TX_CAMBIO_SEGURO'
    end
    object qry_rel_est_mercadoria_imp_DT_REGISTRO_DI: TDateTimeField
      FieldName = 'DT_REGISTRO_DI'
    end
    object qry_rel_est_mercadoria_imp_CD_EXPORTADOR: TStringField
      FieldName = 'CD_EXPORTADOR'
      Size = 5
    end
    object qry_rel_est_mercadoria_imp_NM_EXPORTADOR: TStringField
      FieldName = 'NM_EXPORTADOR'
      Size = 60
    end
    object qry_rel_est_mercadoria_imp_CD_PAIS_ORIGEM: TStringField
      FieldName = 'CD_PAIS_ORIGEM'
      Size = 3
    end
    object qry_rel_est_mercadoria_imp_NM_PAIS_ORIGEM: TStringField
      FieldName = 'NM_PAIS_ORIGEM'
      Size = 120
    end
    object qry_rel_est_mercadoria_imp_CD_AREA: TStringField
      FieldName = 'CD_AREA'
      Size = 2
    end
    object qry_rel_est_mercadoria_imp_NM_AREA: TStringField
      FieldName = 'NM_AREA'
      Size = 50
    end
    object qry_rel_est_mercadoria_imp_TX_CAMBIO_DOLAR: TFloatField
      FieldName = 'TX_CAMBIO_DOLAR'
    end
  end
  object ds_rel_est_mercadoria_imp: TDataSource
    DataSet = qry_rel_est_mercadoria_imp_
    Left = 376
    Top = 96
  end
  object qry_rel_est_mercadoria_exp_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      
        'SELECT R.NR_IDENTIFICADOR, R.CD_UNID_NEG, U.NM_UNID_NEG, R.CD_PR' +
        'ODUTO, R.CD_CLIENTE, E.NM_EMPRESA AS NM_CLIENTE,'
      
        '       R.CD_NCM_SH, R.CD_MERCADORIA, M.AP_MERCADORIA, R.NR_ATO_D' +
        'RAWBACK, R.CD_ENQUAD_OP, OP.NM_ENQUAD_OP,'
      '       R.VL_QTDE, R.VL_FOB, R.VL_PESO_LIQUIDO, R.VL_COM_AGENTE,'
      
        '       R.CD_PAIS_DESTINO, PD.NM_PAIS AS NM_PAIS_DESTINO, R.NR_RE' +
        ', R.NR_ANEXO, R.DT_EMBARQUE,'
      
        '       R.CD_AREA, A.NM_AREA, R.TX_CAMBIO_DOLAR, R.CD_MOEDA_MLE, ' +
        'R.TX_CAMBIO_MLE, M_MLE.AP_MOEDA AS AP_MOEDA_MLE'
      
        'FROM   TREL_EST_MERCADORIA R, TEMPRESA_NAC E, TMERCADORIA_EXP M,' +
        ' TENQUAD_OP OP,'
      
        '       TPAIS_BROKER PD, TUNID_NEG U, TAREA A, TMOEDA_BROKER M_ML' +
        'E'
      'WHERE  R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       R.CD_UNID_NEG = U.CD_UNID_NEG AND'
      '       R.CD_CLIENTE = E.CD_EMPRESA AND'
      '       R.CD_MERCADORIA *= M.CD_MERCADORIA AND'
      '       R.CD_ENQUAD_OP *= OP.CD_ENQUAD_OP AND'
      '       R.CD_PAIS_DESTINO *= PD.CD_PAIS AND'
      '       R.CD_AREA *= A.CD_AREA AND'
      '       R.CD_MOEDA_MLE *= M_MLE.CD_MOEDA'
      
        'ORDER BY R.NR_IDENTIFICADOR, R.CD_UNID_NEG, R.CD_PRODUTO, R.CD_C' +
        'LIENTE,'
      
        '         R.NR_PROCESSO, R.NR_RE, R.NR_ANEXO, R.CD_NCM_SH, R.CD_M' +
        'ERCADORIA,'
      '         R.CD_ENQUAD_OP'
      '')
    Left = 617
    Top = 48
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_est_mercadoria_exp_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
    end
    object qry_rel_est_mercadoria_exp_CD_UNID_NEG: TStringField
      FieldName = 'CD_UNID_NEG'
      Size = 2
    end
    object qry_rel_est_mercadoria_exp_NM_UNID_NEG: TStringField
      FieldName = 'NM_UNID_NEG'
      Size = 50
    end
    object qry_rel_est_mercadoria_exp_CD_PRODUTO: TStringField
      FieldName = 'CD_PRODUTO'
      Size = 2
    end
    object qry_rel_est_mercadoria_exp_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      Size = 5
    end
    object qry_rel_est_mercadoria_exp_NM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Size = 50
    end
    object qry_rel_est_mercadoria_exp_CD_NCM_SH: TStringField
      FieldName = 'CD_NCM_SH'
      Size = 11
    end
    object qry_rel_est_mercadoria_exp_CD_MERCADORIA: TStringField
      DisplayWidth = 40
      FieldName = 'CD_MERCADORIA'
      Size = 40
    end
    object qry_rel_est_mercadoria_exp_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Size = 25
    end
    object qry_rel_est_mercadoria_exp_NR_ATO_DRAWBACK: TStringField
      FieldName = 'NR_ATO_DRAWBACK'
      Size = 13
    end
    object qry_rel_est_mercadoria_exp_CD_ENQUAD_OP: TStringField
      FieldName = 'CD_ENQUAD_OP'
      Size = 5
    end
    object qry_rel_est_mercadoria_exp_NM_ENQUAD_OP: TStringField
      FieldName = 'NM_ENQUAD_OP'
      Size = 60
    end
    object qry_rel_est_mercadoria_exp_VL_QTDE: TFloatField
      FieldName = 'VL_QTDE'
    end
    object qry_rel_est_mercadoria_exp_VL_FOB: TFloatField
      FieldName = 'VL_FOB'
    end
    object qry_rel_est_mercadoria_exp_VL_PESO_LIQUIDO: TFloatField
      FieldName = 'VL_PESO_LIQUIDO'
    end
    object qry_rel_est_mercadoria_exp_VL_COM_AGENTE: TFloatField
      FieldName = 'VL_COM_AGENTE'
    end
    object qry_rel_est_mercadoria_exp_CD_PAIS_DESTINO: TStringField
      FieldName = 'CD_PAIS_DESTINO'
      Size = 3
    end
    object qry_rel_est_mercadoria_exp_NR_RE: TStringField
      FieldName = 'NR_RE'
      Size = 12
    end
    object qry_rel_est_mercadoria_exp_NR_ANEXO: TStringField
      FieldName = 'NR_ANEXO'
      Size = 3
    end
    object qry_rel_est_mercadoria_exp_DT_EMBARQUE: TDateTimeField
      FieldName = 'DT_EMBARQUE'
    end
    object qry_rel_est_mercadoria_exp_NM_PAIS_DESTINO: TStringField
      FieldName = 'NM_PAIS_DESTINO'
      Size = 120
    end
    object qry_rel_est_mercadoria_exp_CD_AREA: TStringField
      FieldName = 'CD_AREA'
      Size = 2
    end
    object qry_rel_est_mercadoria_exp_NM_AREA: TStringField
      FieldName = 'NM_AREA'
      Size = 50
    end
    object qry_rel_est_mercadoria_exp_TX_CAMBIO_DOLAR: TFloatField
      FieldName = 'TX_CAMBIO_DOLAR'
    end
    object qry_rel_est_mercadoria_exp_CD_MOEDA_MLE: TStringField
      FieldName = 'CD_MOEDA_MLE'
      Size = 3
    end
    object qry_rel_est_mercadoria_exp_TX_CAMBIO_MLE: TFloatField
      FieldName = 'TX_CAMBIO_MLE'
    end
    object qry_rel_est_mercadoria_exp_AP_MOEDA_MLE: TStringField
      FieldName = 'AP_MOEDA_MLE'
      Size = 6
    end
  end
  object ds_rel_est_mercadoria_exp: TDataSource
    DataSet = qry_rel_est_mercadoria_exp_
    Left = 616
    Top = 96
  end
  object sp_rel_est_arm_demurrage: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_est_arm_demurrage'
    Left = 40
    Top = 240
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object sp_rel_est_cntr_vl_cif: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_est_cntr_vl_cif'
    Left = 264
    Top = 192
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_ini'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object sp_rel_est_arm_prefixo: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_est_arm_prefixo'
    Left = 40
    Top = 288
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_prefixo'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object sp_rel_fat_saldos: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_fat_saldos'
    Left = 144
    Top = 240
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 10
      end>
  end
  object sp_rel_liq_saldos_pendentes: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_liq_saldos_pendentes'
    Left = 264
    Top = 240
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end
      item
        DataType = ftString
        Name = '@inVenc'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@inDevedor'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@inCredor'
        ParamType = ptInput
      end>
  end
  object sp_rel_liquidacao: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_liquidacao'
    Left = 504
    Top = 48
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object sp_rel_est_desp: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_est_desp'
    Left = 616
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object qry_rel_est_desp_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      
        'SELECT R.*, U.NM_UNID_NEG, P.NM_PRODUTO, E.NM_EMPRESA NM_CLIENTE' +
        ','
      '       CASE R.CD_PRODUTO'
      '         WHEN '#39'01'#39' THEN M.AP_MERCADORIA'
      '         WHEN '#39'02'#39' THEN ME.AP_MERCADORIA'
      '       END AS AP_MERCADORIA, A.NM_ARMAZEM NM_ARMAZEM_ATRACACAO,'
      
        '       PO.NM_PAIS AS NM_PAIS_ORIGEM, V.NM_VIA_TRANSP, L.DESCRICA' +
        'O AS NM_LOCAL_EMBARQUE'
      
        'FROM   TREL_EST_DESP R, TUNID_NEG U, TPRODUTO P, TEMPRESA_NAC E,' +
        ' TMERCADORIA M,'
      
        '       TMERCADORIA_EXP ME, TARMAZEM A, TPAIS_BROKER PO, TVIA_TRA' +
        'NSP_BROKER V, TLOCAL_EMBARQUE L'
      'WHERE  R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       R.CD_UNID_NEG = U.CD_UNID_NEG AND'
      '       R.CD_PRODUTO = P.CD_PRODUTO AND'
      '       R.CD_CLIENTE = E.CD_EMPRESA AND'
      '       R.CD_MERCADORIA *= M.CD_MERCADORIA AND'
      '       R.CD_MERCADORIA *= ME.CD_MERCADORIA AND'
      '       R.CD_ARMAZEM_ATRACACAO *= A.CD_ARMAZEM AND'
      '       R.CD_PAIS_ORIGEM *= PO.CD_PAIS AND'
      '       R.CD_VIA_TRANSPORTE *= V.CD_VIA_TRANSP AND'
      '       R.CD_LOCAL_EMBARQUE *= L.CODIGO'
      
        'ORDER BY R.NR_IDENTIFICADOR, R.CD_UNID_NEG, R.CD_PRODUTO, R.CD_C' +
        'LIENTE, R.NR_PROCESSO, R.NM_ESPECIE')
    Left = 264
    Top = 288
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
  end
  object qry_rel_est_desp_item_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT *'
      'FROM   TREL_EST_DESP_ITEM'
      'WHERE  NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       NR_PROCESSO = :NR_PROCESSO AND'
      '       CD_ITEM = :CD_ITEM'
      '')
    Left = 504
    Top = 96
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'CD_ITEM'
        ParamType = ptUnknown
      end>
  end
  object qry_itens_rel_est_desp_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT DISTINCT R.CD_ITEM, I.NM_ITEM'
      'FROM   TREL_EST_DESP_ITEM R, TITEM I'
      'WHERE  R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       R.CD_ITEM = I.CD_ITEM AND'
      
        '       R.CD_ITEM NOT IN ( SELECT CD_ITEM_II FROM TPARAMETROS ) A' +
        'ND'
      
        '       R.CD_ITEM NOT IN ( SELECT CD_ITEM_IPI FROM TPARAMETROS ) ' +
        'AND'
      
        '       R.CD_ITEM NOT IN ( SELECT CD_ITEM_ICMS FROM TPARAMETROS )' +
        ' AND'
      
        '       R.CD_ITEM NOT IN ( SELECT CD_ITEM_SISCOMEX FROM TPARAMETR' +
        'OS )'
      'ORDER BY I.NM_ITEM'
      '')
    Left = 616
    Top = 192
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
  end
  object sp_rel_dem_estatistico: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_dem_est'
    Left = 40
    Top = 336
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object ds_rel_dem_est: TDataSource
    DataSet = qry_rel_dem_est_
    Left = 504
    Top = 144
  end
  object qry_rel_dem_est_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT R.*,'
      '       U.NM_UNID_NEG, P.NM_PRODUTO, E.NM_EMPRESA AS NM_CLIENTE,'
      
        '       LTRIM( RTRIM( U.NM_UNID_NEG ) ) + '#39' - '#39' + LTRIM( RTRIM( P' +
        '.NM_PRODUTO ) ) + '#39' - '#39' + LTRIM( RTRIM( E.NM_EMPRESA ) ) AS NM_C' +
        'ABEC,'
      
        '       A.NM_AREA, T.NM_TRANSP_ROD AS NM_TRANSP_NAC, SUBSTRING( R' +
        '.NR_PROCESSO, 5, 14 ) AS CalcProcesso'
      
        'FROM   TREL_DEM_EST R, TUNID_NEG U, TPRODUTO P, TEMPRESA_NAC E, ' +
        'TAREA A, TTRANSP_ROD T'
      'WHERE  R.CD_UNID_NEG = U.CD_UNID_NEG AND'
      '       R.CD_PRODUTO = P.CD_PRODUTO AND'
      '       R.CD_CLIENTE = E.CD_EMPRESA AND'
      '       R.CD_AREA *= A.CD_AREA AND'
      '       R.CD_TRANSP_NAC *= T.CD_TRANSP_ROD AND'
      '       R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR'
      'ORDER BY U.NM_UNID_NEG, E.NM_EMPRESA, P.NM_PRODUTO, A.NM_AREA')
    Left = 616
    Top = 240
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_dem_est_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
    end
    object qry_rel_dem_est_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Size = 18
    end
    object qry_rel_dem_est_CD_UNID_NEG: TStringField
      FieldName = 'CD_UNID_NEG'
      Size = 2
    end
    object qry_rel_dem_est_CD_PRODUTO: TStringField
      FieldName = 'CD_PRODUTO'
      Size = 2
    end
    object qry_rel_dem_est_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      Size = 5
    end
    object qry_rel_dem_est_CD_AREA: TStringField
      FieldName = 'CD_AREA'
      Size = 2
    end
    object qry_rel_dem_est_TX_MERCADORIA: TMemoField
      FieldName = 'TX_MERCADORIA'
      BlobType = ftMemo
    end
    object qry_rel_dem_est_CD_TRANSP_NAC: TStringField
      FieldName = 'CD_TRANSP_NAC'
      Size = 4
    end
    object qry_rel_dem_est_DT_ENVIO_NF_TRANSP: TDateTimeField
      FieldName = 'DT_ENVIO_NF_TRANSP'
    end
    object qry_rel_dem_est_NM_UNID_NEG: TStringField
      FieldName = 'NM_UNID_NEG'
      Size = 50
    end
    object qry_rel_dem_est_NM_PRODUTO: TStringField
      FieldName = 'NM_PRODUTO'
      Size = 40
    end
    object qry_rel_dem_est_NM_CLIENTE: TStringField
      FieldName = 'NM_CLIENTE'
      Size = 50
    end
    object qry_rel_dem_est_NM_CABEC: TStringField
      FieldName = 'NM_CABEC'
      Size = 146
    end
    object qry_rel_dem_est_NM_AREA: TStringField
      FieldName = 'NM_AREA'
      Size = 50
    end
    object qry_rel_dem_est_NM_TRANSP_NAC: TStringField
      FieldName = 'NM_TRANSP_NAC'
      Size = 60
    end
    object qry_rel_dem_est_CalcProcesso: TStringField
      FieldName = 'CalcProcesso'
      Size = 14
    end
  end
  object ppBDERelDemEst: TppBDEPipeline
    DataSource = ds_rel_dem_est
    UserName = 'BDERelDemEst'
    Left = 744
    Top = 144
  end
  object ppDesigner: TppDesigner
    Caption = 'ReportBuilder'
    DataSettings.SessionType = 'BDESession'
    DataSettings.AllowEditSQL = False
    DataSettings.CollationType = ctASCII
    DataSettings.DatabaseType = dtParadox
    DataSettings.IsCaseSensitive = True
    DataSettings.SQLType = sqBDELocal
    Position = poScreenCenter
    IniStorageType = 'IniFile'
    IniStorageName = '($WINSYS)\RBuilder.ini'
    WindowHeight = 400
    WindowLeft = 100
    WindowTop = 50
    WindowWidth = 600
    Left = 864
    Top = 96
  end
  object ds_rel_ref_cliente: TDataSource
    DataSet = qry_rel_ref_cliente_
    Left = 504
    Top = 192
  end
  object qry_rel_ref_cliente_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_rel_dem_est
    SQL.Strings = (
      'SELECT CD_REFERENCIA'
      'FROM   TREF_CLIENTE'
      'WHERE  NR_PROCESSO = :NR_PROCESSO'
      'ORDER BY NR_SEQUENCIA')
    Left = 616
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
  end
  object ppBDERelRefCliente: TppBDEPipeline
    DataSource = ds_rel_ref_cliente
    UserName = 'BDERelRefCliente'
    Left = 744
    Top = 192
    object ppBDERelRefClienteppField1: TppField
      FieldAlias = 'CD_REFERENCIA'
      FieldName = 'CD_REFERENCIA'
      FieldLength = 0
      DisplayWidth = 0
      Position = 0
    end
  end
  object ds_rel_cntr: TDataSource
    DataSet = qry_rel_cntr_
    Left = 504
    Top = 240
  end
  object qry_rel_cntr_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_rel_dem_est
    SQL.Strings = (
      
        'SELECT SUBSTRING( NR_CNTR, 1, 4 ) + '#39'-'#39' + SUBSTRING( NR_CNTR, 5,' +
        ' 3 ) + '#39'-'#39' + SUBSTRING( NR_CNTR, 8, 3 ) + '#39'-'#39' +'
      
        '       SUBSTRING( NR_CNTR, 11, 1 ) AS CalcCNTR, DT_ENTREGA_FABRI' +
        'CA'
      'FROM   TPROCESSO_CNTR'
      'WHERE  NR_PROCESSO = :NR_PROCESSO')
    Left = 616
    Top = 336
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
        Size = 19
      end>
  end
  object ppBDERelCNTR: TppBDEPipeline
    DataSource = ds_rel_cntr
    UserName = 'BDERelCNTR'
    Left = 744
    Top = 240
    object ppBDERelCNTRppField1: TppField
      FieldAlias = 'CalcCNTR'
      FieldName = 'CalcCNTR'
      FieldLength = 0
      DisplayWidth = 0
      Position = 0
    end
    object ppBDERelCNTRppField2: TppField
      FieldAlias = 'DT_ENTREGA_FABRICA'
      FieldName = 'DT_ENTREGA_FABRICA'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 18
      Position = 1
    end
  end
  object ds_rel_carga_solta: TDataSource
    DataSet = qry_rel_carga_solta
    Left = 504
    Top = 288
  end
  object qry_rel_carga_solta: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_rel_dem_est
    SQL.Strings = (
      'SELECT NM_ESPECIE, VL_QTDE'
      'FROM    TPROCESSO_CARGA_SOLTA'
      'WHERE NR_PROCESSO = :NR_PROCESSO')
    Left = 616
    Top = 384
    ParamData = <
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
  end
  object ppBDERelCargaSolta: TppBDEPipeline
    DataSource = ds_rel_carga_solta
    UserName = 'BDERelCargaSolta'
    Left = 744
    Top = 288
  end
  object qry_excel_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT R.*,'
      '       U.NM_UNID_NEG, P.NM_PRODUTO, E.NM_EMPRESA AS NM_CLIENTE,'
      
        '       LTRIM( RTRIM( U.NM_UNID_NEG ) ) + '#39' - '#39' + LTRIM( RTRIM( P' +
        '.NM_PRODUTO ) ) + '#39' - '#39' + LTRIM( RTRIM( E.NM_EMPRESA ) ) AS NM_C' +
        'ABEC,'
      
        '       A.NM_AREA, T.NM_TRANSP_ROD AS NM_TRANSP_NAC, SUBSTRING( R' +
        '.NR_PROCESSO, 5, 14 ) AS CalcProcesso,'
      '       C.CD_REFERENCIA, CS.NM_ESPECIE, CS.VL_QTDE,'
      
        '       SUBSTRING( NR_CNTR, 1, 4 ) + '#39'-'#39' + SUBSTRING( NR_CNTR, 5,' +
        ' 3 ) + '#39'-'#39' + SUBSTRING( NR_CNTR, 8, 3 ) + '#39'-'#39' +'
      
        '       SUBSTRING( NR_CNTR, 11, 1 ) AS CalcCNTR, DT_ENTREGA_FABRI' +
        'CA'
      
        'FROM   TREL_DEM_EST R, TUNID_NEG U, TPRODUTO P, TEMPRESA_NAC E, ' +
        'TAREA A, TTRANSP_ROD T, TREF_CLIENTE C,'
      '       TPROCESSO_CNTR PC, TPROCESSO_CARGA_SOLTA CS'
      'WHERE  R.CD_UNID_NEG = U.CD_UNID_NEG AND'
      '       R.CD_PRODUTO = P.CD_PRODUTO AND'
      '       R.CD_CLIENTE = E.CD_EMPRESA AND'
      '       R.CD_AREA *= A.CD_AREA AND'
      '       R.CD_TRANSP_NAC *= T.CD_TRANSP_ROD AND'
      '       R.NR_PROCESSO *= C.NR_PROCESSO AND'
      '       R.NR_PROCESSO *= PC.NR_PROCESSO AND'
      '       R.NR_PROCESSO *= CS.NR_PROCESSO AND'
      '       R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR'
      
        'ORDER BY U.NM_UNID_NEG, E.NM_EMPRESA, P.NM_PRODUTO, A.NM_AREA, C' +
        '.CD_REFERENCIA'
      '')
    Left = 40
    Top = 643
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
  end
  object sp_rel_rent_proc: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_rent_proc'
    Left = 144
    Top = 432
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object qry_rel_rent_proc_capa_: TQuery
    AfterScroll = qry_rel_rent_proc_capa_AfterScroll
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      
        'SELECT C.*, U.NM_UNID_NEG, P.NM_PRODUTO, EN.NM_EMPRESA, SUBSTRIN' +
        'G( C.NR_PROCESSO, 5, 14 ) AS CalcNrProcesso'
      
        'FROM   TREL_RENT_PROC_CAPA C, TUNID_NEG U, TPRODUTO P, TEMPRESA_' +
        'NAC EN'
      'WHERE  C.NR_IDENTIFICADOR =:NR_IDENTIFICADOR AND'
      '       C.CD_UNID_NEG = U.CD_UNID_NEG AND'
      '       C.CD_PRODUTO = P.CD_PRODUTO AND'
      '       C.CD_CLIENTE = EN.CD_EMPRESA '
      'ORDER BY C.NR_PROCESSO')
    Left = 265
    Top = 433
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_rent_proc_capa_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TREL_RENT_PROC_CAPA.NR_IDENTIFICADOR'
    end
    object qry_rel_rent_proc_capa_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TREL_RENT_PROC_CAPA.NR_PROCESSO'
      Size = 18
    end
    object qry_rel_rent_proc_capa_CD_UNID_NEG: TStringField
      FieldName = 'CD_UNID_NEG'
      Origin = 'TREL_RENT_PROC_CAPA.CD_UNID_NEG'
      Size = 2
    end
    object qry_rel_rent_proc_capa_CD_PRODUTO: TStringField
      FieldName = 'CD_PRODUTO'
      Origin = 'TREL_RENT_PROC_CAPA.CD_PRODUTO'
      Size = 2
    end
    object qry_rel_rent_proc_capa_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      Origin = 'TREL_RENT_PROC_CAPA.CD_CLIENTE'
      Size = 5
    end
    object qry_rel_rent_proc_capa_NR_FATURA: TStringField
      FieldName = 'NR_FATURA'
      Origin = 'TREL_RENT_PROC_CAPA.NR_FATURA'
      Size = 6
    end
    object qry_rel_rent_proc_capa_DT_FATURA: TDateTimeField
      FieldName = 'DT_FATURA'
      Origin = 'TREL_RENT_PROC_CAPA.DT_FATURA'
    end
    object qry_rel_rent_proc_capa_TX_MERCADORIA: TMemoField
      FieldName = 'TX_MERCADORIA'
      Origin = 'TREL_RENT_PROC_CAPA.TX_MERCADORIA'
      BlobType = ftMemo
    end
    object qry_rel_rent_proc_capa_NM_UNID_NEG: TStringField
      FieldName = 'NM_UNID_NEG'
      Origin = 'TUNID_NEG.NM_UNID_NEG'
      Size = 50
    end
    object qry_rel_rent_proc_capa_NM_PRODUTO: TStringField
      FieldName = 'NM_PRODUTO'
      Origin = 'TPRODUTO.NM_PRODUTO'
      Size = 40
    end
    object qry_rel_rent_proc_capa_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      Origin = 'TEMPRESA_NAC.NM_EMPRESA'
      Size = 50
    end
    object qry_rel_rent_proc_capa_CalcNrProcesso: TStringField
      FieldName = 'CalcNrProcesso'
      Size = 14
    end
  end
  object ds_rel_rent_proc_capa: TDataSource
    DataSet = qry_rel_rent_proc_capa_
    Left = 376
    Top = 432
  end
  object ppRelSelUnidProd: TppReport
    AutoStop = False
    DataPipeline = ppBDERelRentProcCapa
    PassSetting = psTwoPass
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report2'
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    Template.FileName = 'C:\DDBroker\RBFAT002.rtm'
    Units = utPrinterPixels
    DeviceType = 'Screen'
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = False
    OutlineSettings.Visible = False
    TextSearchSettings.DefaultString = '<FindText>'
    TextSearchSettings.Enabled = False
    Left = 744
    Top = 336
    Version = '7.02'
    mmColumnWidth = 0
    DataPipelineName = 'ppBDERelRentProcCapa'
    object ppHeaderBand2: TppHeaderBand
      BeforePrint = ppHeaderBand4BeforePrint
      mmBottomOffset = 0
      mmHeight = 40481
      mmPrintPosition = 0
      object ppTitle: TppLabel
        UserName = 'Title'
        AutoSize = False
        Caption = 'Rentabilidade de Processos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 13
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 5027
        mmLeft = 83079
        mmTop = 21167
        mmWidth = 112977
        BandType = 0
      end
      object ppImage2: TppImage
        UserName = 'Image2'
        AutoSize = True
        MaintainAspectRatio = False
        Picture.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        mmHeight = 35719
        mmLeft = 3175
        mmTop = 1323
        mmWidth = 79375
        BandType = 0
      end
    end
    object ppDetailBand5: TppDetailBand
      BeforePrint = ppDetailBand5BeforePrint
      mmBottomOffset = 0
      mmHeight = 4233
      mmPrintPosition = 0
      DataPipeline = ppBDERelRentProcItem
      object ppDBText14: TppDBText
        UserName = 'DBText14'
        DataField = 'NM_ITEM'
        DataPipeline = ppBDERelRentProcItem
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        ParentDataPipeline = False
        Transparent = True
        WordWrap = True
        DataPipelineName = 'ppBDERelRentProcItem'
        mmHeight = 4233
        mmLeft = 4498
        mmTop = 0
        mmWidth = 60325
        BandType = 4
      end
      object vl_item_1: TppDBText
        UserName = 'vl_item_1'
        AutoSize = True
        DataField = 'VL_ITEM'
        DataPipeline = ppBDERelRentProcItem
        DisplayFormat = '#0,.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDERelRentProcItem'
        mmHeight = 4022
        mmLeft = 81662
        mmTop = 0
        mmWidth = 14647
        BandType = 4
      end
      object vl_item_2: TppDBText
        UserName = 'vl_item_2'
        DataField = 'VL_ITEM'
        DataPipeline = ppBDERelRentProcItem
        DisplayFormat = '#0,.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDERelRentProcItem'
        mmHeight = 4233
        mmLeft = 119063
        mmTop = 0
        mmWidth = 20638
        BandType = 4
      end
    end
    object ppFooterBand2: TppFooterBand
      mmBottomOffset = 0
      mmHeight = 8467
      mmPrintPosition = 0
      object ppVarRodapeEsq: TppVariable
        UserName = 'VarRodapeEsq'
        CalcOrder = 2
        CalcType = veReportEnd
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        OnCalc = ppVarRodapeEsqCalc
        Transparent = True
        mmHeight = 3704
        mmLeft = 265
        mmTop = 4498
        mmWidth = 19315
        BandType = 8
      end
      object ppNumPag: TppSystemVariable
        UserName = 'NumPag1'
        AutoSize = False
        VarType = vtPageNo
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        Visible = False
        mmHeight = 4233
        mmLeft = 102394
        mmTop = 3969
        mmWidth = 6085
        BandType = 8
      end
      object ppTotPag: TppSystemVariable
        UserName = 'TotPag1'
        AutoSize = False
        VarType = vtPageCount
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        Visible = False
        mmHeight = 4233
        mmLeft = 111390
        mmTop = 3969
        mmWidth = 6085
        BandType = 8
      end
      object ppVarRodapeDir: TppVariable
        UserName = 'VarRodapeDir'
        CalcOrder = 1
        CalcType = vePageStart
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3704
        mmLeft = 179652
        mmTop = 4498
        mmWidth = 17992
        BandType = 8
      end
      object ppLine3: TppLine
        UserName = 'Line3'
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 1588
        mmLeft = 0
        mmTop = 1852
        mmWidth = 197644
        BandType = 8
      end
    end
    object ppSummaryBand4: TppSummaryBand
      mmBottomOffset = 0
      mmHeight = 8467
      mmPrintPosition = 0
      object ppLabel23: TppLabel
        UserName = 'Label23'
        Caption = 'Total Geral'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4233
        mmLeft = 4763
        mmTop = 4233
        mmWidth = 18785
        BandType = 7
      end
      object ppLine4: TppLine
        UserName = 'Line4'
        Pen.Width = 3
        Weight = 2.250000000000000000
        mmHeight = 1588
        mmLeft = 0
        mmTop = 1588
        mmWidth = 197644
        BandType = 7
      end
      object ppDBText7: TppDBText
        UserName = 'DBText7'
        AutoSize = True
        DataField = 'SUM_COLUNA_1'
        DataPipeline = ppBDETotColunaRentProc
        DisplayFormat = '#0,.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDETotColunaRentProc'
        mmHeight = 4233
        mmLeft = 68792
        mmTop = 4233
        mmWidth = 27517
        BandType = 7
      end
      object ppDBText13: TppDBText
        UserName = 'DBText13'
        AutoSize = True
        DataField = 'SUM_COLUNA_2'
        DataPipeline = ppBDETotColunaRentProc
        DisplayFormat = '#0,.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDETotColunaRentProc'
        mmHeight = 4233
        mmLeft = 111919
        mmTop = 4233
        mmWidth = 27517
        BandType = 7
      end
      object ppDBText21: TppDBText
        UserName = 'DBText201'
        AutoSize = True
        DataField = 'RENTABILIDADE'
        DataPipeline = ppBDETotColunaRentProc
        DisplayFormat = '#0,.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDETotColunaRentProc'
        mmHeight = 4233
        mmLeft = 155840
        mmTop = 4233
        mmWidth = 27252
        BandType = 7
      end
    end
    object ppGroup2: TppGroup
      BreakName = 'NR_PROCESSO'
      DataPipeline = ppBDERelRentProcCapa
      OutlineSettings.CreateNode = True
      NewPage = True
      UserName = 'Group2'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppBDERelRentProcCapa'
      object ppGroupHeaderBand2: TppGroupHeaderBand
        PrintHeight = phDynamic
        mmBottomOffset = 0
        mmHeight = 50006
        mmPrintPosition = 0
        object ppLabel10: TppLabel
          UserName = 'Label10'
          Caption = 'Unid. Neg.:'
          Color = clBlack
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 3704
          mmTop = 2117
          mmWidth = 18521
          BandType = 3
          GroupNo = 0
        end
        object ppLabel11: TppLabel
          UserName = 'Label101'
          Caption = 'Processo:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 3704
          mmTop = 8202
          mmWidth = 16140
          BandType = 3
          GroupNo = 0
        end
        object ppLabel12: TppLabel
          UserName = 'Label102'
          Caption = 'N'#186' da Fatura:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 3704
          mmTop = 14552
          mmWidth = 21960
          BandType = 3
          GroupNo = 0
        end
        object ppLabel13: TppLabel
          UserName = 'Label103'
          Caption = 'Data da Fatura:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 96838
          mmTop = 8202
          mmWidth = 25929
          BandType = 3
          GroupNo = 0
        end
        object ppLabel15: TppLabel
          UserName = 'Label105'
          Caption = 'Produto: '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 97367
          mmTop = 2117
          mmWidth = 15346
          BandType = 3
          GroupNo = 0
        end
        object ppDBText9: TppDBText
          UserName = 'DBText9'
          DataField = 'CalcNrProcesso'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 31221
          mmTop = 8202
          mmWidth = 29369
          BandType = 3
          GroupNo = 0
        end
        object ppDBText10: TppDBText
          UserName = 'DBText10'
          DataField = 'NR_FATURA'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 31221
          mmTop = 14552
          mmWidth = 17198
          BandType = 3
          GroupNo = 0
        end
        object ppDBText11: TppDBText
          UserName = 'DBText11'
          DataField = 'DT_FATURA'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 123825
          mmTop = 8213
          mmWidth = 17198
          BandType = 3
          GroupNo = 0
        end
        object ppLabel14: TppLabel
          UserName = 'Label104'
          Caption = 'Cliente:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 3704
          mmTop = 20108
          mmWidth = 12965
          BandType = 3
          GroupNo = 0
        end
        object ppDBText12: TppDBText
          UserName = 'DBText12'
          DataField = 'CD_CLIENTE'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 31221
          mmTop = 20108
          mmWidth = 10583
          BandType = 3
          GroupNo = 0
        end
        object ppDBText15: TppDBText
          UserName = 'DBText15'
          DataField = 'NM_UNID_NEG'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 31221
          mmTop = 2381
          mmWidth = 51065
          BandType = 3
          GroupNo = 0
        end
        object ppDBText16: TppDBText
          UserName = 'DBText16'
          DataField = 'NM_EMPRESA'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 42598
          mmTop = 20108
          mmWidth = 51065
          BandType = 3
          GroupNo = 0
        end
        object ppDBText17: TppDBText
          UserName = 'DBText17'
          DataField = 'NM_PRODUTO'
          DataPipeline = ppBDERelRentProcCapa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = []
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERelRentProcCapa'
          mmHeight = 4233
          mmLeft = 113506
          mmTop = 2117
          mmWidth = 56092
          BandType = 3
          GroupNo = 0
        end
        object ppRegion1: TppRegion
          UserName = 'Region1'
          Brush.Style = bsClear
          Pen.Style = psClear
          Pen.Width = 0
          Stretch = True
          Transparent = True
          mmHeight = 16404
          mmLeft = 0
          mmTop = 24871
          mmWidth = 197115
          BandType = 3
          GroupNo = 0
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          object ppSubReport1: TppSubReport
            UserName = 'SubReport1'
            ExpandAll = False
            NewPrintJob = False
            OutlineSettings.CreateNode = True
            TraverseAllData = False
            DataPipelineName = 'ppBDERelProcCapaPedido'
            mmHeight = 8731
            mmLeft = 0
            mmTop = 30427
            mmWidth = 197115
            BandType = 3
            GroupNo = 0
            mmBottomOffset = 0
            mmOverFlowOffset = 0
            mmStopPosition = 0
            object ppChildReport1: TppChildReport
              AutoStop = False
              DataPipeline = ppBDERelProcCapaPedido
              PrinterSetup.BinName = 'Default'
              PrinterSetup.DocumentName = 'Report2'
              PrinterSetup.PaperName = 'A4'
              PrinterSetup.PrinterName = 'Default'
              PrinterSetup.mmMarginBottom = 6350
              PrinterSetup.mmMarginLeft = 6350
              PrinterSetup.mmMarginRight = 6350
              PrinterSetup.mmMarginTop = 6350
              PrinterSetup.mmPaperHeight = 297000
              PrinterSetup.mmPaperWidth = 210000
              PrinterSetup.PaperSize = 9
              Template.FileName = 'V:\Reports\RBFAT002.rtm'
              Units = utPrinterPixels
              Left = 489
              Top = 342
              Version = '7.02'
              mmColumnWidth = 0
              DataPipelineName = 'ppBDERelProcCapaPedido'
              object ppHeaderBand1: TppHeaderBand
                Visible = False
                mmBottomOffset = 0
                mmHeight = 0
                mmPrintPosition = 0
              end
              object ppDetailBand1: TppDetailBand
                mmBottomOffset = 0
                mmHeight = 4498
                mmPrintPosition = 0
                object ppDBText1: TppDBText
                  UserName = 'DBText1'
                  DataField = 'CD_REFERENCIA'
                  DataPipeline = ppBDERelProcCapaPedido
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Name = 'Arial'
                  Font.Size = 10
                  Font.Style = []
                  ParentDataPipeline = False
                  DataPipelineName = 'ppBDERelProcCapaPedido'
                  mmHeight = 4233
                  mmLeft = 3683
                  mmTop = 265
                  mmWidth = 37835
                  BandType = 4
                end
                object ppDBText2: TppDBText
                  UserName = 'DBText2'
                  DataField = 'AP_MERCADORIA'
                  DataPipeline = ppBDERelProcCapaPedido
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Name = 'Arial'
                  Font.Size = 10
                  Font.Style = []
                  ParentDataPipeline = False
                  DataPipelineName = 'ppBDERelProcCapaPedido'
                  mmHeight = 4233
                  mmLeft = 96859
                  mmTop = 265
                  mmWidth = 81492
                  BandType = 4
                end
              end
              object ppFooterBand1: TppFooterBand
                Visible = False
                mmBottomOffset = 0
                mmHeight = 0
                mmPrintPosition = 0
              end
            end
          end
          object ppLabel1: TppLabel
            UserName = 'Label1'
            Caption = 'Pedido'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Name = 'Arial'
            Font.Size = 10
            Font.Style = [fsBold]
            Transparent = True
            mmHeight = 4233
            mmLeft = 3969
            mmTop = 26194
            mmWidth = 11906
            BandType = 3
            GroupNo = 0
          end
          object ppLabel2: TppLabel
            UserName = 'Label2'
            Caption = 'Produto'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Name = 'Arial'
            Font.Size = 10
            Font.Style = [fsBold]
            Transparent = True
            mmHeight = 4233
            mmLeft = 96838
            mmTop = 25135
            mmWidth = 13229
            BandType = 3
            GroupNo = 0
          end
        end
        object ppRegion2: TppRegion
          UserName = 'Region2'
          Brush.Style = bsClear
          Pen.Style = psClear
          Pen.Width = 0
          ShiftRelativeTo = ppRegion1
          Transparent = True
          mmHeight = 6615
          mmLeft = 3683
          mmTop = 41275
          mmWidth = 197115
          BandType = 3
          GroupNo = 0
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          object ppLabel18: TppLabel
            UserName = 'Label18'
            Caption = 'Itens'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Name = 'Arial'
            Font.Size = 10
            Font.Style = [fsBold]
            Transparent = True
            mmHeight = 4233
            mmLeft = 3704
            mmTop = 42598
            mmWidth = 7938
            BandType = 3
            GroupNo = 0
          end
          object ppLabel19: TppLabel
            UserName = 'Label19'
            AutoSize = False
            Caption = 'Valor Pago'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Name = 'Arial'
            Font.Size = 10
            Font.Style = [fsBold]
            TextAlignment = taCentered
            Transparent = True
            mmHeight = 4233
            mmLeft = 81735
            mmTop = 42863
            mmWidth = 20638
            BandType = 3
            GroupNo = 0
          end
          object ppLabel20: TppLabel
            UserName = 'Label20'
            Caption = 'Valor Cobrado'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Name = 'Arial'
            Font.Size = 10
            Font.Style = [fsBold]
            TextAlignment = taCentered
            Transparent = True
            mmHeight = 4233
            mmLeft = 123296
            mmTop = 42863
            mmWidth = 24342
            BandType = 3
            GroupNo = 0
          end
          object ppLabel21: TppLabel
            UserName = 'Label201'
            Caption = 'Rentabilidade'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Name = 'Arial'
            Font.Size = 10
            Font.Style = [fsBold]
            Transparent = True
            mmHeight = 4233
            mmLeft = 164307
            mmTop = 42863
            mmWidth = 23548
            BandType = 3
            GroupNo = 0
          end
        end
      end
      object ppGroupFooterBand2: TppGroupFooterBand
        mmBottomOffset = 0
        mmHeight = 5292
        mmPrintPosition = 0
        object ppLabel22: TppLabel
          UserName = 'Label22'
          Caption = 'Total'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 4233
          mmLeft = 4763
          mmTop = 529
          mmWidth = 8467
          BandType = 5
          GroupNo = 0
        end
        object ppDBText18: TppDBText
          UserName = 'DBText18'
          AutoSize = True
          DataField = 'SUM_COLUNA_1'
          DataPipeline = ppBDESumColunaRentProc
          DisplayFormat = '#0,.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          ParentDataPipeline = False
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDESumColunaRentProc'
          mmHeight = 4233
          mmLeft = 68792
          mmTop = 529
          mmWidth = 27517
          BandType = 5
          GroupNo = 0
        end
        object ppDBText19: TppDBText
          UserName = 'DBText19'
          AutoSize = True
          DataField = 'SUM_COLUNA_2'
          DataPipeline = ppBDESumColunaRentProc
          DisplayFormat = '#0,.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          ParentDataPipeline = False
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDESumColunaRentProc'
          mmHeight = 4233
          mmLeft = 111919
          mmTop = 529
          mmWidth = 27517
          BandType = 5
          GroupNo = 0
        end
        object ppDBText20: TppDBText
          UserName = 'DBText20'
          AutoSize = True
          DataField = 'RENTABILIDADE'
          DataPipeline = ppBDESumColunaRentProc
          DisplayFormat = '#0,.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 10
          Font.Style = [fsBold]
          ParentDataPipeline = False
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDESumColunaRentProc'
          mmHeight = 4233
          mmLeft = 155840
          mmTop = 529
          mmWidth = 27252
          BandType = 5
          GroupNo = 0
        end
      end
    end
    object ppParameterList1: TppParameterList
    end
  end
  object ppBDERelRentProcCapa: TppBDEPipeline
    DataSource = ds_rel_rent_proc_capa
    UserName = 'BDERelRentProcCapa'
    Left = 744
    Top = 480
  end
  object ppBDERelRentProcItem: TppBDEPipeline
    DataSource = ds_rel_rent_proc_item
    UserName = 'BDERelRentProcItem'
    Left = 744
    Top = 384
    MasterDataPipelineName = 'ppBDERelRentProcCapa'
    object ppBDERelRentProcItemppField1: TppField
      Alignment = taRightJustify
      FieldAlias = 'NR_IDENTIFICADOR'
      FieldName = 'NR_IDENTIFICADOR'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 0
    end
    object ppBDERelRentProcItemppField2: TppField
      FieldAlias = 'NR_PROCESSO'
      FieldName = 'NR_PROCESSO'
      FieldLength = 18
      DisplayWidth = 18
      Position = 1
    end
    object ppBDERelRentProcItemppField3: TppField
      FieldAlias = 'CD_ITEM'
      FieldName = 'CD_ITEM'
      FieldLength = 3
      DisplayWidth = 3
      Position = 2
    end
    object ppBDERelRentProcItemppField4: TppField
      Alignment = taRightJustify
      FieldAlias = 'VL_ITEM'
      FieldName = 'VL_ITEM'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 3
    end
    object ppBDERelRentProcItemppField5: TppField
      FieldAlias = 'CD_COLUNA'
      FieldName = 'CD_COLUNA'
      FieldLength = 1
      DisplayWidth = 1
      Position = 4
    end
    object ppBDERelRentProcItemppField6: TppField
      FieldAlias = 'NM_ITEM'
      FieldName = 'NM_ITEM'
      FieldLength = 30
      DisplayWidth = 30
      Position = 5
    end
  end
  object qry_rel_rent_proc_item_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_rel_rent_proc_capa
    SQL.Strings = (
      'SELECT PI.*, I.NM_ITEM'
      'FROM TREL_RENT_PROC_ITEM PI, TITEM I'
      'WHERE PI.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '               PI.NR_PROCESSO = :NR_PROCESSO AND'
      '               PI.CD_ITEM = I.CD_ITEM ')
    Left = 264
    Top = 480
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
        Size = 4
      end
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
        Size = 19
      end>
    object qry_rel_rent_proc_item_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TREL_RENT_PROC_ITEM.NR_IDENTIFICADOR'
    end
    object qry_rel_rent_proc_item_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TREL_RENT_PROC_ITEM.NR_PROCESSO'
      Size = 18
    end
    object qry_rel_rent_proc_item_CD_ITEM: TStringField
      FieldName = 'CD_ITEM'
      Origin = 'TREL_RENT_PROC_ITEM.CD_ITEM'
      Size = 3
    end
    object qry_rel_rent_proc_item_VL_ITEM: TFloatField
      FieldName = 'VL_ITEM'
      Origin = 'TREL_RENT_PROC_ITEM.VL_ITEM'
    end
    object qry_rel_rent_proc_item_CD_COLUNA: TStringField
      FieldName = 'CD_COLUNA'
      Origin = 'TREL_RENT_PROC_ITEM.CD_COLUNA'
      Size = 1
    end
    object qry_rel_rent_proc_item_NM_ITEM: TStringField
      FieldName = 'NM_ITEM'
      Origin = 'TITEM.NM_ITEM'
      Size = 30
    end
  end
  object ds_rel_rent_proc_item: TDataSource
    DataSet = qry_rel_rent_proc_item_
    Left = 376
    Top = 480
  end
  object qry_sum_coluna_rent_proc_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_rel_rent_proc_capa
    SQL.Strings = (
      'SELECT SUM( CASE WHEN RI.CD_COLUNA = '#39'1'#39' THEN RI.VL_ITEM'
      '                 ELSE 0'
      '                 END ) AS SUM_COLUNA_1,'
      '              SUM( CASE'
      '                 WHEN RI.CD_COLUNA = '#39'2'#39' THEN RI.VL_ITEM'
      '                 ELSE 0'
      '                 END ) AS SUM_COLUNA_2,'
      '          ( ( SUM( CASE'
      '                     WHEN RI.CD_COLUNA = '#39'2'#39' THEN RI.VL_ITEM'
      '                     ELSE 0'
      '                   END ) ) -'
      '          ( SUM( CASE'
      '                   WHEN RI.CD_COLUNA = '#39'1'#39' THEN RI.VL_ITEM'
      '                   ELSE 0'
      '                 END ) ) ) AS RENTABILIDADE'
      'FROM TREL_RENT_PROC_CAPA RC, TREL_RENT_PROC_ITEM RI'
      'WHERE RC.NR_IDENTIFICADOR =:NR_IDENTIFICADOR AND'
      '               RC.NR_PROCESSO =:NR_PROCESSO AND'
      '               RC.NR_PROCESSO = RI.NR_PROCESSO AND '
      '               RC.NR_IDENTIFICADOR = RI.NR_IDENTIFICADOR')
    Left = 40
    Top = 480
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_sum_coluna_rent_proc_SUM_COLUNA_1: TFloatField
      FieldName = 'SUM_COLUNA_1'
    end
    object qry_sum_coluna_rent_proc_SUM_COLUNA_2: TFloatField
      FieldName = 'SUM_COLUNA_2'
    end
    object qry_sum_coluna_rent_proc_RENTABILIDADE: TFloatField
      FieldName = 'RENTABILIDADE'
    end
  end
  object ppBDESumColunaRentProc: TppBDEPipeline
    DataSource = ds_sum_coluna_rent_proc
    UserName = 'BDESumColunaRentProc'
    Left = 504
    Top = 384
  end
  object ds_sum_coluna_rent_proc: TDataSource
    DataSet = qry_sum_coluna_rent_proc_
    Left = 144
    Top = 480
  end
  object sp_rel_demo_cc: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_demo_cc'
    Left = 144
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@nr_relatorio'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_usuario'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 16
      end>
  end
  object qry_ult_relat_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT COUNT(*) AS NUM_RELAT'
      'FROM   TREL_DEMO_CC_NUM'
      'WHERE  DATEPART( YY, DT_EMISSAO ) = DATEPART( YY, GETDATE() )'
      '')
    Left = 40
    Top = 432
    object qry_ult_relat_NUM_RELAT: TIntegerField
      FieldName = 'NUM_RELAT'
    end
  end
  object qry_tot_coluna_rent_proc_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT SUM( CASE WHEN RI.CD_COLUNA = '#39'1'#39' THEN RI.VL_ITEM'
      '                 ELSE 0'
      '                 END ) AS SUM_COLUNA_1,'
      '              SUM( CASE'
      '                 WHEN RI.CD_COLUNA = '#39'2'#39' THEN RI.VL_ITEM'
      '                 ELSE 0'
      '                 END ) AS SUM_COLUNA_2,'
      '          ( ( SUM( CASE'
      '                     WHEN RI.CD_COLUNA = '#39'2'#39' THEN RI.VL_ITEM'
      '                     ELSE 0'
      '                   END ) ) -'
      '          ( SUM( CASE'
      '                   WHEN RI.CD_COLUNA = '#39'1'#39' THEN RI.VL_ITEM'
      '                   ELSE 0'
      '                 END ) ) ) AS RENTABILIDADE'
      'FROM  TREL_RENT_PROC_ITEM RI'
      'WHERE RI.NR_IDENTIFICADOR =:NR_IDENTIFICADOR')
    Left = 40
    Top = 48
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object FloatField1: TFloatField
      FieldName = 'SUM_COLUNA_1'
    end
    object FloatField2: TFloatField
      FieldName = 'SUM_COLUNA_2'
    end
    object FloatField3: TFloatField
      FieldName = 'RENTABILIDADE'
    end
  end
  object ds_tot_coluna_rent_proc: TDataSource
    DataSet = qry_tot_coluna_rent_proc_
    Left = 144
    Top = 48
  end
  object ppBDETotColunaRentProc: TppBDEPipeline
    DataSource = ds_tot_coluna_rent_proc
    UserName = 'BDETotColunaRentProc'
    Left = 40
    Top = 96
    object ppBDETotColunaRentProcppField1: TppField
      FieldAlias = 'SUM_COLUNA_1'
      FieldName = 'SUM_COLUNA_1'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 0
      Searchable = False
      Sortable = False
    end
    object ppBDETotColunaRentProcppField2: TppField
      FieldAlias = 'SUM_COLUNA_2'
      FieldName = 'SUM_COLUNA_2'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 1
      Searchable = False
      Sortable = False
    end
    object ppBDETotColunaRentProcppField3: TppField
      FieldAlias = 'RENTABILIDADE'
      FieldName = 'RENTABILIDADE'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 2
      Searchable = False
      Sortable = False
    end
  end
  object ppBDERelLeadTimePO: TppBDEPipeline
    DataSource = ds_rel_lead_time_po
    UserName = 'BDERelLeadTimePO'
    Left = 864
    Top = 384
  end
  object qry_rel_lead_time_po_: TQuery
    OnCalcFields = qry_rel_lead_time_po_CalcFields
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT *               '
      'FROM TREL_LEAD_TIME_PO '
      'WHERE NR_IDENTIFICADOR =:NR_IDENTIFICADOR'
      'ORDER BY NR_PROCESSO, CD_IMPORTADOR')
    Left = 864
    Top = 336
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_lead_time_po_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TREL_LEAD_TIME_PO.NR_IDENTIFICADOR'
    end
    object qry_rel_lead_time_po_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TREL_LEAD_TIME_PO.NR_PROCESSO'
      Size = 18
    end
    object qry_rel_lead_time_po_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Origin = 'TREL_LEAD_TIME_PO.CD_MERCADORIA'
      Size = 15
    end
    object qry_rel_lead_time_po_CD_EXPORTADOR: TStringField
      FieldName = 'CD_EXPORTADOR'
      Origin = 'TREL_LEAD_TIME_PO.CD_EXPORTADOR'
      Size = 5
    end
    object qry_rel_lead_time_po_CD_IMPORTADOR: TStringField
      FieldName = 'CD_IMPORTADOR'
      Origin = 'TREL_LEAD_TIME_PO.CD_IMPORTADOR'
      Size = 5
    end
    object qry_rel_lead_time_po_DT_REC_PO: TDateTimeField
      FieldName = 'DT_REC_PO'
      Origin = 'TREL_LEAD_TIME_PO.DT_REC_PO'
    end
    object qry_rel_lead_time_po_DT_EMB: TDateTimeField
      FieldName = 'DT_EMB'
      Origin = 'TREL_LEAD_TIME_PO.DT_EMB'
    end
    object qry_rel_lead_time_po_DT_DOCTOS_ORIG: TDateTimeField
      FieldName = 'DT_DOCTOS_ORIG'
      Origin = 'TREL_LEAD_TIME_PO.DT_DOCTOS_ORIG'
    end
    object qry_rel_lead_time_po_DT_CHEGADA: TDateTimeField
      FieldName = 'DT_CHEGADA'
      Origin = 'TREL_LEAD_TIME_PO.DT_CHEGADA'
    end
    object qry_rel_lead_time_po_DT_DESEMB: TDateTimeField
      FieldName = 'DT_DESEMB'
      Origin = 'TREL_LEAD_TIME_PO.DT_DESEMB'
    end
    object qry_rel_lead_time_po_DT_ENTR_MERC: TDateTimeField
      FieldName = 'DT_ENTR_MERC'
      Origin = 'TREL_LEAD_TIME_PO.DT_ENTR_MERC'
    end
    object qry_rel_lead_time_po_DT_REC_PO_DT_EMB: TIntegerField
      FieldName = 'DT_REC_PO_DT_EMB'
      Origin = 'TREL_LEAD_TIME_PO.DT_REC_PO_DT_EMB'
    end
    object qry_rel_lead_time_po_DT_EMB_DT_DOCTOS_ORIG: TIntegerField
      FieldName = 'DT_EMB_DT_DOCTOS_ORIG'
      Origin = 'TREL_LEAD_TIME_PO.DT_EMB_DT_DOCTOS_ORIG'
    end
    object qry_rel_lead_time_po_DT_DOCTOS_ORIG_DT_CHEG: TIntegerField
      FieldName = 'DT_DOCTOS_ORIG_DT_CHEG'
      Origin = 'TREL_LEAD_TIME_PO.DT_DOCTOS_ORIG_DT_CHEG'
    end
    object qry_rel_lead_time_po_DT_CHEG_DT_DESEMB: TIntegerField
      FieldName = 'DT_CHEG_DT_DESEMB'
      Origin = 'TREL_LEAD_TIME_PO.DT_CHEG_DT_DESEMB'
    end
    object qry_rel_lead_time_po_DT_REC_PO_DT_ENTR_MERC: TIntegerField
      FieldName = 'DT_REC_PO_DT_ENTR_MERC'
      Origin = 'TREL_LEAD_TIME_PO.DT_REC_PO_DT_ENTR_MERC'
    end
    object qry_rel_lead_time_po_DT_EMB_DT_ENTR_MERC: TIntegerField
      FieldName = 'DT_EMB_DT_ENTR_MERC'
      Origin = 'TREL_LEAD_TIME_PO.DT_EMB_DT_ENTR_MERC'
    end
    object qry_rel_lead_time_po_DT_CHEG_DT_ENTR_MERC: TIntegerField
      FieldName = 'DT_CHEG_DT_ENTR_MERC'
      Origin = 'TREL_LEAD_TIME_PO.DT_CHEG_DT_ENTR_MERC'
    end
    object qry_rel_lead_time_po_LookMercadoria: TStringField
      FieldKind = fkCalculated
      FieldName = 'LookMercadoria'
      Size = 25
      Calculated = True
    end
    object qry_rel_lead_time_po_LookEmpresaEst: TStringField
      FieldKind = fkCalculated
      FieldName = 'LookEmpresaEst'
      Size = 60
      Calculated = True
    end
  end
  object ds_rel_lead_time_po: TDataSource
    DataSet = qry_rel_lead_time_po_
    Left = 864
    Top = 288
  end
  object sp_rel_lead_time_po: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_lead_time_po'
    Left = 864
    Top = 240
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_fornecedor'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object qry_empresa_est_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CD_EMPRESA, NM_EMPRESA '
      'FROM TEMPRESA_EST'
      'WHERE CD_EMPRESA = :CD_EMPRESA')
    Left = 616
    Top = 576
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_EMPRESA'
        ParamType = ptUnknown
      end>
    object qry_empresa_est_CD_EMPRESA: TStringField
      FieldName = 'CD_EMPRESA'
      Origin = 'TEMPRESA_EST.CD_EMPRESA'
      Size = 5
    end
    object qry_empresa_est_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      Origin = 'TEMPRESA_EST.NM_EMPRESA'
      Size = 60
    end
  end
  object qry_empresa_nac_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CD_EMPRESA, NM_EMPRESA'
      'FROM TEMPRESA_NAC'
      'WHERE CD_EMPRESA =:CD_EMPRESA')
    Left = 496
    Top = 576
    ParamData = <
      item
        DataType = ftString
        Name = 'CD_EMPRESA'
        ParamType = ptUnknown
      end>
    object qry_empresa_nac_CD_EMPRESA: TStringField
      FieldName = 'CD_EMPRESA'
      Origin = 'TEMPRESA_NAC.CD_EMPRESA'
      Size = 5
    end
    object qry_empresa_nac_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      Origin = 'TEMPRESA_NAC.NM_EMPRESA'
      Size = 50
    end
  end
  object sp_rel_est_qt_cntr: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_est_qt_cntr'
    Left = 504
    Top = 432
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_produto'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_area'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo_area'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object ppBDERelEstQtCntr: TppBDEPipeline
    DataSource = ds_rel_est_qt_cntr
    UserName = 'BDERelEstQtCntr'
    Left = 864
    Top = 144
  end
  object qry_rel_est_qt_cntr_: TQuery
    OnCalcFields = qry_rel_est_qt_cntr_CalcFields
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      
        'SELECT R.*, '#39'Margem '#39' + LTRIM( RTRIM( M.NM_MARGEM ) ) AS NM_MARG' +
        'EM, E.NM_EMPRESA'
      'FROM   TREL_EST_QT_CNTR R, TMARGEM M, TEMPRESA_NAC E'
      'WHERE  R.CD_MARGEM = M.CD_MARGEM AND'
      '       R.CD_CLIENTE = E.CD_EMPRESA AND'
      '       R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR'
      'ORDER BY R.CD_MARGEM, R.CD_CLIENTE, R.TP_CNTR')
    Left = 504
    Top = 480
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_est_qt_cntr_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
    end
    object qry_rel_est_qt_cntr_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      Size = 5
    end
    object qry_rel_est_qt_cntr_CD_MARGEM: TStringField
      FieldName = 'CD_MARGEM'
      Size = 1
    end
    object qry_rel_est_qt_cntr_NM_MARGEM: TStringField
      FieldName = 'NM_MARGEM'
      Size = 15
    end
    object qry_rel_est_qt_cntr_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      Size = 50
    end
    object qry_rel_est_qt_cntr_TP_CNTR: TStringField
      FieldName = 'TP_CNTR'
      Size = 2
    end
    object qry_rel_est_qt_cntr_QT_CNTR: TIntegerField
      FieldName = 'QT_CNTR'
    end
    object qry_rel_est_qt_cntr_Qt20: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'Qt20'
      Calculated = True
    end
    object qry_rel_est_qt_cntr_Qt40: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'Qt40'
      Calculated = True
    end
    object qry_rel_est_qt_cntr_QtTank20: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'QtTank20'
      Calculated = True
    end
    object qry_rel_est_qt_cntr_QtTank40: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'QtTank40'
      Calculated = True
    end
  end
  object ds_rel_est_qt_cntr: TDataSource
    DataSet = qry_rel_est_qt_cntr_
    Left = 864
    Top = 192
  end
  object sp_rel_po_emb: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_po_emb'
    Left = 864
    Top = 480
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_ini'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object ppBDERelPOEmb: TppBDEPipeline
    DataSource = ds_rel_po_emb
    UserName = 'BDERelPOEmb'
    Left = 864
    Top = 432
    object ppBDERelPOEmbppField1: TppField
      Alignment = taRightJustify
      FieldAlias = 'NR_IDENTIFICADOR'
      FieldName = 'NR_IDENTIFICADOR'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 0
      Position = 0
    end
    object ppBDERelPOEmbppField2: TppField
      FieldAlias = 'CD_MERCADORIA'
      FieldName = 'CD_MERCADORIA'
      FieldLength = 15
      DisplayWidth = 15
      Position = 1
    end
    object ppBDERelPOEmbppField3: TppField
      FieldAlias = 'DT_EMBARQUE'
      FieldName = 'DT_EMBARQUE'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 10
      Position = 2
    end
    object ppBDERelPOEmbppField4: TppField
      FieldAlias = 'TP_REL'
      FieldName = 'TP_REL'
      FieldLength = 1
      DisplayWidth = 1
      Position = 3
    end
    object ppBDERelPOEmbppField5: TppField
      Alignment = taRightJustify
      FieldAlias = 'QT_MERCADORIA'
      FieldName = 'QT_MERCADORIA'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 4
    end
    object ppBDERelPOEmbppField6: TppField
      FieldAlias = 'AP_MERCADORIA'
      FieldName = 'AP_MERCADORIA'
      FieldLength = 25
      DisplayWidth = 25
      Position = 5
    end
    object ppBDERelPOEmbppField7: TppField
      FieldAlias = 'NM_SIGLA'
      FieldName = 'NM_SIGLA'
      FieldLength = 3
      DisplayWidth = 3
      Position = 6
    end
    object ppBDERelPOEmbppField8: TppField
      FieldAlias = 'NmTpRel'
      FieldName = 'NmTpRel'
      FieldLength = 30
      DisplayWidth = 30
      Position = 7
    end
    object ppBDERelPOEmbppField9: TppField
      FieldAlias = 'NmPrevEmb'
      FieldName = 'NmPrevEmb'
      FieldLength = 20
      DisplayWidth = 20
      Position = 8
    end
  end
  object qry_rel_po_emb_: TQuery
    OnCalcFields = qry_rel_po_emb_CalcFields
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT R.*, M.AP_MERCADORIA, U.NM_SIGLA'
      'FROM   TREL_PO_EMB R, TMERCADORIA M, TUNID_MEDIDA_BROKER U'
      'WHERE  R.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       M.CD_UN_MED_COMERC *= U.CD_UNID_MEDIDA AND '
      '       R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR'
      'ORDER BY R.TP_REL, R.CD_MERCADORIA, R.DT_EMBARQUE')
    Left = 616
    Top = 432
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
        Value = 1
      end>
    object qry_rel_po_emb_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TREL_PO_EMB.NR_IDENTIFICADOR'
    end
    object qry_rel_po_emb_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Size = 15
    end
    object qry_rel_po_emb_DT_EMBARQUE: TDateTimeField
      FieldName = 'DT_EMBARQUE'
      Origin = 'TREL_PO_EMB.DT_EMBARQUE'
    end
    object qry_rel_po_emb_TP_REL: TStringField
      FieldName = 'TP_REL'
      Origin = 'TREL_PO_EMB.TP_REL'
      Size = 1
    end
    object qry_rel_po_emb_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
      Origin = 'TREL_PO_EMB.QT_MERCADORIA'
      DisplayFormat = '#0,.00000'
    end
    object qry_rel_po_emb_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Origin = 'TMERCADORIA.AP_MERCADORIA'
      Size = 25
    end
    object qry_rel_po_emb_NM_SIGLA: TStringField
      FieldName = 'NM_SIGLA'
      Size = 3
    end
    object qry_rel_po_emb_NmTpRel: TStringField
      FieldKind = fkCalculated
      FieldName = 'NmTpRel'
      Size = 30
      Calculated = True
    end
    object qry_rel_po_emb_NmPrevEmb: TStringField
      FieldKind = fkCalculated
      FieldName = 'NmPrevEmb'
      Calculated = True
    end
  end
  object ds_rel_po_emb: TDataSource
    DataSet = qry_rel_po_emb_
    Left = 616
    Top = 480
  end
  object qry_rel_po_emb_graf_: TQuery
    DatabaseName = 'DBBROKER'
    DataSource = ds_rel_po_emb
    SQL.Strings = (
      
        'SELECT G.CD_MERCADORIA, M.AP_MERCADORIA, SUM( G.QT_MERCADORIA ) ' +
        'AS QT_MERCADORIA'
      'FROM   TREL_PO_EMB G, TMERCADORIA M'
      'WHERE  G.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       G.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       G.TP_REL = :TP_REL'
      'GROUP BY G.CD_MERCADORIA, M.AP_MERCADORIA'
      'ORDER BY M.AP_MERCADORIA')
    Left = 864
    Top = 528
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'TP_REL'
        ParamType = ptUnknown
      end>
    object qry_rel_po_emb_graf_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Origin = 'TREL_PO_EMB.CD_MERCADORIA'
      Size = 15
    end
    object qry_rel_po_emb_graf_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Origin = 'TMERCADORIA.AP_MERCADORIA'
      Size = 25
    end
    object qry_rel_po_emb_graf_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
      Origin = 'TREL_PO_EMB.QT_MERCADORIA'
      DisplayFormat = '#0,.00000'
    end
  end
  object qry_rel_nivel_servico_: TQuery
    OnCalcFields = qry_rel_nivel_servico_CalcFields
    DatabaseName = 'DBBROKER'
    RequestLive = True
    SQL.Strings = (
      'SELECT *'
      'FROM TREL_NIVEL_SERVICO_PO'
      'WHERE NR_IDENTIFICADOR =:NR_IDENTIFICADOR AND'
      
        '               ( DT_ENTR_MERC_DT_NEC <= '#39'0'#39' OR DT_ENTR_MERC_DT_N' +
        'EC > '#39'0'#39' )')
    Left = 40
    Top = 528
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_rel_nivel_servico_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TREL_NIVEL_SERVICO_PO.NR_IDENTIFICADOR'
    end
    object qry_rel_nivel_servico_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TREL_NIVEL_SERVICO_PO.NR_PROCESSO'
      Size = 18
    end
    object qry_rel_nivel_servico_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Origin = 'TREL_NIVEL_SERVICO_PO.CD_MERCADORIA'
      Size = 30
    end
    object qry_rel_nivel_servico_CD_EXPORTADOR: TStringField
      FieldName = 'CD_EXPORTADOR'
      Origin = 'TREL_NIVEL_SERVICO_PO.CD_EXPORTADOR'
      Size = 5
    end
    object qry_rel_nivel_servico_CD_IMPORTADOR: TStringField
      FieldName = 'CD_IMPORTADOR'
      Origin = 'TREL_NIVEL_SERVICO_PO.CD_IMPORTADOR'
      Size = 5
    end
    object qry_rel_nivel_servico_DT_ENTR_MERC_FABR: TDateTimeField
      FieldName = 'DT_ENTR_MERC_FABR'
      Origin = 'TREL_NIVEL_SERVICO_PO.DT_ENTR_MERC_FABR'
    end
    object qry_rel_nivel_servico_DT_NECESSIDADE: TDateTimeField
      FieldName = 'DT_NECESSIDADE'
      Origin = 'TREL_NIVEL_SERVICO_PO.DT_NECESSIDADE'
    end
    object qry_rel_nivel_servico_DT_ENTR_MERC_DT_NEC: TIntegerField
      FieldName = 'DT_ENTR_MERC_DT_NEC'
      Origin = 'TREL_NIVEL_SERVICO_PO.DT_ENTR_MERC_DT_NEC'
    end
    object qry_rel_nivel_servico_LookNmMercadoria: TStringField
      FieldKind = fkCalculated
      FieldName = 'LookNmMercadoria'
      Size = 25
      Calculated = True
    end
    object qry_rel_nivel_servico_LookNmEmpresaEst: TStringField
      FieldKind = fkCalculated
      FieldName = 'LookNmEmpresaEst'
      Size = 60
      Calculated = True
    end
  end
  object ds_rel_nivel_servico_: TDataSource
    DataSet = qry_rel_nivel_servico_
    Left = 144
    Top = 528
  end
  object sp_rel_nivel_servico_po: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_nivel_servico_po'
    Left = 264
    Top = 528
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_fornecedor'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
      end>
  end
  object ppBDERelNivelServicoPO: TppBDEPipeline
    DataSource = ds_rel_nivel_servico_
    UserName = 'BDERelNivelServicoPO'
    Left = 376
    Top = 528
    object ppBDERelNivelServicoPOppField1: TppField
      Alignment = taRightJustify
      FieldAlias = 'NR_IDENTIFICADOR'
      FieldName = 'NR_IDENTIFICADOR'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 0
    end
    object ppBDERelNivelServicoPOppField2: TppField
      FieldAlias = 'NR_PROCESSO'
      FieldName = 'NR_PROCESSO'
      FieldLength = 18
      DisplayWidth = 18
      Position = 1
    end
    object ppBDERelNivelServicoPOppField3: TppField
      FieldAlias = 'CD_MERCADORIA'
      FieldName = 'CD_MERCADORIA'
      FieldLength = 15
      DisplayWidth = 15
      Position = 2
    end
    object ppBDERelNivelServicoPOppField4: TppField
      FieldAlias = 'CD_EXPORTADOR'
      FieldName = 'CD_EXPORTADOR'
      FieldLength = 5
      DisplayWidth = 5
      Position = 3
    end
    object ppBDERelNivelServicoPOppField5: TppField
      FieldAlias = 'CD_IMPORTADOR'
      FieldName = 'CD_IMPORTADOR'
      FieldLength = 5
      DisplayWidth = 5
      Position = 4
    end
    object ppBDERelNivelServicoPOppField6: TppField
      FieldAlias = 'DT_ENTR_MERC_FABR'
      FieldName = 'DT_ENTR_MERC_FABR'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 10
      Position = 5
    end
    object ppBDERelNivelServicoPOppField7: TppField
      FieldAlias = 'DT_NECESSIDADE'
      FieldName = 'DT_NECESSIDADE'
      FieldLength = 0
      DataType = dtDateTime
      DisplayWidth = 10
      Position = 6
    end
    object ppBDERelNivelServicoPOppField8: TppField
      Alignment = taRightJustify
      FieldAlias = 'DT_ENTR_MERC_DT_NEC'
      FieldName = 'DT_ENTR_MERC_DT_NEC'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 7
    end
    object ppBDERelNivelServicoPOppField9: TppField
      FieldAlias = 'LookNmMercadoria'
      FieldName = 'LookNmMercadoria'
      FieldLength = 25
      DisplayWidth = 25
      Position = 8
    end
    object ppBDERelNivelServicoPOppField10: TppField
      FieldAlias = 'LookNmEmpresaEst'
      FieldName = 'LookNmEmpresaEst'
      FieldLength = 60
      DisplayWidth = 60
      Position = 9
    end
  end
  object qry_nivel_servico_po_graf_bar_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT COUNT(*) AS QT_TOTAL, '#39' - Depois da Necessidade'#39' AS LABEL'
      'FROM   TREL_NIVEL_SERVICO_PO'
      'WHERE  NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       DT_ENTR_MERC_DT_NEC > '#39'0'#39
      'UNION'
      'SELECT COUNT(*) AS QT_TOTAL, '#39' - Antes da Necessidade'#39' AS LABEL'
      'FROM   TREL_NIVEL_SERVICO_PO'
      'WHERE  NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       DT_ENTR_MERC_DT_NEC <= '#39'0'#39)
    Left = 504
    Top = 528
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_nivel_servico_po_graf_bar_QT_TOTAL: TIntegerField
      FieldName = 'QT_TOTAL'
    end
    object qry_nivel_servico_po_graf_bar_LABEL: TStringField
      FieldName = 'LABEL'
      Size = 24
    end
  end
  object ds_nivel_servico_po_graf: TDataSource
    DataSet = qry_nivel_servico_po_graf_bar_
    Left = 616
    Top = 528
  end
  object ppBDENivelServGRAF: TppBDEPipeline
    DataSource = ds_nivel_servico_po_graf
    UserName = 'BDENivelServGRAF'
    Left = 744
    Top = 528
  end
  object sp_rel_po_emb_web: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_po_emb_web'
    Left = 376
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_ini'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_fim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end>
  end
  object qry_trel_po_emb_web_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT R.*, M.AP_MERCADORIA, U.NM_SIGLA'
      'FROM   TREL_PO_EMB_WEB R, TMERCADORIA M, '
      '             TUNID_MEDIDA_BROKER U'
      'WHERE  R.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       M.CD_UN_MED_COMERC *= U.CD_UNID_MEDIDA AND '
      '       R.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       R.TP_REL = '#39'1'#39
      'ORDER BY R.TP_REL, R.CD_MERCADORIA, R.DT_EMBARQUE')
    Left = 376
    Top = 192
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
        Value = 1
      end>
    object qry_trel_po_emb_web_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
    end
    object qry_trel_po_emb_web_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Size = 30
    end
    object qry_trel_po_emb_web_DT_EMBARQUE: TDateTimeField
      FieldName = 'DT_EMBARQUE'
    end
    object qry_trel_po_emb_web_TP_REL: TStringField
      FieldName = 'TP_REL'
      Size = 1
    end
    object qry_trel_po_emb_web_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
    end
    object qry_trel_po_emb_web_DATA_GRAF: TDateTimeField
      FieldName = 'DATA_GRAF'
    end
    object qry_trel_po_emb_web_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Size = 25
    end
    object qry_trel_po_emb_web_NM_SIGLA: TStringField
      FieldName = 'NM_SIGLA'
      Size = 3
    end
    object qry_trel_po_emb_web_AP_MERC_ANO: TStringField
      FieldName = 'AP_MERC_ANO'
      Size = 35
    end
  end
  object ds_trel_po_emb_web: TDataSource
    DataSet = qry_trel_po_emb_web_
    Left = 376
    Top = 240
  end
  object qry_trel_po_emb_web_graf_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT G.NR_IDENTIFICADOR, G.CD_MERCADORIA, '
      
        '               M.AP_MERCADORIA, SUM( G.QT_MERCADORIA ) AS QT_MER' +
        'CADORIA, '
      '               G.AP_MERC_ANO'
      'FROM   TREL_PO_EMB_WEB G, TMERCADORIA M'
      'WHERE  G.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       G.NR_IDENTIFICADOR =:NR_IDENTIFICADOR AND'
      '       G.TP_REL = '#39'1'#39' '
      'GROUP BY G.NR_IDENTIFICADOR, G.CD_MERCADORIA, '
      '              M.AP_MERCADORIA, G.AP_MERC_ANO'
      'ORDER BY M.AP_MERCADORIA')
    Left = 376
    Top = 288
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_trel_po_emb_web_graf_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TREL_PO_EMB_WEB.NR_IDENTIFICADOR'
    end
    object qry_trel_po_emb_web_graf_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Origin = 'TREL_PO_EMB_WEB.CD_MERCADORIA'
      Size = 30
    end
    object qry_trel_po_emb_web_graf_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Origin = 'TMERCADORIA.AP_MERCADORIA'
      Size = 25
    end
    object qry_trel_po_emb_web_graf_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
      Origin = 'TREL_PO_EMB_WEB.QT_MERCADORIA'
    end
    object qry_trel_po_emb_web_graf_AP_MERC_ANO: TStringField
      FieldName = 'AP_MERC_ANO'
      Origin = 'TREL_PO_EMB_WEB.NR_IDENTIFICADOR'
      Size = 35
    end
  end
  object ppBDETRelPoEmb: TppBDEPipeline
    DataSource = ds_trel_po_emb_web
    UserName = 'BDETRelPoEmb'
    Left = 376
    Top = 384
  end
  object ppBDETRelPoEmbGraf: TppBDEPipeline
    DataSource = ds_trel_po_emb_web_graf_
    UserName = 'BDETRelPoEmbGraf'
    Left = 504
    Top = 336
  end
  object ds_trel_po_emb_web_graf_: TDataSource
    DataSet = qry_trel_po_emb_web_graf_
    Left = 376
    Top = 336
  end
  object qry_graf1_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CONVERT( int,'
      
        '       RIGHT( '#39'0'#39' + CONVERT( varchar, DATEPART ( mm , G.DATA_GRA' +
        'F ) ), 2 ) +'
      
        '                    CONVERT( varchar, DATEPART ( yyyy , G.DATA_G' +
        'RAF ) ) ) AS DATA_GRAF,'
      '       SUM( G.QT_MERCADORIA ) AS QT_MERCADORIA, G.AP_MERC_ANO'
      'FROM   TREL_PO_EMB_WEB G, TMERCADORIA M'
      'WHERE  G.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       G.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       G.CD_MERCADORIA = '#39'50259832'#39' -- LUPRANAT T 80'
      
        'GROUP BY CONVERT( int, RIGHT( '#39'0'#39' + CONVERT( varchar, DATEPART (' +
        ' mm , G.DATA_GRAF ) ), 2 ) +'
      
        #9'                            CONVERT( varchar, DATEPART ( yyyy ,' +
        ' G.DATA_GRAF ) ) ), G.AP_MERC_ANO'
      'ORDER BY AP_MERC_ANO')
    Left = 264
    Top = 643
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_graf1_DATA_GRAF: TIntegerField
      FieldName = 'DATA_GRAF'
    end
    object qry_graf1_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
    end
    object qry_graf1_AP_MERC_ANO: TStringField
      FieldName = 'AP_MERC_ANO'
      Size = 35
    end
  end
  object ds_graf1: TDataSource
    DataSet = qry_graf1_
    Left = 138
    Top = 643
  end
  object qry_graf2_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CONVERT( int,'
      
        '       RIGHT( '#39'0'#39' + CONVERT( varchar, DATEPART ( mm , G.DATA_GRA' +
        'F ) ), 2 ) +'
      
        '                    CONVERT( varchar, DATEPART ( yyyy , G.DATA_G' +
        'RAF ) ) ) AS DATA_GRAF,'
      '       SUM( G.QT_MERCADORIA ) AS QT_MERCADORIA, G.AP_MERC_ANO'
      'FROM   TREL_PO_EMB_WEB G, TMERCADORIA M'
      'WHERE  G.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       G.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       G.CD_MERCADORIA = '#39'51528228'#39' -- PLURACOL 1385 POLYOL'
      
        'GROUP BY CONVERT( int, RIGHT( '#39'0'#39' + CONVERT( varchar, DATEPART (' +
        ' mm , G.DATA_GRAF ) ), 2 ) +'
      
        #9'                            CONVERT( varchar, DATEPART ( yyyy ,' +
        ' G.DATA_GRAF ) ) ), G.AP_MERC_ANO'
      'ORDER BY AP_MERC_ANO')
    Left = 621
    Top = 643
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_graf2_DATA_GRAF: TIntegerField
      FieldName = 'DATA_GRAF'
    end
    object qry_graf2_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
    end
    object qry_graf2_AP_MERC_ANO: TStringField
      FieldName = 'AP_MERC_ANO'
      Size = 35
    end
  end
  object ds_graf2: TDataSource
    DataSet = qry_graf2_
    Left = 504
    Top = 643
  end
  object qry_graf3_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT CONVERT( int,'
      
        '       RIGHT( '#39'0'#39' + CONVERT( varchar, DATEPART ( mm , G.DATA_GRA' +
        'F ) ), 2 ) +'
      
        '                    CONVERT( varchar, DATEPART ( yyyy , G.DATA_G' +
        'RAF ) ) ) AS DATA_GRAF,'
      '       SUM( G.QT_MERCADORIA ) AS QT_MERCADORIA, G.AP_MERC_ANO'
      'FROM   TREL_PO_EMB_WEB G, TMERCADORIA M'
      'WHERE  G.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '       G.NR_IDENTIFICADOR = :NR_IDENTIFICADOR AND'
      '       G.CD_MERCADORIA = '#39'50999818'#39' -- PLURACOL 538 POLYOL'
      
        'GROUP BY CONVERT( int, RIGHT( '#39'0'#39' + CONVERT( varchar, DATEPART (' +
        ' mm , G.DATA_GRAF ) ), 2 ) +'
      
        #9'                            CONVERT( varchar, DATEPART ( yyyy ,' +
        ' G.DATA_GRAF ) ) ), G.AP_MERC_ANO'
      'ORDER BY AP_MERC_ANO')
    Left = 872
    Top = 643
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end>
    object qry_graf3_DATA_GRAF: TIntegerField
      FieldName = 'DATA_GRAF'
    end
    object qry_graf3_QT_MERCADORIA: TFloatField
      FieldName = 'QT_MERCADORIA'
    end
    object qry_graf3_AP_MERC_ANO: TStringField
      FieldName = 'AP_MERC_ANO'
      Size = 35
    end
  end
  object ds_graf3: TDataSource
    DataSet = qry_graf3_
    Left = 751
    Top = 643
  end
  object qry_grupo_area_: TQuery
    DatabaseName = 'DBBROKER'
    Left = 40
  end
  object ds_grupo_area: TDataSource
    DataSet = qry_grupo_area_
    Left = 144
  end
  object ds_rel_rent_proc_capa_pedido: TDataSource
    DataSet = qry_rel_rent_proc_capa_pedido_
    Left = 744
    Top = 576
  end
  object qry_rel_rent_proc_capa_pedido_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT DISTINCT A.*, M.AP_MERCADORIA FROM'
      'TAUX_REL_RENT_PROC A, TMERCADORIA M'
      'WHERE A.CD_MERCADORIA = M.CD_MERCADORIA AND'
      '    NR_IDENTIFICADOR =:NR_IDENTIFICADOR AND'
      '    NR_PROCESSO =:NR_PROCESSO'
      'ORDER BY CD_REFERENCIA')
    Left = 864
    Top = 576
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NR_IDENTIFICADOR'
        ParamType = ptUnknown
      end
      item
        DataType = ftString
        Name = 'NR_PROCESSO'
        ParamType = ptUnknown
      end>
    object qry_rel_rent_proc_capa_pedido_CD_REFERENCIA: TStringField
      FieldName = 'CD_REFERENCIA'
      Origin = 'TREL_RENT_PROC_CAPA.CD_REFERENCIA'
      Size = 15
    end
    object qry_rel_rent_proc_capa_pedido_CD_MERCADORIA: TStringField
      FieldName = 'CD_MERCADORIA'
      Origin = 'TREL_RENT_PROC_CAPA.CD_MERCADORIA'
      Size = 15
    end
    object qry_rel_rent_proc_capa_pedido_AP_MERCADORIA: TStringField
      FieldName = 'AP_MERCADORIA'
      Origin = 'TMERCADORIA.AP_MERCADORIA'
      Size = 25
    end
    object qry_rel_rent_proc_capa_pedido_NR_IDENTIFICADOR: TIntegerField
      FieldName = 'NR_IDENTIFICADOR'
      Origin = 'TAUX_REL_RENT_PROC.NR_IDENTIFICADOR'
    end
    object qry_rel_rent_proc_capa_pedido_NR_PROCESSO: TStringField
      FieldName = 'NR_PROCESSO'
      Origin = 'TAUX_REL_RENT_PROC.NR_IDENTIFICADOR'
      Size = 18
    end
  end
  object ppBDERelProcCapaPedido: TppBDEPipeline
    DataSource = ds_rel_rent_proc_capa_pedido
    UserName = 'BDERelProcCapaPedido'
    Left = 744
    Top = 432
    object ppBDERelProcCapaPedidoppField1: TppField
      FieldAlias = 'CD_REFERENCIA'
      FieldName = 'CD_REFERENCIA'
      FieldLength = 0
      DisplayWidth = 0
      Position = 0
    end
    object ppBDERelProcCapaPedidoppField2: TppField
      FieldAlias = 'CD_MERCADORIA'
      FieldName = 'CD_MERCADORIA'
      FieldLength = 15
      DisplayWidth = 15
      Position = 1
    end
    object ppBDERelProcCapaPedidoppField3: TppField
      FieldAlias = 'AP_MERCADORIA'
      FieldName = 'AP_MERCADORIA'
      FieldLength = 25
      DisplayWidth = 25
      Position = 2
    end
    object ppBDERelProcCapaPedidoppField4: TppField
      Alignment = taRightJustify
      FieldAlias = 'NR_IDENTIFICADOR'
      FieldName = 'NR_IDENTIFICADOR'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 3
    end
    object ppBDERelProcCapaPedidoppField5: TppField
      FieldAlias = 'NR_PROCESSO'
      FieldName = 'NR_PROCESSO'
      FieldLength = 18
      DisplayWidth = 18
      Position = 4
    end
  end
  object qry_prev_po_nacion_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      '                ')
    Left = 744
    Top = 96
    object qry_prev_po_nacion_NR_REFERENCIA: TStringField
      FieldName = 'NR_REFERENCIA'
      Size = 18
    end
    object qry_prev_po_nacion_NR_ITEM_CLIENTE: TStringField
      DisplayWidth = 4
      FieldName = 'NR_ITEM_CLIENTE'
      Size = 4
    end
    object qry_prev_po_nacion_VL_MERC_DOLAR: TFloatField
      FieldName = 'VL_MERC_DOLAR'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_VL_MERC_REAL: TFloatField
      FieldName = 'VL_MERC_REAL'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_VL_ALIQ_II: TFloatField
      FieldName = 'VL_ALIQ_II'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_VL_II: TFloatField
      FieldName = 'VL_II'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_VL_ALIQ_IPI: TFloatField
      FieldName = 'VL_ALIQ_IPI'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_VL_IPI: TFloatField
      FieldName = 'VL_IPI'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_VL_ICMS: TFloatField
      FieldName = 'VL_ICMS'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_DT_PREV_NACION: TDateTimeField
      FieldName = 'DT_PREV_NACION'
    end
    object qry_prev_po_nacion_DT_NACIONALIZACAO: TDateTimeField
      FieldName = 'DT_NACIONALIZACAO'
    end
    object qry_prev_po_nacion_VL_PIS: TFloatField
      FieldName = 'VL_PIS'
      Origin = 'TREL_PREV_PO_NACION.VL_MERC_DOLAR'
    end
    object qry_prev_po_nacion_VL_COFINS: TFloatField
      FieldName = 'VL_COFINS'
      Origin = 'TREL_PREV_PO_NACION.VL_MERC_REAL'
    end
    object qry_prev_po_nacion_VL_ALIQ_PIS: TFloatField
      FieldName = 'VL_ALIQ_PIS'
      Origin = 'TREL_PREV_PO_NACION.VL_ALIQ_II'
    end
    object qry_prev_po_nacion_VL_ALIQ_COFINS: TFloatField
      FieldName = 'VL_ALIQ_COFINS'
      Origin = 'TREL_PREV_PO_NACION.VL_II'
    end
  end
  object qry_prev_po_nacion_totais_: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      '')
    Left = 744
    Top = 48
    object qry_prev_po_nacion_totais_REFERENCIAS: TIntegerField
      FieldName = 'REFERENCIAS'
    end
    object qry_prev_po_nacion_totais_VL_DOLAR: TFloatField
      FieldName = 'VL_DOLAR'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_totais_VL_REAL: TFloatField
      FieldName = 'VL_REAL'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_totais_VL_II: TFloatField
      FieldName = 'VL_II'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_totais_VL_IPI: TFloatField
      FieldName = 'VL_IPI'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_totais_VL_ICMS: TFloatField
      FieldName = 'VL_ICMS'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_totais_VL_PIS: TFloatField
      FieldName = 'VL_PIS'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
    object qry_prev_po_nacion_totais_VL_COFINS: TFloatField
      FieldName = 'VL_COFINS'
      DisplayFormat = '#0,.00'
      EditFormat = '#0.00'
    end
  end
  object qry_prev_po_nacion_subtotais_: TQuery
    DatabaseName = 'DBBROKER'
    Left = 864
    Top = 48
    object qry_prev_po_nacion_subtotais_VL_DOLAR: TFloatField
      FieldName = 'VL_DOLAR'
    end
    object qry_prev_po_nacion_subtotais_VL_REAL: TFloatField
      FieldName = 'VL_REAL'
    end
    object qry_prev_po_nacion_subtotais_VL_II: TFloatField
      FieldName = 'VL_II'
    end
    object qry_prev_po_nacion_subtotais_VL_IPI: TFloatField
      FieldName = 'VL_IPI'
    end
    object qry_prev_po_nacion_subtotais_VL_ICMS: TFloatField
      FieldName = 'VL_ICMS'
    end
    object qry_prev_po_nacion_subtotais_VL_PIS: TFloatField
      FieldName = 'VL_PIS'
    end
    object qry_prev_po_nacion_subtotais_VL_COFINS: TFloatField
      FieldName = 'VL_COFINS'
    end
  end
  object sp_rel_previsao_po_nacionalizar: TStoredProc
    DatabaseName = 'DBBROKER'
    StoredProcName = 'dbo.sp_rel_previsao_po_nacionalizar'
    Left = 745
    ParamData = <
      item
        DataType = ftInteger
        Name = 'Result'
        ParamType = ptResult
        Value = 0
      end
      item
        DataType = ftString
        Name = '@cd_unid_neg'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_grupo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_cliente'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = '@cd_area'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_inicio'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = '@dt_final'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = '@nr_identificador'
        ParamType = ptOutput
        Value = 1
      end
      item
        DataType = ftInteger
        Name = '@nr_registros'
        ParamType = ptOutput
        Value = 0
      end>
  end
  object ds_saldo_pendente: TDataSource
    DataSet = qry_saldo_pendente_
    Left = 960
    Top = 488
  end
  object qry_saldo_pendente_: TQuery
    DatabaseName = 'DBBROKER'
    Left = 960
    Top = 440
    object qry_saldo_pendente_CD_GRUPO: TStringField
      FieldName = 'CD_GRUPO'
      FixedChar = True
      Size = 3
    end
    object qry_saldo_pendente_NM_GRUPO: TStringField
      FieldName = 'NM_GRUPO'
      FixedChar = True
      Size = 50
    end
    object qry_saldo_pendente_CD_CLIENTE: TStringField
      FieldName = 'CD_CLIENTE'
      FixedChar = True
      Size = 5
    end
    object qry_saldo_pendente_NM_EMPRESA: TStringField
      FieldName = 'NM_EMPRESA'
      FixedChar = True
      Size = 50
    end
    object qry_saldo_pendente_AP_UNID_NEG: TStringField
      FieldName = 'AP_UNID_NEG'
      FixedChar = True
      Size = 10
    end
    object qry_saldo_pendente_NM_PRODUTO: TStringField
      FieldName = 'NM_PRODUTO'
      FixedChar = True
      Size = 40
    end
    object qry_saldo_pendente_PROCESSO: TStringField
      FieldName = 'PROCESSO'
      FixedChar = True
      Size = 10
    end
    object qry_saldo_pendente_Pedido: TStringField
      FieldName = 'Pedido'
      FixedChar = True
    end
    object qry_saldo_pendente_DT_FATURA: TDateTimeField
      FieldName = 'DT_FATURA'
    end
    object qry_saldo_pendente_NR_SOLICITACAO: TStringField
      FieldName = 'NR_SOLICITACAO'
      FixedChar = True
      Size = 6
    end
    object qry_saldo_pendente_DT_VENCTO_FATURA: TDateTimeField
      FieldName = 'DT_VENCTO_FATURA'
    end
    object qry_saldo_pendente_VL_FATURA: TFloatField
      FieldName = 'VL_FATURA'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
    object qry_saldo_pendente_DT_LIQUIDACAO: TDateTimeField
      FieldName = 'DT_LIQUIDACAO'
    end
    object qry_saldo_pendente_VL_LIQUIDACAO: TFloatField
      FieldName = 'VL_LIQUIDACAO'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
    object qry_saldo_pendente_Saldo: TFloatField
      FieldName = 'Saldo'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
    object qry_saldo_pendente_DiasAtrasado: TIntegerField
      FieldName = 'Dias  Atrasado'
    end
    object qry_saldo_pendente_COMISSAO: TFloatField
      FieldName = 'COMISSAO'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
  end
  object qry_tot_saldo_pendente: TQuery
    DatabaseName = 'DBBROKER'
    Left = 960
    Top = 576
    object qry_tot_saldo_pendenteAP_UNID_NEG: TStringField
      FieldName = 'AP_UNID_NEG'
      FixedChar = True
      Size = 10
    end
    object qry_tot_saldo_pendenteNM_PRODUTO: TStringField
      FieldName = 'NM_PRODUTO'
      FixedChar = True
      Size = 40
    end
    object qry_tot_saldo_pendenteVL_FATURA: TFloatField
      FieldName = 'VL_FATURA'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
    object qry_tot_saldo_pendenteVL_LIQUIDACAO: TFloatField
      FieldName = 'VL_LIQUIDACAO'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
    object qry_tot_saldo_pendenteVL_SALDO_PENDENTE: TFloatField
      FieldName = 'VL_SALDO_PENDENTE'
      DisplayFormat = '#.##0,00'
      EditFormat = '#.##0,00'
    end
  end
  object ds_tot_saldo_pendente: TDataSource
    DataSet = qry_tot_saldo_pendente
    Left = 960
    Top = 640
  end
  object ppBDERel_TotSaldoPendente: TppBDEPipeline
    DataSource = ds_tot_saldo_pendente
    UserName = 'BDERel_TotSaldoPendente'
    Left = 952
    Top = 536
    object ppBDERel_TotSaldoPendenteppField1: TppField
      FieldAlias = 'AP_UNID_NEG'
      FieldName = 'AP_UNID_NEG'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 0
      Searchable = False
      Sortable = False
    end
    object ppBDERel_TotSaldoPendenteppField2: TppField
      FieldAlias = 'NM_PRODUTO'
      FieldName = 'NM_PRODUTO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 1
      Searchable = False
      Sortable = False
    end
    object ppBDERel_TotSaldoPendenteppField3: TppField
      FieldAlias = 'VL_FATURA'
      FieldName = 'VL_FATURA'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 2
      Searchable = False
      Sortable = False
    end
    object ppBDERel_TotSaldoPendenteppField4: TppField
      FieldAlias = 'VL_LIQUIDACAO'
      FieldName = 'VL_LIQUIDACAO'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 3
      Searchable = False
      Sortable = False
    end
    object ppBDERel_TotSaldoPendenteppField5: TppField
      FieldAlias = 'VL_SALDO_PENDENTE'
      FieldName = 'VL_SALDO_PENDENTE'
      FieldLength = 0
      DataType = dtNotKnown
      DisplayWidth = 0
      Position = 4
      Searchable = False
      Sortable = False
    end
  end
  object ppBDERel_SaldoPendente: TppBDEPipeline
    DataSource = ds_saldo_pendente
    UserName = 'BDERel_SaldoPendente'
    Left = 960
    Top = 408
  end
  object ppReport1: TppReport
    AutoStop = False
    DataPipeline = ppBDERel_SaldoPendente
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report2'
    PrinterSetup.Orientation = poLandscape
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 210000
    PrinterSetup.mmPaperWidth = 297000
    PrinterSetup.PaperSize = 9
    Template.FileName = 'C:\DDBroker\Rel_SaldoPendente.rtm'
    Units = utPrinterPixels
    DeviceType = 'Screen'
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = False
    OutlineSettings.Visible = False
    TextSearchSettings.DefaultString = '<FindText>'
    TextSearchSettings.Enabled = False
    Left = 952
    Top = 352
    Version = '7.02'
    mmColumnWidth = 0
    DataPipelineName = 'ppBDERel_SaldoPendente'
    object ppHeaderBand3: TppHeaderBand
      mmBottomOffset = 0
      mmHeight = 34925
      mmPrintPosition = 0
      object ppLabel3: TppLabel
        UserName = 'Label1'
        Caption = 'Rela'#231#227'o de Saldos Pendentes'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 16
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 6615
        mmLeft = 115888
        mmTop = 21696
        mmWidth = 79640
        BandType = 0
      end
      object ppImage1: TppImage
        UserName = 'Image1'
        AutoSize = True
        MaintainAspectRatio = False
        Picture.Data = {
          
          889560199662399664599666799606689996551004003B}
        mmHeight = 18256
        mmLeft = 8213
        mmTop = 1058
        mmWidth = 47625
        BandType = 0
      end
      object ppLine2: TppLine
        UserName = 'Line2'
        Pen.Width = 2
        Weight = 1.500000000000000000
        mmHeight = 529
        mmLeft = 0
        mmTop = 29369
        mmWidth = 274373
        BandType = 0
      end
      object ppShape1: TppShape
        UserName = 'Shape1'
        Brush.Color = 13882323
        Pen.Color = 13882323
        mmHeight = 3969
        mmLeft = 265
        mmTop = 30163
        mmWidth = 274109
        BandType = 0
      end
      object ppLabel4: TppLabel
        UserName = 'Label2'
        Caption = 'Processo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 31750
        mmTop = 30427
        mmWidth = 16933
        BandType = 0
      end
      object ppLabel5: TppLabel
        UserName = 'Label3'
        Caption = 'Ref. Cliente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 51858
        mmTop = 30427
        mmWidth = 17463
        BandType = 0
      end
      object ppLabel6: TppLabel
        UserName = 'Label4'
        Caption = 'Dt Fatura'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 81227
        mmTop = 30427
        mmWidth = 13229
        BandType = 0
      end
      object ppLabel7: TppLabel
        UserName = 'Label5'
        Caption = 'N'#250'mero'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 99484
        mmTop = 30427
        mmWidth = 12965
        BandType = 0
      end
      object ppLabel8: TppLabel
        UserName = 'Label6'
        Caption = 'Dt Venc'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 118004
        mmTop = 30427
        mmWidth = 11642
        BandType = 0
      end
      object ppLabel9: TppLabel
        UserName = 'Label7'
        Caption = 'Valor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 142082
        mmTop = 30427
        mmWidth = 8202
        BandType = 0
      end
      object ppLabel16: TppLabel
        UserName = 'Label8'
        Caption = 'Dt Liq.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3440
        mmLeft = 178859
        mmTop = 30427
        mmWidth = 8731
        BandType = 0
      end
      object ppLabel17: TppLabel
        UserName = 'Label9'
        Caption = 'Pagto/Receb'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3440
        mmLeft = 195263
        mmTop = 30427
        mmWidth = 21431
        BandType = 0
      end
      object ppLabel24: TppLabel
        UserName = 'Label10'
        Caption = 'Sd. Pendente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3440
        mmLeft = 225690
        mmTop = 30427
        mmWidth = 21167
        BandType = 0
      end
      object ppLabel25: TppLabel
        UserName = 'Label11'
        Caption = 'Dias em atraso'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3440
        mmLeft = 249767
        mmTop = 30427
        mmWidth = 21696
        BandType = 0
      end
      object ppLine6: TppLine
        UserName = 'Line6'
        Pen.Width = 2
        Weight = 1.500000000000000000
        mmHeight = 529
        mmLeft = 0
        mmTop = 34396
        mmWidth = 274373
        BandType = 0
      end
      object ppLabel35: TppLabel
        UserName = 'Label15'
        Caption = 'Comiss'#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3440
        mmLeft = 158221
        mmTop = 30692
        mmWidth = 13494
        BandType = 0
      end
    end
    object ppDetailBand2: TppDetailBand
      mmBottomOffset = 0
      mmHeight = 3704
      mmPrintPosition = 0
      object ppDBText3: TppDBText
        UserName = 'DBText1'
        DataField = 'PROCESSO'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 31750
        mmTop = 0
        mmWidth = 17727
        BandType = 4
      end
      object ppDBText4: TppDBText
        UserName = 'DBText2'
        DataField = 'Pedido'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 51858
        mmTop = 0
        mmWidth = 26194
        BandType = 4
      end
      object ppDBText5: TppDBText
        UserName = 'DBText3'
        DataField = 'DT_FATURA'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 79375
        mmTop = 0
        mmWidth = 15663
        BandType = 4
      end
      object ppDBText6: TppDBText
        UserName = 'DBText4'
        DataField = 'VL_FATURA'
        DataPipeline = ppBDERel_SaldoPendente
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 135467
        mmTop = 0
        mmWidth = 20373
        BandType = 4
      end
      object ppDBText8: TppDBText
        UserName = 'DBText5'
        DataField = 'NR_SOLICITACAO'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 98690
        mmTop = 0
        mmWidth = 12435
        BandType = 4
      end
      object ppDBText22: TppDBText
        UserName = 'DBText6'
        DataField = 'DT_VENCTO_FATURA'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 115359
        mmTop = 0
        mmWidth = 15663
        BandType = 4
      end
      object ppDBText23: TppDBText
        UserName = 'DBText7'
        DataField = 'DT_LIQUIDACAO'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 175155
        mmTop = 0
        mmWidth = 15663
        BandType = 4
      end
      object ppDBText24: TppDBText
        UserName = 'DBText8'
        DataField = 'Dias  Atrasado'
        DataPipeline = ppBDERel_SaldoPendente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taCentered
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 250032
        mmTop = 0
        mmWidth = 19579
        BandType = 4
      end
      object ppDBText25: TppDBText
        UserName = 'DBText9'
        DataField = 'VL_LIQUIDACAO'
        DataPipeline = ppBDERel_SaldoPendente
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 191294
        mmTop = 0
        mmWidth = 25400
        BandType = 4
      end
      object ppDBText26: TppDBText
        UserName = 'DBText101'
        DataField = 'Saldo'
        DataPipeline = ppBDERel_SaldoPendente
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        ParentDataPipeline = False
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 222250
        mmTop = 0
        mmWidth = 20902
        BandType = 4
      end
      object ppDBText38: TppDBText
        UserName = 'DBText108'
        DataField = 'COMISSAO'
        DataPipeline = ppBDERel_SaldoPendente
        DisplayFormat = '#,0.00;-#,0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 8
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppBDERel_SaldoPendente'
        mmHeight = 3175
        mmLeft = 157163
        mmTop = 0
        mmWidth = 15610
        BandType = 4
      end
    end
    object ppFooterBand3: TppFooterBand
      PrintOnFirstPage = False
      mmBottomOffset = 0
      mmHeight = 0
      mmPrintPosition = 0
    end
    object ppGroup1: TppGroup
      BreakName = 'ppLabel3'
      BreakType = btCustomField
      KeepTogether = True
      OutlineSettings.CreateNode = True
      UserName = 'Group2'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = ''
      object ppGroupHeaderBand1: TppGroupHeaderBand
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand1: TppGroupFooterBand
        PrintHeight = phDynamic
        mmBottomOffset = 0
        mmHeight = 37306
        mmPrintPosition = 0
        object ppSubReport2: TppSubReport
          UserName = 'SubReport1'
          ExpandAll = False
          NewPrintJob = False
          OutlineSettings.CreateNode = True
          TraverseAllData = False
          DataPipelineName = 'ppBDERel_TotSaldoPendente'
          mmHeight = 37571
          mmLeft = 0
          mmTop = 0
          mmWidth = 284300
          BandType = 5
          GroupNo = 0
          mmBottomOffset = 0
          mmOverFlowOffset = 0
          mmStopPosition = 0
          object ppChildReport2: TppChildReport
            AutoStop = False
            DataPipeline = ppBDERel_TotSaldoPendente
            PrinterSetup.BinName = 'Default'
            PrinterSetup.DocumentName = 'Report2'
            PrinterSetup.Orientation = poLandscape
            PrinterSetup.PaperName = 'A4'
            PrinterSetup.PrinterName = 'Default'
            PrinterSetup.mmMarginBottom = 6350
            PrinterSetup.mmMarginLeft = 6350
            PrinterSetup.mmMarginRight = 6350
            PrinterSetup.mmMarginTop = 6350
            PrinterSetup.mmPaperHeight = 210000
            PrinterSetup.mmPaperWidth = 297000
            PrinterSetup.PaperSize = 9
            Units = utPrinterPixels
            Version = '7.02'
            mmColumnWidth = 0
            DataPipelineName = 'ppBDERel_TotSaldoPendente'
            object ppTitleBand1: TppTitleBand
              mmBottomOffset = 0
              mmHeight = 6615
              mmPrintPosition = 0
              object ppLabel26: TppLabel
                UserName = 'Label14'
                Caption = 'TOTAIS'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                Transparent = True
                mmHeight = 3810
                mmLeft = 9260
                mmTop = 2646
                mmWidth = 11599
                BandType = 1
              end
              object ppLabel27: TppLabel
                UserName = 'Label1'
                Caption = 'Valor Fatura'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                TextAlignment = taRightJustified
                Transparent = True
                mmHeight = 3810
                mmLeft = 126143
                mmTop = 2646
                mmWidth = 18584
                BandType = 1
              end
              object ppLabel28: TppLabel
                UserName = 'Label2'
                Caption = 'Valor Liquida'#231#227'o'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                TextAlignment = taRightJustified
                Transparent = True
                mmHeight = 3810
                mmLeft = 166423
                mmTop = 2646
                mmWidth = 25696
                BandType = 1
              end
              object ppLabel29: TppLabel
                UserName = 'Label3'
                Caption = 'Valor Saldo'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                TextAlignment = taRightJustified
                Transparent = True
                mmHeight = 3810
                mmLeft = 206577
                mmTop = 2646
                mmWidth = 17526
                BandType = 1
              end
            end
            object ppDetailBand3: TppDetailBand
              mmBottomOffset = 0
              mmHeight = 4233
              mmPrintPosition = 0
              object ppDBText27: TppDBText
                UserName = 'DBText11'
                DataField = 'NM_PRODUTO'
                DataPipeline = ppBDERel_TotSaldoPendente
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                ParentDataPipeline = False
                Transparent = True
                DataPipelineName = 'ppBDERel_TotSaldoPendente'
                mmHeight = 3810
                mmLeft = 42069
                mmTop = 0
                mmWidth = 66940
                BandType = 4
              end
              object ppDBText28: TppDBText
                UserName = 'DBText10'
                DataField = 'AP_UNID_NEG'
                DataPipeline = ppBDERel_TotSaldoPendente
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                ParentDataPipeline = False
                Transparent = True
                DataPipelineName = 'ppBDERel_TotSaldoPendente'
                mmHeight = 3810
                mmLeft = 9260
                mmTop = 0
                mmWidth = 29633
                BandType = 4
              end
              object ppDBText29: TppDBText
                UserName = 'DBText1'
                DataField = 'VL_FATURA'
                DataPipeline = ppBDERel_TotSaldoPendente
                DisplayFormat = '#,0.00;-#,0.00'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                ParentDataPipeline = False
                TextAlignment = taRightJustified
                Transparent = True
                DataPipelineName = 'ppBDERel_TotSaldoPendente'
                mmHeight = 3810
                mmLeft = 112448
                mmTop = 0
                mmWidth = 32279
                BandType = 4
              end
              object ppDBText30: TppDBText
                UserName = 'DBText2'
                DataField = 'VL_LIQUIDACAO'
                DataPipeline = ppBDERel_TotSaldoPendente
                DisplayFormat = '#,0.00;-#,0.00'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                ParentDataPipeline = False
                TextAlignment = taRightJustified
                Transparent = True
                DataPipelineName = 'ppBDERel_TotSaldoPendente'
                mmHeight = 3810
                mmLeft = 159809
                mmTop = 0
                mmWidth = 32279
                BandType = 4
              end
              object ppDBText31: TppDBText
                UserName = 'DBText3'
                DataField = 'VL_SALDO_PENDENTE'
                DataPipeline = ppBDERel_TotSaldoPendente
                DisplayFormat = '#,0.00;-#,0.00'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Name = 'Arial'
                Font.Size = 9
                Font.Style = [fsBold]
                ParentDataPipeline = False
                TextAlignment = taRightJustified
                Transparent = True
                DataPipelineName = 'ppBDERel_TotSaldoPendente'
                mmHeight = 3810
                mmLeft = 195792
                mmTop = 0
                mmWidth = 28310
                BandType = 4
              end
            end
            object ppSummaryBand1: TppSummaryBand
              mmBottomOffset = 0
              mmHeight = 0
              mmPrintPosition = 0
            end
          end
        end
      end
    end
    object ppGroup3: TppGroup
      BreakName = 'CD_GRUPO'
      DataPipeline = ppBDERel_SaldoPendente
      OutlineSettings.CreateNode = True
      UserName = 'Group1'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppBDERel_SaldoPendente'
      object ppGroupHeaderBand3: TppGroupHeaderBand
        mmBottomOffset = 0
        mmHeight = 4233
        mmPrintPosition = 0
        object ppLabel30: TppLabel
          UserName = 'Label12'
          Caption = 'Grupo:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3440
          mmLeft = 5027
          mmTop = 0
          mmWidth = 9525
          BandType = 3
          GroupNo = 0
        end
        object ppDBText32: TppDBText
          UserName = 'DBText102'
          DataField = 'CD_GRUPO'
          DataPipeline = ppBDERel_SaldoPendente
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 15875
          mmTop = 0
          mmWidth = 6879
          BandType = 3
          GroupNo = 0
        end
        object ppDBText33: TppDBText
          UserName = 'DBText103'
          DataField = 'NM_GRUPO'
          DataPipeline = ppBDERel_SaldoPendente
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3969
          mmLeft = 23813
          mmTop = 0
          mmWidth = 92604
          BandType = 3
          GroupNo = 0
        end
      end
      object ppGroupFooterBand3: TppGroupFooterBand
        mmBottomOffset = 0
        mmHeight = 5556
        mmPrintPosition = 0
        object ppLabel31: TppLabel
          UserName = 'Label28'
          Caption = 'Total do Grupo'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3440
          mmLeft = 5027
          mmTop = 529
          mmWidth = 27517
          BandType = 5
          GroupNo = 0
        end
        object ppDBCalc4: TppDBCalc
          UserName = 'DBCalc4'
          DataField = 'VL_FATURA'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 135996
          mmTop = 529
          mmWidth = 19844
          BandType = 5
          GroupNo = 0
        end
        object ppDBCalc5: TppDBCalc
          UserName = 'DBCalc5'
          DataField = 'VL_LIQUIDACAO'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 199496
          mmTop = 529
          mmWidth = 17198
          BandType = 5
          GroupNo = 0
        end
        object ppDBCalc6: TppDBCalc
          UserName = 'DBCalc6'
          DataField = 'Saldo'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup3
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 226219
          mmTop = 529
          mmWidth = 17198
          BandType = 5
          GroupNo = 0
        end
        object ppLine5: TppLine
          UserName = 'Line1'
          Pen.Width = 2
          Weight = 1.500000000000000000
          mmHeight = 529
          mmLeft = 0
          mmTop = 4498
          mmWidth = 274373
          BandType = 5
          GroupNo = 1
        end
        object ppDBText41: TppDBText
          UserName = 'DBText401'
          DataField = 'COMISSAO'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 157692
          mmTop = 529
          mmWidth = 15610
          BandType = 5
          GroupNo = 1
        end
      end
    end
    object ppGroup6: TppGroup
      BreakName = 'CD_CLIENTE'
      DataPipeline = ppBDERel_SaldoPendente
      OutlineSettings.CreateNode = True
      UserName = 'Group6'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppBDERel_SaldoPendente'
      object ppGroupHeaderBand6: TppGroupHeaderBand
        mmBottomOffset = 0
        mmHeight = 4233
        mmPrintPosition = 0
        object ppShape2: TppShape
          UserName = 'Shape2'
          Brush.Color = 13882323
          Pen.Color = 13882323
          mmHeight = 3440
          mmLeft = 20108
          mmTop = 265
          mmWidth = 254265
          BandType = 3
          GroupNo = 2
        end
        object ppDBText34: TppDBText
          UserName = 'DBText104'
          DataField = 'CD_CLIENTE'
          DataPipeline = ppBDERel_SaldoPendente
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 37571
          mmTop = 265
          mmWidth = 9525
          BandType = 3
          GroupNo = 1
        end
        object ppDBText35: TppDBText
          UserName = 'DBText105'
          DataField = 'NM_EMPRESA'
          DataPipeline = ppBDERel_SaldoPendente
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 49742
          mmTop = 265
          mmWidth = 119327
          BandType = 3
          GroupNo = 1
        end
        object ppLabel32: TppLabel
          UserName = 'Label14'
          Caption = 'Cliente:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3387
          mmLeft = 20638
          mmTop = 265
          mmWidth = 11906
          BandType = 3
          GroupNo = 2
        end
      end
      object ppGroupFooterBand6: TppGroupFooterBand
        mmBottomOffset = 0
        mmHeight = 5027
        mmPrintPosition = 0
        object ppLabel33: TppLabel
          UserName = 'Label29'
          Caption = 'Total do Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3387
          mmLeft = 20638
          mmTop = 529
          mmWidth = 21251
          BandType = 5
          GroupNo = 1
        end
        object ppDBCalc7: TppDBCalc
          UserName = 'DBCalc7'
          DataField = 'VL_FATURA'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup6
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 135996
          mmTop = 529
          mmWidth = 19844
          BandType = 5
          GroupNo = 1
        end
        object ppDBCalc8: TppDBCalc
          UserName = 'DBCalc8'
          DataField = 'VL_LIQUIDACAO'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup6
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 199232
          mmTop = 529
          mmWidth = 17198
          BandType = 5
          GroupNo = 1
        end
        object ppDBCalc9: TppDBCalc
          UserName = 'DBCalc9'
          DataField = 'Saldo'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup6
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 225955
          mmTop = 529
          mmWidth = 17198
          BandType = 5
          GroupNo = 1
        end
        object ppDBText40: TppDBText
          UserName = 'DBText40'
          DataField = 'COMISSAO'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 157427
          mmTop = 529
          mmWidth = 15610
          BandType = 5
          GroupNo = 2
        end
      end
    end
    object ppGroup7: TppGroup
      BreakName = 'AP_UNID_NEG'
      DataPipeline = ppBDERel_SaldoPendente
      OutlineSettings.CreateNode = True
      UserName = 'Group7'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppBDERel_SaldoPendente'
      object ppGroupHeaderBand7: TppGroupHeaderBand
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand7: TppGroupFooterBand
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
    end
    object ppGroup8: TppGroup
      BreakName = 'NM_PRODUTO'
      DataPipeline = ppBDERel_SaldoPendente
      OutlineSettings.CreateNode = True
      UserName = 'Group8'
      mmNewColumnThreshold = 0
      mmNewPageThreshold = 0
      DataPipelineName = 'ppBDERel_SaldoPendente'
      object ppGroupHeaderBand8: TppGroupHeaderBand
        mmBottomOffset = 0
        mmHeight = 0
        mmPrintPosition = 0
      end
      object ppGroupFooterBand8: TppGroupFooterBand
        mmBottomOffset = 0
        mmHeight = 4498
        mmPrintPosition = 0
        object ppLabel34: TppLabel
          UserName = 'Label13'
          Caption = 'Total'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          Transparent = True
          mmHeight = 3387
          mmLeft = 28840
          mmTop = 0
          mmWidth = 6731
          BandType = 5
          GroupNo = 3
        end
        object ppDBText36: TppDBText
          UserName = 'DBText106'
          DataField = 'AP_UNID_NEG'
          DataPipeline = ppBDERel_SaldoPendente
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 41804
          mmTop = 0
          mmWidth = 34396
          BandType = 5
          GroupNo = 3
        end
        object ppDBText37: TppDBText
          UserName = 'DBText107'
          DataField = 'NM_PRODUTO'
          DataPipeline = ppBDERel_SaldoPendente
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 77788
          mmTop = 0
          mmWidth = 57944
          BandType = 5
          GroupNo = 3
        end
        object ppDBCalc1: TppDBCalc
          UserName = 'DBCalc1'
          DataField = 'VL_FATURA'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3440
          mmLeft = 135996
          mmTop = 0
          mmWidth = 19844
          BandType = 5
          GroupNo = 3
        end
        object ppDBCalc2: TppDBCalc
          UserName = 'DBCalc2'
          DataField = 'VL_LIQUIDACAO'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 199496
          mmTop = 0
          mmWidth = 17198
          BandType = 5
          GroupNo = 3
        end
        object ppDBCalc3: TppDBCalc
          UserName = 'DBCalc3'
          DataField = 'Saldo'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          ParentDataPipeline = False
          ResetGroup = ppGroup8
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 225955
          mmTop = 0
          mmWidth = 17198
          BandType = 5
          GroupNo = 3
        end
        object ppDBText39: TppDBText
          UserName = 'DBText39'
          DataField = 'COMISSAO'
          DataPipeline = ppBDERel_SaldoPendente
          DisplayFormat = '#,0.00;-#,0.00'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Name = 'Arial'
          Font.Size = 8
          Font.Style = [fsBold]
          TextAlignment = taRightJustified
          Transparent = True
          DataPipelineName = 'ppBDERel_SaldoPendente'
          mmHeight = 3387
          mmLeft = 157957
          mmTop = 0
          mmWidth = 15610
          BandType = 5
          GroupNo = 4
        end
      end
    end
    object ppParameterList2: TppParameterList
    end
  end
  object qryReferencia: TQuery
    DatabaseName = 'DBBROKER'
    SQL.Strings = (
      'SELECT * FROM TTP_REFERENCIA')
    Left = 870
    Top = 691
    object qryReferenciaCD_REFERENCIA: TStringField
      FieldName = 'CD_REFERENCIA'
      Origin = 'DBBROKER.TTP_REFERENCIA.CD_REFERENCIA'
      FixedChar = True
      Size = 2
    end
    object qryReferenciaNM_REFERENCIA: TStringField
      FieldName = 'NM_REFERENCIA'
      Origin = 'DBBROKER.TTP_REFERENCIA.NM_REFERENCIA'
      FixedChar = True
    end
  end
  object dsReferencia: TDataSource
    DataSet = qryReferencia
    Left = 962
    Top = 691
  end
end
