...
var
  frm_imp_adm_old : Tfrm_imp_adm_old;
...
uses GSMLIB, PGGP001, PGGP017, PGPG020, PGSM018, PGSM041, PGPG020_OLD;

{$R *.DFM}

procedure Tfrm_imp_adm_old.btn_sairClick(Sender: TObject);
begin
  Close;
end;

procedure Tfrm_imp_adm_old.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  with datm_imp_adm_old do
  begin
...
procedure Tfrm_imp_adm_old.FormCreate(Sender: TObject);
begin
  ...
  Application.CreateForm(Tdatm_imp_adm_old, datm_imp_adm_old );
...
procedure Tfrm_imp_adm_old.btn_cancelarClick(Sender: TObject);
begin
  if Solicitando then CancelaOperacao := True;
  btn_desmarca_todasClick(nil);
end;

procedure Tfrm_imp_adm_old.Abre_Adm;
begin
  with datm_imp_adm_old do
  begin
...
procedure Tfrm_imp_adm_old.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #27 then Close;
end;

procedure Tfrm_imp_adm_old.btn_marca_todasClick(Sender: TObject);
var
  i : Integer;
begin
  ...
  datm_imp_adm_old.qry_lib_adm_.First;
  for i := 0 to datm_imp_adm_old.qry_lib_adm_.RecordCount - 1 do
  begin
    dbgrdAdm.SelectedRows.CurrentRowSelected := True;
    datm_imp_adm_old.qry_lib_adm_.Next;
  end;
end;

procedure Tfrm_imp_adm_old.btn_desmarca_todasClick(Sender: TObject);
var
  i : Integer;
begin
...
procedure Tfrm_imp_adm_old.msk_dt_baseEnter(Sender: TObject);
begin
  dt_pagamento := msk_dt_base.Text;
end;

procedure Tfrm_imp_adm_old.msk_dt_baseExit(Sender: TObject);
begin
  if msk_dt_base.Text = dt_pagamento then Exit;
  Abre_Adm;
end;

procedure Tfrm_imp_adm_old.btn_imprimirClick(Sender: TObject);
begin
...
procedure Tfrm_imp_adm_old.msk_cd_bancoExit(Sender: TObject);
begin
...
procedure Tfrm_imp_adm_old.msk_cd_bancoKeyDown(Sender: TObject;
...
procedure Tfrm_imp_adm_old.btn_co_bancoClick(Sender: TObject);
begin
  with datm_imp_adm_old Do
  begin
...
procedure Tfrm_imp_adm_old.Dados(Copia : Boolean);
begin
  try
    with datm_imp_adm_old do
    begin
    ...
    datm_imp_adm_old.qry_solic_adm_.Close;
    CloseFile( Txt );
    Exit
  end;
end;

procedure Tfrm_imp_adm_old.btn_canc_lancClick(Sender: TObject);
var i : Integer;
...
procedure Tfrm_imp_adm_old.msk_cd_unid_negExit(Sender: TObject);
begin
...
procedure Tfrm_imp_adm_old.FormShow(Sender: TObject);
begin
...
procedure Tfrm_imp_adm_old.rdgrp_tp_naturezaClick(Sender: TObject);
begin
  if rdgrp_tp_natureza.ItemIndex = 0 then
...
procedure Tfrm_imp_adm_old.ImpCheque;
var
  cNomeTxt : String;
  cd_banco: String[3];
  i : Integer;
...
procedure Tfrm_imp_adm_old.ImpNota;
var
  i : Byte;
  in_soma : Boolean;
begin
  ...
  try
    datm_main.db_broker.StartTransaction;
    datm_imp_adm_old.qry_limpa_usuario_.Close;
    datm_imp_adm_old.qry_limpa_usuario_.ParamByName('CD_USUARIO_IMP').AsString := str_cd_usuario;
    datm_imp_adm_old.qry_limpa_usuario_.Prepare;
    datm_imp_adm_old.qry_limpa_usuario_.ExecSQL;
    datm_main.db_broker.Commit;
  except
    if datm_main.db_broker.InTransaction then datm_main.db_broker.Rollback;
  end;
...
procedure Tfrm_imp_adm_old.rdgrp_tp_lancamentoClick(Sender: TObject);
begin
  if rdgrp_tp_lancamento.ItemIndex = 0 then //Cheque
  begin
...
procedure Tfrm_imp_adm_old.msk_nr_chequeExit(Sender: TObject);
begin
  if Trim( msk_nr_cheque.Text ) <> '' then
     msk_nr_cheque.Text := StrZero( StrToInt( msk_nr_cheque.Text ), 6 );
end;

procedure Tfrm_imp_adm_old.msk_dt_pagtoExit(Sender: TObject);
begin
  with datm_imp_adm_old do
  begin
    qry_caixa_ctrl_.Close;
...
procedure Tfrm_imp_adm_old.btn_co_unid_negClick(Sender: TObject);
begin
