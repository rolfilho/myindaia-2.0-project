unit FrmFaturadosSaldoFaturaCtx;
...
type
  TFormFaturadosSaldoFaturaCtx = class(TForm)
    lbl_produto: TLabel;
...
var
  FormFaturadosSaldoFaturaCtx: TFormFaturadosSaldoFaturaCtx;
...
function TFormFaturadosSaldoFaturaCtx.GetSQL: string;
begin
  Result :=
  '     SELECT                                                                                                                  ' + sLineBreak +
  ...
  ' 		 CONVERT(VARCHAR(10),EV041.DT_REALIZACAO,103)  AS [Data Liberação para transporte],                                     ' + sLineBreak +
	'      CONVERT(VARCHAR(10),EV114.DT_REALIZACAO,103)  AS [Data envio ao faturamento],                                          ' + sLineBreak +
  ...
  '      B.TOTAL * -1                                  AS [Saldo Fatura],                                                       ' + sLineBreak +
  ...
  '     LEFT JOIN BROKER.DBO.TFOLLOWUP EV041 (NOLOCK) ON EV041.NR_PROCESSO = TP.NR_PROCESSO AND EV041.CD_EVENTO=''041''         ' + sLineBreak +
  '	    LEFT JOIN BROKER.DBO.TFOLLOWUP EV114 (NOLOCK) ON EV114.NR_PROCESSO = TP.NR_PROCESSO AND EV114.CD_EVENTO=''114''         ' + sLineBreak +
  '                                                                                                                             ' + sLineBreak +
...
function TFormFaturadosSaldoFaturaCtx.GetOrderBy: string;
begin
  Result := ' ORDER BY G.CD_GRUPO, B.CREATED, TP.NR_PROCESSO, B.ID ';
end;

procedure TFormFaturadosSaldoFaturaCtx.BitBtn2Click(Sender: TObject);
begin
  Close;
end;

procedure TFormFaturadosSaldoFaturaCtx.BitBtn1Click(Sender: TObject);
var
  Form: TfmConsulta;
  Filtros: string;
begin
...
procedure TFormFaturadosSaldoFaturaCtx.btn_co_produtoClick(
  Sender: TObject);
begin
...
procedure TFormFaturadosSaldoFaturaCtx.msk_cd_produtoExit(Sender: TObject);
begin
  btn_co_produtoClick(msk_cd_produto);
  ValCodEdt( msk_cd_produto );
end;

procedure TFormFaturadosSaldoFaturaCtx.btn_co_grupoClick(Sender: TObject);
begin
  if Sender is TSpeedButton then
