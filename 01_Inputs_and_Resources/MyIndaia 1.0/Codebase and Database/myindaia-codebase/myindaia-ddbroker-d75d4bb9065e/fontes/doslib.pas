...
//procedure BrokerCopyFile(const FileName, DestName: string);
//procedure BrokerCopyFileLpt1(const FileName, DestName: string);
...
uses Forms, ShellAPI, FileUtil;

const
  SInvalidDest = 'Destination %s does not exist';
  SFCantMove = 'Cannot move file %s';

procedure BrokerCopyFile(const FileName, DestName: TFileName);
var
...
procedure BrokerCopyFileLpt1(const FileName, DestName: TFileName);
var
  CopyBuffer: Pointer; { buffer for copying }
  BytesCopied: Longint;
  Source, Dest: Integer; { handles }
...
begin
  ...
  begin
    if HasAttr(FileName, faReadOnly) then  { if it's read-only... }
      ...
      BrokerCopyFile(FileName, Destination); { copy it over to destination...}
