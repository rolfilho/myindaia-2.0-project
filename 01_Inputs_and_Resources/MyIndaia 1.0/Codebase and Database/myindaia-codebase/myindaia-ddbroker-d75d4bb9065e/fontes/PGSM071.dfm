object frm_cliente_repres: Tfrm_cliente_repres
  Left = 316
  Top = 162
  Width = 657
  Height = 495
  Caption = 'Cliente X Representante'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  Menu = menu_cadastro
  OldCreateOrder = True
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl_cadastro: TPanel
    Left = 0
    Top = 0
    Width = 317
    Height = 44
    BevelOuter = bvLowered
    TabOrder = 0
    object btn_incluir: TSpeedButton
      Left = 8
      Top = 12
      Width = 25
      Height = 25
      Hint = 'Incluir'
      Flat = True
      Glyph.Data = {
        
        3BB33773333773333773B333333B3333333B7333333733333337}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_incluirClick
    end
    object btn_excluir: TSpeedButton
      Left = 92
      Top = 12
      Width = 25
      Height = 25
      Hint = 'Excluir'
      Flat = True
      Glyph.Data = {
        
        3333333337FFF7F3333333333000003333333333377777333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_excluirClick
    end
    object btn_sair: TSpeedButton
      Left = 276
      Top = 12
      Width = 25
      Height = 25
      Hint = 'Sair'
      Flat = True
      Glyph.Data = {
        
        0333337F777FFFFF7F3333000000000003333377777777777333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_sairClick
    end
    object btn_salvar: TSpeedButton
      Left = 36
      Top = 12
      Width = 25
      Height = 25
      Hint = 'Salvar'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        0003737FFFFFFFFF7F7330099999999900333777777777777733}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_salvarClick
    end
    object btn_cancelar: TSpeedButton
      Left = 64
      Top = 12
      Width = 25
      Height = 25
      Hint = 'Cancelar'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      OnClick = btn_cancelarClick
    end
    object dbnvg: TDBNavigator
      Left = 144
      Top = 12
      Width = 104
      Height = 25
      DataSource = datm_cliente_repres.ds_cliente_repres
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      Flat = True
      Hints.Strings = (
        'Primeira'
        'Anterior'
        'Pr'#243'xima'
        #218'ltima')
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      BeforeAction = dbnvgBeforeAction
    end
  end
  object pnl_pesquisa: TPanel
    Left = 331
    Top = 0
    Width = 309
    Height = 44
    BevelOuter = bvLowered
    TabOrder = 1
    object lbl_pesquisa: TLabel
      Left = 12
      Top = 5
      Width = 56
      Height = 13
      Caption = 'Pesquisar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbl_ordem_pesquisa: TLabel
      Left = 148
      Top = 5
      Width = 37
      Height = 13
      Caption = 'Ordem'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edt_chave: TEdit
      Left = 12
      Top = 20
      Width = 129
      Height = 21
      TabStop = False
      TabOrder = 0
      OnChange = edt_chaveChange
    end
    object cb_ordem: TComboBox
      Left = 148
      Top = 20
      Width = 153
      Height = 21
      Style = csDropDownList
      ItemHeight = 13
      TabOrder = 1
      TabStop = False
      OnClick = cb_ordemClick
    end
  end
  object pgctrl_cliente_repres: TPageControl
    Left = 0
    Top = 47
    Width = 641
    Height = 390
    ActivePage = ts_lista
    Align = alBottom
    TabOrder = 2
    OnChange = pgctrl_cliente_represChange
    OnChanging = pgctrl_cliente_represChanging
    object ts_lista: TTabSheet
      Caption = '    Lista    '
      object lbl_Cliente: TLabel
        Left = 16
        Top = 1
        Width = 40
        Height = 13
        Caption = 'Cliente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lbl_unid_neg: TLabel
        Left = 16
        Top = 39
        Width = 48
        Height = 13
        Caption = 'Unidade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lbl_produto: TLabel
        Left = 16
        Top = 77
        Width = 45
        Height = 13
        Caption = 'Produto'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label11: TLabel
        Left = 16
        Top = 118
        Width = 44
        Height = 13
        Caption = 'Servi'#231'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object dbg_cadastro: TDBGrid
        Left = 16
        Top = 162
        Width = 586
        Height = 190
        Color = clWhite
        DataSource = datm_cliente_repres.ds_cliente_repres
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'LookRepresentante'
            Title.Color = clBlack
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWhite
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'LookAtivo'
            Title.Color = clBlack
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWhite
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = True
          end>
      end
      object dbedt_cd_cliente: TDBEdit
        Left = 16
        Top = 17
        Width = 41
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'CD_CLIENTE'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 1
      end
      object dbedt_nm_cliente: TDBEdit
        Left = 61
        Top = 17
        Width = 313
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'LookCliente'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 2
      end
      object dbedt_cd_unid_neg: TDBEdit
        Left = 16
        Top = 53
        Width = 33
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'CD_UNID_NEG'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 3
      end
      object dbedt_nm_unid_neg: TDBEdit
        Left = 61
        Top = 53
        Width = 313
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'LookUnidNeg'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 4
      end
      object dbedt_cd_produto: TDBEdit
        Left = 16
        Top = 94
        Width = 33
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'CD_PRODUTO'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 5
      end
      object dbedt_nm_produto: TDBEdit
        Left = 61
        Top = 94
        Width = 313
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'LookProduto'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 6
      end
      object dbedt_cd_servico: TDBEdit
        Left = 16
        Top = 135
        Width = 33
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'CD_SERVICO'
        DataSource = datm_cliente_repres.ds_cliente_repres
        MaxLength = 2
        ReadOnly = True
        TabOrder = 7
      end
      object dbedt_nm_servico: TDBEdit
        Left = 61
        Top = 135
        Width = 313
        Height = 21
        TabStop = False
        Color = clMenu
        DataField = 'LookServico'
        DataSource = datm_cliente_repres.ds_cliente_repres
        ReadOnly = True
        TabOrder = 8
      end
    end
    object ts_dados_basicos: TTabSheet
      Caption = '   Dados B'#225'sicos   '
      object pnl_entrada_de_dados: TPanel
        Left = 3
        Top = 8
        Width = 610
        Height = 345
        BevelOuter = bvLowered
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        object lbl_representante: TLabel
          Left = 25
          Top = 174
          Width = 84
          Height = 13
          Caption = 'Representante'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbl_in_ativo: TLabel
          Left = 426
          Top = 174
          Width = 30
          Height = 13
          Caption = 'Ativo'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label4: TLabel
          Left = 25
          Top = 16
          Width = 40
          Height = 13
          Caption = 'Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label5: TLabel
          Left = 25
          Top = 54
          Width = 48
          Height = 13
          Caption = 'Unidade'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label6: TLabel
          Left = 25
          Top = 92
          Width = 45
          Height = 13
          Caption = 'Produto'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object btn_co_representante: TSpeedButton
          Left = 391
          Top = 190
          Width = 25
          Height = 21
          Glyph.Data = {
            
            5555555555555555555555555555555555555555555555555555}
          NumGlyphs = 2
          OnClick = btn_co_representanteClick
        end
        object Label12: TLabel
          Left = 25
          Top = 133
          Width = 44
          Height = 13
          Caption = 'Servi'#231'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label13: TLabel
          Left = 25
          Top = 216
          Width = 58
          Height = 13
          Caption = 'Numer'#225'rio'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label14: TLabel
          Left = 25
          Top = 256
          Width = 201
          Height = 13
          Caption = 'Fatura/Demonstrativo de Despesas'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label15: TLabel
          Left = 25
          Top = 296
          Width = 136
          Height = 13
          Caption = 'Nota Fiscal de Servi'#231'os'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object dbedt_nm_representante: TDBEdit
          Left = 70
          Top = 190
          Width = 313
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'LookRepresentante'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 5
        end
        object dbedt_cd_representante: TDBEdit
          Left = 25
          Top = 190
          Width = 41
          Height = 21
          Color = clWhite
          DataField = 'CD_REPRESENTANTE'
          DataSource = datm_cliente_repres.ds_cliente_repres
          TabOrder = 0
          OnChange = dbedt_cd_representanteChange
          OnExit = dbedt_cd_representanteExit
          OnKeyDown = dbedt_cd_representanteKeyDown
        end
        object dblkpcbox_ativo: TDBLookupComboBox
          Left = 426
          Top = 190
          Width = 50
          Height = 21
          DataField = 'IN_ATIVO'
          DataSource = datm_cliente_repres.ds_cliente_repres
          DropDownRows = 3
          KeyField = 'CD_YESNO'
          ListField = 'TX_YESNO'
          ListSource = datm_cliente_repres.ds_yesno
          TabOrder = 1
          OnClick = dbedt_cd_representanteChange
        end
        object dbedt_cd_cliente2: TDBEdit
          Left = 25
          Top = 32
          Width = 41
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'CD_CLIENTE'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 6
        end
        object dbedt_nm_cliente2: TDBEdit
          Left = 70
          Top = 32
          Width = 313
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'LookCliente'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 7
        end
        object dbedt_cd_unid_neg2: TDBEdit
          Left = 25
          Top = 68
          Width = 33
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'CD_UNID_NEG'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 8
        end
        object dbedt_nm_unid_neg2: TDBEdit
          Left = 70
          Top = 68
          Width = 313
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'LookUnidNeg'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 9
        end
        object dbedt_cd_produto2: TDBEdit
          Left = 25
          Top = 109
          Width = 33
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'CD_PRODUTO'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 10
        end
        object dbedt_nm_produto2: TDBEdit
          Left = 70
          Top = 109
          Width = 313
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'LookProduto'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 11
        end
        object dbedt_cd_servico2: TDBEdit
          Left = 25
          Top = 150
          Width = 33
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'CD_SERVICO'
          DataSource = datm_cliente_repres.ds_cliente_repres
          MaxLength = 2
          ReadOnly = True
          TabOrder = 12
        end
        object dbedt_nm_servico2: TDBEdit
          Left = 70
          Top = 150
          Width = 313
          Height = 21
          TabStop = False
          Color = clMenu
          DataField = 'LookServico'
          DataSource = datm_cliente_repres.ds_cliente_repres
          ReadOnly = True
          TabOrder = 13
        end
        object dblkpcbox_numerario: TDBLookupComboBox
          Left = 25
          Top = 230
          Width = 50
          Height = 21
          DataField = 'IN_DEST_NUM'
          DataSource = datm_cliente_repres.ds_cliente_repres
          KeyField = 'CD_YESNO'
          ListField = 'TX_YESNO'
          ListSource = datm_cliente_repres.ds_yesno
          TabOrder = 2
          OnClick = dbedt_cd_representanteChange
        end
        object dblkpcbox_fatura: TDBLookupComboBox
          Left = 25
          Top = 270
          Width = 50
          Height = 21
          DataField = 'IN_DEST_FAT'
          DataSource = datm_cliente_repres.ds_cliente_repres
          KeyField = 'CD_YESNO'
          ListField = 'TX_YESNO'
          ListSource = datm_cliente_repres.ds_yesno
          TabOrder = 3
          OnClick = dbedt_cd_representanteChange
        end
        object dblkpcbox_nota: TDBLookupComboBox
          Left = 25
          Top = 310
          Width = 50
          Height = 21
          DataField = 'IN_DEST_NF'
          DataSource = datm_cliente_repres.ds_cliente_repres
          KeyField = 'CD_YESNO'
          ListField = 'TX_YESNO'
          ListSource = datm_cliente_repres.ds_yesno
          TabOrder = 4
          OnClick = dbedt_cd_representanteChange
        end
      end
    end
    object ts_repasse: TTabSheet
      Caption = '   Repasse   '
      object pgctrl_cliente_repasse: TPageControl
        Left = 1
        Top = 3
        Width = 618
        Height = 347
        ActivePage = ts_lista2
        TabOrder = 0
        OnChange = pgctrl_cliente_represChange
        OnChanging = pgctrl_cliente_represChanging
        object ts_lista2: TTabSheet
          Caption = '    Lista    '
          object Label1: TLabel
            Left = 12
            Top = -1
            Width = 40
            Height = 13
            Caption = 'Cliente'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label2: TLabel
            Left = 12
            Top = 37
            Width = 48
            Height = 13
            Caption = 'Unidade'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label3: TLabel
            Left = 12
            Top = 75
            Width = 45
            Height = 13
            Caption = 'Produto'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object lblRepresentante: TLabel
            Left = 12
            Top = 154
            Width = 84
            Height = 13
            Caption = 'Representante'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label16: TLabel
            Left = 12
            Top = 114
            Width = 44
            Height = 13
            Caption = 'Servi'#231'o'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object dbg_contatos: TDBGrid
            Left = 12
            Top = 198
            Width = 586
            Height = 117
            DataSource = datm_cliente_repres.ds_cliente_repasse
            Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
            ReadOnly = True
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -11
            TitleFont.Name = 'MS Sans Serif'
            TitleFont.Style = []
            Columns = <
              item
                Expanded = False
                FieldName = 'LookItem'
                Title.Color = clBlack
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clWhite
                Title.Font.Height = -11
                Title.Font.Name = 'MS Sans Serif'
                Title.Font.Style = [fsBold]
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'PC_REPASSE'
                Title.Color = clBlack
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clWhite
                Title.Font.Height = -11
                Title.Font.Name = 'MS Sans Serif'
                Title.Font.Style = [fsBold]
                Width = 76
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'LookAtivo'
                Title.Color = clBlack
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clWhite
                Title.Font.Height = -11
                Title.Font.Name = 'MS Sans Serif'
                Title.Font.Style = [fsBold]
                Visible = True
              end>
          end
          object dbedt_cd_cliente3: TDBEdit
            Left = 12
            Top = 15
            Width = 41
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'CD_CLIENTE'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 1
          end
          object dbedt_nm_cliente3: TDBEdit
            Left = 57
            Top = 15
            Width = 313
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'LookCliente'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 2
          end
          object dbedt_cd_unid_neg3: TDBEdit
            Left = 12
            Top = 51
            Width = 33
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'CD_UNID_NEG'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 3
          end
          object dbedt_nm_unid_neg3: TDBEdit
            Left = 57
            Top = 51
            Width = 313
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'LookUnidNeg'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 4
          end
          object dbedt_cd_produto3: TDBEdit
            Left = 12
            Top = 92
            Width = 33
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'CD_PRODUTO'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 5
          end
          object dbedt_nm_produto3: TDBEdit
            Left = 57
            Top = 92
            Width = 313
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'LookProduto'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 6
          end
          object dbedt_cd_representante2: TDBEdit
            Left = 12
            Top = 170
            Width = 41
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'CD_REPRESENTANTE'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 9
          end
          object dbedt_nm_representante2: TDBEdit
            Left = 57
            Top = 170
            Width = 313
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'LookRepresentante'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 10
          end
          object dbedt_cd_servico3: TDBEdit
            Left = 12
            Top = 131
            Width = 33
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'CD_SERVICO'
            DataSource = datm_cliente_repres.ds_cliente_repres
            MaxLength = 2
            ReadOnly = True
            TabOrder = 7
          end
          object dbedt_nm_servico3: TDBEdit
            Left = 57
            Top = 131
            Width = 313
            Height = 21
            TabStop = False
            Color = clMenu
            DataField = 'LookServico'
            DataSource = datm_cliente_repres.ds_cliente_repres
            ReadOnly = True
            TabOrder = 8
          end
        end
        object ts_dados_basicos2: TTabSheet
          Caption = '   Dados B'#225'sicos   '
          object Panel1: TPanel
            Left = 7
            Top = 7
            Width = 598
            Height = 306
            BevelOuter = bvLowered
            ParentShowHint = False
            ShowHint = True
            TabOrder = 0
            object btn_co_item: TSpeedButton
              Left = 378
              Top = 222
              Width = 25
              Height = 21
              Glyph.Data = {
                
                5555555555555555555555555555555555555555555555555555}
              NumGlyphs = 2
              OnClick = btn_co_itemClick
            end
            object Label7: TLabel
              Left = 15
              Top = 12
              Width = 40
              Height = 13
              Caption = 'Cliente'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label8: TLabel
              Left = 15
              Top = 50
              Width = 48
              Height = 13
              Caption = 'Unidade'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label9: TLabel
              Left = 15
              Top = 88
              Width = 45
              Height = 13
              Caption = 'Produto'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label10: TLabel
              Left = 15
              Top = 168
              Width = 84
              Height = 13
              Caption = 'Representante'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object lbl_item: TLabel
              Left = 15
              Top = 207
              Width = 25
              Height = 13
              Caption = 'Item'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object lbl_pc_repasse: TLabel
              Left = 418
              Top = 207
              Width = 63
              Height = 13
              Caption = '% Repasse'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object lbl_in_ativo2: TLabel
              Left = 498
              Top = 207
              Width = 30
              Height = 13
              Caption = 'Ativo'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Label17: TLabel
              Left = 15
              Top = 128
              Width = 44
              Height = 13
              Caption = 'Servi'#231'o'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object dbedt_cd_item: TDBEdit
              Left = 15
              Top = 222
              Width = 33
              Height = 21
              DataField = 'CD_ITEM'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              TabOrder = 0
              OnChange = dbedt_cd_itemChange
              OnExit = dbedt_cd_itemExit
              OnKeyDown = dbedt_cd_representanteKeyDown
            end
            object dbedt_nm_item: TDBEdit
              Left = 58
              Top = 222
              Width = 313
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'LookItem'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 3
            end
            object dbedt_cd_cliente4: TDBEdit
              Left = 15
              Top = 28
              Width = 41
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'CD_CLIENTE'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 4
            end
            object dbedt_nm_cliente4: TDBEdit
              Left = 58
              Top = 28
              Width = 313
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'LookCliente'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 5
            end
            object dbedt_cd_unid_neg4: TDBEdit
              Left = 15
              Top = 64
              Width = 33
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'CD_UNID_NEG'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 6
            end
            object dbedt_nm_unid_neg4: TDBEdit
              Left = 58
              Top = 64
              Width = 313
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'LookUnidNeg'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 7
            end
            object dbedt_cd_produto4: TDBEdit
              Left = 15
              Top = 105
              Width = 33
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'CD_PRODUTO'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 8
            end
            object dbedt_nm_produto4: TDBEdit
              Left = 58
              Top = 105
              Width = 313
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'LookProduto'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 9
            end
            object dbedt_cd_representante3: TDBEdit
              Left = 15
              Top = 183
              Width = 41
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'CD_REPRESENTANTE'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 12
            end
            object dbedt_nm_representante3: TDBEdit
              Left = 58
              Top = 183
              Width = 313
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'LookRepresentante'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 13
            end
            object dbedt_pc_repasse: TDBEdit
              Left = 418
              Top = 222
              Width = 50
              Height = 21
              DataField = 'PC_REPASSE'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              MaxLength = 5
              ParentFont = False
              TabOrder = 1
              OnChange = dbedt_cd_itemChange
            end
            object dblckboc_in_ativo2: TDBLookupComboBox
              Left = 498
              Top = 222
              Width = 50
              Height = 21
              DataField = 'IN_ATIVO'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              DropDownRows = 3
              KeyField = 'CD_YESNO'
              ListField = 'TX_YESNO'
              ListSource = datm_cliente_repres.ds_yesno
              TabOrder = 2
              OnClick = dbedt_cd_itemChange
            end
            object dbedt_cd_servico4: TDBEdit
              Left = 15
              Top = 145
              Width = 33
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'CD_SERVICO'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              MaxLength = 2
              ReadOnly = True
              TabOrder = 10
            end
            object dbedt_nm_servico4: TDBEdit
              Left = 58
              Top = 145
              Width = 313
              Height = 21
              TabStop = False
              Color = clMenu
              DataField = 'LookUnidNeg'
              DataSource = datm_cliente_repres.ds_cliente_repasse
              ReadOnly = True
              TabOrder = 11
            end
          end
        end
      end
    end
  end
  object menu_cadastro: TMainMenu
    Left = 556
    Top = 368
    object mi_incluir: TMenuItem
      Caption = '&Incluir'
      Hint = ' '
      OnClick = btn_incluirClick
    end
    object mi_salvar: TMenuItem
      Caption = '&Salvar'
      Enabled = False
      OnClick = btn_salvarClick
    end
    object mi_cancelar: TMenuItem
      Caption = '&Cancelar'
      Enabled = False
      OnClick = btn_cancelarClick
    end
    object mi_excluir: TMenuItem
      Caption = '&Excluir'
      Hint = ' '
      OnClick = btn_excluirClick
    end
    object mi_sair: TMenuItem
      Caption = 'Sai&r'
      Hint = ' '
      OnClick = btn_sairClick
    end
  end
end
