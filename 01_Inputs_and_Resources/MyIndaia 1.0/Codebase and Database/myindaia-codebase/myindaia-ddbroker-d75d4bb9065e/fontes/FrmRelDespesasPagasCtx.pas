...
type
  TFormRelDepesasPagasCtx = class(TForm)
    lbl_produto: TLabel;
...
var
  FormRelDepesasPagasCtx: TFormRelDepesasPagasCtx;
...
function TFormRelDepesasPagasCtx.GetGroupBy: string;
begin                                //nao é usado
  Result := '		GROUP BY CC.CUSTOMID, '+sLineBreak+
            ...
            '		B.CREATED,            '+sLineBreak+
            '		B.SENIORKEY,          '+sLineBreak+
            '		B.ID                  ';
end;

function TFormRelDepesasPagasCtx.GetSQL: string;
begin
  Result :=
  ' SELECT	                                                                                                                         '+sLineBreak +
  ...
  '      P.NM_PAIS                                        AS [País Destino]                                                           '+sLineBreak +
  ...
  '   LEFT JOIN BROKER.DBO.TPAIS_BROKER                              P    (NOLOCK) ON P.CD_PAIS                 = TP.CD_PAIS_DESTINO '+sLineBreak +
...
function TFormRelDepesasPagasCtx.GetWhere: string;
begin
  Result := ' WHERE PR.STATUS=''APPROVED'' ';
end;

procedure TFormRelDepesasPagasCtx.BitBtn2Click(Sender: TObject);
begin
  Close;
end;

procedure TFormRelDepesasPagasCtx.BitBtn1Click(Sender: TObject);
var
  Form: TfmConsulta;
  Filtros: string;
begin
...
procedure TFormRelDepesasPagasCtx.btn_co_produtoClick(Sender: TObject);
begin
...
procedure TFormRelDepesasPagasCtx.msk_cd_produtoExit(Sender: TObject);
begin
  btn_co_produtoClick(msk_cd_produto);
  ValCodEdt( msk_cd_produto );
end;

procedure TFormRelDepesasPagasCtx.btn_co_grupoClick(Sender: TObject);
begin
  if Sender is TSpeedButton then
...
procedure TFormRelDepesasPagasCtx.msk_cd_grupoChange(Sender: TObject);
begin
  if Sender is TSpeedButton then
    msk_cd_grupo.Text := ConsultaOnLineEx('TGRUPO','Grupo',['CD_GRUPO','NM_GRUPO'],['Código','Nome'],'CD_GRUPO')
  else
...
procedure TFormRelDepesasPagasCtx.SpeedButton1Click(Sender: TObject);
begin
  if Sender is TSpeedButton then
    mskItem.Text := ConsultaOnLineExFiltro('MYINDAIAV2.DBO.ITEM','Item',
                                          ['id','name'],['Código','Nome'],'id', 'type = ''D''',nil)
  else
