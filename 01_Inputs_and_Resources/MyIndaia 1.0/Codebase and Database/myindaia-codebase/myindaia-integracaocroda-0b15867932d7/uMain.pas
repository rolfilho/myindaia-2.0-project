...
type
  ...
  private
    ...
    procedure processaIntegracaoCroda(arquivo: String);
...
begin
  ...
  Caption := 'Integração Croda - ' + GetBuildInfoAsString;
end;
...
begin
  ...
  //FTP Indaiá
  Pasta_FTP := '\\192.168.100.5\sftp\Croda';
...
procedure TFormMain.processaIntegracaoCroda(arquivo: String);
var
...
//  ConfigurarFTP(LocalSettings.URLFTP, LocalSettings.UsuarioFTP, LocalSettings.SenhaFTP);
  ...
  ArquivoCroda := TArquivoCroda.Create(CaminhoArquivoFTP);
  IDCriada := false;

  Headers := TObjectList<THeader>.Create;
//  dmdMain.dbBroker.StartTransaction;
  try
    ArquivoCroda.Processa(Headers, Logar);
...
//      dmdMain.dbBroker.Rollback;
      Log('ERRO na integração Croda: ' + e.Message, Logar);
...
//        exit;
    ...
    for i := 0 to ListaNovosArquivos.Count - 1 do
      processaIntegracaoCroda(ListaNovosArquivos.Strings[i]);
  finally
...
begin
  ...
  if dmdMain.queryAuxiliar.RecordCount = 1 then
    dmdMain.EnviaEmail('', [dmdMain.queryAuxiliar.FieldByName('NM_EMAIL').AsString], [], [],
      'Integração Croda - Pedido: ' + nrPedido + ' Instrucao Desembaraço: ' + nrInstrucaoDesembaraco,
...
//    if dmdMain.EnviaEmail('', ['ti@myindaia.com.br', dmdMain.queryAuxiliar.FieldByName('NM_EMAIL').AsString, LocalSettings.emailerro], [], [],
    if dmdMain.EnviaEmail('', EmailsDestino, [], [],
      'Integração Croda - Erro ao tentar integrar o arquivo: - ' + nrArquivo ,
