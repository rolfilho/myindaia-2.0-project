...
uses
  ...
  sCheckBox, sSpeedButton{, JvHtControls};
...
type
  ...
  private
    ...
    procedure SetOriginalFileName(const Value: TFileName);
  protected
  ...
  public
    constructor Create(AOwner: TColecaoArquivos); reintroduce;
  ...
  public
    ...
    function Add(AFileName, AOriginalFileName: TFileName): TItemArquivo;
    function AddParented(AFileName, AOriginalFileName: TFileName; AParentItem: TItemArquivo): TItemArquivo;
    function FileNameExists(AFileName, AOriginalFileName: TFileName; const ACaseInsensitive: Boolean = True): Boolean;
    function GetItemByFileName(AFileName, AOriginalFileName: TFileName; const ACaseInsensitive: Boolean = True): TItemArquivo;
...
begin
  ...
  //FComboType             := TJvHTComboBox.Create(FOwner.Owner);
...
function TItemArquivo.GetOriginalFileName: TFileName;
begin
  Result := FOriginalFileName;
end;

procedure TItemArquivo.SetOriginalFileName(const Value: TFileName);
begin
...
function TColecaoArquivos.Add(AFileName, AOriginalFileName: TFileName): TItemArquivo;
begin
  Result := AddParented(AFileName, AOriginalFileName, nil);
end;

function TColecaoArquivos.AddParented(AFileName, AOriginalFileName: TFileName; AParentItem: TItemArquivo): TItemArquivo;
begin
  Result := nil;
  if FileNameExists(AFileName, AOriginalFileName) then
  begin
    with Self.GetItemByFileName(AFileName, AOriginalFileName) do
    begin
      if Visible then
        SetFocus
  ...
  begin
    ...
    Result.OriginalFileName := AOriginalFileName;
...
function TColecaoArquivos.FileNameExists(AFileName, AOriginalFileName: TFileName; const ACaseInsensitive: Boolean = True): Boolean;
begin
  Result := GetItemByFileName(AFileName, AOriginalFileName, ACaseInsensitive) <> nil;
end;
...
function TColecaoArquivos.GetItemByFileName(AFileName, AOriginalFileName: TFileName;
  ...
  for I := 0 to Count -1 do
    if ((AFileName = IfThen(ACaseInsensitive, AnsiUpperCase(Item[I].FFullFileName), Item[I].FFullFileName)) or
       (AFileName = IfThen(ACaseInsensitive, AnsiUpperCase(ExtractFileName(Item[I].FFullFileName)),
          ExtractFileName(Item[I].FFullFileName)))) or
       ((AOriginalFileName = IfThen(ACaseInsensitive, AnsiUpperCase(Item[I].OriginalFileName), Item[I].OriginalFileName)) or
       (AOriginalFileName = IfThen(ACaseInsensitive, AnsiUpperCase(ExtractFileName(Item[I].OriginalFileName)),
          ExtractFileName(Item[I].OriginalFileName)))) then
      Break;
  if I < Count then
    Result := Item[I]
  else
