...
function UltimoProcesso(TipoConsulta :string): String;
function Posicao(TipoConsulta :string): Integer;
procedure SetProcesso(Processo: string; Pos: Integer; TipoConsulta :string);
...
function URL_Imp: String;
begin
  Result := Trim(WebConfig.ReadString('Imp Web', 'DI.URL', 'https://www1c.siscomex.receita.fazenda.gov.br/'));
  if Result = '' then
...
procedure SetProcesso(Processo: string; Pos: Integer; TipoConsulta : String);
begin
  if TipoConsulta = 'P' then
  begin
    WebConfig.WriteString('Fila Consulta', 'LOG_PARAMETRIZACAO', Processo);
    WebConfig.WriteInteger('Fila Consulta', 'POS_PARAMETRIZACAO', Pos);
  end
  else
  if TipoConsulta = 'DI' then
  begin
    WebConfigDi.WriteString('Fila Consulta', 'ULT_PROCESSO_DI', Processo);
    WebConfigDi.WriteInteger('Fila Consulta', 'POS_DI', Pos);
  end;
...
begin
  if TipoConsulta = 'P' then
  begin
    Result := Trim(WebConfig.ReadString('Fila Consulta', 'LOG_PARAMETRIZACAO', ''));
  end
  else
  if TipoConsulta = 'DI' then
  begin
    Result := Trim(WebConfigDi.ReadString('Fila Consulta', 'ULT_PROCESSO_DI', ''));
  end;
end;
...
begin
  if TipoConsulta = 'P' then
  begin
    Result := WebConfig.ReadInteger('Fila Consulta', 'POS_PARAMETRIZACAO', 1);
  end
  else
  if TipoConsulta = 'DI' then
  begin
    Result := WebConfigDi.ReadInteger('Fila Consulta', 'POS_DI', 1);
  end;
...
initialization
  ...
  if not FileExists(FDataPath + 'WebConfigDi.ini') then
  begin
    WebConfigDi     := TIniFile.Create(FDataPath + 'WebConfigDi.ini');
    WebConfigDi.WriteString('Fila Consulta', 'ULT_PROCESSO_DI', '');
    WebConfigDi.WriteString('Fila Consulta', 'POS_DI', '');
  end
  else
    WebConfigDi     := TIniFile.Create(FDataPath + 'WebConfigDi.ini');
