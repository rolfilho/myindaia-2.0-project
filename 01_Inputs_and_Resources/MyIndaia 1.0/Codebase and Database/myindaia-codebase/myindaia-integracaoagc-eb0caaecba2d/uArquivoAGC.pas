...
constructor TArquivoAGC.Create(const NomeArquivo: string);
begin
...
destructor TArquivoAGC.Destroy;
begin
  FArquivo.Free;
  inherited;
end;

function TArquivoAGC.Processa(var Headers: TObjectList<THeader>; CallBack: TLogCallBack): Boolean;
var
  I: Integer;
...
function TArquivoAGC.GetTipoAcaoItem(value: String): TAcaoItem;
begin
  if value = '' then
    result := taInserir
...
function TArquivoAGC.GetCodigoModal(value: String): String;
begin
  if UpperCase(value) = UpperCase('AEREO') then result := '04' else
  if UpperCase(value) = UpperCase('MARITIMO') then result := '01' else
  if UpperCase(value) = UpperCase('RODOVIARIO') then result := '07';
end;

function TArquivoAGC.GetCodigoMoeda(value: String): String;
begin
...
function TArquivoAGC.GetCodigoUnMedida(value: String): String;
begin
...
function TArquivoAGC.GetEmpresa(value: String): String;
begin
  if upperCase(value) = ' ' then
    result := '03209'
  else if upperCase(value) = '' then
    result := '03209';
end;

function TArquivoAGC.GetCodigoPais(value: String): String;
begin
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('Bélgica') then result := '087' else
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('Colômbia') then result := '169' else
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('Eslováquia') then result := '247' else
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('EUA') then result := '249' else
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('Índia') then result := '361' else
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('Países Baixos') then result := '573' else
  ...
  if AnsiUpperCase(value) = AnsiUpperCase('Tailândia') then result := '776';
  if AnsiUpperCase(value) = AnsiUpperCase('Tailandia') then result := '776';
