...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If Trim(Request("retorno")) <> "" And Session("in_interno") = "0" Then
            ...
            If contador_permissao = 0 Then
                Response.Redirect("digitalizacao_principal_e.aspx?&produto=" & Session("produto") & "&usuario=" + Session("usuario") + "&pg=1")
            End If
        End If
    ...
    Sub EnviarArquivo(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If PrefixoArquivo <> "" Then
            ...
            If Arq.Exists Then
                ...
                CnnAux.Close()
                Page.DataBind()
                Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "script", "location.href='digitalizacao_principal_e.aspx?produto=" + Session("produto") + "&usuario=" + Session("usuario") + "&pg=3&retorno=" & NrProcesso & "';", True)
            Else
                Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "script", "alert('ATENCIÓN: Error al intentar enviar el archivo!');location.href='digitalizacao_principal_e.aspx?produto=" + Session("produto") + "&usuario=" + Session("usuario") + "&pg=3&retorno=" & NrProcesso & "';", True)
            End If
        Else
            Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "script", "alert('ATENCIÓN: Error al intentar enviar el archivo!');location.href='digitalizacao_principal_e.aspx?produto=" + Session("produto") + "&usuario=" + Session("usuario") + "&pg=3&retorno=" & NrProcesso & "';", True)
        End If
    ...
    Sub EnviarEmail(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Page.DataBind()
        Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "script", "location.href='digitalizacao_principal_e.aspx?produto=" + Session("produto") + "&usuario=" + Session("usuario") + "&pg=2&retorno=" & NrProcesso & "';", True)
    End Sub
...
<script type="text/javascript">
    ...
    function validarData(campo, valor) {
        ...
        if (Trim(date) != '') {
            ...
            if (erro) {
                alert("ATENCIÓN: Fecha no válida!");
                campo.focus();
                campo.value = "";
                return false;
            }
    ...
    function findInPage(str) {
        ...
        if (NS4) {
            if (!win.find(str))
                ...
                n++;
            if (n == 0) alert(str + " no está en esta página.");
        }
        ...
        if (IE4) {
            ...
            if (found) {
                ...
                if (n > 0) {
                    ...
                    alert(str + " no está en esta página.");
            }
        }
        return false;
    }
                        ...
                        if (ajax.readyState == 4) { //agora exibe o que interessa
                            if (ajax.status == 200) {
                                ...
                                if (vResult.indexOf('erro') != 0) {
                                    ...
                                    alert('ATENCIÓN: Código no válido!!\nUtilice la consulta para buscar el código correcto.');
                                    vCodigo.style.background = 'red';
                                    vDesc.value = "";
                                }
                            } else {
                                alert('Se ha producido un error desconocido, por favor póngase en contacto con ti@myindaia.com.br!');
                                vDesc.value = "";
                            }
                        }
                    }
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4) {
                if (ajax.status == 200) {
                    ...
                    vResult = vResult.replace(/\+/g, " ");
                    vResult = unescape(vResult);
                    document.location = 'digitalizacao_principal_e.aspx?produto=<%=Session("produto")%>&usuario=<%=Session("usuario")%>&pg=2&retorno=' + vResult;
                }
            }
        }
        ajax.send(null);                  
            ...
            ajax.onreadystatechange = function () {
                if (ajax.readyState == 4) {
                    if (ajax.status == 200) {
                        ...
                        //alert(vResult);
                        if (vResult == "2") {
                            alert("ATENCIÓN: Customer Order/Indaia #/ID invalid!");
                            document.getElementById(campo).focus();
                        } else {
                            if (vResult == "1") {
                                alert("ATENCIÓN: Customer Order duplicate.\nPlease fill the field 'Customer Order'!");
                                document.getElementById(campo).focus();
                            } else {
                                if (tipo == '1') {
                                    retorno = unid_neg + produto + pedido;
                                    document.location = 'digitalizacao_principal_e.aspx?produto=<%=Session("produto")%>&usuario=<%=Session("usuario")%>&pg=2&retorno=' + retorno;
                                }
                                if (tipo == '2') {
                                    retorno = pedido;
                                    document.location = 'digitalizacao_principal_e.aspx?produto=<%=Session("produto")%>&usuario=<%=Session("usuario")%>&pg=2&retorno=' + retorno;
    ...
    function mostraEmail(arquivo) {
        if (arquivo != '' && document.getElementById('frmDigitalizacaoPrincipal_txtAnexoSelecionado').value != "") {
            alert("ATENÇÃO: Utilice la opción: 'Enviar archivo(s) seleccionado(s) por e-mail'!");
        } else {
...
<body class="body" style="text-align:center;background-color:#f0f1f6;margin:0px 0px 0px 0px;width:100%" onload="if('<%= Request("pg")%>'==1){carregaTelas('divDigitalizacao','divArquivos','divUpload');}if('<%= Request("pg")%>'==2){carregaTelas('divArquivos','divDigitalizacao','divUpload');}if('<%= Request("pg")%>'==3){carregaTelas('divUpload','divArquivos','divDigitalizacao');}">
    ...
    <div style="right:0;width:18px;"><a href="#" onclick="FechaDetalhes('divInfoCorpo','divInfo');"><img src="/imagens/30/btn_fechar24.gif" width="18" height="17" border="0" alt="Cerrar"></a></div>
    ...
    <div id="divInfoCorpoEmail" class="divDetalhes3" style="padding-left:20px;"> 
      ...
      <asp:Label ID="lblEmail" runat="server" Text="Para" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="68px" CssClass="lbl"/>&nbsp;
      ...
      <asp:Label ID="lblAssunto" runat="server" Text="Tema" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="68px" CssClass="lbl"/>&nbsp;
      <asp:TextBox ID="txtAssunto" runat="server" CssClass="textbox" Width="290px"/><br />
      <asp:Label ID="lblMensagem" runat="server" Text="Mensaje" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="68px" CssClass="lbl"/>&nbsp;
    ...
    <div id="divDigitalizacaoPrincipal" style="display:block;">
      ...
      <div style="width:600px;text-align:left;"><br />
        <asp:RadioButton runat="server" Checked="true" onclick="selecionaOpcao(this);" ID="chkProcesso"/><asp:Label ID="lblProcessoCheck" runat="server" Text="Proceso" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="100px" CssClass="lbl" style="vertical-align:middle;"/>&nbsp;<br /><br />
      </div>
      <asp:Label ID="lblUnidade" runat="server" Text="Unidad" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="100px" CssClass="lbl"/>&nbsp;
      ...
      <img id="imgUnidade" src="../../imagens/icones/16x16/btn_folder.gif" alt="Buscar" align="middle" style="cursor:pointer;" onclick="ExibeDetalhes('BUs','divInfoCorpo','divInfo','divInfoRodape','codigo=CD_UNID_NEG&nome=AP_UNID_NEG&tabela=TUNID_NEG&usuario=<%=Session("usuario")%>&campo=frmDigitalizacaoPrincipal_txtCdUnidade','digitalizacao_consulta', 431, 205, 31,0);"/><br /><br />
      <asp:Label ID="lblProcesso" runat="server" Text="Proceso" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="100px" CssClass="lbl"/>&nbsp;
      ...
      <img id="imgProcesso" src="../../imagens/icones/16x16/btn_folder.gif" alt="Buscar" align="middle" style="cursor:pointer;" onclick="ExibeDetalhes('Processos','divInfoCorpo','divInfo','divInfoRodape','cd_unid_neg='+document.getElementById('frmDigitalizacaoPrincipal_txtCdUnidade').value+'&cd_produto=<%=Session("produto") %>&usuario=<%=Session("usuario")%>&cd_cargo=<%=Session("cd_cargo")%>&codigo=NR_PROCESSO&campo=frmDigitalizacaoPrincipal_txtNrProcesso','digitalizacao_pedido', 350, 200,-9,40);"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <img id="imgID" src="../../imagens/icones/16x16/btn_folder.gif" alt="Buscar" align="middle" style="cursor:pointer;" onclick="ExibeDetalhes('Instruções de desembaraço','divInfoCorpo','divInfo','divInfoRodape','cd_unid_neg='+document.getElementById('frmDigitalizacaoPrincipal_txtCdUnidade').value+'&usuario=<%=Session("usuario")%>&cd_cargo=<%=Session("cd_cargo")%>&codigo=NR_PROCESSO&campo=frmDigitalizacaoPrincipal_txtID','digitalizacao_pedido', 250, 200,-330,40);"/><br /><br />
      <br />
      <div style="width:600px;text-align:left;">
        <asp:RadioButton runat="server" Checked="false" onclick="selecionaOpcao(this);" ID="chkPedido"/><asp:Label ID="lblPedidoCheck" runat="server" Text="Pedido" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="200px" CssClass="lbl" style="vertical-align:middle;" />&nbsp;<br /><br />
      </div>
      <asp:Label ID="lblCliente" runat="server" Text="Cliente" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="100px" CssClass="lbl"/>&nbsp;
      ...
      <img id="imgCliente" src="../../imagens/icones/16x16/btn_folder.gif" alt="Buscar" align="middle" style="cursor:pointer;" onclick="ExibeDetalhes('Customers','divInfoCorpo','divInfo','divInfoRodape','codigo=CD_EMPRESA&nome=NM_EMPRESA&tabela=TEMPRESA_NAC&usuario=<%=Session("usuario")%>&cd_cargo=<%=Session("cd_cargo")%>&campo=frmDigitalizacaoPrincipal_txtCdCliente','digitalizacao_consulta', 431, 205, 31,134);" disabled="true"/><br /><br />
      <asp:Label ID="lblPedido" runat="server" Text="Pedido" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="100px" CssClass="lbl"/>&nbsp;
      ...
      <img id="imgPedido" src="../../imagens/icones/16x16/btn_folder.gif" alt="Buscar" align="middle" style="cursor:pointer;" onclick="ExibeDetalhes('Pedidos','divInfoCorpo','divInfo','divInfoRodape','cd_cliente='+document.getElementById('frmDigitalizacaoPrincipal_txtCdCliente').value+'&usuario=<%=Session("usuario")%>&cd_cargo=<%=Session("cd_cargo")%>&codigo=NR_REFERENCIA&campo=frmDigitalizacaoPrincipal_txtNrPedido','digitalizacao_pedido', 250, 130,-60,209);" disabled="true"/>&nbsp;&nbsp;
      ...
      <img id="imgID2" src="../../imagens/icones/16x16/btn_folder.gif" alt="Buscar" align="middle" style="cursor:pointer;" onclick="" style="visibility:hidden;"/><br /><br />
    ...
    <div id="divArquivosPrincipal" style="display:none;">
      <asp:UpdatePanel ID="upArquivosPrincipal" runat="server">    
        ...
        <div style="height: 20px;width:660px;overflow:hidden;border-color:#f8a310;border-style:solid;border-width:1px;text-align:center;">
          ...
          <td style="width:260px">Descripción</td>
          <td style="width:180px">Registro Creador</td>
          <td style="width:170px">Fecha de la creación</td>
          </tr></table>
        </div>
          ...
          <asp:GridView ID="grdArquivos" runat="server" DataSourceID="SqlArquivos"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText=" " ItemStyle-Width="40px">
                  <ItemTemplate>
                     <asp:HyperLink ID="ibVisualizaArquivo" runat="server" style="cursor:pointer;" ToolTip="Ver" Target="_blank" NavigateUrl='<%# Eval("CODIGO", "..\..\redireciona_doc.asp?cod={0}")%>'><img id="imgVisualiza" src="../../imagens/30/view_doctos.gif" alt="Ver" align="absmiddle" style="cursor:pointer;border:0;" /></asp:HyperLink>&nbsp;
                     <img id="imgEmailDoctos" src="../../imagens/30/mail_doctos.gif" align="middle" onclick="mostraEmail('<%# Eval("RESULTADO") %>');" style="cursor:pointer;border:0;" alt="Enviar por e-mail"/>   
                  </ItemTemplate>
                </asp:TemplateField>
        ...
        <div style="width:640px;text-align:right;">       
          <img id="imgEmailDoctos" src="../../imagens/30/mail_doctos.gif" align="middle"/>&nbsp;<span onclick="mostraEmail('');" class="lbl" style="font-size:9pt;cursor:pointer;">Enviar archivo(s) seleccionado(s) por e-mail</span>
        </div>
        </ContentTemplate>
      </asp:UpdatePanel>
    </div>
    ...
    <asp:Button ID="btnArquivos" runat="server" Text="Enviar" CssClass="textbox" OnClick="EnviarArquivo"  Width="55px" Height="20px" style="text-transform:none;font-size:9pt;font-weight:normal;background-color:#f8a310;color:#FFFFFF;font-family:Verdana;cursor:pointer;"/><br /><br />
    ...
    <br /><br />
    <asp:Label ID="lblArquivo" runat="server" Text="Descripción" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="70px" CssClass="lbl"/>&nbsp;
