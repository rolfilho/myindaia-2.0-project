...
<%
Session("sql_pendencias") = Null
If Not fnc_TestaVar(Session("sql_pendencias")) Then

sql_s = "SELECT DISTINCT R.NR_PROCESSO_DES, R.NR_REFERENCIA, R.NR_ITEM_CLIENTE, R.NR_PARCIAL, R.CD_EMPRESA, PR.CD_PRODUTO, R.CD_PENDENCIA,  "&_
	        " (SELECT MASTER.DBO.ROWCONCAT((RTRIM(LTRIM(EP.NR_REFERENCIA))+'/'+EP.NR_ITEM_CLIENTE)+'<br>') "&_
            " FROM TESTAGIO_PROCESSO EP (NOLOCK) "&_
            " WHERE EP.NR_PROCESSO_DES = R.NR_PROCESSO_DES "&_
            " AND EP.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE "&_
            " AND EP.NR_PARCIAL = R.NR_PARCIAL) AS REFERENCIA,  "
	'tabelas da busca

  sql_f = " FROM TREL_PENDENCIAS R (NOLOCK) "&_
					"  LEFT JOIN TPROCESSO PR (NOLOCK) ON (PR.NR_PROCESSO = R.NR_PROCESSO_DES) "&_
					"  INNER JOIN TPROCESSO_PENDENCIAS PP (NOLOCK) ON (PP.NR_PROCESSO = R.NR_PROCESSO_DES AND PP.CD_PENDENCIA = R.CD_PENDENCIA) "&_
					"  LEFT JOIN TPROCESSO GP (NOLOCK) ON (GP.NR_PROCESSO = R.NR_PROCESSO_ADM) "&_
					" LEFT JOIN TFLP_IMPORT1 FI1 (NOLOCK) ON (FI1.NR_PROCESSO_DES = PR.NR_PROCESSO) "&_
                    " LEFT JOIN TFLP_IMPORT2 FI2 (NOLOCK) ON (FI2.NR_REFERENCIA = R.NR_REFERENCIA AND FI2.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND FI2.NR_PARCIAL = R.NR_PARCIAL AND FI2.CD_EMPRESA = R.CD_EMPRESA) "
'Response.Redirect("teste.asp?var1="&sql_s&sql_f&"")
  ...
  'dados básicos do relatório
	sql_s_a = "SELECT DISTINCT R.NR_PROCESSO_DES, R.NR_REFERENCIA, R.NR_ITEM_CLIENTE, R.NR_PARCIAL, R.CD_EMPRESA, PR.CD_PRODUTO, R.CD_PENDENCIA,  "&_
	        " (SELECT MASTER.DBO.ROWCONCAT((RTRIM(LTRIM(EP.NR_REFERENCIA))+'/'+EP.NR_ITEM_CLIENTE)+'<br>') "&_
            " FROM TESTAGIO_PROCESSO EP (NOLOCK) "&_
            " WHERE EP.NR_REFERENCIA = R.NR_REFERENCIA AND EP.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE "&_
            " AND EP.NR_PARCIAL = R.NR_PARCIAL AND EP.CD_EMPRESA = R.CD_EMPRESA) AS REFERENCIA,  "
	'tabelas da busca

  sql_f_a = " FROM TREL_PENDENCIAS R (NOLOCK) "&_
					"  LEFT JOIN TPROCESSO PR (NOLOCK) ON (PR.NR_REFERENCIA = R.NR_REFERENCIA AND PR.CD_CLIENTE = R.CD_EMPRESA) "&_
					"  INNER JOIN TPROCESSO_PENDENCIAS PP (NOLOCK) ON (PP.CD_PENDENCIA = R.CD_PENDENCIA AND PP.NR_REFERENCIA = R.NR_REFERENCIA AND PP.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND PP.NR_PARCIAL = R.NR_PARCIAL AND PP.CD_EMPRESA = R.CD_EMPRESA) "&_
					"  LEFT JOIN TPROCESSO GP (NOLOCK) ON (GP.NR_REFERENCIA = R.NR_REFERENCIA AND GP.CD_CLIENTE = R.CD_EMPRESA) "&_
					" LEFT JOIN TFLP_IMPORT1 FI1 (NOLOCK) ON (FI1.NR_REFERENCIA = R.NR_REFERENCIA AND FI1.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND FI1.NR_PARCIAL = R.NR_PARCIAL AND FI1.CD_EMPRESA = R.CD_EMPRESA) "&_
                    " LEFT JOIN TFLP_IMPORT2 FI2 (NOLOCK) ON (FI2.NR_REFERENCIA = R.NR_REFERENCIA AND FI2.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND FI2.NR_PARCIAL = R.NR_PARCIAL AND FI2.CD_EMPRESA = R.CD_EMPRESA) "
'Response.Redirect("teste.asp?var1="&sql_s&sql_f&"")
	'requisitos da pesquisa
	sql_w_a = "WHERE R.CD_USUARIO ='"& Session("cd_usuario") &"' "&_
					"  AND R.NR_IDENT   = "& nr_ident & sql_w_a&" "
	' ordenação dos registros  
	...
	Else
	sql_o = sql_o &" R.NR_PROCESSO_DES ASC" 
	End If
	...
	Do While Not objrsv.Eof
		If objrsv.Fields.Item("M12_IN_MONTA_SQL").Value Then
			...
			If objrsv.Fields.Item("M10_TP_CONSULTA") = "0" Then
				Select Case objrsv.Fields.Item("M10_TP_DADO").Value
					Case "0","5" 'String/Text
						...
						sql_s_a = sql_s_a &" ISNULL("&objrsv.Fields.Item("M12_NM_CH_ORIGEM").Value&",'') AS "& objrsv.Fields.Item("M10_AP_CAMPO").Value &", "
					Case "1" 'Datetime
						...
						Else
							...
							sql_s_a = sql_s_a &" "& objrsv.Fields.Item("M12_NM_CH_ORIGEM").Value &" AS RLZ_"& objrsv.Fields.Item("M10_AP_CAMPO").Value &", "
						End If
					Case "2","3","4","6" 'Decimal/Moeda/Inteiro
						...
						sql_s_a = sql_s_a &" ISNULL("& objrsv.Fields.Item("M12_NM_CH_ORIGEM").Value &", 0) AS "& objrsv.Fields.Item("M10_AP_CAMPO").Value &", "
				End Select
			End If
			'Lookup
			...
			If objrsv.Fields.Item("M10_TP_CONSULTA") = "2" Then
				...
				sql_s_a = sql_s_a &" "& objrsv.Fields.Item("M12_NM_CH_ORIGEM").Value &" AS "& objrsv.Fields.Item("M10_AP_CAMPO").Value &", "
			End If
			'INNER JOIN
		...
		If objrsv.Eof Then
			...
			sql_s_a = Left( sql_s_a, Len(sql_s_a)-2 ) &" "
			Exit Do
		End If
	Loop
	...
	Session("sql_pendencias") = sql_s & sql_f & sql_w & sql_u & sql_s_a & sql_f_a & sql_w_a & sql_o
	'response.Write(Session("sql_pendencias"))
	'response.end
End If
...
tx_path_file  = "/controle/pendencias/controle_pendencias_excel"
tx_path_excel = "/controle/pendencias/controle_pendencias_excel"
...
if Len(vTxLinha(0)) > 0 then
	vTxLinha(0) = "<th#ROW# width=160 class='gridtit' style='background-color:" & cabec_result & ";'>Anexos</th>"&vTxLinha(0)
  	nr_cols = nr_cols + 1
...
Do While nrec < objrs.PageSize and Not objrs.Eof
	...
	'link para o detalhe do processo
		...
		"tp_relat=2&filtro=nr_processo&nr_processo=" & nr_processo
		else
		
		If objrs.Fields.Item("CD_PRODUTO").Value = "06" Then
			txLinkProcesso = "/Detalhe/imp_proc_result.asp?cd_menu_rel=01&cd_subm_rel=01&cd_view_rel=1801-0101-01&tp_relat=2"&tp_relat&"&filtro=nr_referencia,nr_item_cliente,nr_parcial,cd_empresa&nr_referencia="&objrs.Fields.Item("NR_REFERENCIA").Value&"&nr_item_clnte="&objrs.Fields.Item("NR_ITEM_CLIENTE").Value&"&nr_parcial="&objrs.Fields.Item("NR_PARCIAL").Value&"&cd_empresa="&objrs.Fields.Item("CD_EMPRESA").Value				
			else
			txLinkProcesso ="/Detalhe/exp_proc_result.asp?cd_menu_rel=02&cd_subm_rel=01&cd_view_rel=1630-0201-03&filtro=nr_processo&nr_processo=" & nr_processo
			End If
		end if 
...
%>
					...
					<td align="right"><nobr>&nbsp;<a href='#' onClick="abreDocs('<%=nr_processo %>','','1');">Ver Anexos</a>&nbsp;</nobr></td>
                    <%
	...
	Do While Not objrsv.Eof
		...
		Select Case objrsv.Fields.Item("M10_CD_DADO").Value
			...
			"FROM TFLP_IMPORT FI1 "&_
            "   LEFT JOIN TFLP_IMPORT2 FI2 (NOLOCK) ON (FI2.NR_REFERENCIA = R.NR_REFERENCIA AND FI2.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND FI2.NR_PARCIAL = R.NR_PARCIAL AND FI2.CD_EMPRESA = R.CD_EMPRESA) "
			If tp_relat = 0 then
				sql = sql & " WHERE FI1.NR_PROCESSO_DES = '" & nr_processo & "'"
			Else
				sql = sql & " WHERE FI1.NR_REFERENCIA = '" & nr_referencia & "'"&_
				" AND FI1.NR_ITEM_CLIENTE = '" & nr_item_clnte & "'"&_
				" AND FI1.CD_EMPRESA = '" & cd_empresa & "'"&_
				" AND FI1.NR_PARCIAL = '" & nr_parcial & "'"		
			End If
			...
			If in_kpi_opcao = "1" Then
				...
				total_fds & ") AS DIFERENCA_TOTAL "&_
                "FROM TFLP_IMPORT FI1 "&_
                "   LEFT JOIN TFLP_IMPORT2 FI2 (NOLOCK) ON (FI2.NR_REFERENCIA = R.NR_REFERENCIA AND FI2.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND FI2.NR_PARCIAL = R.NR_PARCIAL AND FI2.CD_EMPRESA = R.CD_EMPRESA) "
...
%>
			...
			If objrsx.state = adstateOpen then objrsx.close()
					  sql = " SELECT U.NM_USUARIO AS CONTATO "&_
                      " FROM TPROCESSO_PENDENCIAS P (NOLOCK) "&_
                      "  INNER JOIN TUSUARIO U (NOLOCK) ON U.CD_USUARIO = P.CD_USUARIO_CONTATO "&_
            ...
            <%            	
             Case "0718" ' 
					...
					End If
			 %>  
						<td><nobr>&nbsp;<a href="#" onClick="abreHistorico('<%= nr_processo_pendencia%>', '<%= objrs.Fields.Item("CD_PENDENCIA").Value%>','<%= nr_referencia_pendencia%>','<%= nr_item_cliente_pendencia%>', '<%= nr_parcial_pendencia%>', '<%= cd_empresa_pendencia%>');">Ver Histórico</a>&nbsp;</nobr></td>		           
