...
Response.Redirect("cadastro_viagem_filtros.asp?cd_menu="& cd_menu & "&cd_subm="& cd_subm& "&in_pesquisa=1")
End If

Session("grupo_utilizado") = "X"
If fnc_TestaVar(Request("cd_grupo")) Then
	Session("grupo_utilizado") = Request("cd_grupo")
End If
...
If tx_erro <> "" Then
  ...
  Response.Redirect("controle_pendencias_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If
		...
		if cd_filtros2_total(cont_cd_filtros2_total) <> "pesquisa" Then
			If InStr(tx_filtros2_total(cont_cd_filtros2_total), "*") > 0 Then
				...
				For cont_tx_filtros2_parcial = 0 to UBound(tx_filtros2_parcial)
					sql = "INSERT INTO M17_VIEW_CONS (M17_CD_VIEW, M17_CD_FILTRO_DADO, M17_CD_FILTRO_CONS, M17_CD_FILTRO_COMBO) "&_
					" VALUES ('" & cd_view & "','" & cd_filtros2_total(cont_cd_filtros2_total) & "','" & tx_filtros2_parcial(cont_tx_filtros2_parcial) & "','" & combo_filtros_total(cont_cd_filtros2_total) & "') "					
					objcnn.Execute(sql)
				Next	
			Else	
				sql = "INSERT INTO M17_VIEW_CONS (M17_CD_VIEW, M17_CD_FILTRO_DADO, M17_CD_FILTRO_CONS, M17_CD_FILTRO_COMBO) "&_
				" VALUES ('" & cd_view & "','" & cd_filtros2_total(cont_cd_filtros2_total) & "','" & tx_filtros2_total(cont_cd_filtros2_total) & "','" & combo_filtros_total(cont_cd_filtros2_total) & "') "
				objcnn.Execute(sql)
			End If
		End If
	Next
...
'response.end
  ...
  str = "/controle/viagem/cadastro_viagem_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = tx_lang_A00002(cd_lang) &": "& tx_lang_A00029(cd_lang) &".\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/controle/viagem/cadastro_viagem_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
