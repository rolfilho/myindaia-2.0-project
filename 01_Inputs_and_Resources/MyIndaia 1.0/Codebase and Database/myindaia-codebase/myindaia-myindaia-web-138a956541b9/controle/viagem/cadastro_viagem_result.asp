...
'---------------------------------------------------------------------------
autoriza = Request.QueryString("autoriza")
in_default = Request.QueryString("in_default")
%>
<!--#include virtual="/includes/inc_utils.asp"-->
<%If autoriza <> "1" Then%>
<!--#include virtual="/includes/inc_execs.asp"-->
<%Else
  Session("grupo_utilizado") = Request.QueryString("cd_grupo")
...
If Not fnc_TestaVar(Session("sql_viagem")) Then
  ...
  If autoriza <> "1" Then
    sql_s = "SELECT DISTINCT R.CD_UNID_NEG AS CD_UNID_NEG, R.CD_PRODUTO AS CD_PRODUTO, R.CD_EMBARCACAO AS CD_EMBARCACAO, R.CD_VIAGEM AS CD_VIAGEM, R.CD_ARMAZEM, E.NM_EMBARCACAO AS NM_EMBARCACAO, V.CD_LOCAL, LE.NM_LOCAL_EMB AS LOCAL_EMB, "
	
    sql_f = " FROM TREL_VIAGEM R (NOLOCK) "&_
    " INNER JOIN BROKER.DBO.TVIAGEM V ON (V.CD_UNID_NEG = R.CD_UNID_NEG AND V.CD_PRODUTO = R.CD_PRODUTO AND V.CD_EMBARCACAO = R.CD_EMBARCACAO AND V.NR_VIAGEM = R.CD_VIAGEM) "&_
    " INNER JOIN TEMBARCACAO E ON (E.CD_EMBARCACAO = R.CD_EMBARCACAO)  "&_
    " LEFT JOIN TPROCESSO PR ON (PR.CD_EMBARCACAO = R.CD_EMBARCACAO AND PR.NR_VIAGEM = R.CD_VIAGEM AND "&_
    "(CASE PR.CD_UNID_NEG WHEN '07' THEN '01' ELSE PR.CD_UNID_NEG END) = R.CD_UNID_NEG AND PR.CD_PRODUTO = R.CD_PRODUTO) "&_
  ...
  Else
    sql_s = "SELECT DISTINCT V.CD_UNID_NEG AS CD_UNID_NEG, V.CD_PRODUTO AS CD_PRODUTO, V.CD_EMBARCACAO AS CD_EMBARCACAO, V.NR_VIAGEM AS CD_VIAGEM, V.CD_ARMAZEM_ATRACACAO, E.NM_EMBARCACAO AS NM_EMBARCACAO, V.CD_LOCAL, LE.NM_LOCAL_EMB AS LOCAL_EMB, "
	
    sql_f = " FROM BROKER.DBO.TVIAGEM V  "&_
    " INNER JOIN TEMBARCACAO E ON (E.CD_EMBARCACAO = V.CD_EMBARCACAO)  "&_
    " LEFT JOIN TPROCESSO PR ON (PR.CD_EMBARCACAO = V.CD_EMBARCACAO AND PR.NR_VIAGEM = V.NR_VIAGEM AND "&_
    "(CASE PR.CD_UNID_NEG WHEN '07' THEN '01' ELSE PR.CD_UNID_NEG END) = V.CD_UNID_NEG AND PR.CD_PRODUTO = V.CD_PRODUTO) "&_
    " INNER JOIN TLOCAL_EMBARQUE LE ON (LE.CD_LOCAL_EMB = V.CD_LOCAL) "
            
    sql_w = " WHERE V.DT_ENT IS NULL AND PR.CD_GRUPO = '" & Request.QueryString("cd_grupo") & "' AND PR.CD_PRODUTO = '" & Request.QueryString("cd_produto") & "' AND SUBSTRING(PR.NR_PROCESSO, 5, 2) NOT IN ('TD') AND V.DT_ESPERADA > GETDATE() - 16"
  End If
	' ordenação dos registros  
	sql_o = " ORDER BY V.CD_LOCAL ASC, V.DT_ESPERADA "
	...
	'response.Write(Session("sql_viagem"))
	'response.end
End If
...
url_mais = "?cd_menu="& cd_menu &_
			"&cd_subm="& cd_subm &_
			"&cd_view="& cd_view &_
			"&exibe_mais=1" &_
			"&nr_ident="& nr_ident & url			

url_menos = "?cd_menu="& cd_menu &_
			"&cd_subm="& cd_subm &_
			"&cd_view="& cd_view &_
			"&exibe_mais=0" &_
			"&nr_ident="& nr_ident & url		
...
tx_path_file  = "/controle/viagem/cadastro_viagem_excel"
tx_path_excel = "/controle/viagem/cadastro_viagem_excel"
...
//cd_unid_neg, cd_produto, cd_viagem, cd_emb
function abreDetalhesNavio(cd_div, total){
   var img_mais  = new Image(13,15);
   var img_menos = new Image(13,15);
   var img_atual = eval("document.getElementById('img_"+cd_div+"')");
   
   img_mais.src  = "/imagens/30/btn_mais.gif";
   img_menos.src = "/imagens/30/btn_menos.gif";

   var div = document.getElementById(cd_div);
   var total_div = parseInt(document.getElementById(total).value);
   if (div.style.display == 'block'){
	 ...
	 for (i = 0; i <= total_div-1;i++){
		if (div.id != i){
 		  if (eval('document.getElementById('+i+')')){			
			  var div_retira = eval('document.getElementById('+i+')');	  
			  var img_retira = eval("document.getElementById('img_"+i+"')");	
			  div_retira.style.display = 'none';		
			  img_retira.src = img_mais.src;	
		  }		
		}
	 }
   }
   //tableScroll('TabelaPrincipal');
	...
	//window.open('/controle/viagem/cadastro_viagem_detalhe.asp?cd_unid_neg='+cd_unid_neg+'&cd_produto='+cd_produto+'&cd_viagem='+cd_viagem+'&cd_embarcacao='+cd_emb,"win0",features);
}
</script>

</head>

<body <%If autoriza <> "1" Then %>onLoad="document.getElementById('colunaseta').background='/imagens/30/seta.jpg';"<%End If %>>
<table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="0"  class="table">
<%if in_default <> "1" Then %>
  <tr valign="top">
    <td <%If autoriza <> "1" Then %>height="<%= Application("vl_width_header")%>"<%Else%>height="20px"<%End If %> colspan="2"><!-- :::::::::: INICIO DO CABEÇALHO :::::::::: -->
    <%If autoriza <> "1" Then %>
 ...
 <td width="10" rowspan="2" valign="top"><!-- :::::::::: INICIO DO MENU PRINCIPAL :::::::::: -->
  ...
  <%Else %>
  <tr height="20" bgcolor="#b4c7db" align="center">
  <td style="text-align:center;"><b><font size="2">Posição de Navios</font></b></td>
  </tr>
  <%End If %>
	<tr>
    <td valign="top"><!-- :::::::::: INICIO DO CORPO CENTRAL :::::::::: -->
<div id="dv_corpo" class="divMainMid" <%If autoriza = "1" Then%><%if in_default = "1" Then%>style="top:20px;left:0px"<%Else%>style="top:100px;left:5px"<%End If%><%End If%>>
...
%>
	...
	Do While Not objrsv.Eof
		...
		Select Case objrsv.Fields.Item("M10_CD_DADO").Value
		    Case "0044"            
			%>
				<td align="right"><nobr><img src="/imagens/30/btn_mais.gif"  border="0" alt="Informações" align="absmiddle" style="cursor:pointer" onClick="javascript:abreDetalhesNavio('<%= nrec%>','registro_pagina');" id="img_<%= nrec%>">&nbsp;<%=objrs.Fields.Item("NM_EMBARCACAO").Value%></nobr></td>
                <%
			Case "0746"
...
sql = " SELECT PR.NR_PROCESSO AS NR_PROCESSO, ISNULL((SELECT TOP 1 CD_REFERENCIA FROM TREF_CLIENTE (NOLOCK) WHERE NR_PROCESSO = PR.NR_PROCESSO AND TP_REFERENCIA = '01'),' - ') AS NR_REFERENCIA, "&_
    " ISNULL((SELECT TOP 1 CD_REFERENCIA FROM TREF_CLIENTE (NOLOCK) WHERE NR_PROCESSO = PR.NR_PROCESSO AND TP_REFERENCIA = '01'),' - ') AS NR_REFERENCIA, "&_
    ...
    "   AND SUBSTRING(PR.NR_PROCESSO, 5, 2) NOT IN ('TD')"
	  If Not Session("cargo_interno") = "1" and autoriza <> "1" Then sql = sql & " AND PR.CD_GRUPO = '" & session("GrupoUsuario2") & "'"
...
%>
 <tr bgcolor="<%= bgcolor%>">
                <td colspan="<%= nr_cols%>">
                    <div id="<%= nrec%>" style="display:none;margin:6px 6px 6px 6px;width:100%">
          <table border="1" cellspacing="0" cellpadding="0" bgcolor="#336699" style="padding: 1px 1px 1px 1px;" bordercolor="#b4c7db" width="90%" align="center">
				<tr align="center" >
					<td style="background-color:#f0f1f6;font-weight:bold;text-align:center;" width="20%"><nobr>&nbsp;Nr. Processo&nbsp;</nobr></td>
					<td style="background-color:#f0f1f6;font-weight:bold;text-align:center;" width="20%"><nobr>&nbsp;Referência&nbsp;</nobr></td>
                    <td style="background-color:#f0f1f6;font-weight:bold;text-align:center;" width="20%"><nobr>&nbsp;Main Leg&nbsp;</nobr></td>
                    <td style="background-color:#f0f1f6;font-weight:bold;text-align:center;" width="20%"><nobr>&nbsp;Abertura&nbsp;</nobr></td>
                    <%If autoriza <> "1" Then %>
                    <td style="background-color:#f0f1f6;font-weight:bold;text-align:center;" width="20%"><nobr>&nbsp;Doctos.&nbsp;</nobr></td>
                    <%End If %>
				</tr>
<%

Do While Not objrsx.Eof 
		If Trim(objrs.Fields.Item("CD_PRODUTO").Value) = "01" then
		...
		"tp_relat=2&filtro=nr_processo&nr_processo=" & objrsx.Fields.Item("NR_PROCESSO").Value
		Else
			txLinkProcesso ="/Detalhe/exp_proc_result.asp?cd_menu_rel=02&cd_subm_rel=01&cd_view_rel=1630-0201-03&filtro=nr_processo&nr_processo=" & objrsx.Fields.Item("NR_PROCESSO").Value
		End if 

%>
				<tr align="center" >
					<td style="background-color:#f0f1f6;text-align:center;"><nobr>&nbsp;<a href="<%=txLinkProcesso%>" style="color:#97ABC1;" onMouseOut="this.style.color='#97ABC1';" onMouseOver="this.style.color='#ff0000';"><%=Mid(objrsx.Fields.Item("NR_PROCESSO").Value,5,15)%></a>&nbsp;</nobr></td>
					<td style="background-color:#f0f1f6;color:#97ABC1;text-align:center;"><nobr>&nbsp;<%=objrsx.Fields.Item("NR_REFERENCIA").Value%>&nbsp;</nobr></td>
                    <td style="background-color:#f0f1f6;color:#97ABC1;text-align:center;"><nobr>&nbsp;<%=objrsx.Fields.Item("NR_MAIN_LEG").Value%>&nbsp;</nobr></td>
                    <td style="background-color:#f0f1f6;color:#97ABC1;text-align:center;"><nobr>&nbsp;<%=objrsx.Fields.Item("DT_ABERTURA").Value%>&nbsp;</nobr></td>
                    <%If autoriza <> "1" Then %>
                    <td style="background-color:#f0f1f6;color:#97ABC1;text-align:center;"><nobr>&nbsp;
                    ...
                    <a href='#' onClick="abreDocs('<%= objrsx.Fields.Item("NR_PROCESSO").Value %>','<%= objrsx.Fields.Item("NR_PROCESSO").Value %>','2');"><img src="/imagens/30/button_view.gif" width="16" height="16" align="absmiddle" vspace="1" border="0" /></a>
                    <%Else%>-
                    <%End If%>
                    &nbsp;</nobr></td>
                    <%End If%>
...
%>
					...
					%>
                    <script>
					document.getElementById('img_<%=nrec%>').style.visibility='hidden';
					</script>
                    <%
					End If
	nrec = nrec + 1
