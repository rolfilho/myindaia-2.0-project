...
if request.form("checkSubmit") = "duplicar" then 'duplica a rap selecionada
			...
			If vedtCodRap2=""  then
					...
					'		vNrCDE           = UCase(Request.Form("edtNrCDE"))
							...
							sql = "INSERT INTO TRAP ("&_
													...
													"  VL_CUBAGEM, IN_COURRIER, VL_QTD_VOL ) "&_
													...
													"  "&fncValSQL(vVlCubagem, 3)&", "&vInCourrier&", "&fncValSQL(vVlQtdVolume, 3)&" )"
						Else
							vInFabrExpo      = 0
							vInTriangulacao  = 0
							vInPermisso      = 0
			...
			If InStr("040;055;060", Session("cd_cargo")) = 0 Then							
				if objrsx.State = adStateOpen then objrsx.Close()						
				sql = " SELECT G.EMAIL_CRIACAO AS EMAIL, G.IN_EMAIL_CRIACAO AS IN_EMAIL, G.IN_INTEGRA_FORNECIMENTO AS IN_FORNECIMENTO, G.IN_INTEGRA_PACKING_LIST AS IN_PACKING_LIST FROM TEMPRESA_NAC E (NOLOCK)" &_
...
corpo = corpo &	"		<col width='100'>"
corpo = corpo &	"		<col width='220>"
corpo = corpo &	"		<col width='100'>"
corpo = corpo &	"		<col width='240'>"
	If Session("InModoSimples")<>"1" Then
...
corpo = corpo &	"			<td class='c1'><b>CADIVI?</b></td>"
corpo = corpo &	"			<td class='c2'>" & sim_nao(vInCadivi) & "&nbsp;</td>"
corpo = corpo &	"		</tr>"
corpo = corpo &	"		<tr height=" & vl_heigth & ">"
...
corpo = corpo &	"	</table>"
		...
		If (session("EmailCriacao")="1" or vInEmail="1") and vCodExportador <> "" Then
			Set mail = Server.CreateObject("Persits.MailSender") 
				...
				mail.From = "ti@myindaia.com.br" 
				mail.FromName = "myIndaiá TI" 
				If session("EmailCriacaoEmail") <> "" Then
					mail.AddAddress session("EmailCriacaoEmail")
				Else
					mail.AddAddress vEmail
				End If
				mail.Subject = "[myIndaiáWeb] Novo Embarque Criado (duplicado) - RAP: "& Left(Trim(vnrrap),6) & "/" & Right(Trim(vnrrap),2)
				mail.IsHTML = True
				mail.Body = corpo
				mail.Send 
			set Mail = nothing
		...
		If (session("EmailCriacao")="1" or vInEmail="1") and vCodExportador <> "" Then
			Set mail = Server.CreateObject("Persits.MailSender") 
				...
				Else
					mail.AddAddress vEmail
				End If
				mail.Subject = "[myIndaiáWeb] Embarque Alterado - RAP: "&  Left(Trim(vnrrap),6) & "/" & Right(Trim(vnrrap),2)
				mail.IsHTML = True
				mail.Body = corpo
				mail.Send 
			set Mail = nothing
...
corpo = corpo &	"	</table>"
		...
		If (session("EmailCriacao")="1" or vInEmail="1") and vCodExportador <> "" Then
			Set mail = Server.CreateObject("Persits.MailSender") 
				...
				Else
					mail.AddAddress vEmail
				End If
				mail.Subject = "[myIndaiáWeb] Novo Embarque Criado - RAP: "& Left(Trim(vnrrap),6) & "/" & Right(Trim(vnrrap),2)
				mail.IsHTML = True
				mail.Body = corpo
				mail.Send 
			set Mail = nothing
	...
	if vInTestarRap then
		if objrs.State = adStateOpen then objrs.Close()
		sql = "SELECT CD_EXPORTADOR, NR_FORNECIMENTO, NR_NOTA_CREDITO, CD_MOEDA_MLE, CD_INCOTERM, CD_TERMO_PAGTO, IN_AMOSTRA, CD_STATUS, IN_ANTECIPADO, "&_
		...
		If InStr("040;055;060", Session("cd_cargo")) = 0 Then
			if objrsx.State = adStateOpen then objrsx.Close()						
			sql = " SELECT G.EMAIL_CRIACAO AS EMAIL, G.IN_EMAIL_CRIACAO AS IN_EMAIL, G.IN_INTEGRA_FORNECIMENTO AS IN_FORNECIMENTO, G.IN_INTEGRA_PACKING_LIST AS IN_PACKING_LIST, G.IN_MODO_SIMPLES AS IN_MODO_SIMPLES, G.CD_GRUPO AS GRUPO FROM TEMPRESA_NAC E (NOLOCK)" &_
				" LEFT JOIN TGRUPO G ON (G.CD_GRUPO = E.CD_GRUPO)"&_
	...
	'Response.Redirect("../IMPORT/teste.asp?var1="&checkrap&"")
		...
		if vInForn  and (Session("InPackingList")="1" or vInPackList="1") then
			if objrsx.State = adStateOpen then objrsx.Close()
	...
	if (document.getElementById) {
		...
		ajax.onreadystatechange = function() {
			if(ajax.readyState == 1) { //enquanto estiver processando exibe mensagem para acalmar
				vCorpo.innerHTML = "<div style='position:absolute;top:40%;left:50%;'><center><img src='/imagens/icones/32x32/loading.gif'></center></div>";
			}
...
<body onLoad="PreencheForm('<%= vTpAcao%>', '<%= vTpExibe%>');">
...
<%If vTpAcao="duplicar" then%>
				<td width="45"><input type="image" name="btnSalvar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Salvar cópia desta instrução" onClick = "return verificaSalvar();"></td>
...
If vTpExibe <> 1 Then
  ...
  <td width="45"><input type="image" name="btnSalvar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Esta instrução não pode ser alterada" disabled="true"></td>
  <%else%>
				<td width="45"><input type="image" name="btnSalvar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Você deve estar visualizando a instrução para poder salvá-la" disabled="true"></td>
  <%End If%>              
 <%else%>
 <td width="45"><input type="image" name="btnSalvar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Salvar Edição" onClick = "return verificaSalvar();"></td>
<%end if end if%>				
        ...
        <% end if %>
  	  	...
  	  	<% end if 
                ...
                <%Else%>
                <td width="45"><img name="btnIntegraForn" src="/imagens/icones/32x32/btn_txt_1.gif" class="btnImagem" alt="Integrar Processo" onClick="abreIntegra(3);"></td>
                <%End If%>
				<% end if 
...
%>
			<tr height="25">
			  <td colspan="15" style="border-top: 1px solid #CCCCCC; padding: 5px 0px 0px 0px;">&nbsp;<b>Exibir instruções:</b>
				...
				<input name="pNrRap" type="text" id="pNrRap" maxlength="9" class="editbox" style="width:80px;" onBlur="pesqNrRap=this.value;" onKeyPress="if(event.keyCode == 13){pesqNrRap=this.value;ExibeListaRap('DivCorpo',5,'cd_menu=<%=cd_menu%>&cd_subm=<%=cd_subm%>&cd_docto=<%=cd_docto%>');document.getElementById('cmbExibeInst').value='5';vTpExibe=5;}">
				<label for="pNrCDE" class="label">CDE:</label>
                <input name="pNrCDE" type="text" id="pNrCDE" maxlength="15" class="editbox" style="width:60px;" onBlur="pesqNrCDE=this.value;" onKeyPress="if(event.keyCode == 13){pesqNrCDE=this.value;ExibeListaRap('DivCorpo',5,'cd_menu=<%=cd_menu%>&cd_subm=<%=cd_subm%>&cd_docto=<%=cd_docto%>');document.getElementById('cmbExibeInst').value='5';vTpExibe=5;}">
				<a href="#" onClick="ExibeListaRap('DivCorpo',5,'cd_menu=<%=cd_menu%>&cd_subm=<%=cd_subm%>&cd_docto=<%=cd_docto%>');document.getElementById('cmbExibeInst').value='5';vTpExibe=5;"><img src="/imagens/icones/16x16/button_view1.gif" alt="Pesquisar" width="14" height="13" border="0"></a></td>
			</tr>
<%else%>
			<tr height="25">
				...
				<%
	if vTpAcao <> "novo" and vTpAcao <> "duplicar" then
		if vValida then
			If vCdStatus = "2" Then
				Response.Write("<a href='#' onClick=""ExibeProblemasRap('"&vNrRap&"', '0');""><img src='/imagens/icones/16x16/leg_verde2.gif' class='imgValida'> Instrução de Embarque válida e pronta para embarque.</a>")
			Else
				Response.Write("<a href='#' onClick=""ExibeProblemasRap('"&vNrRap&"', '0');""><img src='/imagens/icones/16x16/leg_amarelo2.gif' class='imgValida'> Instrução de Embarque válida e pronta para embarque.</a>")
			End If
		else
  		if vInAntecipado then
				if vInRemessa then
					If vCdStatus = "2" Then
						Response.Write("<a href='#' onClick=""ExibeProblemasRap('"&vNrRap&"', '1');""><img src='/imagens/icones/16x16/leg_verde2.gif' class='imgValida'> Instrução de Embarque pronta para Autorização de Emissão de Doctos.</a><br>")
					Else
						Response.Write("<a href='#' onClick=""ExibeProblemasRap('"&vNrRap&"', '1');""><img src='/imagens/icones/16x16/leg_amarelo2.gif' class='imgValida'> Instrução de Embarque pronta para Autorização de Emissão de Doctos.</a><br>")
					End If
				else
					Response.Write("<a href='#' onClick=""ExibeProblemasRap('"&vNrRap&"', '1');""><img src='/imagens/icones/16x16/leg_vermelho2.gif' class='imgValida'> Verifique os erros da instrução clicando aqui. (Autorização de Doctos.)</a><br>")
				end if
			end if
			Response.Write("<a href='#' onClick=""ExibeProblemasRap('"&vNrRap&"', '0');""><img src='/imagens/icones/16x16/leg_vermelho2.gif' class='imgValida'> Verifique os erros da instrução clicando aqui. (Embarque)</a>")
		end if
...
if vTpAcao <> "listar"  and vTpAcao <> "listarPE" then
	if vTpAcao = "novo" then 
		...
		if objrsx.State = adStateOpen then objrsx.Close()	
		sql = " SELECT  TG.IN_MODO_SIMPLES AS IN_MODO_SIMPLES " &_
				...
				<table width="750" border="0" cellspacing="0" cellpadding="2">
					...
					<tr height="<%= vl_heigth%>">
						...
						if vTpAcao = "novo" or vTpAcao = "duplicar" then%>
								<td class="c2">
								<%If vTpAcao = "duplicar" then%> 
								<input name="edtCodRap2" type="text" class="editbox" id="rap" style="width: 100px;" tabindex="1"  size="6" maxlength="6" onBlur="verificaRap_Ano(edtCodRapAno)" onFocus="this.select();" <% If session("InRapSequencial") = "1" Then%> readonly <%End If%>>
								<%else%>
								<input name="edtCodRap" type="text" class="editbox" id="rap" style="width: 100px;" tabindex="1"  size="6" maxlength="6" onBlur="verificaRap_Ano(edtCodRapAno)" onFocus="this.select();" <% If session("InRapSequencial") = "1" Then%> readonly <%End If%>>
...
<%end if%>
					...
					<tr height="<%= vl_heigth%>">
                        ...
                        <!-- Function 'RapAcao' inserida no onBlur dos editbox's para a verificação da RAP-->
						<td class="c2"><input name="rdgInAmostra" type="radio" id="nao10" tabindex="3" value="0" onBlur="RapAcao('<%= vTpAcao%>');">
								<label class="label" for="nao10">N&atilde;o</label>
								<input name="rdgInAmostra" type="radio" id="sim10" value="1" onBlur="RapAcao('<%= vTpAcao%>');">
								<label class="label" for="sim10">Sim</label></td>
						<td class="c1"><b>Antecipado?</b></td>
						<td class="c2"><input name="rdgInAntecipado" type="radio" id="nao11" tabindex="4" value="0" onBlur="RapAcao('<%= vTpAcao%>');">
...
%>
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmExportador" width="16" height="16" border="0" align="absmiddle" id="img_edtNmExportador" style="cursor: hand;" onClick="<%= str%>"></nobr></td>
                        <% If Session("InModoSimples")<>"1" Then%>
						<td class="c1"><label class="label" for="logo">Logomarca:</label></td>
						<td class="c2"><select name="cmbCodLogomarca" class="select" id="logo" style="width:100px;" tabindex="6" onBlur="RapAcao('<%= vTpAcao%>');">
								<option value=""></option>
					...
					<tr height="<%= vl_heigth%>">
						...
						<%If Session("InModoSimples")<>"1" Then%>
                            ...
                            Nr. Fornecimento:
                            <%Else%>
                            Nr. Fornecimento/<br>Nr. Processo
                            <%End If%>
						<%Else%>
							<%If InStr("040;055;060", Session("cd_cargo")) > 0 Then%>
                            Nr. Processo:
						...
						<% If Session("InModoSimples")<>"1" Then%><br><%End If%>
						    <textarea name="edtNrFornec" wrap="virtual" class="editbox" id="fornec" cols="30" rows="5" style="width:250px;height:<% If Session("InModoSimples")<>"1" Then%>70px<%Else%>35px<%End If%>;text-transform:uppercase;" tabindex="9" onChange="ApenasNumero(this,';');" onBlur="RapAcao('<%= vTpAcao%>');"></textarea>
						<% If Session("InModoSimples")<>"1" Then%><br><%End If%>
                        </td>
                        <% If Session("InModoSimples")<>"1" Then%>
...
%>
                        <img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descri&ccedil;&atilde;o" name="img_edtNmFabricante" width="16" height="16" border="0" align="absmiddle" id="img_edtNmFabricante" style="cursor: hand;" onClick="<%= str%>"></td>
				  </tr>
                  <% If Session("InModoSimples")<>"1" Then%>
					<tr height="<%= vl_heigth%>">
					  <td class="c1" ><strong>Dt. Faturamento:</strong></td>
				      <td class="c2"><input name="edtDtFaturamento" type="text" class="editbox" id="edtDtFaturamento" style="width: 80px;" tabindex="8" onKeyPress="return(MascaraData(this,'/',event));" onBlur="RapAcao('<%= vTpAcao%>');">
...
%>
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmCliente" width="16" height="16" border="0" align="absmiddle" id="img_edtNmCliente" style="cursor: hand;" onClick="<%= str%>"></nobr></td>
						...
						<td class="c2"><nobr>
							...
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmPais" width="16" height="16" border="0" align="absmiddle" id="img_edtNmPais" style="cursor: hand;" onClick="ConsultaOnLine('TPAIS','CODIGO,DESCRICAO','Código,Descrição','CODIGO','','edtCodPais');"></nobr></td>
					</tr>
                    <% If Session("InModoSimples")<>"1" Then%>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="dtemb">Dt. Acordada de Embarque:</label></td>
						<td class="c2"><input name="edtDtEmbarque" type="text" class="editbox" id="edtDtEmbarque" style="width: 80px;" tabindex="11" onKeyPress="return(MascaraData(this,'/',event));" onBlur="RapAcao('<%= vTpAcao%>');">
						...
						<td class="c2"><input name="rdgInCadivi" type="radio" id="nao9" tabindex="14" value="0" onBlur="RapAcao('<%= vTpAcao%>');">
								<label class="label" for="nao9">N&atilde;o</label>
								<input name="rdgInCadivi" type="radio" id="sim9" value="1" onBlur="RapAcao('<%= vTpAcao%>');">
								<label class="label" for="sim9">Sim</label></td>
					</tr>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="drawback">Drawback</label></td>
						<td class="c2"><select name="cmbTpDrawback" class="select" id="drawback" style="width:100px;" tabindex="15" onChange="MudaDrawback(this);" onBlur="RapAcao('<%= vTpAcao%>');">
								<%= fncCriaOption("drawback")%>
						</select></td>
						<td class="c1"><label class="label" for="edtNrAtoDrawback">Nr. Ato Consessório</label></td>
						<td class="c2"><input name="edtNrAtoDrawback" type="text" class="editbox" id="edtNrAtoDrawback" style="width:214px;text-transform:uppercase;" tabindex="16" maxlength="13" onBlur="RapAcao('<%= vTpAcao%>');"></td>
					</tr>
					<tr height="<%= vl_heigth%>">
                   ...
                   <%End if%>
					...
					<tr height="<%= vl_heigth%>">
						...
						<td class="c2"><select name="cmbCodIncoterm" class="select" id="incoterm" style="width: 100px;" tabindex="26" onBlur="RapAcao('<%= vTpAcao%>');">
								<option value="">&nbsp;</option>
...
%>
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmCondPagto" width="16" height="16" border="0" align="absmiddle" id="img_edtNmCondPagto" style="cursor: hand;" onClick="<%= str%>"></nobr></td>
					</tr>
                    <% If Session("InModoSimples")<>"1" Then%>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="vlmle">Valor FOB:</label></td>
						<td class="c2"><input name="edtVlMLE" type="text" class="editbox" id="vlmle" style="width: 100px;" tabindex="28" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
						...
						<td class="c2"><input name="edtCodMoedaMLE" type="text" class="editbox" id="moeda" style="width: 40px;" tabindex="29" onBlur="RapAcao('<%= vTpAcao%>');FastLookup('TMOEDA_BROKER','CD_MOEDA','NM_MOEDA',this,'edtNmMoedaMLE');" maxlength="3">
								...
								<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmMoedaMLE" width="16" height="16" border="0" align="absmiddle" id="img_edtNmMoedaMLE" style="cursor: hand;" onClick="ConsultaOnLine('TMOEDA_BROKER','CD_MOEDA,NM_MOEDA','Código,Descrição','CD_MOEDA','','edtCodMoedaMLE');"></td>
					</tr>
                    <%End If%>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="vlacres">Acréscimo:</label></td>
						<td class="c2"><input name="edtVlAcrescimo" type="text" class="editbox" id="vlacres" style="width: 100px;" tabindex="30" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
                   <% If Session("InModoSimples")<>"1" Then%>     
						<td class="c1">&nbsp;</td>
						<td class="c2">&nbsp;</td>
					</tr>
                    ...
                    <% End If%>
						...
						<td class="c2" <% If Session("InModoSimples")="1" Then%>colspan="3"<%End if%>><input name="edtVlDesconto" type="text" class="editbox" id="vldesc" style="width: 100px;" tabindex="31" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
                        <% If Session("InModoSimples")<>"1" Then%>
					...
					<tr height="<%= vl_heigth%>">
						...
						<td class="c2"><input name="edtVlFreteInt" type="text" class="editbox" id="vlfrete" style="width: 100px;" tabindex="33" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
                        <% If Session("InModoSimples")<>"1" Then%>
						<td class="c1"><label class="label" for="vlpb">Peso Bruto:</label></td>
						<td class="c2"><input name="edtVlPesoBruto" type="text" class="editbox" id="vlpb" style="width: 100px;" tabindex="34" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>	
                        <%End if%>	
                     <% If Session("InModoSimples")<>"1" Then%>   				
					</tr>
					<tr height="<%= vl_heigth%>">
                    <%End If%>
						...
						<td class="c2"><input name="edtVlSeguro" type="text" class="editbox" id="seguro" style="width: 100px;" tabindex="35" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
                        <% If Session("InModoSimples")<>"1" Then%>
						<td class="c1"><label class="label" for="vlpl">Peso L&iacute;quido:</label></td>
						<td class="c2"><input name="edtVlPesoLiq" type="text" class="editbox" id="vlpl" style="width: 100px;" tabindex="36" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
                        <% End If %>
					</tr>
                    <% If Session("InModoSimples")<>"1" Then%>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="cubagem">Cubagem:</label></td>
						<td class="c2"><input name="edtVlCubagem" type="text" class="editbox" id="cubagem" style="width: 100px;" tabindex="37" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
						<td class="c1">&nbsp;</td>
						<td class="c2">&nbsp;</td>
					</tr>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="edtTxEmbalagem">Embalagens:</label></td>
						<td class="c2"><input name="edtTxEmbalagem" type="text" class="editbox" id="edtTxEmbalagem"  value="<%=vTxEmbalagem%>" style="width:214px;text-transform:uppercase;" tabindex="38" maxlength="255" onBlur="RapAcao('<%= vTpAcao%>');"></td>
						<td class="c1"><label class="label" for="edtVlQtdVolume">Qtde. Volumes:</label></td>
						<td class="c2"><input name="edtVlQtdVolume" type="text" class="editbox" id="edtVlQtdVolume" style="width:100px;" tabindex="39" onBlur="RapAcao('<%= vTpAcao%>');ValidaValor(this);"></td>
					</tr>
					<tr height="<%= vl_heigth%>">
...
%>
								<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmAgenteComissao" width="16" height="16" border="0" align="absmiddle" id="img_edtNmAgenteComissao" style="cursor: hand;" onClick="<%= str%>"></td>
					</tr>
					<tr height="<%= vl_heigth%>">
...
%>
								<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmDepColeta" width="16" height="16" border="0" align="absmiddle" id="img_edtNmDepColeta" style="cursor: hand;" onClick="<%= str%>"></td>
						<td class="c1">&nbsp;</td>
						<td class="c2">&nbsp;</td>
					</tr>
                    <%End if%>
					<tr height="<%= vl_heigth%>">
						...
						<td class="c2"><select name="cmbCodViaTransp" class="select" id="modal" style="width: 100px;" tabindex="44"  onChange="verificaTransporte(this.value);" onBlur="RapAcao('<%= vTpAcao%>');">
                       ...
                       Courrier <img src="../../imagens/icones/16x16/icon_mini_faq.gif" alt="O checkbox Courrier somente poder&aacute; ser marcado para a op&ccedil;&atilde;o Via de Transporte a&eacute;rea!" width="15" height="13"></label></td>
						...
						<td class="c2"><input name="edtCodEmbarcador" type="text" class="editbox" id="embac" style="width: 40px;" tabindex="45" onBlur="RapAcao('<%= vTpAcao%>');FastLookup('TAGENTE','CD_AGENTE','NM_AGENTE',this,'edtNmEmbarcador');" maxlength="4">
								...
								<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmEmbarcador" width="16" height="16" border="0" align="absmiddle" id="img_edtNmEmbarcador" style="cursor: hand;" onClick="ConsultaOnLine('TAGENTE','CD_AGENTE,NM_AGENTE','Código,Descrição','CD_AGENTE','','edtCodEmbarcador');"></td>
					</tr>
					<tr height="<%= vl_heigth%>">
						...
						<td class="c2"><input name="edtCodEmbarcacao" type="text" class="editbox" id="navio" style="width: 40px;" tabindex="46" onBlur="RapAcao('<%= vTpAcao%>');FastLookup('TEMBARCACAO','CD_EMBARCACAO','NM_EMBARCACAO',this,'edtNmEmbarcacao');" maxlength="4">
								...
								<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmEmbarcacao" width="16" height="16" border="0" align="absmiddle" id="img_edtNmEmbarcacao" style="cursor: hand;" onClick="ConsultaOnLine('TEMBARCACAO','CD_EMBARCACAO,NM_EMBARCACAO','Código,Descrição','CD_EMBARCACAO','','edtCodEmbarcacao');"></td>
						...
						<td class="c2"><input name="edtNrViagem" type="hidden" class="editbox" id="viagem" style="width:80px;" tabindex="47" onKeyUp="ApenasNumero(this,'/');" maxlength="5" onBlur="RapAcao('<%= vTpAcao%>');">
								<img src="/imagens/icones/16x16/btn_folder.gif" width="16" height="16" border="0" alt="Consulta por descrição" style="cursor: hand;visibility:hidden" align="absmiddle" onClick="ConsultaOnLineSQL(5,'Viagem,Dt. Esperada,Dt. Entrada,Local Embarque,Arm. Atracação,Nº Manifesto','NR_VIAGEM','edtNrViagem',edtCodEmbarcacao.value);"></td>
					</tr>
					<tr height="<%= vl_heigth%>">
						...
						<td class="c2"><nobr>
							...
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmLocalEmb" width="16" height="16" border="0" align="absmiddle" id="img_edtNmLocalEmb" style="cursor: hand;" onClick="ConsultaOnLine('TLOCAL_EMBARQUE','CD_LOCAL_EMB,NM_LOCAL_EMB','Código,Descrição','CD_LOCAL_EMB','','edtCodLocalEmb');"></nobr></td>
						...
						<td class="c2"><nobr>
							...
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="img_edtNmLocalDesemb" width="16" height="16" border="0" align="absmiddle" id="img_edtNmLocalDesemb" style="cursor: hand;" onClick="ConsultaOnLine('TLOCAL_EMBARQUE','CD_LOCAL_EMB,NM_LOCAL_EMB','Código,Descrição','CD_LOCAL_EMB','','edtCodLocalDesemb');"></nobr></td>
					</tr>
					<tr height="<%= vl_heigth%>">
						<td class="c1"><label class="label" for="notify">Notify:</label></td>
						<td class="c2"><select name="cmbTpNotify" class="select" id="notify" style="width:100px;" tabindex="50" onChange="HabNotifyConsignee(this,document.forms[0].edtCodNotify);" onBlur="RapAcao('<%= vTpAcao%>');">
								<%= fncCriaOption("pessoa_exp")%>
						</select></td>
						<td class="c1"><label class="label" for="consignee">Consignee:</label></td>
						<td class="c2"><select name="cmbTpConsignee" class="select" id="consignee" style="width:100px;" tabindex="51" onChange="HabNotifyConsignee(this,document.forms[0].edtCodConsignee);" onBlur="RapAcao('<%= vTpAcao%>');">
								<%= fncCriaOption("pessoa_exp")%>
						</select></td>
					</tr>
					<tr height="<%= vl_heigth%>">
						...
						<td class="c2"><nobr>
							...
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="btnNotify" width="16" height="16" border="0" align="absmiddle" id="btnNotify" style="cursor: hand;" onClick="ConsNotify('N',1,document.forms[0].edtCodNotify,'');"></nobr></td>
						<td class="c1">&nbsp;</td>
						<td class="c2"><nobr>
							...
							<img src="/imagens/icones/16x16/btn_folder.gif" alt="Consulta por descrição" name="btnConsignee" width="16" height="16" border="0" align="absmiddle" id="btnConsignee" style="cursor: hand;" onClick="ConsNotify('C',1,document.forms[0].edtCodConsignee,'');"></nobr></td>
					</tr>
                    <% If Session("InModoSimples")<>"1" Then%>
					<tr height="<%= vl_heigth%>">
                    ...
                    <tr>
                    <td colspan="4" class="c2" align="right"><img src="/imagens/icones/16x16/leg_vermelho.gif" border="1" align="absmiddle"> Cadastros Inativos&nbsp;&nbsp;</td>
                    </tr>
				</table>
      <%
...
%>
				<table width="750" border="0" cellspacing="1" cellpadding="0" bgcolor="#336699" style="padding: 0px 1px 0px 1px;">
					<tr height="18">
                        ...
                        <%Else%>
                        <td class="gridtit">Nr. Processo</td>
                        <%End If%>
...
function PreencheForm(vTpAcao, vTpExibe) {
	if (vTpAcao == 'listar') {
		...
		document.getElementById('colunaseta').background='/imagens/seta.jpg';
