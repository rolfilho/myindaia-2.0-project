...
<tr style="vertical-align:top;">
 <td colspan="3" align="left" style="background-color:#E9E9E9;">
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#DEDEDE">
  <form name="frmEmbarque" action="ccembarque.asp" method="post" onSubmit="return ValidaForm(this);">
    ...
    <tr>
      <td><div id="embarque" class="popupClass" style="width:700px;">
          <table width="100%" border="0" cellpadding="0" cellspacing="2" bgcolor="#336699">
            <tr height="18">
              <td class="gridtit">&nbsp;Enviar</td>
              <td class="gridtit"> Instrução/CDE </td>
              <td class="gridtit">&nbsp;Cliente/Importador</td>
              <td class="gridtit"><nobr>&nbsp;País de Destino</nobr></td>
              <% If Session("InModoSimplesCheck") <>"1" Then%>
              <td class="gridtit">&nbsp;Fornecimentos</td>
              <%Else%>
              <td class="gridtit">&nbsp;Processo</td>              
              <%End If%>
...
if objrs.RecordCount > 0 then
	...
	do while not objrs.Eof
		...
		if objrsx.State = adStateOpen then objrsx.Close()						
            ...
            in_doctos_rap = 1
            If objrsx.RecordCount > 0 then                  
                If Not objFSO.FolderExists(Server.MapPath("/docs_rap/rap"& vNrRap & "_3")) Then	
                    in_doctos_rap = 0
                End If
            End If
%>
            <tr bgcolor="#FFFFFF">
              <td align="center"><input type="checkbox" name="chkCodRapAno" value="<%= vNrRap%>" vInDoctosRap="<%= in_doctos_rap %>" vNrCDE="<%= vNrCDE%>" vCodImp="<%= vCodImp%>" vCodPais="<%= vCodPais%>" vCodColeta="<%= vCodColeta%>" vNmColeta1="<%= vNmColeta1%>" vCodIncoterm="<%= vCodIncoterm%>" vCodTermoPgto="<%= vCodTermoPgto%>" vCodExp="<%= vCodExp%>" vCodLogo="<%= vCodLogo%>" vCodViaTransp="<%= vCodViaTransp%>" vModoSimples="<%= Session("InModoSimplesCheck")%>" vCodEmbarcador="<%= vCodEmbarcador%>"/></td>
              <td><%= Left(vNrRap, 6)&"/"&Right(vNrRap, 2)%>&nbsp;</td>
              <td><%= objrs.Fields.Item("NM_EMPRESA").Value%>&nbsp;</td>
              <td><%= objrs.Fields.Item("NM_PAIS").Value%>&nbsp;</td>
              <td><%= objrs.Fields.Item("NR_FORNECIMENTO").Value%>&nbsp;</td>
