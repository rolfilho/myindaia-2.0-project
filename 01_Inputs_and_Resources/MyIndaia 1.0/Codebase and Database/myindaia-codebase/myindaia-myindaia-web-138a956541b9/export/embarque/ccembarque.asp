...
sub subInsereEmbarque(vListaRap)
	...
	sql = "SELECT RIGHT('0000' + CONVERT(VARCHAR, ISNULL(MAX(CAST(CD_EMBARQUE AS INTEGER)), 0) + 1), 8) AS EMBARQUE, " &_
	...
	for x = 0 to Ubound(aListaRap) - 1
		...
		sql = "INSERT INTO TRAP_EMBARQUE(CD_EMBARQUE, CD_EMBARQUE_ANO, CD_RAP, CD_RAP_ANO, CD_GRUPO, "&_
					...
					"VALUES ('"&Left(aCodEmbarque, 8)&"', '"&Right(aCodEmbarque, 2)&"', '"&Left(vRapAux, 6)&"', "&_
	...
	sql = "SELECT CD_RAP, CD_RAP_ANO, CD_STATUS, CD_USUARIO_CANC, CD_USUARIO_FIM, CD_USUARIO_CRIADOR, DT_INSERCAO, DT_CANCELADO, DT_FINALIZADO,CD_CLIENTE,CD_EMBARCADOR,CD_DEP_COLETA, NM_DEP_COLETA_1,CD_FABRICANTE,CD_EXPORTADOR, NR_FORNECIMENTO, "&_
            ...
            "  ISNULL(NR_CAIXAS,0) AS NR_CAIXAS, ISNULL(NR_PALLETS,0) AS NR_PALLETS, ISNULL(NR_JUMBOS,0) AS NR_JUMBOS, ISNULL(NR_PACOTES,0) AS NR_PACOTES,  ISNULL(NR_NOTAS_FISCAIS, '&nbsp;') AS NR_NOTAS_FISCAIS, ISNULL(NR_EASY, '&nbsp;') AS NR_EASY, " &_
...
objEmail.Configuration.Fields.Item ("http://schemas.microsoft.com/cdo/configuration/sendusername") = "AKIAU3FXQJOE7T5S552J"
objEmail.Configuration.Fields.Item ("http://schemas.microsoft.com/cdo/configuration/sendpassword") = "BL+u/elS1EZDfsmQFphyVQT5j3SlwR+vh4YmRDoh/6PO"
...
objEmail.Configuration.Fields.Item ("http://schemas.microsoft.com/cdo/configuration/smtpconnectiontimeout") = 90
  ...
  aListaRap2 = Split(vListaRap, ",", - 1)
  for x = 0 to Ubound(aListaRap2) - 1
	vRapAux = Trim(aListaRap2(x))
    
    corpo = ""	
	rap_assunto = ""
    if objrsx.State = adStateOpen then objrsx.Close()
	sql = "SELECT MASTER.DBO.FN_CRIPROGRAFA('" & vRapAux & Session("GrupoUsuario") & "') AS RAP "
    objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText

    rap_criptografada = objrsx.Fields.Item("RAP").Value
   
    link_nota = "http://www.myindaiaweb.com.br/export/embarque/ccprint_nota.asp?cod1=" & rap_criptografada & "&in_email=1"

    corpo = "<html>"
    corpo = corpo & "<body style='font-family:Calibri;'>"
    corpo = corpo & "<IMG SRC=http://www.myindaiaweb.com.br/imagens/logo2012_2.jpg><br>"
    corpo = corpo & "<IMG SRC=http://www.myindaiaweb.com.br/imagens/barra_tracking.gif><br><br>"
    corpo = corpo & "<a href="& link_nota & " style='text-decoration:none;font-weight: bold;'><img src='http://www.myindaiaweb.com.br/imagens/30/rap/caminhao.gif' style='vertical-align:middle;'>&nbsp;Controle de Despachos - RAP: " & vNrCde & "</a><br>"
    corpo = corpo & "<br>Esta é uma mensagem gerada automaticamente pelo MyIndaiá.<br>"
    corpo = corpo & "Por favor, não responda esse e-mail.<br>"
    corpo = corpo & "Caso tenha dúvidas ou se desejar não receber mais esta mensagem, favor entrar em contato com <b>myindaia@myindaia.com.br</b>.<br><br>"
    corpo = corpo & "Obrigado,<br>"
    corpo = corpo & "<b>Departamento de TI</b><br>"
    corpo = corpo & "<IMG SRC=http://www.myindaiaweb.com.br/imagens/logo_indaia.gif><br>"
    corpo = corpo & "<IMG SRC=http://www.myindaiaweb.com.br/imagens/recicle2.jpg><br>"
    corpo = corpo & "</html>"
    corpo = corpo & "</body>"


If (session("EmailAutorizacao")="1" or vInEmail="1") and Session("GrupoUsuario") = "028" Then
			Set objEmail = CreateObject("CDO.Message")
			objEmail.From 		= "ti@myindaia.com.br" 
			If session("EmailAutorizacaoEmail") <> "" Then
				objEmail.To 		= session("EmailAutorizacaoEmail")
			...
			objEmail.Subject = "[myIndaiáWeb] Controle de Despachos - RAP: "& vNrCde 
...
if objrs.RecordCount > 0 then
  ...
  <td colspan="8">
  <img name="imgAlerta" src="/imagens/30/alerta_32.gif" style="border:0;" align="absmiddle">Não existem instruções válidas para se criar um embarque
  </td>
  </tr>
  <%
end if
