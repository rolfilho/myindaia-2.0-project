...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#DEDEDE">
		<tr height="25">
			<td background="/imagens/bg_popup.gif" style="background-repeat: repeat-x"><table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr height="25">
						<td class="titpopup">&nbsp;Alterar Pedidos integrados na RAP <%= Mid(vNrRap, 1, 6) + "/" + Mid(vNrRap, 7, 2)%></td>
...
For Each check in Request.Form("checar")
  ...
  %>
    ...
    <%
  	...
  	sql = "UPDATE TPE_ITEM SET QT_BAIXADA =  ISNULL("& Replace(vQtde, ",", ".") &", 0) " &_
...
<script language="javascript">
<!--
 alert("Pedidos alterados com sucesso!");
 window.close();
...
sql_s = "SELECT RTRIM(LTRIM(CD_RAP)) + '/' + RTRIM(LTRIM(CD_RAP_ANO)) AS RAP,  "&_
        "RTRIM(LTRIM(CD_RAP)) + RTRIM(LTRIM(CD_RAP_ANO)) AS VRAP, "&_
				"RTRIM(LTRIM(NR_PEDIDO)) AS NR_PEDIDO, "&_
        "RTRIM(LTRIM(NR_ITEM)) AS NR_ITEM, "&_
        "ISNULL(QT_FORN_PEDIDO, 0) AS QT_TOTAL, "&_
				"ISNULL(QT_FORN_PEDIDO, 0) - ISNULL(QT_BAIXADA, 0) AS QT_RESTANTE, "&_
        "ISNULL(QT_BAIXADA, 0) AS QT_BAIXADA, "&_
        "CD_PRODUTO, "&_
        "TX_DESC "
sql_f = "FROM TPE_ITEM (NOLOCK) "
sql_w = "WHERE ISNULL(QT_BAIXADA, 0) > 0  AND "&_
        "      ((RTRIM(LTRIM(CD_RAP)) = '"& Mid(vNrRap, 1, 6) &"' AND CD_RAP_ANO = '"& Mid(vNrRap, 7, 2) &"') OR (ISNULL(CD_RAP, '') = '')) "
sql_o = "ORDER BY CD_RAP_ANO DESC , CD_RAP DESC "
'verifica o cargo do usuario
if InStr("040;055;060", Session("cd_cargo")) > 0 then
	sql_f = sql_f&" INNER JOIN TRAP R (NOLOCK) ON R.CD_RAP = P.CD_RAP AND R.CD_RAP_ANO = P.CD_RAP_ANO "&_
...
%>
 <tr height="100%">
   ...
   <table>
	   <tr height="25">
       <td> Selecione os pedidos que deseja alterar e insira a quantidade que deve estar integrada... </td>
     </tr>
	</table> 
   <table width="100%" border="0" cellpadding="0" cellspacing="1" style="padding: 2px 2px 0px 2px;" bgcolor="#336699">
	    <tr height="18">
    		...
    		<td width="50"  class="gridtit">Nova Qtde. Baixada&nbsp;</td>
    		<td width="70"  class="gridtit">Qtde. Baixada&nbsp;</td>
...
%>
	...
	<tr height="15" bgcolor="<%= bgcolor%>" align="center" valign="top">
		...
		<input type="hidden" name="hdnQtde" id="hdnQtde<%= nRec %>" value="<%= objrs.Fields.Item("QT_TOTAL").Value %>">
<%
    if vCheck then
...
%>
		...
		<td><input name="qtde" id="qtde<%= nRec %>" class="editbox" value="<%= FormatNumber(objrs.Fields.Item("QT_BAIXADA").Value, 2)%>" size="5"></td>
		<td align="right"><%= FormatNumber(objrs.Fields.Item("QT_BAIXADA").Value, 2)%></td>
...
'objrs.Close()
%>
  <input name='btnLiquidar' type='submit' class='button' value='Alterar Quantidades' style='width:150px;' >
	</form>
     </table>
   </div></td>
  </tr>
...
%>
   <tr> <td> <p align="center"><b> Não existem arquivos a serem alterados... </b></p> </td> </tr> 
 </table>
<%
end if
%>
