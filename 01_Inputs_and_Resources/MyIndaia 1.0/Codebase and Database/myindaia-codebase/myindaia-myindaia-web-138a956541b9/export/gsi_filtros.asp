...
'Arquivo de Script e HTML: Filtros do Relatório de GSI - General Shipment Instruction
'
...
%>
							...
							<%
	Case "0005","0006","0007","0008","0297" 'Selects - Grupo, Clientes, Área e Sub-área
%>
             <td align="right"><input type="checkbox" name="filtro" id="<%= objrsv("M10_AP_FILTRO")%>X" value="<%= objrsv("M10_AP_FILTRO")%>" onClick="clickCheck(this)"<%= checked%>></td>
		    ...
		    If objrsx.State = adStateOpen Then objrsx.Close()
						...
						If objrsx.RecordCount = 0 Then
							sql = "SELECT A.CD_AREA AS CODIGO, A.NM_AREA AS DESCRICAO " &_
									...
									" ORDER BY A.NM_AREA"
						End If	
			 Case "0297" 'SELECIONA O NOME DOS GRUPOS DE EMPRESAS DO CLIENTE
		    sql = "SELECT DISTINCT EE.CD_EMPRESA AS CODIGO, EE.NM_EMPRESA AS DESCRICAO " &_
				  " FROM TEMPRESA_EST EE ( NOLOCK ) " &_
				  " INNER JOIN TEMPRESA_EST_GRUPO EG (NOLOCK) ON (EG.CD_EMPRESA = EE.CD_EMPRESA) " &_
				  " INNER JOIN TPERMISSAO_GRUPO P ON ( P.CD_GRUPO   = EG.CD_GRUPO ) " &_
				  " WHERE P.CD_USUARIO = '"& Session("cd_usuario") &"' " &_
				  " ORDER BY EE.NM_EMPRESA"
		End Select
		If objrsx.State = adStateOpen Then objrsx.Close()
		objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
...
%>
			...
			Case "0297" 'Sub-área
%>
                  <!--#include virtual="/selects/option_exp_importador.txt"-->
                  <%	  
		End Select
	  End If
%>
                                ...
                                <td align="left" width="17"><%If objrsv.Fields.Item("M11_IN_VARIOS").Value Then%><a href="#" onClick="criaCombo('<%= CStr(vl_width_form)%>','<%= objrsv("M10_AP_FILTRO")%>', document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'));"><img id="img_<%= objrsv("M10_AP_FILTRO")%>" src='/imagens/30/btn_mais.gif' style='border:0;display:none;' class='top' alt='<%=tx_lang_TF017(cd_lang)%>' disabled="true"></a><img align="left" id="img_<%= objrsv("M10_AP_FILTRO")%>2" src='/imagens/30/btn_mais.gif' style='border:0;visibility:hidden' class='top'><input type="text" style="display:none" value="0" name="contador_<%= objrsv("M10_AP_FILTRO")%>" id="contador_<%= objrsv("M10_AP_FILTRO")%>"><input type="text" style="display:none" value="0" name="contador_total_<%= objrsv("M10_AP_FILTRO")%>" id="contador_total_<%= objrsv("M10_AP_FILTRO")%>"><%End If%></td>
								<%
	Case "0011","0088" 'Selects - País de Destino, Família de Mercadoria
%>
             <td align="right"><input type="checkbox" name="filtro" id="<%= objrsv("M10_AP_FILTRO")%>X" value="<%= objrsv("M10_AP_FILTRO")%>" onClick="clickCheck(this)"<%= checked%>></td>
							<td width="320px"><nobr><input type="text" style="display:none" value="<%= objrsv("M10_NM_DADO")%>" name="nome_<%= objrsv("M10_AP_FILTRO")%>" id="nome_<%= objrsv("M10_AP_FILTRO")%>"><label for="<%= objrsv("M10_AP_FILTRO")%>X" class="label"><%= objrsv("M10_NM_DADO")%>&nbsp;</label></nobr></td><td><select class="select" style="width:90px;" name="combo_aux_<%= objrsv("M10_AP_FILTRO")%>" onFocus="enterInput(this);" id="combo_aux_<%= objrsv("M10_AP_FILTRO")%>" onChange="fnc_comboEdt('<%= objrsv("M10_AP_FILTRO")%>',this);">
<option value="1"><%=tx_lang_TF002(cd_lang)%></option>
	  ...
	  'seleciona todas as informações para os usuários funcionários
	  Select Case objrsv.Fields.Item("M10_CD_DADO").Value
		Case "0011" 'País de Destino
%>
                  <!--#include virtual="/selects/option_paises.txt"-->
                  <%
		Case "0088" 'Família de Mercadoria
%>
                  <!--#include virtual="/selects/option_familia_prod_exp.txt"-->
                  <%
	  End Select
%>
              </select></td>
...
%>
              <td align="right"><input type="checkbox" name="filtro" id="<%= objrsv("M10_AP_FILTRO")%>X" value="<%= objrsv("M10_AP_FILTRO")%>" onClick="clickCheck(this)"<%= checked%>></td>
                            ...
                            <td align="left" width="17"><%If objrsv.Fields.Item("M11_IN_VARIOS").Value Then%><a href="#" onClick="criaEdit('<%= CStr(vl_width_form)%>','<%= objrsv("M10_AP_FILTRO")%>', document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'));"><img align="left" id="img_<%= objrsv("M10_AP_FILTRO")%>" src='/imagens/btn_mais.gif' style='border:0;display:none;' class='top' alt='<%=tx_lang_TF017(cd_lang)%>'></a><img align="left" id="img_<%= objrsv("M10_AP_FILTRO")%>2" src='/imagens/btn_mais.gif' style='border:0;visibility:hidden' class='top'><input type="text" style="display:none" value="0" name="contador_<%= objrsv("M10_AP_FILTRO")%>" id="contador_<%= objrsv("M10_AP_FILTRO")%>"><input type="text" style="display:none" value="0" name="contador_total_<%= objrsv("M10_AP_FILTRO")%>" id="contador_total_<%= objrsv("M10_AP_FILTRO")%>"><%End If%></td>
