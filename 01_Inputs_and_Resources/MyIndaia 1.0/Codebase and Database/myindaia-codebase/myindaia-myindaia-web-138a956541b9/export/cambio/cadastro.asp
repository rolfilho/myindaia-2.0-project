...
dim vVlContrato, vVlProjecao, vVlTaxaCambio, vVlTaxaJuros, vVlComissaoAg,vNrDias
...
function ExibeJuros(nNrContrato, nNrBanco, NrDias, nExibir) {
	var divJuros = document.getElementById('divJuros');
	if (nExibir) {
		CarregaViaGet('divJuros', 'ccjuros', 'vNrContrato='+nNrContrato+'&vNrBanco='+nNrBanco+'&vNrDias='+NrDias);
		if (document.layers) getMouseLoc;     //NS
		else if (document.all) getMouseLoc(); //IE
		...
		with (vForm) {
			...
			if (vTpAcao=='novo') {
				if (edtNrContrato.value == '' || edtNrBanco.value == '') {
					...
					if (edtDtFechamento.value  == '') txErro += '\nO Data de fechamento deve ser informado!';
					if (edtDtVenctoLiq.value  == '') txErro += '\nO Vencimento da liquidação deve ser informada!';
		...
		<tr height="75">
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="1">
					...
					<tr height="40" align="center">
                        ...
                        <%else%>
                        <td width="45"><input type="image" name="btnSalvar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Você deve estar no modo de edição para poder salvá-la" value="Submit" disabled="true"></td>
                        <%end if%>
...
'					"WHERE NR_CONT_CAMB = '"&vNrContrato&"' AND NR_BANCO = '"&vNrBanco&"'"
		...
		'vVlProjecao	= (objrsx.Fields.Item(0).Value/12) + vVlContrato
		vNrDias=DateDiff("d",objrs.Fields.Item("DT_FECHAMENTO").Value, objrs.Fields.Item("DT_VENC_LIQ").Value)
		vVlProjecao	= (FormatNumber(objrs.Fields.Item("VL_NEGOC").Value+(FormatNumber(objrs.Fields.Item("VL_NEGOC").Value*vNrDias*FormatNumber(objrs.Fields.Item("VL_TAXA_JUROS").Value)/36000, 2))))
					...
					<table width="750" border="0" cellspacing="0" cellpadding="0" style="padding: 0px 0px 0px 2px;">
						...
						<tr height="<%= vl_heigth%>">
							...
							<td><nobr><input name="edtVlProjecao" type="text" class="editbox" id="edtVlProjecao" style="width:100px; text-align:right;" readonly>
								<img src="/imagens/icones/16x16/btn_juros.gif" width="16" height="16" border="0" alt="Visualizar juros mensal" style="cursor: hand;" align="absmiddle" onClick="ExibeJuros('<%= vNrContrato%>', '<%= vNrBanco%>','<%= vNrDias%>', true)"></nobr></td>
						</tr>
						<tr height="<%= vl_heigth%>">
...
%>
						...
						<tr height="<%= vl_heigth%>">
							...
							<td><label class="label" for="edtVlTaxaJuros">Taxa de Juros Anual:</label></td>
						...
						<tr height="<%= vl_heigth%>">
							...
							<td colspan="2"><input name="edtDtVenctoLiq" type="text" class="editbox" id="edtDtVenctoLiq" style="width: 80px;" tabindex="10" onKeyPress="return(MascaraData(this,'/',event));">
								<img id="btnDtVenctoLiq" height=16 width=16 border=0 src="/imagens/icones/16x16/calenda.gif" align="absmiddle">(<%=vNrDias%> dias)</td>
