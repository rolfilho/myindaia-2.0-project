...
If Not fnc_TestaVar(Session("sql_exp_track")) Then
  ...
  sql_s=sql_s & " PEI.NR_PROC_PE, PEI.NR_ITEM_PE, "
  end if
...
/* Flávio Henrique - Cabeçalho fixo Julho/2008 */
...
Do While Not objrsv.Eof
	select case objrsv.Fields.Item("M10_CD_DADO").Value
		case "0118"
			...
			vTxLinha(1) = vTxLinha(1) &"<th>"&tx_lang_A00095(cd_lang)&"</th><th>"&tx_lang_A00096(cd_lang)&"</th><th>"&tx_lang_A00097(cd_lang)&"</th>"
			vInRowspan = true
			nr_cols = nr_cols + 3
		case else
			select case objrsv.Fields.Item("M10_TP_DADO").Value
				case "1" 'datas
					if fnc_TestaVar(objrsv.Fields.Item("M10_CD_BROKER").Value) and in_dt_prev then
						...
						vTxLinha(0) = vTxLinha(0) &"<th#ROW#>"&objrsv.Fields.Item("M10_NM_DADO").Value&"</th>" ' Título da coluna do relatório com o nome do campo selecionado.(datas)
						nr_cols = nr_cols + 1
					end if
				case else
					...
					End If
					' Título da coluna do relatório com o nome do campo selecionado.
					nr_cols = nr_cols + 1
			end select
	end select
  objrsv.MoveNext
...
Do While nrec < objrs.PageSize and Not objrs.Eof
			...
			%>
									<td align="center"><nobr>&nbsp;<a href="/detalhe/exp_proc_result.asp?cd_menu_rel=<%= cd_menu%>&cd_subm_rel=<%= cd_subm%>&cd_view_rel=<%= cd_view%>&filtro=nr_processo&nr_processo=<%= nr_processo%>"><%= Mid(nr_processo, 5, 10)%></a>&nbsp;</nobr></td>
									<%	
						Case "0009"
...
<%                      Case "0669"				
						...
						'===================================================================	
						Case "0018","0094","0096","0097","0098","0099","0100","0101","0374","0486","0780" 'Nr.Referencia - Item
									If objrsx.State = adStateOpen Then objrsx.Close()
