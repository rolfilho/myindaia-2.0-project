...
Dim sql_u_s 'monta o select da query com os campos a serem exibidos
Dim sql_u_f 'monta o from da query com as tabelas extras para a pesquisa
Dim sql_u_w 'monta o where da query
...
If Not fnc_TestaVar(Session("sql_exp_track_itens")) Then
  ...
  sql_s =  "SELECT DISTINCT                                               "&_
           "M2.AP_MERCADORIA                  AS AP_MERCADORIA,           "&_
           "A.NR_RE                           AS NR_RE,                   "&_
           "P.QT_MERCADORIA                   AS QT_MERCADORIA,           "&_
           "P.CD_NCM                          AS CD_NCM,                  "&_
           "I.VL_UNITARIO                     AS VL_UNITARIO,             "&_
           "P.VL_MLE                          AS VL_MLE,                  "&_
           "P.VL_MCV                          AS VL_MCV,                  "&_
           "P.VL_TOT_PESO_LIQ                 AS VL_TOT_PESO_LIQ,         "&_
           "CONVERT(VARCHAR, C.DT_DATA, 103)  AS DT_RE,                   "&_
           "D.NR_ATO_CONCESSORIO              AS ATO,                     "&_
           "ISNULL( M.AP_MERCADORIA,'')       AS TX_MERCADORIA,           "&_
           "M.CD_MERCADORIA                   AS CD_MERCADORIA,           "&_
           "R.NR_PROCESSO                     AS NR_PROCESSO,             "&_
           "R.CD_GRUPO                        AS CD_GRUPO,                "&_
           "R.CD_CLIENTE                      AS CD_CLIENTE,              "&_
           "R.CD_AREA                         AS CD_AREA,                 "&_
           "R.CD_VIA_TRANSPORTE               AS CD_VIA_TRANSPORTE,       "&_
	       "EE.NM_EMPRESA                     AS NM_IMPORTADOR,           "&_
	       "PD.NM_PAIS                        AS NM_PAIS_DESTINO,         "&_
	       "MF.AP_MOEDA                       AS NM_MOEDA_FRETE,          "&_
	       "A.VL_FRETE_RATEADO                AS VL_FRETE_PREPAID_MNEG,   "&_
	       "A.PESO_LIQUIDO                    AS VL_PESO_LIQ_UNIT,        "&_
	       "ISNULL(PR.IN_COURRIER,0)          AS IN_COURRIER,             "&_
	       "ISNULL(PR.IN_PRODUTO_IMPORTADO,0) AS IN_PRODUTO_IMPORTADO,    "&_
           "ISNULL(UNM.NM_UNID_MEDIDA,'')     AS NM_UNID_MEDIDA,          "&_
           "ISNULL(UNM1.NM_SIGLA,'')          AS NM_SIGLA,                "&_
           "I.NR_PEDIDO                       AS NR_PEDIDO1,              "&_
           "I.NR_PEDIDO_IMP                   AS NR_PEDIDO_IMP,           "&_
           "I.NR_ITEM_PE                      AS NR_ITEM_PE,              "&_
           "NULL                              AS NR_NF,                   "&_
           "NULL                              AS CHAVE_NOTA,              "&_
           "NULL                              AS DT_NOTA,                 "&_
           "NULL                              AS VL_NOTA_ITEM, "
  ...
  sql_f =           "   FROM TREL_EXP_TRACK                    R   (NOLOCK)                                                          "&_
					...
					"  LEFT  JOIN TPROCESSO_EXP_SAQUE          SQ  (NOLOCK) ON (SQ.NR_PROCESSO           = R.NR_PROCESSO)            "&_
                    "  LEFT JOIN TPROCESSO_EXP_ITEM            I   (NOLOCK) ON (I.NR_PROCESSO            = PR.NR_PROCESSO)           "&_
                    "  INNER JOIN TMERCADORIA_EXP              M   (NOLOCK) ON (M.CD_MERCADORIA          = I.CD_MERCADORIA)          "&_                  
                    "  INNER JOIN BROKER.DBO.TRE_ANEXO_PURO    P   (NOLOCK) ON P.NR_PROCESSO             = PR.NR_PROCESSO            "&_ 
                    "										               AND M.CD_MERCADORIA           = P.CD_MERCADORIA           "&_
                    ...
                    "  INNER JOIN BROKER.DBO.TMERCADORIA_EXP   M2 (NOLOCK)  ON M2.CD_MERCADORIA          = P.CD_MERCADORIA           "&_
                    "  OUTER APPLY (SELECT TOP 1 NM_UNID_MEDIDA FROM TUNID_MEDIDA_BROKER UMB WHERE UMB.CD_UNID_MEDIDA = I.CD_UNID_MEDIDA) UNM "&_
                    "  OUTER APPLY (SELECT TOP 1 NM_SIGLA FROM TUNID_MEDIDA_BROKER UMB WHERE UMB.CD_UNID_MEDIDA = I.CD_UNID_MEDIDA) UNM1 "&_
                    "  LEFT JOIN BROKER.DBO.TEMPRESA_EST       EE (NOLOCK) ON EE.CD_EMPRESA = PR.CD_IMPORTADOR " &_
                    "  LEFT JOIN TPAIS_BROKER                   PD (NOLOCK) ON PD.CD_PAIS = PR.CD_PAIS_DESTINO " &_
                    "  LEFT JOIN TMOEDA_BROKER                  MF (NOLOCK) ON MF.CD_MOEDA = PR.CD_MOEDA_FRETE "
...
End if 
		...
		'*=======================================================================================================
			...
			Select Case objrsv.Fields.Item("M10_TP_DADO").Value & objrsv.Fields.Item("M10_TP_CONSULTA") 
							...
							Select Case objrsv.Fields.Item("M10_CD_DADO").Value
										...
										Case "0621","0616","2345","2346","2347","2348","0681" ,"0619","0623","0789" 'Ítens - Cód. Mercadoria
								            sql_s = sql_s									
						  ...
						  '============================================================================================
							Case "01","21","31","41"'String - Tipo de consulta = Lookup //CÉLULA//
								...
								Case "0618","0126","0011","0061","0624","0622"
								sql_s = sql_s 
						  ...
						  '============================================================================================
							Case "20","30"'Decimal, Moeda
								Select Case objrsv.Fields.Item("M10_CD_DADO").Value
										Case "0063","0173","0617","0547","0620" 'Peso liquido/bruto
												sql_s = sql_s '&_
	...
	'---------union
      ...
      sql_u_s = "    UNION ALL                                                              "&_
                "  SELECT     distinct                                                      "&_
                "         M2.AP_MERCADORIA      					AS AP_MERCADORIA,       "&_
                "         PR.CD_DUE             					AS NR_RE,               "&_
                "         NFI.QT_COMERCIAL      					AS QT_MERCADORIA,       "&_
                "         NFI.CD_NCM            					AS CD_NCM,              "&_
                "         (NFI.VL_LOCAL_EMBARQUE / QT_COMERCIAL)    AS VL_UNITARIO,         "&_
                "         ISNULL(NFI.VL_LOCAL_EMBARQUE,0)   		AS VL_MLE,              "&_
                "         ISNULL(NFI.VL_CONDICAO_VENDA,0) 			AS VL_MCV,              "&_
                "         ISNULL(NFI.PL_MERCADORIA,0)     			AS VL_TOT_PESO_LIQ,     "&_
                "         CONVERT(VARCHAR, PE.DT_ENVIO_DUE, 103)    AS DT_RE,                "&_
                "         NULL                                      AS ATO,                  "&_
                "         ISNULL(NFI.AP_MERCADORIA,'')              AS TX_MERCADORIA,        "&_
                "         NFI.CD_MERCADORIA                         AS CD_MERCADORIA,       "&_
                "         R.NR_PROCESSO                             AS NR_PROCESSO,          "&_
                "         R.CD_GRUPO                                AS CD_GRUPO,             "&_
                "         R.CD_CLIENTE                              AS CD_CLIENTE,           "&_
                "         R.CD_AREA                                 AS CD_AREA,              "&_
                ...
                "         PD.NM_PAIS                                AS NM_PAIS_DESTINO,      "&_
                "         MF.AP_MOEDA                               AS NM_MOEDA_FRETE,       "&_
                "         NFI.VL_TOTAL_FRETE                        AS VL_FRETE_PREPAID_MNEG,"&_
                "         NFI.PL_MERCADORIA                         AS VL_PESO_LIQ_UNIT,             "&_
                "         ISNULL(PR.IN_COURRIER,0)                  AS IN_COURRIER,          "&_
                "         ISNULL(PR.IN_PRODUTO_IMPORTADO,0)         AS IN_PRODUTO_IMPORTADO, "&_
                "         ISNULL(UNM.NM_UNID_MEDIDA,'')             AS NM_UNID_MEDIDA,       "&_
                "         ISNULL(UNM1.NM_SIGLA,'')                  AS NM_SIGLA,             "&_
                "         ISNULL(PE.NR_FATURA, '')                  AS NR_PEDIDO1,           "&_
                "         TPEI.NR_PEDIDO_IMP                        AS NR_PEDIDO_IMP,        "&_
                "         TPEI.NR_ITEM_PE                           AS NR_ITEM_PE,           "&_
                "         NFI.NR_NF                                 AS NR_NF,                "&_
                "         NF.CHAVE_NOTA                             AS CHAVE_NOTA,           "&_
                "         NF.DT_NOTA                                AS DT_NOTA,              "&_
                "         (NFI.VL_ITEM + NFI.VL_OUTRAS_DESP + NFI.VL_TOTAL_FRETE)  AS VL_NOTA_ITEM, "
	
		     
     
  If tp_relat=1 then '1=Pedido 0-embarque
    sql_u_s = sql_u_s & " NULL AS NR_PROC_PE, NFI.NR_ITEM, "
  End If

  If tp_relat=3 then '1=Pedido 0-embarque
    sql_u_s = sql_u_s & " NULL AS NR_PEDIDO_EXP,  "
  end if
  

  sql_u_f =   "  FROM TREL_EXP_TRACK                    	R   (NOLOCK)                                           "&_           
              " INNER JOIN BROKER.DBO.TPROCESSO         	PR  (NOLOCK) ON (PR.NR_PROCESSO   = R.NR_PROCESSO)     "&_            
              " LEFT  JOIN BROKER.DBO.TPROCESSO_EXP     	PE  (NOLOCK) ON (PE.NR_PROCESSO   = R.NR_PROCESSO)     "&_   
              " LEFT  JOIN BROKER.DBO.TPROCESSO_EXP_NF      NF  (NOLOCK) ON NF.NR_PROCESSO    = PR.NR_PROCESSO     "&_ 
              
                
              " LEFT  JOIN BROKER.DBO.TPROCESSO_EXP_NF_ITEM NFI (NOLOCK) ON NFI.NR_PROCESSO   = NF.NR_PROCESSO     "&_
              "                                                         AND NFI.NR_NF         = NF.NR_NF           "&_
              
              " LEFT  JOIN BROKER.DBO.TMERCADORIA_EXP       M2  (NOLOCK) ON M2.CD_MERCADORIA  = NFI.CD_MERCADORIA  "&_
		      " LEFT  JOIN TFLP_EXPORT1                     FE  (NOLOCK) ON (FE.NR_PROCESSO   = R.NR_PROCESSO)     "&_
              " LEFT  JOIN TFLP_EXPORT2                     FE2 (NOLOCK) ON (FE2.NR_PROCESSO  = R.NR_PROCESSO)     "&_
              " LEFT  JOIN BROKER.DBO.TRE_CAPA              C   (NOLOCK) ON C.NR_PROCESSO     = R.NR_PROCESSO      "&_
              " OUTER APPLY (SELECT TOP 1 NM_UNID_MEDIDA FROM TUNID_MEDIDA_BROKER UMB WHERE UMB.NM_SIGLA = NFI.UN_COMERCIAL) UNM "&_
              " OUTER APPLY (SELECT TOP 1 NM_SIGLA FROM TUNID_MEDIDA_BROKER UMB WHERE UMB.NM_SIGLA = NFI.UN_COMERCIAL) UNM1 "&_
              "  LEFT JOIN BROKER.DBO.TEMPRESA_EST       EE (NOLOCK) ON EE.CD_EMPRESA = PR.CD_IMPORTADOR "       &_
              "  LEFT JOIN TPAIS_BROKER                   PD (NOLOCK) ON PD.CD_PAIS = PR.CD_PAIS_DESTINO "  &_
              "  LEFT JOIN BROKER.DBO.TPROCESSO_EXP_ITEM TPEI (NOLOCK) ON TPEI.NR_PROCESSO = PR.NR_PROCESSO AND TPEI.CD_MERCADORIA = NFI.CD_MERCADORIA AND TPEI.QT_MERCADORIA = NFI.QT_COMERCIAL "  &_
              "  LEFT JOIN TMOEDA_BROKER                  MF (NOLOCK) ON MF.CD_MOEDA = PR.CD_MOEDA_FRETE "
                    

					
	'requisitos da pesquisa
	sql_u_w = "WHERE C.NR_PROCESSO IS NULL AND R.CD_USUARIO = '"& Session("cd_usuario")&"' AND R.NR_IDENT = "& nr_ident &" "
	...
	sql_u_s = Left( sql_u_s, Len(sql_u_s)-2 ) &" "

	'finaliza a construção do select do relatório e salva na Session
	
	'-----------union
	
	Session("sql_exp_track_itens") = sql_s & sql_f & sql_w & sql_u_s & sql_u_f & sql_u_w & sql_o
 'Response.Redirect("../EXPORT/teste.asp?var1="&sql_s & sql_f & sql_w & sql_o&"")
...
'AÇÃO 3
'============================================================================================
'add por agatha nigro - se tiver vazio volta pra pagina anterior
If objrs.RecordCount = 0 then
  response.Write("<script language=""JavaScript"">"&_
        "alert('ATENÇÃO:\n\n - Busca terminada e nenhum registro encontrado.');"&_
        "document.location.href='javascript:history.back()';"&_
        "</script>")
...
tx_path_file  = "/export/trackitens_print"
tx_path_excel = "/export/trackitens_excel"
...
<!--
    function MM_jumpMenu(targ, selObj, restore) { //v3.0
        var target = "trackitens_result.asp<%= url%>&p=" + selObj.options[selObj.selectedIndex].value;
...
Do While Not objrsv.Eof
	...
	select case objrsv.Fields.Item("M10_CD_DADO").Value
		case "0118"
			vTxLinha(0) = vTxLinha(0) &"<th colspan='4' style='background-color:" & cabec_result & ";'>"&objrsv.Fields.Item("M10_NM_DADO").Value&"</td>"
			vTxLinha(1) = vTxLinha(1) &"<th>"&tx_lang_A00095(cd_lang)&"</td><th>"&tx_lang_A00096(cd_lang)&"</td><th>"&tx_lang_A00097(cd_lang)&"</td><th>"&tx_lang_A00098(cd_lang)&"</td>"
			vInRowspan = true
			nr_cols = nr_cols + 4
		case else
...
<%                      Case "0669"				
						...
						'===================================================================	
						Case "0018","0094","0096","0097","0098","0099","0100","0101","0374","0486","0678","0780","0899","0920","1051","1066","1067","1087","2017","2018","2024","2027","2029","2142","2157","2158","2159","2261","2317","2396","2400","2402","2408" 'Nr.Referencia - Item
									If objrsx.State = adStateOpen Then objrsx.Close()
						...
						dim tp_consignatario
								...
								If tp_consignatario = 2 Then 'Transportadora
									sql = "SELECT TEE.DESCRICAO AS NOME, LTRIM(RTRIM(ISNULL(TEE.END_CIA_TRANSP,''))) + ' - ' + LTRIM(RTRIM(ISNULL(TEE.END_COMPL_CIA_TRANSP,''))) AS EMPRESA_END, TEE.END_CIDADE, TPEE.NM_PAIS "&_
										  " FROM TPROCESSO_EXP TPE (NOLOCK) "&_
										  " INNER JOIN BROKER.DBO.TTRANSPORTADOR_ITL TEE  (NOLOCK) ON TEE.CODIGO = TPE.CD_CONSIGNATARIO"&_
                                          " INNER JOIN TPAIS_BROKER                  TPEE (NOLOCK) ON TPEE.CD_PAIS = TEE.CD_PAIS "&_
										  " WHERE TPE.NR_PROCESSO ='"& nr_processo & "'"
									objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
								...
								If tp_consignatario = 4 Then 'Despachante
									sql = "SELECT TDE.NM_DESPACHANTE AS NOME, '' AS EMPRESA_END, '' END_CIDADE, '' NM_PAIS"&_
									      " FROM TPROCESSO_EXP      TPE (NOLOCK) "&_
									      " INNER JOIN TDESPACHANTE TDE (NOLOCK) ON TDE.CD_DESPACHANTE = TPE.CD_CONSIGNATARIO"&_
									      " WHERE TPE.NR_PROCESSO ='"& nr_processo & "'"
									objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
...
'									End If
			...
			Case "0758"
			If objrsx.State = adStateOpen Then objrsx.Close()
					sql = "SELECT TX_OBSERVACAO FROM TFOLLOWUP_OBS (NOLOCK) "&_
							"WHERE NR_PROCESSO= '"& nr_processo &"' "
					objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText	
					If objrsx.RecordCount > 0 Then
...
<%			Case "0800" ' Qtde Volumes
			  ...
			  If objrsx.state = adstateOpen then objrsx.close()
						sql = "SELECT COUNT(PV.CD_VOLUME) AS VOLUME"&_ 
								  " FROM TRAP_PACK_VOLUME PV( NOLOCK )"&_
								  " INNER JOIN TRAP       TR (NOLOCK ) ON (TR.CD_RAP+TR.CD_RAP_ANO = PV.CD_RAP+PV.CD_RAP_ANO)"&_
								  " WHERE TR.NR_PROCESSO ='"& nr_processo & "'"
					  Objrsx.Open sql, objcnn, adOpenStatic, adlockReadOnly, adcmdtext
...
<%			Case "2339" ' Nr. Chave de acesso DUE
			  ...
			  If objrsx.state = adstateOpen then objrsx.close()
					  ...
					  str= objrsx.fields.item("CHAVE_DUE").value
...
<%			Case "0182","0485" ' Analista Cliente 23/05/2008
			...
			Else
			  If objrsx.state = adstateOpen then objrsx.close()
					SQL=" select tp.cd_contato,tct.nm_contato as CONTATO,tp.cd_cliente "&_
					    "   from tprocesso             tp  (NOLOCK)"&_
...
<%   					Case Else 'FLAVIO - MONTANDO O RELATÓRIO (TRAZENDO OS DADOS)
									Select Case objrsv.Fields.Item("M10_TP_DADO").Value
										...
										Case "2" 'exibe os campos com valor decimal
											Response.Write(fncExibeNumeroPadrao(0, objrs.Fields.Item(nm_campo_rel).Value, 6))
