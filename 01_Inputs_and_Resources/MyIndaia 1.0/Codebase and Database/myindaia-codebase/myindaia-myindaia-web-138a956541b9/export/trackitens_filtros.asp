...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" class="table" >
  ...
  <img src="/imagens/30/barra_tracking<%=barra_tracking %>.gif"  width="565" border="0"><br><br>                       
    ...
    <tr align="left">                    
     <%if tp_relat = "0" then%>
	 <td valign="top">&nbsp;&nbsp;&nbsp;&nbsp;- <%=tx_lang_004066(cd_lang)%>;</td>
     <%else%>
     <%if tp_relat = "1" then%>
     <td valign="top">&nbsp;&nbsp;&nbsp;&nbsp;- <%=tx_lang_004067(cd_lang)%>;</td><%else%>  
      <%if tp_relat = "3" then%>
     <td valign="top">&nbsp;&nbsp;&nbsp;&nbsp;- <%=tx_lang_004082(cd_lang)%>;</td> 
     <%end if%> 
     <%end if%>     
      <%end if%>  
      </tr>                      
...
%>
							...
							<%
		Case "0005","0006","0007","0008","0661","0594","0471" ',"0126" 'Selects - Grupo, Clientes, Área e Sub-área
%>
							<td align="right"><input type="checkbox" name="filtro" id="<%= objrsv("M10_AP_FILTRO")%>X" value="<%= objrsv("M10_AP_FILTRO")%>" onClick="clickCheck(this)"<%= checked%>></td>
							<td width="320px"><nobr><input type="text" style="display:none" value="<%= objrsv("M10_NM_DADO")%>" name="nome_<%= objrsv("M10_AP_FILTRO")%>%>" id="nome_<%= objrsv("M10_AP_FILTRO")%>"><label for="<%= objrsv("M10_AP_FILTRO")%>X" class="label"><%= objrsv("M10_NM_DADO")%>&nbsp;</label></nobr></td>
                            <td><select class="select" style="width:90px;" name="combo_aux_<%= objrsv("M10_AP_FILTRO")%>" onFocus="enterInput(this);" id="combo_aux_<%= objrsv("M10_AP_FILTRO")%>" onChange="fnc_comboEdt('<%= objrsv("M10_AP_FILTRO")%>',this);">
...
%>
			...
			Else
				...
				Select Case objrsv.Fields.Item("M10_CD_DADO").Value
					...
					Case "0006","0661" %><!--#include virtual="/selects/option_cliente.txt"--><%'Cliente
					...
					Case "0126" Server.Execute("/selects/option_importador"&grupo&".txt") 'IMPORTADOR - ALTERAÇÃO GREGORY	
...
%>
                                ...
                                <td align="left" width="17"><%If objrsv.Fields.Item("M11_IN_VARIOS").Value Then%><a href="#" onClick="criaCombo('<%= CStr(vl_width_form)%>','<%= objrsv("M10_AP_FILTRO")%>', document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'));"><img id="img_<%= objrsv("M10_AP_FILTRO")%>" src='/imagens/30/btn_mais.gif' style='border:0;display:none;' class='top' alt='<%=tx_lang_TF017(cd_lang)%>' disabled="true"></a><img align="left" id="img_<%= objrsv("M10_AP_FILTRO")%>2" src='/imagens/30/btn_mais.gif' style='border:0;visibility:hidden' class='top'><input type="text" style="display:none" value="0" name="contador_<%= objrsv("M10_AP_FILTRO")%>" id="contador_<%= objrsv("M10_AP_FILTRO")%>"><input type="text" style="display:none" value="0" name="contador_total_<%= objrsv("M10_AP_FILTRO")%>" id="contador_total_<%= objrsv("M10_AP_FILTRO")%>"><%End If%></td>
							<%
		Case "0009","0011","0012","0013","0017","0044","0058","0068","0085","0126","0132","0133","0136","0576","0654","0768","0854" 'Outros Selects
%>
							<td align="right"><input type="checkbox" name="filtro" id="<%= objrsv("M10_AP_FILTRO")%>X" value="<%= objrsv("M10_AP_FILTRO")%>" onClick="clickCheck(this)"<%= checked%>></td>
...
<option value="1"><%=tx_lang_TF002(cd_lang)%></option>
			...
			Select Case objrsv.Fields.Item("M10_CD_DADO").Value
				...
				case "0654" %><!--#include virtual="/selects/option_continentes.txt"--><%'CONTINENTE
				...
				case "0126" 
  			        If Session("cargo_interno") = "1" Then
					   Server.Execute("/selects/option_exp_importador.txt")
					else	
					If objrsx.State = adStateOpen Then objrsx.Close()			
						...
						Objrsx.Open sql, objcnn,adOpenStatic,adLockReadOnly,adCmdText
							...
							If objrsx.Eof Then Exit Do Else Server.Execute("/selects/option_exp_importador"&str&".txt") 'IMPORTADOR - ALTERAÇÃO GREGORY	
							objrsx.MoveNext
							Loop
					end if				
				...
				case "0768"
				If objrsx.State = adStateOpen Then objrsx.Close()
				sql = " SELECT CD_CARRETA AS CODIGO, NM_CARRETA AS DESCRICAO FROM TTP_CARRETA "
		 		objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
				'Preenche o Select com os dados pesquisados na tabela
				Do While Not objrsx.Eof
%>
...
<option value="1"><%=tx_lang_TF002(cd_lang)%></option>
							...
							<td><nobr><input type="text" style="display:none" value="combo" name="controle_<%= objrsv("M10_AP_FILTRO")%>" id="controle_<%= objrsv("M10_AP_FILTRO")%>"><select class="select" style="width: 75px;" name="<%= objrsv("M10_AP_FILTRO")%>" onFocus="enterInput(this);" <%If objrsv.Fields.Item("M11_IN_VARIOS").Value Then%>onKeyPress="if(window.event.shiftKey && event.keyCode == 45){return false;}if(window.event.shiftKey && event.keyCode == 43){criaCombo('<%= CStr(vl_width_form)%>','<%= objrsv("M10_AP_FILTRO")%>', document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'));return false;}" <%End If%> onBlur="escreveValor('<%= objrsv("M10_AP_FILTRO")%>',document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'),3)">
...
%>
							...
							<td><nobr>
								<input class="editbox" type="text" name="<%= objrsv("M10_AP_FILTRO")%>i" style="width: 70px;" maxlength="10" value="<%= fnc_Mascara(dt_eventoi, 5)%>" onFocus="enterInput(this);" onBlur="escreveValor('<%= objrsv("M10_AP_FILTRO")%>',document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'),4)">
								...
								<input class="editbox" type="text" name="<%= objrsv("M10_AP_FILTRO")%>f" style="width: 70px;" maxlength="10" value="<%= fnc_Mascara(dt_eventof, 5)%>" onFocus="enterInput(this);" onBlur="escreveValor('<%= objrsv("M10_AP_FILTRO")%>',document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'),5)">
...
%>
						...
						"  M10_CD_DADO, RTRIM("& tx_lang_dado &") AS M10_NM_DADO, CASE WHEN M10_TP_DADO  = '1' AND M10_CD_BROKER IS NOT NULL THEN 'RLZ_' + RTRIM(M10_AP_CAMPO) ELSE CASE WHEN M10_CD_DADO IN ('0003') THEN 'R.' + UPPER(RTRIM(M10_AP_CAMPO)) ELSE UPPER(RTRIM(M10_AP_CAMPO)) END END AS M10_AP_CAMPO, M10_TP_DADO, "&_
						"  M10_TP_CONSULTA, M10_NM_TABELA, M10_NM_CHAVE, M10_NM_DESCRICAO, M10_CD_BROKER, " &_
						...
						"  AND M12_IN_ATIVO     = 1 " &_
						"  AND M10_IN_ATIVO     = 1 " &_
						"  AND (M12_IN_MONTA_SQL = 1 OR M10_CD_DADO IN ('0003'))" &_
						"  AND M10_CD_DADO NOT IN ('"& tx_nao_exibe &"') "&_
