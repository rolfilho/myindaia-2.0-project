...
'Response.write Session("sql_alerta_exp_track")
'Response.end
...
If objrs.RecordCount > 0 and Request("vTxBusca") <> "" Then
  ...
  End If
Else
  if nr_ident <> "" Then objrs.AbsolutePage = CInt(pagina)
End If
...
tx_path_excel = "/export/alerta_excel"

'retorna para o primeiro registro do resultado
If nr_ident <> "" Then If objrs.Eof  Then objrs.MoveFirst
...
<!--
			function MM_jumpMenu(targ,selObj,restore){ //v3.0
				var target = "alerta_result.asp<%= url%>&p="+selObj.options[selObj.selectedIndex].value;
...
<Table id="TabelaPrincipal" width="100%" border="0" cellspacing="0" cellpadding="0" valign="middle" style="padding: 0px 1px 0px 1px;" bordercolor="#FFFFFF"  bgcolor="#336699">
<%
...
Do While Not objrsv.Eof
	...
	select case objrsv.Fields.Item("M10_CD_DADO").Value
		case "0118"
			...
			vTxLinha(1) = vTxLinha(1) &"<th style='background-color:" & cabec_result & ";'>"&tx_lang_A00095(cd_lang)&"</td><th style='background-color:" & cabec_result & ";'>"&tx_lang_A00096(cd_lang)&"</td><th style='background-color:" & cabec_result & ";'>"&tx_lang_A00097(cd_lang)&"</td><th style='background-color:" & cabec_result & ";'>"&tx_lang_A00098(cd_lang)&"</td>"
			vInRowspan = true
			nr_cols = nr_cols + 4
		case else
			select case objrsv.Fields.Item("M10_TP_DADO").Value
				case "1" 'datas
					...
					if fnc_TestaVar(objrsv.Fields.Item("M10_CD_BROKER").Value) and in_dt_prev then
						vTxLinha(0) = vTxLinha(0) &"<th colspan='4' style='background-color:" & cabec_result & ";'>"&objrsv.Fields.Item("M10_NM_DADO").Value&"</th>"
...
<%                      Case "0669"				
						...
						'===================================================================	
						Case "0018","0094","0096","0097","0098","0099","0100","0101","0374","0486","0780","0899","0920","1051","1066","2331","2332","1067","1087","2017","2018","2024","2027","2029","2142","2157","2158","2159","2317","2336","2261","2396","2400","2402","2408" 'Nr.Referencia - Item
									If objrsx.State = adStateOpen Then objrsx.Close()
...
%>
                ...
                <%			
                        ...
                        Case "0863" 'peso_liq_unit
							If objrsx.State = adStateOpen Then objrsx.Close()
								sql = " SELECT SUM(ISNULL(VL_PESO_LIQ_UNIT, 0) * ISNULL(QT_MERCADORIA, 0)) AS VL_PESO "&_
								...
								str = ""
								If objrsx.RecordCount > 0 Then
                                  if fnc_TestaVar(objrsx.Fields.Item("VL_PESO").Value) Then    
                        ...
                        Case "0864" 'peso_liq_unit
							If objrsx.State = adStateOpen Then objrsx.Close()
								sql = " SELECT ISNULL(SUM(VL_MLE), 0) AS VL_MLE "&_
                                " FROM TPROCESSO_EXP_ITEM "&_ 
                                " WHERE NR_PROCESSO = '" & nr_processo & "' "&_
                                "  AND NR_PEDIDO = '" & nr_fatura_exp & "' "
								objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText	
								str = ""
								If objrsx.RecordCount > 0 Then
                                  if fnc_TestaVar(objrsx.Fields.Item("VL_MLE").Value) Then    
						...
						dim tp_notify1
								...
								If tp_notify1 = 3 Then 'Banco
									...
									If 	objrsx.RecordCount > 0 Then						
                                      ...
                                      If objrsv.Fields.Item("M10_CD_DADO").Value = "2383" Then str = Rtrim(objrsx.Fields.Item("EMPRESA_END").Value) End If
                                      If objrsv.Fields.Item("M10_CD_DADO").Value = "2384" Then str = Rtrim(objrsx.Fields.Item("END_CIDADE").Value) End If
                                      If objrsv.Fields.Item("M10_CD_DADO").Value = "2384" Then str = Rtrim(objrsx.Fields.Item("NM_PAIS").Value) End If
									End If	
								End If
			...
			<td align="center">&nbsp;<%= str%>&nbsp;</td>	
              ...
              If fnc_TestaVar(objrs.Fields.Item(campo_arq).Value) then
                str= "<a href='/popup/ver_arquivos.asp?cod=" & objrs.Fields.Item(campo_arq2).Value & "' target='_blank'><img src='/imagens/30/open.gif' border=0 alt='Ver Arquivo(s)'></a>"  
              Else
                str = ""
              End If
              %><td class="center"><nobr>&nbsp;<%=str%>&nbsp;</nobr></td><%
              If objrsx.State = adStateOpen Then objrsx.Close		
						'===================================================================	
...
<%   					Case Else 'FLAVIO - MONTANDO O RELATÓRIO (TRAZENDO OS DADOS)
									Select Case objrsv.Fields.Item("M10_TP_DADO").Value
										...
										Case "2" 'exibe os campos com valor decimal
											Response.Write(fncExibeNumeroPadrao(0, objrs.Fields.Item(nm_campo_rel).Value, 3))
