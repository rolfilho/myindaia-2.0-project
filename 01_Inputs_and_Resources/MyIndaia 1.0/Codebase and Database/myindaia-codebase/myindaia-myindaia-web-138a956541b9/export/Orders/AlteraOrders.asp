...
<%
if request("TX_PO_CUSTOMER")="" and request("vTpacao")<>"Duplicar"  then
...
Dim pagina,campo,vTpAcao,vTpacaoDuplica 'pagina inicial da ordenação
Dim LOADING_PLACE,PRODUCTION_PLANT,OLD_TX_PO_CUSTOMER
Dim CD_MATERIAL,DT_PRODUCTION,DT_RELEASE,NR_QTY_REAL,NR_QTY_PLANNED,NR_NET_WEIGTH,NR_GROSS_WEIGTH,NR_NET_VOLUME,NR_GROSS_VOLUME
...
OLD_TX_PO_CUSTOMER= Request("TX_PO_CUSTOMER")
...
If  request("vTpacao")="Alterar" or request("vTpacao")="Exibir" or request("vTpacao")="Incluir" or  request("vTpacao")="Gravar"  or  request("vTpacao")="Duplicar"  then
	sql= "SELECT TX_PO_CUSTOMER,NR_1ST_WEEK,NR_REVISED_WEEK,TX_ORDER_NUMBER,dt_order_issue,TX_PROFORM_INVOICE,TX_INVOICE,TX_DELIVERY_NUMBER, "&_
		 ...
		 " TX_MONTH_YEAR,DT_LOADING,CD_VESSEL,CD_EQUIP_TYPE,DT_ETD,NR_DEPARTURE_WEEK,IN_LOADED,DT_DEADLINE,DT_ETA,TX_MEMO "&_
		 " FROM TPEDIDO_VENDA_NESTLE WHERE TX_PO_CUSTOMER ='"&Replace(request("TX_PO_CUSTOMER"),"'","0")&"'"
		 objrs.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
		 'Response.Redirect("../../import/teste.asp?var1="&sql&"")
	If not objrs.eof then 'Registro encontrado
		'CheckCustomer=0 ' 
		If  request("vTpacao")="Incluir" or request("vTpacaoDuplica")="Duplicar" then
		    Response.Redirect("Listagem.asp?mensagem=Order já cadastrada!&TpExibir=1")
		End if
		If  request("vTpacao")="Exibir" or request("vTpacao")="Alterar" or request("vTpacao")="Duplicar"   then 'Se o registro existir seta as variáveis com o conteúdo do banco de dados
			TX_PO_CUSTOMER= objrs("TX_PO_CUSTOMER")
			OLD_TX_PO_CUSTOMER= objrs("TX_PO_CUSTOMER") 
	...
	Else ' registro não existe, seta as variáveis para inclusão na tabela
		...
		If  request("vTpacao")="Gravar" or request("vTpacaoDuplica")="Duplicar" then
			If  request("TX_PO_CUSTOMER")="" then ' Se for alteração e o registro não existe retorna a lista exibindo mensagem
		        Response.Redirect("AlteraOrders.asp?TX_PO_CUSTOMER="&Request("OLD_TX_PO_CUSTOMER")&"&mensagem=Digite um numero de Order!&vTpacao=Duplicar")
		    End if
		sql = "INSERT INTO TPEDIDO_VENDA_NESTLE (TX_PO_CUSTOMER,NR_1ST_WEEK,NR_REVISED_WEEK,TX_ORDER_NUMBER,DT_ORDER_ISSUE, "&_
			  ...
			  " CD_PRODUCTION_PLANT,CD_LOADING_PLACE,CD_CUSTOMER_PLANT,CD_CUSTOMER,CD_CONSIGNEE,CD_COUNTRY,TX_REGION,CD_PORT_OF_DESTINATION, "&_
			  " TX_MONTH_YEAR,DT_LOADING,CD_VESSEL,CD_EQUIP_TYPE,DT_ETD,IN_LOADED,DT_DEADLINE,DT_ETA,TX_MEMO) "&_
			  ...
			  " "&fncValSQL(CD_VESSEL,0)&","&fncValSQL(CD_EQUIP_TYPE,0)&","&fncValSQL(DT_ETD,1)&","&fncValSQL(IN_LOADED,0)&","&fncValSQL(DT_DEADLINE,1)&","&fncValSQL(DT_ETA,1)&","&fncValSQL(TX_MEMO,0)&")"
			  'Response.Redirect("../../import/teste.asp?var1="&sql&"")
			objcnn.Execute(sql)
			%><script>
				alert("Pedido Gravado!");
			</script><% 
			If request("vTpacaoDuplica")="Duplicar" then 'Rotina para gravar os itens da Po duplicada.
				if objrsv.State = adStateOpen then objrsv.Close
				sql="select * from tpedido_venda_item_nestle where TX_PO_CUSTOMER='"&Request("OLD_TX_PO_CUSTOMER")&"'"
				objrsv.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
				Do while not objrsv.eof 
					NR_ITEM_PO= objrsv("NR_ITEM_PO")
					CD_MATERIAL= objrsv("CD_MATERIAL")
					DT_PRODUCTION= objrsv("DT_PRODUCTION")
					DT_RELEASE= objrsv("DT_RELEASE")
					NR_QTY_REAL= objrsv("NR_QTY_REAL")
					NR_QTY_PLANNED= objrsv("NR_QTY_PLANNED")
					NR_NET_WEIGTH= objrsv("NR_NET_WEIGTH")
					NR_GROSS_WEIGTH= objrsv("NR_GROSS_WEIGTH")
					NR_NET_VOLUME= objrsv("NR_NET_VOLUME")
					NR_GROSS_VOLUME= objrsv("NR_GROSS_VOLUME")
				   sql="INSERT INTO  TPEDIDO_VENDA_ITEM_NESTLE (NR_ITEM_PO,TX_PO_CUSTOMER,CD_MATERIAL,DT_PRODUCTION,DT_RELEASE,NR_QTY_REAL,NR_QTY_PLANNED,NR_NET_WEIGTH,NR_GROSS_WEIGTH,NR_NET_VOLUME,NR_GROSS_VOLUME)" &_
						  " VALUES   ("&fncValSQL(NR_ITEM_PO,0)&","&fncValSQL(TX_PO_CUSTOMER,0)&","&fncValSQL(CD_MATERIAL,0)&","&fncValSQL(DT_PRODUCTION,1)&","&fncValSQL(DT_RELEASE,1)&","&NR_QTY_REAL&","&NR_QTY_PLANNED&","&NR_NET_WEIGTH&","&NR_GROSS_WEIGTH&","&NR_NET_VOLUME&","&NR_GROSS_VOLUME&")"
				   objcnn.Execute(sql)
				   	nrec = nrec + 1
					objrsv.MoveNext
			...
			End if
			Response.Redirect("AlteraOrders.asp?vTpacao=Exibir&TX_PO_CUSTOMER="&Request("TX_PO_CUSTOMER")&" &mensagem=PO/Customer:"&Replace(request("TX_PO_CUSTOMER"),"'","0")&" cadastrado com sucesso! Você já pode incluir ítens para este pedido!&TpExibir=1")
	   End if 
...
<table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="0" background="<%= fcn_TrocaFundo()%>" class="table">
  ...
  <tr>
    <td width="10" valign="top"><!-- :::::::::: INICIO DO MENU PRINCIPAL :::::::::: -->
      <!--#include virtual="/includes/lay_menuRap.asp" -->
      <!-- :::::::::: FIM DO MENU PRINCIPAL    :::::::::: -->    </td>
    <td width="100%" valign="top"><div id="dv_corpo" class="divMainMidBusca">
      <form action="AlteraOrders.asp" method="get" name="OrderForm" id="OrderForm" >
        <table width="95%" border="1" align="center" cellpadding="0" cellspacing="3" bordercolor="#CCCCCC">
          <tr>
            <!--Botao salvar para Inclus&atilde;o e Alterar para edi&ccedil;&atilde;o-->
            ...
            <th width="4%" bordercolor="#FFFFFF" scope="col"> <%If request("vTpacao")="Incluir" then%>
                <input type="image" name="btnSalvar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Incluir" value="Submit();" tabindex="29">
                <input name="vTpacao" type="hidden" id="vTpacao" value="Gravar">
                <input name="TX_PO_CUSTOMER" type="hidden" id="TX_PO_CUSTOMER" value="<%=request("TX_PO_CUSTOMER")%>">
                <%End if%>
                <%If request("vTpacao")="Exibir" or request("vTpacao")="Alterar" then%>
                <input type="image" name="btnAlterar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Alterar" value="Submit();" tabindex="29">
                <input name="vTpacao" type="hidden" id="vTpacao" value="GravaAlteracao">
                <input name="TX_PO_CUSTOMER" type="hidden" id="TX_PO_CUSTOMER" value="<%=request("TX_PO_CUSTOMER")%>">
                <%End if%>
                <%If request("vTpacao")="Duplicar" then%>
                <input type="image" name="btnAlterar" src="/imagens/icones/32x32/btn_salvar.gif" class="btnImagem" alt="Duplicar" value="Submit();" tabindex="29">
                <input name="vTpacao" type="hidden" id="vTpacao" value="Duplicar">
                <%End if%>            </th>
            ...
            <th width="33%" valign="top" bordercolor="#FFFFFF" scope="col"><div align="right">
                <p class="style6"><span class="style7">
                  <%if request("vTpacao")<>"Duplicar" then%>
                  PO Customer<br>
                  <span class="style8"><%=Replace(request("TX_PO_CUSTOMER"),"'","0")%></span></span></p>
              <%Else%>
                <div id="apDiv7">
                  <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                      <td><div align="center"><img src="../../imagens/icones/16x16/SetaAnimada.gif" width="14" height="14">
                              <input name="OLD_TX_PO_CUSTOMER" type="hidden" id="OLD_TX_PO_CUSTOMER" value="<%=REQUEST("TX_PO_CUSTOMER")%>">
                              <input name="vTpacaoDuplica" type="hidden" id="hiddenField" value="Duplicar">
                              <input name="TX_PO_CUSTOMER" type="text" class="editbox" id="TX_PO_CUSTOMER" size="15" maxlength="15" >
                              <input type="image" src="../../imagens/icones/16x16/Up.gif" alt="Duplicar" width="17" height="17" value="Submit();">
                      </div></td>
                    </tr>
                  </table>
        ...
        <br>
        <table width="95%" border="0" align="center" cellpadding="0" cellspacing="5" bordercolor="#CCCCCC" class="table">
          <td height="27"><table width="59%" border="0" cellpadding="0" cellspacing="0" bordercolor="0">
            ...
            <tr>
              ...
              <th width="72%" align="left" valign="top" scope="col"><div align="left">
                <select name="NR_REVISED_WEEK" id="NR_REVISED_WEEK" tabindex="2">
          ...
          <tr>
            <td bordercolor="#CCCCCC"><table width="99%" border="0" align="left" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
              ...
              <tr>
                <th width="33%" valign="top" bordercolor="#FFFFFF" scope="col"><div align="left"> <span class="c2">
                  <%
                ...
                <th bordercolor="#FFFFFF" scope="col" width="49%"> <div align="left">
                  <label>
                    <input type="radio" name="IN_ORDER_CONSOLIDATED" value="S" tabindex="9" id="RadioGroup1_2"  <%if IN_ORDER_CONSOLIDATED="S" then response.Write("checked") end if%>>
                    Sim</label>
                  <label>
                    <input type="radio" name="IN_ORDER_CONSOLIDATED" value="N" tabindex="9" id="RadioGroup1_3" <%if IN_ORDER_CONSOLIDATED="N" then response.Write("checked") end if%>>
                    N&atilde;o</label>
                </div></th>
              </tr>
            </table></td>
          ...
          <tr>
            <td bordercolor="#CCCCCC"><table width="99%" border="0" align="left" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
			  ...
			  Case "1211"
			      PRODUCTION_PLANT="Ca&ccedil;apava"
			  Case "1215"
			...
			%>
                  <select name="CD_PRODUCTION_PLANT" id="CD_PRODUCTION_PLANT">
                    ...
                    <%if PRODUCTION_PLANT<>"Aracatuba" then%>
                    <option value="0460">Aracatuba </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Araraquara" then%>
                    <option value="1209">Araraquara </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Araras" then%>
                    <option value="1210">Araras </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Cacapava" then%>
                    <option value="1211">Cacapava </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Ibia" then%>
                    <option value="1215">Ibia </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Ituiutaba" then%>
                    <option value="1217">Ituiutaba </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Marilia" then%>
                    <option value="1221">Marilia </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Rio Pardo" then%>
                    <option value="1228">Rio Pardo </option>
                    <%end if%>
                    <%if PRODUCTION_PLANT<>"Montes Claros" then%>
                    <option value="1238">Montes Claros </option>
                    <%end if%>
                  </select>
                </div>
                  </label></th>
			  ...
			  Case "1202"
			      LOADING_PLACE="S&atilde;o Bernardo"
			  Case "1208"
			      LOADING_PLACE="Ibia"
			  Case "1246"
			...
			%>
                  <select name="CD_LOADING_PLACE" id="CD_LOADING_PLACE">
                    ...
                    <%end if%>
                    <%if LOADING_PLACE<>"S&atilde;o Bernardo" then%>
                    <option value="1202">S&atilde;o Bernardo</option>
                    <%end if%>
...
%>
          ...
          <tr>
            <td bordercolor="#FFFFFF"><table width="99%" border="0" align="left" cellpadding="0" cellspacing="0">
              <tr>
                <th width="46%" bordercolor="#FFFFFF" scope="col"><div align="left">Vessel  / Carrier</div></th>
                <th width="5%" height="12" bordercolor="#FFFFFF" scope="col">&nbsp;</th>
                <th width="49%" bordercolor="#FFFFFF" scope="col"><div align="left">Equipment type</div></th>
              </tr>
              <tr>
                <th bordercolor="#FFFFFF" scope="col" width="46%"><div align="left">
...
%>
                ...
                <th height="20" bordercolor="#FFFFFF" scope="col" width="5%"><label></label></th>
                <th bordercolor="#FFFFFF" scope="col" width="49%"> <div align="left">
...
%>
          ...
          <tr>
            <td bordercolor="#FFFFFF"><table width="99%" border="0" align="left" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <th width="33%" valign="top" bordercolor="#FFFFFF" scope="col"><div align="left">
                  <label>
                    <input type="radio" name="IN_LOADED" value="S" tabindex="24"  id="RadioGroup1_4" <%if IN_LOADED="S" then response.Write("checked") end if%>>
                    Sim </label>
                  <label>
                    <input type="radio" name="IN_LOADED" value="N" id="RadioGroup1_5" <%if IN_LOADED="N" then response.Write("checked") end if%>>
                    N&atilde;o </label>
                </div></th>
                <th height="20" bordercolor="#FFFFFF" scope="col" width="18%"><label></label></th>
                <th width="49%" valign="top" bordercolor="#FFFFFF" scope="col"> <div align="left">
          ...
          <tr>
            <td height="162" valign="top" bordercolor="#FFFFFF"><table width="99%" border="0" align="left" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC">
              ...
              <tr>
                <th height="146" valign="top" bordercolor="#FFFFFF" scope="col"><label></label>
                  <div align="left">
                    <p>
...
<%End if%>
    ...
    <!-- :::::::::: INICIO DO CORPO CENTRAL :::::::::: -->      <!-- :::::::::: FIM DO CORPO CENTRAL    :::::::::: --></td>
  </tr>
...
function PreencheForm(vTpAcao) {
    ...
    if ( '<%=request("vTpacao")%>' =='Duplicar')                  	  document.OrderForm.TX_PO_CUSTOMER.focus();
    if ( '<%=request("TX_PO_CUSTOMER")%>' =='')                  	  document.OrderForm.TX_PO_CUSTOMER.focus();
    if ( '<%=request("CD_CUSTOMER")%>' !='')                  	      document.OrderForm.CD_CUSTOMER.focus();
