...
'Arquivo de Script e HTML: Busca de processos utilizando os filtros
'
...
sql = "SELECT M10_CD_DADO, RTRIM(M10_NM_DADO) AS M10_NM_DADO, M10_AP_FILTRO, M10_CD_BROKER, M11_TP_FROM "&_
...
If tx_erro <> "" Then
  ...
  tx_erro = "ATENÇÃO: os seguintes filtros foram selecionados e deixados em branco.\n"& tx_erro
  Response.Redirect("kpi_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If
...
If in_dt_null or fnc_TestaVar(cd_evento) Then
  ...
  If Not fnc_TestaVar(cd_evento) Then cd_evento = "132"
	...
	If Not fnc_TestaVar(nr_dias) Then nr_dias = DatePart("d", Date)
...
If nr_result > 0 Then
  ...
  str = "/export/kpi_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = "Não foram encontrados registros com os seguintes filtros:\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/export/kpi_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
