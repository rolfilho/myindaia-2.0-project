...
<%
'---------------------------------------------------------------------------
'Sistema MyIndaiá WEB - Exportação / Alerta
		...
		<td width="100%" valign="top"><!-- :::::::::: INICIO DO CORPO CENTRAL :::::::::: -->
			<div id="dv_corpo" class="divMainTop">
				<table width="<%= CStr(vl_width)%>" border="0" align="center" cellpadding="0" cellspacing="0">
					<form name="frmbusca" method="post" action="/export/alerta_busca.asp" onSubmit="return validSubmit()">
						<tr>
							<td height="50" <%If cd_lang = 0 Then%><%Else%><%End If%>colspan="<%= CStr(vl_colspan) + 2%>" align="left"><font size="2"><br><b><%= tx_lang_A00020(cd_lang)%></b></font><%If cd_lang = 0 or cd_lang = 2 Then%><% If Session("cd_cargo") <> "088" Then%><span style="margin-left:20px"><%Else%><span style="margin-left:150px"><%End If%><%Else%><span style="margin-left:100px"><%End If%><font color="#B4C7DB"><b><a href="/personaliza/cad_view.asp?in_acao=editar&cd_menu=<%= cd_menu%>&cd_subm=<%= cd_subm%>&cd_view=<%= cd_view%>"><font color="#B4C7DB"><%= tx_lang_004004(cd_lang)%></font></a> / <a href="#" onClick="javascript:ExibeDetalhes('divInfo','divInfoCorpo', 'divInfoRodape',2);"><font color="#B4C7DB"><%= tx_lang_TF001(cd_lang)%></font> </a>/ <a href="#" onClick="javascript:carregaPesquisa('<%=cd_view %>','alerta_filtros');"><font color="#B4C7DB"><%= tx_lang_TF043(cd_lang)%></font></a><% If Session("cd_cargo") <> "088" Then%> / <a href="#" onClick="javascript:ExibeDetalhesCR();"><font color="#B4C7DB"><%=tx_lang_TF044(cd_lang)%></font></a><%End If%></b></font></span><br>
  <img src="/imagens/30/barra_tracking.gif"  width="565" border="0"><br><br>                       
								</td>
						</tr>
                        <%
	  sql_campos = "SELECT " &_
...
%>
              ...
              <td><label for="<%= objrsv("M10_AP_FILTRO")%>X" class="label"><%= objrsv("M10_NM_DADO")%></label></td>
                  ...
                  <option value="02" selected><%= tx_lang_A00011(cd_lang)%></option>
                </select></td>
              <%
...
%>
                                ...
                                <td align="left" width="17"><%If objrsv.Fields.Item("M11_IN_VARIOS").Value Then%><a href="#" onClick="criaCombo('<%= CStr(vl_width_form)%>','<%= objrsv("M10_AP_FILTRO")%>', document.getElementById('combo_aux_<%= objrsv("M10_AP_FILTRO")%>'));"><img id="img_<%= objrsv("M10_AP_FILTRO")%>" src='/imagens/icones/16x16/btn_mais20.gif' style='border:0;display:none;' class='top' alt='<%=tx_lang_TF017(cd_lang)%>' disabled="true"></a><img align="left" id="img_<%= objrsv("M10_AP_FILTRO")%>2" src='/imagens/icones/16x16/btn_mais20.gif' style='border:0;visibility:hidden' class='top'><input type="text" style="display:none" value="0" name="contador_<%= objrsv("M10_AP_FILTRO")%>" id="contador_<%= objrsv("M10_AP_FILTRO")%>"><input type="text" style="display:none" value="0" name="contador_total_<%= objrsv("M10_AP_FILTRO")%>" id="contador_total_<%= objrsv("M10_AP_FILTRO")%>"><%End If%></td>
							<%
