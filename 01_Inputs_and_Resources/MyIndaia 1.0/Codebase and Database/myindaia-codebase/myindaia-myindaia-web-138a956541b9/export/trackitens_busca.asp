...
Response.Redirect("trackitens_filtros.asp?cd_menu="& cd_menu & "&cd_subm="& cd_subm& "&cd_view="& cd_view & "&in_pesquisa=1")
End If
...
Do While Not objrsx.Eof
	...
	If objrsx.Fields.Item("M11_TP_FROM").Value = "0" Then
		If (fnc_TestaVar(Request(ap_filtro)) and flag_dif <> 1) or (flag_dif = 1)  Then
			If InStr("0548", objrsx.Fields.Item("M11_CD_DADO").Value) > 0 and (Request(combo_aux) <> 2) Then
				str_1 = ""
				str = ""
				...
				For cont = 1 To len(Trim(Request(ap_filtro)))
					If IsNumeric(Mid(Trim(Request(ap_filtro)), cont, 1)) Then str_1 = str_1 & Mid(Trim(Request(ap_filtro)), cont, 1)
				Next
				If Len(str_1) < 8 then str_1 = fnc_AddZeros(str_1, 8)
				tx_filtros = tx_filtros & str_1
					
				For cont_filtro = 0 To total_filtro-1
				If not IsEmpty(Request(ap_filtro & "_" & cont_filtro)) Then
					For cont = 1 To len(Trim(Request(ap_filtro &"_"& cont_filtro)))
					  If IsNumeric(Mid(Trim(Request(ap_filtro &"_"& cont_filtro)), cont, 1)) then str = str & Mid(Trim(Request(ap_filtro &"_"& cont_filtro)), cont, 1)
					Next
					If Len(str) < 8 Then str = fnc_AddZeros(str, 8)
					tx_filtros_varios = "*" & str
...
If tx_erro <> "" Then
  ...
  Response.Redirect("trackitens_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If
...
If nr_result > 0 Then
  ...
  str = "/export/trackitens_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = tx_lang_A00002(cd_lang) &": "& tx_lang_A00029(cd_lang) &".\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/export/trackitens_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
