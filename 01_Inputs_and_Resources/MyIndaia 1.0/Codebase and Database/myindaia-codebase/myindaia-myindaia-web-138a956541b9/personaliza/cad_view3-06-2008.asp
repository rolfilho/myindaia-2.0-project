...
sql = " SELECT  TP.CD_USUARIO, TP.CD_CLIENTE, TE.CD_EMPRESA , TE.CD_GRUPO, TG.CD_GRUPO, TG.IN_EXIBE_PEDIDO_SITE AS PERMISSO" &_
...
If ex_relat Then
 Select Case cd_menu&cd_subm
   Case "0201"
		...
		'tp_exibir_Export=tp_relat
              ...
              <td>&nbsp;&nbsp; <% tp_exibir_Export=tp_relat
								...
								<input name="tp_relat" type="radio"  onClick="javascript:submit();" value="0" id="embarque"<%If tp_relat = "0" Then Response.Write(" checked")%>>
                <label for="embarque"><%= tx_lang_004066(cd_lang)%> <%'=tx_lang_004066(cd_lang)%></label>
								<input name="tp_relat" type="radio" onClick="javascript:submit();" value="1" id="pedido"<%If tp_relat = "1" Then Response.Write(" checked")%>>
   ...
   Case "0101" %>
            ...
            <tr>
              <td align="right" class="label"><%= tx_lang_004031(cd_lang)%></td>
              <td>&nbsp;&nbsp; <%tp_exibir_Export=2 'Usuário não tem opção de exibição - mostra todos os campos%>
...
'Response.Redirect("../import/teste.asp?var1="&tp_exibir_Export&"")
	If objrs.State = 1 Then objrs.Close
	sql_s = "SELECT M12_CD_CAMPO, RTRIM("& tx_lang_dado &") AS M10_NM_DADO, M10_TP_DADO, M10_TP_CONSULTA, M10_CD_BROKER "&_
				"FROM M12_CAMPO ( NOLOCK ), M10_DADOS ( NOLOCK ) "&_
				"WHERE M12_CD_DADO    = M10_CD_DADO "&_
				"  AND M12_CD_MENU    = "& fnc_QuotedSQL(cd_menu) &_
				"  AND M12_CD_SUBM    = "& fnc_QuotedSQL(cd_subm) &_
...
%>
                    ...
                    <td><input type="button" value=">>" class="button" onClick="combo_altera(document.frmview.campo,document.frmview.array_campo,true);" name="button1">
                      ...
                      <input type="button" value="<<" class="button" onClick="combo_altera(document.frmview.array_campo,document.frmview.campo,true);" name="button2">
											...
											<br>
											<br>
											<span style="color:#999999;"><%=tp_exibir_Export%><%= tx_lang_004055(cd_lang)%><br>
