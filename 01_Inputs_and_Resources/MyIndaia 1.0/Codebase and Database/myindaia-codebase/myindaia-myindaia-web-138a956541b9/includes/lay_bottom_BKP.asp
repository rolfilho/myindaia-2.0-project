...
setTimeout("reloadPagina();ExibeDicas_alert();",100);
...
function ExibeDetalhes_alert() {
	if (document.getElementById) {
		...
		ajax.open("POST","/detalhe_div.asp?mostra_pendencia=0", true); 
...
function ExibePendencias_alert(){
	if (document.getElementById) {
		...
		var ajax = openAjax();
		//usado o GET para que se possa usar filtros querystring
		ajax.open("POST","/detalhe_div.asp?mostra_pendencia=1", true); 
		ajax.onreadystatechange = function() {
			...
			if(ajax.readyState == 4) { //agora exibe o que interessa
				if(ajax.status == 200) {
					var vResult = ajax.responseText; //resultado da busca
					if (vResult == 'ERRO'){
						FechaPendencias('divInfoCorpoAlert','divInfoRodapeAlert','divInfoAlert','divFrameAlert');
					}else{	
						vResult = vResult.replace(/\+/g," "); //resolve problemas de acentos
						vResult = unescape(vResult); //continua resolvendo problemas de acento
						vCorpo.innerHTML = vResult;
...
function ExibeHistorico_alert(pendencia,processo){
	var x = screen.width;
	var y = screen.height;
	var w = 760;
	var z = 360;
	...
	var ajax = openAjax();
		//usado o GET para que se possa usar filtros querystring
		ajax.open("POST","/detalhe_div.asp?mostra_pendencia=2&cd_pendencia="+pendencia+"&nr_processo="+processo, true); 
		ajax.onreadystatechange = function() {
			if(ajax.readyState == 1) { //enquanto estiver processando exibe mensagem para acalmar
...
function ExibeRep_alert(pendencia,processo){
	var x = screen.width;
	var y = screen.height;
	var w = 350;
	var z = 90;
	...
	var ajax = openAjax();
		//usado o GET para que se possa usar filtros querystring
		ajax.open("POST","/detalhe_div.asp?mostra_pendencia=4&cd_pendencia="+pendencia+"&nr_processo="+processo, true); 
		ajax.onreadystatechange = function() {
			if(ajax.readyState == 1) { //enquanto estiver processando exibe mensagem para acalmar
...
function EnviaMensagem_alert(objeto, pendencia, processo){
	var texto = objeto.value;
	//texto = texto.replace(/\n{1}/gi,"|");
	if (document.getElementById) {
		var ajax = openAjax();
		ajax.open("POST","/detalhe_div.asp?mostra_pendencia=3&nm_texto="+escape(texto)+"&cd_pendencia="+pendencia+"&nr_processo="+processo, true);
		ajax.onreadystatechange = function() {
			if(ajax.readyState == 4) { //agora exibe o que interessa
				if(ajax.status == 200) {
					var vResult = ajax.responseText; //resultado da busca
	...
	ExibeHistorico_alert(pendencia,processo);
}

function RepassaPendencia_alert(objeto, nome, pendencia, processo){
	var contato = objeto.value;
	if (document.getElementById) {
		var ajax = openAjax();
		ajax.open("POST","/detalhe_div.asp?mostra_pendencia=5&cd_contato="+contato+"&nm_contato="+nome+"&cd_pendencia="+pendencia+"&nr_processo="+processo, true);
		ajax.onreadystatechange = function() {
			if(ajax.readyState == 4) { //agora exibe o que interessa
				if(ajax.status == 200) {
					var vResult = ajax.responseText; //resultado da busca
...
#divDicas {
	background-color:#f0f1f6;
	background-image:url(../imagens/fundo_dicas.gif);
	width:210px;
	height:200px;
	position:absolute;
	bottom:30px;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr height="16" bgcolor="#f0f1f6">
		<td style="FILTER: progid:DXImageTransform.Microsoft.Gradient(startColorStr='#f0f1f6', endColorStr='#f0f1f6', gradientType='0');"><table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="40"><p style="color: #BBBBBB;">
					<%If Session("login_valido") Then%>
                    <iframe id="divFrameAlert" src="" style="visibility: hidden; position: absolute; z-index:0;"></iframe>
...
<td width="2%" align="left" ><img id="imgAlert" src="/imagens/icones/16x16/btn_fechar20.gif" width="18" height="17" border="0" alt="<%= tx_lang_POP060(cd_lang)%>" onclick="FechaPendencias('divInfoCorpoAlert','divInfoRodapeAlert','divInfoAlert','divFrameAlert');" style="cursor:pointer;">&nbsp;</td>
</tr>
</table>
 </div>
...
<table width="100%" height="26" border="0" cellpadding="0" cellspacing="0" bgcolor="#f0f1f6" >
<tr>
<td width="98%" align="center" valign="middle">&nbsp;&nbsp;&nbsp;<font style="color:#0066CC;font-size:18px;font-weight:bold;">Você Sabia?</font><br /></td>
<td width="2%" align="left" ><img id="imgDicas" src="/imagens/icones/16x16/btn_fechar21.gif" width="18" height="17" border="0" alt="<%= tx_lang_POP060(cd_lang)%>" onclick="FechaDetalhes_alert();" style="cursor:pointer;">&nbsp;</td>
</tr>
</table> 
 </div>
 <div id="divDicasCorpo" style="color:#000000;font-weight:normal;"></div>
</div>
                    <div id="divAlert"><a href="#" onclick="ExibePendencias_alert();"><div id="divAlertCorpo"></div></a>
                   </div>
                   <iframe id="divFrameRep" src="" style="visibility: hidden; position: absolute; z-index:0;"></iframe>
                   <div id="divRep"><div id="divRepBotao"><img src="/imagens/icones/16x16/btn_fechar22.gif" width="18" height="17" border="0" alt="<%= tx_lang_POP060(cd_lang)%>" align="right" onclick="FechaRep_alert();" style="cursor:pointer;"></div><div id="divRepCorpo"></div></div>
                   <iframe id="divFrameHistorico" src="" style="visibility: hidden; position: absolute; z-index:0;"></iframe>
                   <div id="divHistorico"><div id="divHistoricoBotao"><img src="/imagens/icones/16x16/btn_fechar211.gif" width="36" height="17" border="0" alt="<%= tx_lang_POP098(cd_lang)%>" align="right" onclick="FechaHistorico_alert();" style="cursor:pointer;"></div><div id="divHistoricoCorpo"></div></div>
					<%End If%></p></td>
					<td><p align="center">&nbsp;&nbsp;&copy;<%= Year(Date())%> MyIndai&aacute; - <b><a href="http://www.myindaia.com.br/" target="_blank" >Indai&aacute; Log&iacute;stica Internacional</a></b></p></td>
                    <td align="right" width="100">
                <% If flag_autentica <> 1 then%>
				<a href="<%= tx_url_lang &"0"%>" target="_self" onclick="mudaIdioma(0);"><img src="/imagens/icones/16x16/flag_brazil.gif" alt="português-br" width="23" height="14" border="0" align="absmiddle"/></a> <a href="<%= tx_url_lang &"1"%>" target="_self" onclick="mudaIdioma(1);"><img src="/imagens/icones/16x16/flag_uk.gif" alt="english" width="24" height="14" border="0" align="absmiddle"/></a> <a href="<%= tx_url_lang &"2"%>" target="_self" onclick="mudaIdioma(2);"><img src="/imagens/icones/16x16/flag_spain.gif" alt="español" width="23" height="14" border="0" align="absmiddle"/></a>&nbsp;&nbsp;
                <%End if%>
                </td>
				</tr>
			</table></td>
