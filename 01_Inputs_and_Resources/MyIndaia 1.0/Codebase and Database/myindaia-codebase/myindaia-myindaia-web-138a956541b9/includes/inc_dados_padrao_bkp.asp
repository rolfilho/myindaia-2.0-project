...
function fncExibeDataEventoFup(nExibir, nInData, nDataPrev, nDataReal)
	...
	if not fnc_TestaVar(nInData) or (nInData = 0) then
		...
		if IsDate(nDataPrev) then
			if IsDate(nDataReal) then
				vNrKPI = fnc_DiasUteisKPI(nDataPrev, nDataReal, "0") 
			else
				vNrKPI = fnc_DiasUteisKPI(nDataPrev, Date(), "0")
			end if
			'muda a cor do lead-time conforme o resultado
			if vNrKPI < 0 then 
				color = "#0000FF" 'azul
			elseif vNrKPI > 0 then
	...
	select case nExibir
		case 0
			vReturn = "<td><nobr><font color='"&color&"'>"&fnc_Mascara(nDataReal, 5)&"&nbsp;</font></nobr></td>"
		...
		case 1
			vReturn = "<td class='pgridsub'><nobr><font color='"&color&"'>"&fnc_Mascara(nDataReal, 5)&"&nbsp;</font></nobr></td>"
			'verifica se exibe a data prevista
			if in_dt_prev then
				vReturn = "<td class='pgridsub'><nobr>"&fnc_Mascara(nDataPrev, 5)&"&nbsp;</nobr></td>"&_
...
function fncExibeNumeroPadrao(nExibir, nNumero, nDecimal)
	...
	if IsNumeric(nNumero) then nNumero = FormatNumber(nNumero, nDecimal) else nNumero = ""
		...
		Case 0
			If Mid(nr_processo, 5, 2) = "01" Then
				vReturn = "<td align='center'><a href=""javascript:abreParametrizacao('"& nr_processo &"','"& tp_relat &"');"">"&_
									"<img src='/imagens/icones/16x16/tp_canal_"&Trim(nCanal)&".gif' width=17 height=16 border=0></a></td>"
			Else
				vReturn = "<td align='center'><img src='/imagens/icones/16x16/tp_canal_"&Trim(nCanal)&".gif' width=17 height=16 border=0></td>"
			End If
		Case 1
			vReturn = "<td align='center' class='pgridsub'><img src='/imagens/icones/16x16/tp_canal_"&Trim(nCanal)&".gif' width=17 height=16></td>"
		Case 2
...
function fncExibeTabelaExp(nExibir, nCodigo, tpProduto)
	...
	select case nCodigo
		case "0118" 'Nr. Nota Fiscal
			...
			aCampos(1) = ""
			aCampos(2) = ""
			sql = "SELECT NR_NF, DT_NOTA, ISNULL(VL_NOTA, '') "&_
			...
			select case nCodigo
				case "0118"
					vReturn = "<td><nobr>"&aCampos(0)&"</nobr></td>"&_
										...
										"<td align='right'><nobr>"&aCampos(2)&"</nobr></td>"
				case "0123"
			...
			select case nCodigo
				case "0118"
					vReturn = "<td class='pgridsub'><nobr>"&aCampos(0)&"&nbsp;</nobr></td>"&_
										...
										"<td class='pgridsub' align='right'><nobr>&nbsp;"&aCampos(2)&"</nobr></td>"
				case "0123"
			...
			select case nCodigo
				case "0118"
					vReturn = "<td bgcolor='"& bgcolor &"'><nobr>"&aCampos(0)&"&nbsp;</nobr></td>"&_
										...
										"<td align='right' bgcolor='"& bgcolor &"'><nobr>&nbsp;"&aCampos(2)&"</nobr></td>"
				case "0123"
	...
	if nCodigo = "0545" then 'qtde de mercadoria inicial - PO
		if tp_relat = 0 then 'desembaraço
			sql = "SELECT ISNULL(SUM(I.QT_INICIAL), 0) AS QT_INICIAL, ISNULL(RTRIM(UM.NM_UNID_MEDIDA), '') AS NM_UNID_MEDIDA "&_ 
						...
						"  INNER JOIN TPO_ITEM I (NOLOCK) ON (I.NR_PROCESSO = RC.CD_REFERENCIA AND I.NR_ITEM_CLIENTE = RC.NR_ITEM_PO AND I.NR_PARCIAL = RC.NR_PARCIAL) "&_
			...
			sql = " SELECT ISNULL(I.QT_INICIAL,0) AS QT_INICIAL, ISNULL(RTRIM(UM.NM_UNID_MEDIDA), '') AS NM_UNID_MEDIDA  "&_
				...
				" AND I.NR_PARCIAL = '"& Trim( objrs.Fields.Item("NR_PARCIAL").Value ) &"' "
