...
function preload() {
  if(document.images) {
	...
	pasta_open.src  = "/imagens/icones/16x16/folder_open20.gif";
	pasta_close.src = "/imagens/icones/16x16/folder_close20.gif";
	lupa.src        = "/imagens/icones/16x16/button_view20.gif";
	menu_open.src   = "/imagens/icones/16x16/minus_t.gif";
	menu_close.src  = "/imagens/icones/16x16/plus_t.gif";
	seta.src        = "/imagens/seta.jpg";
	seta_virada.src = "/imagens/seta_virada.jpg";
  }
}

function mostra(item, item2, item3, tipo, flag_lupa) {
	var x = screen.width;
	var w = 500;
...
function ConsultaOnLineSQL(vSQL, vTitulo, vReturn, vEdit, vEditAux) {
	...
	if (vSQL == 4) vSQL = "SELECT DISTINCT E.CD_EMPRESA, NM_EMPRESA, AP_EMPRESA, END_EMPRESA, CGC_EMPRESA FROM TEMPRESA_NAC E (NOLOCK), TPERMISSAO P (NOLOCK) WHERE P.CD_CLIENTE = E.CD_EMPRESA AND P.CD_USUARIO = '"+vCodUser+"' ORDER BY E.NM_EMPRESA"
	...
	if (vSQL == 6) vSQL = "SELECT DISTINCT EE.CD_EMPRESA,EE.NR_CLIENTE,EE.NM_EMPRESA, EE.AP_EMPRESA, EE.END_EMPRESA, EE.NR_CLIENTE FROM TEMPRESA_EST EE (NOLOCK), TEMPRESA_EST_GRUPO EG (NOLOCK), TEMPRESA_NAC EN (NOLOCK) WHERE ISNULL(EE.IN_IMPORTADOR,'0') = '1'  AND ISNULL(EE.IN_EXPORTADOR,'0') = '0'  AND EG.CD_EMPRESA = EE.CD_EMPRESA  AND EG.CD_GRUPO = EN.CD_GRUPO  AND (((EN.CD_EMPRESA) IN (SELECT P.CD_CLIENTE FROM TPERMISSAO P (NOLOCK) WHERE P.CD_USUARIO = '"+vCodUser+"'))  OR  ((SELECT CD_CARGO FROM TUSUARIO U (NOLOCK) WHERE CD_USUARIO = '"+vCodUser+"') NOT IN ('040', '055', '060'))) ORDER BY EE.NM_EMPRESA";
	...
	if (vSQL == 8) vSQL = "SELECT DISTINCT EE.CD_EMPRESA,EE.NR_CLIENTE, EE.NM_EMPRESA, EE.AP_EMPRESA, EE.END_EMPRESA FROM TEMPRESA_EST EE (NOLOCK), TEMPRESA_EST_GRUPO EG (NOLOCK), TEMPRESA_NAC EN (NOLOCK), TPERMISSAO P (NOLOCK) WHERE ISNULL(EE.IN_AGENTE,'0') = '1' AND EG.CD_EMPRESA = EE.CD_EMPRESA AND EG.CD_GRUPO = EN.CD_GRUPO AND EN.CD_EMPRESA = P.CD_CLIENTE AND P.CD_USUARIO = '"+vCodUser+"' ORDER BY EE.NM_EMPRESA";
