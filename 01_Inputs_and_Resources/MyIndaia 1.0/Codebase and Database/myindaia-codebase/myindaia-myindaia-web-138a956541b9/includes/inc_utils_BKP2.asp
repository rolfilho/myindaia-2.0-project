...
Session.Timeout  = 30		'Tempo (em minutos) para a sessão expirar
Session("sql_pagina") = ""
...
Const ERROEMAIL   = "andre.borges@myindaia.com.br"'e-mail para envio de erros ocorridos no site
...
Dim cnnBroker_aux, cmdBroker_aux, rsBroker_aux  
Dim strHOST_MAIL
Dim strPORT_MAIL
Dim strUSER_MAIL
Dim strPWD_MAIL 
...
Select Case cd_lang
	Case 1
		...
		tipo_canal      = Array("Red","Yellow","Green","Gray","Waiting","Blocked Green","All")
		...
		tipo_referencia = Array("ORDER","ORDER","DELIVERY","OTHERS","BOOKING","SEEK","SAP USA","EASY","COURRIER","DRAWBACK", "SAP TM","COTAÇÃO","NÚMERO DO LOTE","BOOKING CO-LOADER","MAP","PROTOCOLO BB","RECOPI")
	...
	Case 2
		...
		tipo_canal      = Array("Vermelho","Amarelo","Verde","Cinza","Aguardando","Verde Bloqueado","Todos")
		...
		tipo_referencia = Array("PEDIDO","ORDEM","REMESSA","OUTROS","BOOKING","SEEK","SAP USA","EASY","COURRIER","DRAWBACK", "SAP TM","COTAÇÃO","NÚMERO DO LOTE","BOOKING CO-LOADER","MAPA","PROTOCOLO BB","RECOPI")
	...
	Case Else
		...
		tipo_periodo    = Array("Dias","Meses")
		nm_tp_quebra    = Array("Grupo de Empresas","Empresa","Modalidade","Área","Empresa Estrangeira","Exportador/Fornecedor")
		tipo_referencia = Array("PEDIDO","ORDEM","REMESSA","OUTROS","BOOKING","SEEK","SAP USA","CLIENTE","COURRIER","ATO CONCESSORIO", "SAP TM","COTAÇÃO","NÚMERO DO LOTE","BOOKING CO-LOADER","MAP","PROTOCOLO BB","RECOPI")
		sim_nao         = Array("Não","Sim")
		tipo_local_doc  = Array("Indaiá","Cliente","Outros")
...
tipo_cntr     = Array("PIER","CNTR 20'DRY","CNTR 40'DRY","TANK 20'","TANK 40'","REFFER","20'REEFER","40'REEFER","20'OPEN TOP","40'OPEN TOP","40'HC","40'HC REEFER","FR 40","FR 20","40'OH","20´OTC","20'OTS","FT20'","FT40'","MF 40")
...
'RETORNA:
  ...
  fundo(1) = "/imagens/fundo_01.jpg"
  fundo(2) = "/imagens/fundo_03.jpg"
  fundo(3) = "/imagens/fundo_03.jpg"
  fundo(4) = "/imagens/fundo_04.jpg"
  
  Randomize
  f = Int(4 * rnd + 1)
  
  If Session("ap_usuario") <> "cebrace.vasa" Then
  	fcn_TrocaFundo = ""
  Else
 	 fcn_TrocaFundo = "/imagens/fundo_05.jpg"
  End If	
End Function
...
sub subConectaBrokerAux(servidor, database, usuario, password)
	dim cnstr_aux 'string de conexão
	set cnnBroker_aux = Server.CreateObject("ADODB.Connection")
	set cmdBroker_aux = Server.CreateObject("ADODB.Command")
	set rsBroker_aux  = Server.CreateObject("ADODB.Recordset")
	cnstr_aux = "Provider=SQLOLEDB;Persist Security Info=True;"&_
					"Data Source="&servidor&";Initial Catalog="&database&";"&_
					"User ID="&usuario&";Password="&password&";"
	On error resume next
	cnnBroker_aux.Open cnstr_aux
	cmdBroker_aux.ActiveConnection = cnnBroker_aux
end sub

sub subFechaBrokerAux()
	if rsBroker_aux.State  = adStateOpen then rsBroker_aux.Close
	if cmdBroker_aux.State  = adStateOpen then cmdBroker_aux.Close
	if cnnBroker_aux.State  = adStateOpen then cnnBroker_aux.Close
	set rsBroker_aux = nothing
	set cmdBroker_aux = nothing
	set cnnBroker_aux = nothing
end sub
