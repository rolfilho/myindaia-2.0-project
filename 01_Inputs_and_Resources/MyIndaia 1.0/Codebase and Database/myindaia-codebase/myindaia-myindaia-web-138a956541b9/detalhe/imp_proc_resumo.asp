...
Dim nr_processo_des_null, flag_resumo
...
tp_relat_rel = Session("tp_relat_rel")
...
in_btn_excel = 0
in_btn_lupa = 1
Session("sql_imp_proc_resumo") = Null
If Not fnc_TestaVar(Session("sql_imp_proc_resumo")) Then
  ...
  sql_f = "FROM TPROCESSO PR (NOLOCK) "&_
	...
	Session("sql_imp_proc_resumo") = sql
End If
Session("sql_pagina") = Session("sql_pagina") &"<br>"& Session("sql_imp_proc_resumo") &"<br>"

if objrs.State = adStateOpen then objrs.Close()
With objcmd
  .CommandTimeOut = 120
  .CommandText = Session("sql_imp_proc_resumo")
...
%>
        ...
        <%End If%>
        <td valign="top" width="120"><table width="100%" border="0" cellspacing="0" cellpadding="0" style="text-align:center;">
								<%
If objrs.State = adStateOpen Then objrs.Close
...
%>
								<tr height="100">
									<td align="center">&nbsp;<a href="javascript:abreInstDespachoCusto('<%= nr_processo %>');"><img src="/imagens/30/despacho_custo.gif" alt="<%= tx_lang_A00094(cd_lang)%>" width="59" height="66" border="0"><br>
									<nobr><%= tx_lang_A00094(cd_lang)%></nobr></a></td>
								</tr>
