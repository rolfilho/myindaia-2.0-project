...
Dim nr_processo_des_null, nr_processo_li_null
...
function ExibeHistoricoDetalhe(pendencia, processo, referencia, item_cliente, parcial, empresa){
	...
	img_mais.src  = "/imagens/30/btn_mais20.gif";
	img_menos.src = "/imagens/30/btn_menos20.gif";
...
<body onLoad="document.getElementById('colunaseta').background='/imagens/30/seta.jpg';<%if Session("exibe_detalhe") = 0 Then%> <%Else%>exibeResumo('divResumo','<%= url%>');<%End If%>" style="scrollbar-face-color: <%=rolagem_menu1%>;scrollbar-track-color: <%=rolagem_menu2%>;scrollbar-arrow-color: <%=rolagem_menu3%>;scrollbar-shadow-color: <%=rolagem_menu4%>;scrollbar-3dlight-color: <%=rolagem_menu5%>;scrollbar-highlight-color: <%=rolagem_menu6%>;scrollbar-darkshadow-color: <%=rolagem_menu7%>;">
...
'					If fnc_TestaVar(str) Then str = CInt(str) Else str = 0 [OCORRENCIA NO.: 0399/08]
			...
			End If
			%>
									<td class="detcampo" style="background-color:<%=bg_detalhes_campo%>;"><%= Right(objrsv.Fields.Item("M10_NM_DADO").Value, 11)%></td>
...
%>
						...
						<td valign="top" width="120"><table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#FFFFFF;">
								<%
...
%>
<tr height="100">
	<td align="center">&nbsp;<a href='#' onClick="abreDocs('<%= nr_processo %>','<%= nr_processo %>','<%= tp_relat_rel%>');"><img src="/imagens/30/digitalizado.gif"  border="0" alt="Clique para visualizar os documentos digitalizados."><br>
...
If Not objrs.eof then
	If Not fnc_testavar(objrs.Fields.Item("NR_PROCESSO").Value) then
		nr_processo_li_null = 1
	End if
Else
	nr_processo_li_null = 0
End if
...
%>
								<tr height="100">
									<td align="center">&nbsp;<a href="javascript:abreLI('<%= Trim(objrs.Fields.Item("NR_PROCESSO").Value)%>','');"><img src="/imagens/30/libera_imp_on.gif" border="0" alt="Clique para visualizar os dados da LI."><br>
...
%>
								<tr height="100">
									<td align="center">&nbsp;<a href="javascript:abreFaturamento('<%= nr_processo%>');"><img src="/imagens/30/fatura_on.gif"  border="0" alt="Clique para visualizar os dados do Faturamento."><br>
...
if objrsx.State = adStateOpen then objrsx.Close()
sql = "SELECT ISNULL(G.IN_PLANILHA_CUSTOS,'0') AS IN_PLANILHA_CUSTOS "&_
...
If Not Session("cargo_interno") = "1" Then
	If (Session("cd_cargo") = "040") and objrsx.Fields.Item("IN_PLANILHA_CUSTOS").Value = "1" and nr_processo_des_null = 0 Then in_custo = True
Else
	If objrsx.Fields.Item("IN_PLANILHA_CUSTOS").Value = "1" and nr_processo_des_null = 0 Then in_custo = True
End If
objrsx.Close
...
%>
								<tr height="100">
									<td align="center">&nbsp;<a href="javascript:abreInstDespachoCusto('<%= nr_processo %>');"><img src="/imagens/30/despacho_custo.gif" alt="<%= tx_lang_A00094(cd_lang)%>"  border="0"><br>
...
If nr_parcial <> "" or  (nr_processo_adm_null = 0 and nr_processo_li_null = 1) Then
    If nr_parcial = "" Then
        If objrs.State = adStateOpen Then objrs.Close
...
Do While Not objrs.Eof
    ...
    If objrs.Fields.Item("CD_EVENTO").Value = "164" Then	
	
%>        
		<tr bgcolor="#33CCFF" height="18" border=1>
		<td title="<%= str2%>"><b> <%= Left(str, 45)%><% if Len(str) > 45 then Response.Write("...")%></b></td>
		<td align="right" ><b> <%= x%>&nbsp;</b></td>
		<td align="center" ><b> <%= "<font color="& color &">"& z &"</font>"%></b></td>
		<td align="right" ><b> <%= "<font color="& color &">"& y & img_marcador & "</font>"%>&nbsp;</b></td>
		</tr>
...
%>
								<tr bgcolor="<%=bg_detalhes %>" style="color:#546678;">
									<td height="<%= x%>">
                                    <%if Session("cd_cargo") <> "131" Then%>
                                    <div style="scrollbar-face-color: <%=rolagem_menu10%>;scrollbar-track-color: <%=rolagem_menu20%>;scrollbar-arrow-color: <%=rolagem_menu30%>;scrollbar-shadow-color: <%=rolagem_menu40%>;scrollbar-3dlight-color: <%=rolagem_menu50%>;scrollbar-highlight-color: <%=rolagem_menu60%>;scrollbar-darkshadow-color: <%=rolagem_menu70%>;overflow:auto;height:<%= x+40%>" ><%= Replace(Replace(str_obs, "'", "´"), vbcrlf, "<br>")%></div>                                   
									<%End If%>
                                    </td>
								</tr>
							</table></td>
			...
			If objrs.State = adStateOpen Then objrs.Close					
			sql = "SELECT DISTINCT CONVERT(VARCHAR,MAX(PL.DT_TEXTO),103)+ ' ' + CONVERT(VARCHAR,MAX(PL.DT_TEXTO),108) AS DT_ULTIMA, U.NM_USUARIO AS NM_USUARIO, U2.NM_USUARIO AS NM_USUARIO2,"&_ 
            " P.NM_PENDENCIA AS NM_PENDENCIA, P.NR_PROCESSO AS NR_PROCESSO, PL.CD_PENDENCIA AS CD_PENDENCIA, "&_ 
            " DATEDIFF(MINUTE,  ISNULL(P.DT_SOLICITACAO,P.DT_PRIMEIRA_SOLICITACAO), ISNULL(DT_RESPOSTA_SOLICITA,GETDATE())) AS DIFERENCA, "&_ 
            " DATEDIFF(MINUTE,  P.DT_PRIMEIRA_SOLICITACAO, ISNULL(P.DT_FINALIZACAO, GETDATE())) AS DIFERENCA2, SP.NM_STATUS, P.CD_STATUS "&_
			...
			"  AND P.IN_ATIVO = '1' "&_
			"GROUP BY P.CD_PENDENCIA, U.NM_USUARIO, U2.NM_USUARIO, P.NM_PENDENCIA, P.NR_PROCESSO, PL.CD_PENDENCIA, P.DT_PRIMEIRA_SOLICITACAO, P.DT_FINALIZACAO, DT_SOLICITACAO, DT_RESPOSTA_SOLICITA, SP.NM_STATUS, P.CD_STATUS "
			...
			<div id='divControle'>
          <table width="100%" border="0" cellspacing="2" cellpadding="0">
						<tr height="30"><td valign="top"><font size="2"><b>Controle de Pendências</b></font><br /><img src="/imagens/30/barra_tracking<%=barra_tracking %>.gif"  width="565" border="0"><br><br />
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="border: solid 1px <%=bg_detalhes_borda%>;">
		<tr class="gridtop" height="16" style="background-color:<%=cabec_result%>;">
          ...
          <tr bgcolor="<%= bgcolor%>" height="18px">	
                ...
                <td width="140px" style="text-align:left;"><nobr>&nbsp;<%= objrs.Fields.Item("NM_USUARIO2").Value%></nobr></td>
