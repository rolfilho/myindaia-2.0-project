	 	...
	 	Case "0225" 'Nr de LI
			...
			If objrsv.RecordCount > 0 Then 
				Do While Not objrsv.Eof
					str_li = str_li &"<a href=""javascript:abreLI('"& Trim(objrs.Fields.Item("NR_PROCESSO").Value)& "','" & Trim(objrsv.Fields.Item(nm_campo_rel).Value) & "');"">"& fnc_Mascara(objrsv.Fields.Item(nm_campo_rel), 1) &"</a>"
					objrsv.MoveNext 
					If objrsv.Eof Then Exit Do Else str_li = str_li &", "
				Loop
			End If
%>
									<td class="gridtit" width="30%"><nobr>&nbsp;<%=objrsx.Fields.Item("M10_NM_DADO").Value%>&nbsp;</nobr></td>
									<td align="left" bgcolor="#FFFFFF">&nbsp;<%= str_li%>&nbsp;</td>
									<%
			objrsv.Close
			Case Else
