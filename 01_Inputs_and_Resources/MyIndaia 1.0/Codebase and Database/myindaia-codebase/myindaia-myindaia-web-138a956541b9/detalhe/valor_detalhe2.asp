...
nr_parcial = CStr(Request.QueryString("nr_parcial"))
		...
		If objrsx.Fields.Item("M12_IN_MONTA_SQL").Value Then
			Select Case objrsx.Fields.Item("M10_TP_DADO").Value & objrsx.Fields.Item("M10_TP_CONSULTA")
				...
				Case "20","30","40"'Decimal, Moeda, Inteiro
					sql_s = sql_s &_
						" ISNULL("& objrsx.Fields.Item("M12_NM_CH_ORIGEM").Value &", 0) AS "& objrsx.Fields.Item("M10_AP_CAMPO").Value &", "
  ...
  sql_f = "FROM TPROCESSO PR (NOLOCK) "&_
	...
	Session("sql_valor_detalhe") = sql
	Session("sql_pagina") = Session("sql_pagina") &"<br>"& Session("sql_valor_detalhe") &"<br>"
...
%>
		...
		Case "0350" 'qtde de pallets
			...
			str_lcl = ""
			If objrsv.Fields.Item("VL_QTDE").Value > 0 then
				Do While Not objrsx.Eof
					str_lcl = str_lcl & objrsv.Fields.Item("VL_QTDE").Value '&" "& tipo_cntr(CInt(objrsx.Fields.Item("TP_CNTR").Value)-1)
					objrsv.MoveNext
					If objrsv.Eof Then Exit Do Else str_lcl = str_lcl &"; "
				Loop
			Else
...
%>
									<td class="gridtit" width="30%"><nobr>&nbsp;<%=objrsx.Fields.Item("M10_NM_DADO").Value%>&nbsp;</nobr></td>
									<td align="left" bgcolor="#FFFFFF">&nbsp;<%= str_lcl%>&nbsp;</td>
									<%												
		Case "0170" 'qtde de containers
			...
			If objrsv.RecordCount > 0 then
				Do While Not objrsv.Eof
					str_cntr = str_cntr & objrsv.Fields.Item("VL_QTDE").Value &" "& tipo_cntr(CInt(objrsv.Fields.Item("TP_CNTR").Value)-1)
					objrsv.MoveNext
					If objrsv.Eof Then Exit Do Else str_cntr = str_cntr &"; "
				Loop
			Else
...
%>
									<td class="gridtit" width="30%"><nobr>&nbsp;<%=objrsx.Fields.Item("M10_NM_DADO").Value%>&nbsp;</nobr></td>
									<td align="left" bgcolor="#FFFFFF">&nbsp;<%= str_cntr%>&nbsp;</td>
									<%
			Case Else
