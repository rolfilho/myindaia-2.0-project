...
<body onLoad="document.getElementById('colunaseta').background='/imagens/seta.jpg';" bgcolor="#FFFFFF">
...
<td width="4%" align="left" ><a href="#" onClick="FechaDetalhes('divInfoCorpo2','divInfoRodape2','divInfo2');"><img src="/imagens/icones/16x16/btn_fechar20.gif" width="18" height="17" border="0" alt="Fechar"></a>&nbsp;</td>
</tr>
</table>
 </div>
...
<tr><td valign="top"><table width="100%" border="0" cellspacing="2" cellpadding="0">
					<tr>
...
%>
								<tr class="gridtop">
									<td colspan="4"><table width="100%" cellpadding="0" cellspacing="0" border="0" class="table"><tr class="gridtop"><td width="70%">&nbsp;<%= tx_lang_A00051(cd_lang)%></td><td align="right" width="30%" style="font-style:normal;"><a href="#" onClick="ExibeDetalhes('<%= tx_lang_POP091(cd_lang)%>','divInfoCorpo2','divInfo2','divInfoRodape2','<%= url%>','dados_detalhe',510,280)" style="color:#FFFFFF;"><img src="/imagens/icones/16x16/btn_mais20.gif"  border="0" alt="<%= tx_lang_POP091(cd_lang)%>" align="absmiddle" height="13" width="15">&nbsp;<%=tx_lang_POP092(cd_lang)%>&nbsp;</a></td></tr></table></td>
								</tr>
								<%
		End Select
		cont = 1
...
%>
								...
								<tr class="gridtop">
								  <td colspan="4"><table width="100%" cellpadding="0" cellspacing="0" border="0" class="table"><tr class="gridtop"><td width="70%">&nbsp;<%= tx_lang_A00052(cd_lang)%></td><td align="right" width="30%" style="font-style:normal;"><a href="#" onClick="ExibeDetalhes('<%= tx_lang_POP090(cd_lang)%>','divInfoCorpo2','divInfo2','divInfoRodape2','<%= url%>','valor_detalhe',350,420)" style="color:#FFFFFF;"><img src="/imagens/icones/16x16/btn_mais20.gif"  border="0" alt="<%= tx_lang_POP090(cd_lang)%>" align="absmiddle" height="13" width="15">&nbsp;<%=tx_lang_POP092(cd_lang)%>&nbsp;</a></td></tr></table></td>
								</tr>
                                						</table></td>
				...
				<table width="100%" border="0" cellspacing="2" cellpadding="0">
					<tr>
						<td width="<%If Session("cd_cargo") <> "088" Then%>45%<%Else%>100%<%End If%>" valign="top" style="border: solid 1px #336699;">
<!-- **************************************************************** 
   ...
   Else
       if objrs.Fields.Item("CD_EVENTO").Value = "164" Then	
		...
		sql = "SELECT "&_
			...
			"  LEFT JOIN TEVENTO E ( NOLOCK ) ON ( E.CD_EVENTO = '164' ) "&_
			"  WHERE F.NR_REFERENCIA = '"&nr_referencia&"' AND F.NR_ITEM_CLIENTE = '"&fnc_AddZeros(nr_item_clnte, 4)&"' AND F.NR_PARCIAL = '"& nr_parcial &"' AND F.CD_EMPRESA = '"&cd_empresa&"'"
		Session("sql_pagina") = Session("sql_pagina") &"<br>"& sql &"<br>"
		objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
		If bgcolor = "#FEFEFE" Then bgcolor = "#EDEDED" Else bgcolor = "#FEFEFE"
...
%>
						...
						<td width="55%" valign="top" style="border: solid 1px #336699;" bgcolor="#EEEEEE" cellspacing="1" >
<!-- **************************************************************** 
...
%>
								<tr bgcolor="#EEEEEE" valign="top">
									<td height="<%= x%>"><div style="overflow:auto;height:<%= x%>" ><%= Replace(Replace(str_obs, "'", "´"), vbcrlf, "<br>")%></div></td>
								</tr>
							</table></td>
					</tr>
				</table>
