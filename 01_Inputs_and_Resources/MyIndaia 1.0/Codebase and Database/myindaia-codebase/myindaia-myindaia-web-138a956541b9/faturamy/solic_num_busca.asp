...
'Arquivo de Script e HTML: Preparação de dados e Busca pela informação
'
...
If tx_erro <> "" Then
  ...
  Response.Redirect("solic_num_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If
...
If in_dt_null or fnc_TestaVar(cd_evento) Then
  ...
  'If Not fnc_TestaVar(cd_evento) Then cd_evento = "162"
	'verifica se a qtde de dias é valido
	If fnc_TestaVar(cd_evento) Then
        If Not fnc_TestaVar(nr_dias) Then nr_dias = DatePart("d", Date)-1
...
With objcmd
	.CommandTimeOut = 9999
	.CommandText    = "sp_rel_fat_solic_num_money"
...
If nr_result > 0 Then
  ...
  str = "/faturamy/solic_num_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = tx_lang_A00002(cd_lang) &": "& tx_lang_A00029(cd_lang) &".\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/faturamy/solic_num_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
