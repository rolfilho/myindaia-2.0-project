...
If Not fnc_TestaVar(Session("sql_fat_solic_num")) Then
  'dados básicos do relatório
	sql_s = "SELECT DISTINCT R.NR_PROCESSO, R.NR_REFERENCIA, R.CD_GRUPO, R.CD_CLIENTE, R.CD_AREA, R.CD_VIA_TRANSPORTE, DN.NR_EMISSAO, DN.ANO_EMISSAO, CONVERT(VARCHAR, DN.DT_EMISSAO,103) AS DT_RECEBIMENTO,"
	'tabelas da busca
  sql_f = "FROM TREL_FAT_SOLIC_NUM R ( NOLOCK ) "&_
					"  INNER JOIN TPROCESSO  PR ( NOLOCK ) ON ( PR.NR_PROCESSO = R.NR_PROCESSO ) "&_					
					"  INNER JOIN MYMONEY.DBO.VW_DOCUMENTO_SOLICITACAO_NUMERARIO_ATIVO DN (NOLOCK) ON (DN.NR_PROCESSO = PR.NR_PROCESSO)  "&_
                    "  INNER JOIN MYMONEY.DBO.VW_DOCUMENTO_ITENS DI (NOLOCK) ON (DI.NR_EMISSAO = DN.NR_EMISSAO AND DI.ANO_EMISSAO = DN.ANO_EMISSAO)  "&_					
					"  LEFT JOIN TPO_ITEM POI (NOLOCK) ON (POI.NR_PROCESSO = R.NR_PROCESSO AND POI.NR_ITEM_CLIENTE = DI.CD_ITEM) "&_
					"  INNER JOIN TITEM TI ( NOLOCK ) ON ( TI.CD_ITEM = DI.CD_ITEM ) "
...
tx_path_file  = "/faturamy/solic_num_print"
tx_path_excel = "/excel/fat_solic_num_my"
...
If in_dt_prev Then 
  vl_colspan  = " colspan='3'"
	...
	Next
	tx_sub_cols = "<tr height='15' class='gridsub' style='background-color:"& cabec_result &"'>"& tx_sub_cols &"</tr>"
Else
  vl_colspan  = ""
	vl_rowspan  = ""
End If
...
%>
	...
	If objrsv.Eof Then objrsv.MoveFirst
	'dados do prcesso
    nr_emissao = Trim(objrs.Fields.Item("NR_EMISSAO").Value)
    ano_emissao = Trim(objrs.Fields.Item("ANO_EMISSAO").Value)
...
%>
						...
						<%
			Case "0018","0094","0096","0097","0098","0099","0100","0101","0374","0899" 'Nr.Referencia - Item
...
%>
						...
						<%	
            Case "0834"
                sql = " SELECT SUM(VL_EMISSAO) AS VL_EMISSAO, NR_NOTA_AMARRADA "&_
                " FROM (SELECT DISTINCT (SELECT DISTINCT E1.NR_NOTA FROM MYMONEY.DBO.TB_EMISSAO E1 "&_
                " WHERE E1.NR_EMISSAO = E.NR_EMISSAO_AMARRADO  "&_
                "  AND E1.ANO_EMISSAO = E.ANO_EMISSAO_AMARRADO "&_
                "  AND E1.IN_CANCELADO = '0') AS NR_NOTA_AMARRADA, "&_
                "  ED.VL_EMISSAO, E.NR_EMISSAO  "&_                                                                          
                " FROM MYMONEY.DBO.TB_EMISSAO_DETALHE ED "&_
                " INNER JOIN MYMONEY.DBO.TB_EMISSAO E ON E.NR_EMISSAO = ED.NR_EMISSAO AND E.ANO_EMISSAO = ED.ANO_EMISSAO AND E.IN_CANCELADO = 0 "&_
                " INNER JOIN MYMONEY.DBO.TB_LANCTO_FUP LF ON LF.NR_EMISSAO = ED.NR_EMISSAO AND LF.ANO_EMISSAO = ED.ANO_EMISSAO AND LF.CD_ROTINA = 102 "&_
                " WHERE LF.NR_LANCTO IN (SELECT NR_LANCTO FROM MYMONEY.DBO.TB_EMISSAO_DETALHE WHERE NR_EMISSAO = " & nr_emissao & " AND ANO_EMISSAO = " & ano_emissao & ") "&_
                "  AND NR_EMISSAO_AMARRADO IS NOT NULL) TABELA "&_
                " GROUP BY NR_NOTA_AMARRADA  " 
                objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
                If objrsx.RecordCount > 0 Then
                str = objrsx("NR_NOTA_AMARRADA")
                Else
                str = ""
                End If
                %>
              ...
              Case "0835"
                ...
                objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
                If objrsx.RecordCount > 0 Then
                str = FormatNumber(objrsx("VL_EMISSAO"),2)
                Else
                ...
                <%
                objrsx.Close()
			Case "0797","0704" 'Valor total da solicitação do numerário
