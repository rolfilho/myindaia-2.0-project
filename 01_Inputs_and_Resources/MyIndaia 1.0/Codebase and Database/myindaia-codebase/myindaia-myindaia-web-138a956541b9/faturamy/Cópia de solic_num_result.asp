...
If Not fnc_TestaVar(Session("sql_fat_solic_num")) Then
  'dados básicos do relatório
	sql_s = "SELECT DISTINCT R.NR_PROCESSO, R.NR_REFERENCIA, R.CD_GRUPO, R.CD_CLIENTE, R.CD_AREA, R.CD_VIA_TRANSPORTE, CONVERT(VARCHAR, DN.DT_EMISSAO,103) AS DT_RECEBIMENTO,"
	'tabelas da busca
  sql_f = "FROM TREL_FAT_SOLIC_NUM R ( NOLOCK ) "&_
					"  INNER JOIN TPROCESSO  PR ( NOLOCK ) ON ( PR.NR_PROCESSO = R.NR_PROCESSO ) "&_					
					"  INNER JOIN MYMONEY.DBO.VW_DOCUMENTO_SOLICITACAO_NUMERARIO DN (NOLOCK) ON (DN.NR_PROCESSO = PR.NR_PROCESSO)  "&_
  ...
  vl_colspan  = " colspan='3'"
	...
	For x = 1 to nr_data
		tx_sub_cols = tx_sub_cols &_
			"<td>"& tx_lang_A00033(cd_lang) &"</td>"&_
			"<td>"& tx_lang_A00034(cd_lang) &"</td>"&_
...
Do While Not objrsv.Eof
	If objrsv.Fields.Item("M10_TP_DADO").Value & objrsv.Fields.Item("M10_TP_CONSULTA") = "10" Then
		tx_tit_cols = tx_tit_cols &"<td"& vl_colspan &">"& objrsv.Fields.Item("M10_NM_DADO").Value &"</td>"
	Else
    tx_tit_cols = tx_tit_cols &"<td"& vl_rowspan &">"& objrsv.Fields.Item("M10_NM_DADO").Value &"</td>"
	End If
	objrsv.MoveNext
	If objrsv.Eof Then Exit Do
Loop
Response.Write("<tr class='gridtit'>"& tx_tit_cols &"</tr>"& tx_sub_cols)
