...
<%
Session("sql_fat_notas_fiscais_my") = Null
If Not fnc_TestaVar(Session("sql_fat_notas_fiscais_my")) Then

sql_s = " SELECT R.*, CASE WHEN CONVERT(DECIMAL(28,2), R.FATURA) = CONVERT(DECIMAL(28,2), R.LIQUIDACAO) THEN 'Liquidada' ELSE 'Pendente' END PAGA_SIM_NAO, "
sql_s = sql_s & "(SELECT TOP 1 MASTER.DBO.FN_CRIPROGRAFA(RTRIM(LEFT(R.PROCESSO,CHARINDEX(';', R.PROCESSO)-1)) + ';' + CONVERT(VARCHAR, PD.NM_ARQUIVO)) "&_
                "  FROM BROKER.DBO.TPROCESSO_DOCTOS PD "&_
                "   WHERE PD.NR_PROCESSO = LEFT(R.PROCESSO,CHARINDEX(';', R.PROCESSO)-1) "&_
                "     AND PD.NM_DESCRICAO LIKE '%'+ CONVERT(VARCHAR, R.NOTA) +'%' "&_
                "     AND PD.IN_ATIVO = '1' "&_
                "     AND PD.CD_TIPO_DOCTO = 63) AS CRIPTOGRAFADO "
sql_f = sql_f & " FROM TREL_FAT_NOTAS_FISCAIS R " 
...
'Response.Redirect("teste.asp?var1="&sql_s&sql_f&"")
	...
	Session("sql_fat_notas_fiscais_my") = sql_s & sql_f & sql_w & sql_o
	'response.Write(Session("sql_fat_notas_fiscais_my"))
	'response.end
End If
...
tx_path_file  = "/faturamy/not_fiscal/not_fiscal_excel"
tx_path_excel = "/faturamy/not_fiscal/not_fiscal_excel"
...
Do While nrec < objrs.PageSize and Not objrs.Eof
    ...
    str_docto= "<a href='/popup/ver_arquivos.asp?in_nota=1&cod=" & objrs.Fields.Item("CRIPTOGRAFADO").Value & "' target='_blank'><img src='/imagens/30/open.gif' border=0 alt='Ver Nota(s)'></a>"  
...
%>
					...
					<td class="center"><nobr>&nbsp;<%=str_docto%>&nbsp;</nobr></td>
                    <%
