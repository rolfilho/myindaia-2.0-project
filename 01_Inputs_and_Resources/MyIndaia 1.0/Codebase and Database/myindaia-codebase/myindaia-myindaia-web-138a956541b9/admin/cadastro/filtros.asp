...
'Arquivo de Script e HTML: Cadastro de Filtros
'
...
tx_link   = "filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_docto="&cd_docto
'verifica se as variaves não são nulas/vazio
If Not fnc_TestaVar(tx_campo) Then tx_campo = "M11_NR_ORDEM"
...
cd_filtro   = CStr(Request.QueryString("cd_filtro"))
...
sql = "SELECT M11_CD_FILTRO, M11_CD_MENU, M11_CD_SUBM, M11_CD_DADO, M11_NR_ORDEM, M11_IN_ATIVO, "&_
			"  M01_NM_MENU, M02_NM_SUBM, M10_NM_DADO, M10_AP_FILTRO, M10_CD_BROKER "&_
			"FROM M11_FILTRO (NOLOCK) "&_
			"  INNER JOIN M01_MENU  (NOLOCK) ON (M01_CD_MENU = M11_CD_MENU) "&_
			"  INNER JOIN M02_SUBM  (NOLOCK) ON (M02_CD_MENU+M02_CD_SUBM = M11_CD_MENU+M11_CD_SUBM) "&_
			"  INNER JOIN M10_DADOS (NOLOCK) ON (M10_CD_DADO = M11_CD_DADO) "&_ 
			"ORDER BY M11_CD_MENU, M11_CD_SUBM, "& tx_campo &" "& tx_ordem
'Response.Write(sql)
With objcmd
  .CommandTimeOut = 120
  .CommandText    = sql
...
If Request.QueryString("in_acao") = "lista" Then
	str = "M11_CD_FILTRO = '"& Request("cd_filtro") &"'"
	'busca o registro
...
function fnc_AbreEdicao(tx_link, in_acao, cd_filtro) {
  features = "top=20,left=20,width=400,height=310,scrollbars=no,menubar=no,status=no";
	window.open(tx_link+'<%= "&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&nr_pagina="&nr_pagina%>&in_acao='+in_acao+'&cd_filtro='+cd_filtro,'EditaFiltro',features);
}
//-->
</script>
</head>
...
End If
%>
							<a href="#" onClick="fnc_AbreEdicao('<%= Replace(tx_link,"filtros","filtros_edt",1,1)%>','novo','','');"><b>Clique para criar um Filtro</b></a></p></td>
				</tr>
			</table></td>
	</tr>
	<tr>
		<td valign="top"><!-- :::::::::: INICIO DO CORPO CENTRAL :::::::::: -->
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr height="25">
						<td width="75" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M11_CD_FILTRO&tx_ordem=ASC&nr_pagina=<%= nr_pagina%>';">C&oacute;digo</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M10_NM_DADO&tx_ordem=ASC&nr_pagina=<%= nr_pagina%>';">Nome</td>
						<td width="50" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M10_AP_FILTRO&tx_ordem=ASC&nr_pagina=<%= nr_pagina%>';">Apelido</td>
						...
						<td width="35" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M11_NR_ORDEM&tx_ordem=ASC&nr_pagina=<%= nr_pagina%>';">Ordem</td>
						<td width="30" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M11_IN_ATIVO&tx_ordem=ASC&nr_pagina=<%= nr_pagina%>';">Ativo</td>
...
Do While cont < objrs.PageSize and Not objrs.Eof
	If str <> objrs.Fields.Item("M11_CD_MENU").Value&objrs.Fields.Item("M11_CD_SUBM").Value Then
%>
					<tr>
					...
					<%
  End If
  str = objrs.Fields.Item("M11_CD_MENU").Value&objrs.Fields.Item("M11_CD_SUBM").Value
%>
					<tr>
						<td align="right" class="admcel"><%= objrs.Fields.Item("M11_CD_FILTRO").Value%>&nbsp;</td>
						...
						<td align="right" class="admcel"><%= objrs.Fields.Item("M11_NR_ORDEM").Value%>&ordm;&nbsp;</td>
						<td align="center" class="admcel"><a href="<%= tx_link&"&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&nr_pagina="&nr_pagina&"&cd_filtro="&objrs.Fields.Item("M11_CD_FILTRO").Value&"&in_ativo="&objrs.Fields.Item("M11_IN_ATIVO").Value%>"><img src="/imagens/icones/<%= fnc_CheckImg(objrs("M11_IN_ATIVO"))%>.gif" width="12" height="12" border="0" align="Clique para ativar/desativar"></a></td>
						<td class="admcom"><table width="100%" border="0" cellspacing="1" cellpadding="0">
								<tr align="center">
									<td width="50%"><a href="#" onClick="fnc_AbreEdicao('<%= Replace(tx_link,"filtros","filtros_edt",1,1)%>','editar','<%= objrs.Fields.Item("M11_CD_FILTRO").Value%>');"><img src="/imagens/icones/16x16/button_edit.gif" height="13" width="12" border="0" alt="Editar"></a></td>
									<td width="50%"><a href="<%= tx_link&"&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&nr_pagina="&nr_pagina&"&cd_filtro="&objrs.Fields.Item("M11_CD_FILTRO").Value&"&in_acao=excluir"%>"><img src="/imagens/icones/16x16/button_drop.gif" height="13" width="11" border="0" alt="Excluir"></a></td>
								</tr>
							</table></td>
					</tr>
					<%
...
%>
					<tr height="25">
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M11_CD_FILTRO&tx_ordem=DESC&nr_pagina=<%= nr_pagina%>';">C&oacute;digo</td>
						...
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M11_NR_ORDEM&tx_ordem=DESC&nr_pagina=<%= nr_pagina%>';">Ordem</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M11_IN_ATIVO&tx_ordem=DESC&nr_pagina=<%= nr_pagina%>';">Ativo</td>
