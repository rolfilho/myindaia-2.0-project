...
'Arquivo de Script e HTML: Cadastro de Sub-Menu
'
...
tx_link   = "subm.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_docto="&cd_docto
'verifica se as variaves não são nulas/vazio
If Not fnc_TestaVar(tx_campo) Then tx_campo = "M02_NR_ORDEM"
...
If fnc_TestaVar(in_ativo) Then
	Call sub_AtivaRegistro(cd_menu_cad&cd_subm_cad, "M02_SUBM", in_ativo)
End If
  ...
  Else
    ...
    Else
      objrs.Close
			...
			nr_ordem = objrs.Fields.Item("M02_NR_ORDEM").Value - 1
...
sql = "SELECT M02_CD_MENU, M01_NM_MENU, M02_CD_SUBM, M02_NM_SUBM, M02_AP_SUBM, M02_NR_ORDEM, M02_IN_ATIVO, " &_
			"  M02_CD_MENU+M02_CD_SUBM AS CODIGO "&_
			"FROM M02_SUBM ( NOLOCK ) " &_
			"  INNER JOIN M01_MENU ( NOLOCK ) ON ( M01_CD_MENU = M02_CD_MENU ) " &_ 
			"ORDER BY M02_CD_MENU, "& tx_campo &" "& tx_ordem
objrs.Open sql, objcnn, 3, 3
'coloca na pagina que for encontrado o registro
If Request.QueryString("in_acao") = "lista" Then
	str = "CODIGO = '"& cd_menu_cad&cd_subm_cad &"'"
	'busca o registro
...
function fnc_AbreEdicao(tx_link, in_acao, cd_menu_cad, cd_subm_cad) {
  features = "top=20,left=20,width=410,height=310,scrollbars=no,menubar=no,status=yes";
	window.open(tx_link+'<%= "&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&nr_pagina="&nr_pagina%>&in_acao='+in_acao+'&cd_menu_cad='+cd_menu_cad+'&cd_subm_cad='+cd_subm_cad,'EditaMenu',features);
}
//-->
</script>
</head>
...
End If
%>
							<a href="#" onClick="fnc_AbreEdicao('<%= Replace(tx_link,"subm","subm_edt",1,1)%>','novo','','');"><b>Clique para criar um Sub-Menu</b></a></p></td>
				</tr>
			</table></td>
	</tr>
	<tr>
		<td valign="top"><!-- :::::::::: INICIO DO CORPO CENTRAL :::::::::: -->
			<div id="dv_corpo" class="divMainQld">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr height="25">
						<td width="25" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_CD_SUBM&tx_ordem=ASC&p=<%= nr_pagina%>';">C&oacute;d.</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_NM_SUBM&tx_ordem=ASC&p=<%= nr_pagina%>';">Nome</td>
						<td width="70" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_AP_SUBM&tx_ordem=ASC&p=<%= nr_pagina%>';">Apelido</td>
						<td width="35" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_NR_ORDEM&tx_ordem=ASC&p=<%= nr_pagina%>';">Ordem</td>
						<td width="30" class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_IN_ATIVO&tx_ordem=ASC&p=<%= nr_pagina%>';">Ativo</td>
...
Do While cont < objrs.PageSize and Not objrs.Eof
	If str <> objrs.Fields.Item("M02_CD_MENU").Value Then
%>
					<tr>
						<td colspan="6" class="admsub"><%= objrs.Fields.Item("M01_NM_MENU").Value%></td>
					</tr>
					<%
  End If
  str = objrs.Fields.Item("M02_CD_MENU").Value
%>
					<tr>
						<td align="right" class="admcel"><%= objrs.Fields.Item("M02_CD_SUBM").Value%>&nbsp;</td>
						<td class="admcel"><%= objrs.Fields.Item("M02_NM_SUBM").Value%>&nbsp;</td>
						<td class="admcel"><%= objrs.Fields.Item("M02_AP_SUBM").Value%>&nbsp;</td>
						<td align="right" class="admcel"><%= objrs.Fields.Item("M02_NR_ORDEM").Value%>&ordm;&nbsp;</td>
						<td align="center" class="admcel"><a href="<%= tx_link&"&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&nr_pagina="&nr_pagina&"&cd_menu_cad="&objrs.Fields.Item("M02_CD_MENU")&"&cd_subm_cad="&objrs.Fields.Item("M02_CD_SUBM").Value&"&in_ativo="&objrs.Fields.Item("M02_IN_ATIVO").Value%>"><img src="/imagens/icones/<%= fnc_CheckImg(objrs("M02_IN_ATIVO"))%>.gif" width="12" height="12" border="0" align="Clique para ativar/desativar"></a></td>
						<td class="admcom"><table width="100%" border="0" cellspacing="1" cellpadding="0">
								<tr align="center">
									<td width="50%"><a href="#" onClick="fnc_AbreEdicao('<%= Replace(tx_link,"subm","subm_edt",1,1)%>','editar','<%= objrs.Fields.Item("M02_CD_MENU").Value%>','<%= objrs.Fields.Item("M02_CD_SUBM").Value%>');"><img src="/imagens/icones/16x16/button_edit.gif" height="13" width="12" border="0" alt="Editar"></a></td>
									<td width="50%"><a href="<%= tx_link&"&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&nr_pagina="&nr_pagina&"&cd_menu_cad="&objrs.Fields.Item("M02_CD_MENU").Value&"&cd_subm_cad="&objrs.Fields.Item("M02_CD_SUBM").Value&"&in_acao=excluir"%>"><img src="/imagens/icones/16x16/button_drop.gif" height="13" width="11" border="0" alt="Excluir"></a></td>
								</tr>
							</table></td>
					</tr>
					<%
...
%>
					<tr height="25">
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_CD_SUBM&tx_ordem=DESC&p=<%= nr_pagina%>';">C&oacute;d.</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_NM_SUBM&tx_ordem=DESC&p=<%= nr_pagina%>';">Nome</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_AP_SUBM&tx_ordem=DESC&p=<%= nr_pagina%>';">Apelido</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_NR_ORDEM&tx_ordem=DESC&p=<%= nr_pagina%>';">Ordem</td>
						<td class="admtit" onClick="document.location='<%= tx_link%>&tx_campo=M02_IN_ATIVO&tx_ordem=DESC&p=<%= nr_pagina%>';">Ativo</td>
