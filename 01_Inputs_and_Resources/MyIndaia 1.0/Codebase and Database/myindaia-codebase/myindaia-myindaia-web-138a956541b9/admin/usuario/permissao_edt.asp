...
tx_link   = "permissao_edt.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_docto="&cd_docto&"&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem
...
If in_acao = "incluir" Then
	...
	nm_user = Request.Form("nm_user")
	in_tipo = "alterar"
	'verifica se o usuário foi informado corretamente
	If cd_user <> nm_user Then
		tx_erro = "Usuário não especificado/inválido."
	Else
		cont = 0
		...
		Else
			...
			Response.Write("<script>window.opener.location.href='"&Replace(tx_link,"permissao_edt","permissao",1,1)&"&in_acao=lista&cd_user="&cd_user&"';"&_
										 "document.location='"&tx_link&"&in_acao=editar&cd_user="&cd_user&"&tx_erro="& Server.URLEncode(tx_erro)&"';</script>")
		End If
	End If
End If
'edita os registros
...
If in_acao = "alterar" Then
	cd_user = Trim(Request.Form("cd_user"))
	nm_user = Trim(Request.Form("nm_user"))
	in_tipo = "alterar"
	'verifica se o usuário foi informado corretamente
	If cd_user = "" Then
		tx_erro = "Usuário não especificado/inválido."
	Else
		...
		Else
			nm_user = fnc_ConsultaLookup("TUSUARIO","CD_USUARIO",cd_user,"NM_USUARIO")
			Response.Write("<script>window.opener.location.href='"&Replace(tx_link,"permissao_edt","permissao",1,1)&"&in_acao=lista&cd_user="&cd_user&"'</script>")
		End If
	End If
End If
'exclui o registro
  ...
  cd_user = Trim(Request.QueryString("cd_user"))
	...
	Else
		...
		If Err.Number = 0 Then
			tx_erro = "CAD(13):\n - Código: " & cd_user &"."
			Response.Write("<script>window.opener.location.href='"&Replace(tx_link,"permissao_edt","permissao",1,1)&"&in_acao=lista&cd_user="&cd_user&"';window.close();</script>")
		Else
			tx_erro = "CAD(07):\n -"
			Response.Redirect(tx_link&"&in_acao=novo&cd_user=&tx_erro="& Server.URLEncode(tx_erro))
		End If
...
If in_acao = "novo" or in_acao = "incluir" Then
	Response.Write("left: 77px;")
Else
	Response.Write("left: 82px;")
End If
%>
	width: 100%;
	height: 100%;
...
<body<%If in_acao = "novo" or in_acao = "incluir" Then Response.Write(" onLoad='document.frmCadastroAcesso.cd_user.focus();'")%>>
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
	<form action="<%= tx_link%>" method="post" name="frmCadastroAcesso" onSubmit="return fnc_ValidaForm(this);">
		<tr>
			<td height="30" class="admtitulo">&nbsp;Acesso de Usuários</td>
		</tr>
		<tr>
			<td valign="top"><table height="100%" border="0" cellpadding="0" cellspacing="1">
					<tr height="30">
						<td class="admlabel"><label for="user">Usuário:</label></td>
						<%
If in_acao = "novo" or in_acao = "incluir" Then
%>
						<td class="admdados" colspan="3"><input name="cd_user" id="user" type="text" class="editbox" style="width:30px;" maxlength="4" onBlur="fnc_ConsultaCodigo();">
							<select name="nm_user" class="select" style="width:250px;" onChange="fnc_InsereCodigo();">
...
sql = "SELECT CD_USUARIO, NM_USUARIO, CD_CARGO " &_
			...
			"WHERE CD_USUARIO NOT IN ( SELECT DISTINCT M09_CD_USUARIO FROM M09_ACESSO ( NOLOCK ) ) "&_
			"ORDER BY CD_CARGO"
Session("sql_pagina") = Session("sql_pagina") &"<br> "& sql &"<br>"
objrs.Open sql, objcnn, 3, 1

Do While Not objrs.Eof
  If Trim(objrs.Fields.Item("CD_CARGO").Value) = "040" Then str = "(cliente)" Else str = "(interno)"
%>
...
Else
%>
						<td class="admdados" colspan="3"><%= cd_user &" - "& nm_user%>
							<input name="cd_user" type="hidden" value="<%= Trim(cd_user)%>">
							<input name="nm_user" type="hidden" value="<%= Trim(cd_user)%>">
						</td>
						<%
End If
%>
					...
					<tr height="30">
						<td width="75" class="admlabel" rowspan="2" valign="top">Acesso:</td>
						<td width="265" class="admdados">Menus</td>
						<td width="25" class="admdados">Sim</td>
						<td width="45" class="admdados">N&atilde;o</td>
					</tr>
					<%
If in_acao = "novo" or in_acao = "incluir" Then
...
%>
					<tr>
						<td colspan="3"><div id="dv_corpo" class="divEdicao">
...
%>
									<tr>
										<td width="265" class="admdados"><%= objrsx.Fields.Item("NM_MENU").Value%>
