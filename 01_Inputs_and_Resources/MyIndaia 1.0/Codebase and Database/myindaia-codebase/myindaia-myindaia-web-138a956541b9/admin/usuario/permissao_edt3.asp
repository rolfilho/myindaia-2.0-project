...
tx_link   = "permissao_edt2.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_docto="&cd_docto&"&tx_campo="&tx_campo&"&tx_ordem="&tx_ordem&"&cd_user="&cd_user&"&in_interno="&in_interno&"&cd_cargo="&cd_cargo
	...
	Else
        If objrsx.Fields.Item("M09_IN_ACESSO").Value = "X" Then
           if objrsx.Fields.Item("M02_CD_MENU").Value = "01" Then
                If objrsv.State = adStateOpen Then objrsv.Close
                sql = " SELECT (SELECT ISNULL(M09_IN_ACESSO, 0) FROM M09_ACESSO "&_
                      "          WHERE M09_CD_USUARIO = '" & cd_user & "' "&_
                      "            AND M09_CD_MENU = '01'"&_
                      "            AND M09_CD_SUBM = '01') AS TRACKING,  "&_
                      "         (SELECT ISNULL(M09_IN_ACESSO, 0) FROM M09_ACESSO "&_
                      "          WHERE M09_CD_USUARIO = '" & cd_user & "' "&_
                      "            AND M09_CD_MENU = '01'"&_
                      "            AND M09_CD_SUBM = '09') AS BUSCA,  "&_
                      "         (SELECT ISNULL(M09_IN_ACESSO, 0) FROM M09_ACESSO "&_
                      "          WHERE M09_CD_USUARIO = '" & cd_user & "' "&_
                      "            AND M09_CD_MENU = '01'"&_
                      "            AND M09_CD_SUBM = '03') AS ALERTA  "
                objrsv.Open sql, objcnn, adOpenStatic, adLockReadOnly,adCmdText
            
                If in_interno = 1 or InStr("040;088", cd_cargo) > 0 Then
                   soma_acessos = 0
                   soma_acessos = CInt(objrsv("TRACKING")) + CInt(objrsv("BUSCA")) + CInt(objrsv("ALERTA"))
                   If soma_acessos = 3 Then in_acesso = 1 Else in_acesso = 0
                Else
                    If cd_cargo = "131" Then
                        soma_acessos = 0
