...
'monta a instrução sql
    ...
    Response.Cookies("pendencia")("codigo") = ""
    Response.Cookies("pendencia")("processo") = ""
    Response.Cookies("pendencia")("nome") = ""
	...
	Case 1
		...
		Else
			...
			" INNER JOIN BROKER.DBO.TUSUARIO U (NOLOCK) ON (U.CD_USUARIO = P.CD_USUARIO_SOLICITANTE)"&_ 
		...
		"tp_relat=2&filtro=nr_processo&nr_processo=" & rsMy.Fields.Item("NR_PROCESSO").Value 
                tpReferenciaPend = session("TipoPedidoImp")
		else
			If rsMy.Fields.Item("CD_PRODUTO").Value = "06" Then
                ...
                tpReferenciaPend = session("TipoPedido")
			End If	
		end if 
		...
		sql = "SELECT DISTINCT RC.CD_REFERENCIA "&_
					...
					"WHERE RC.NR_PROCESSO = '"& rsMy.Fields.Item("NR_PROCESSO").Value  &"' AND RC.TP_REFERENCIA = '" &  tpReferenciaPend & "' "&_
		...
		%>
			<tr bgcolor="<%= bgcolor%>" height="18px">	
				...
				<td width="<%If Session("supervisa_pendencias") = "1" Then%>100px<%Else%>130px<%End If%>" style="text-align:left;">&nbsp;<a href="#" onclick="ExibeHistorico_alert('<%= rsMy.Fields.Item("CD_PENDENCIA").Value%>','<%=rsMy.Fields.Item("NR_PROCESSO").Value%>','<%=rsMy.Fields.Item("NR_REFERENCIA").Value%>', '<%=rsMy.Fields.Item("NR_ITEM_CLIENTE").Value%>', '<%=rsMy.Fields.Item("NR_PARCIAL").Value%>', '<%=rsMy.Fields.Item("CD_EMPRESA").Value%>','<%= rsMy.Fields.Item("NM_PENDENCIA").Value%>');"><%= rsMy.Fields.Item("NM_PENDENCIA").Value%></a></td>	
	...
	Case 2 
		...
		If rsMy.State = adStateOpen Then rsMy.Close()
		sql = "SELECT P.*, CONVERT(VARCHAR, DT_TEXTO, 103) AS DT_TEXTO_DATA, CONVERT(VARCHAR, DT_TEXTO, 108) AS DT_TEXTO_HORA, U.NM_USUARIO AS NM_USUARIO, U2.NM_USUARIO AS NM_USUARIO2, "&_
        " MASTER.DBO.FN_CRIPROGRAFA('/docs_novo/'+P.NR_PROCESSO+'/'+P.NM_ARQUIVO) AS ARQUIVO "&_
        " FROM TPROCESSO_PENDENCIAS_LOG P (NOLOCK) "&_
        " INNER JOIN TUSUARIO U (NOLOCK) ON U.CD_USUARIO = P.CD_USUARIO"&_
        " INNER JOIN TUSUARIO U2 (NOLOCK) ON U2.CD_USUARIO = P.CD_USUARIO_PARA"&_
        ...
        cd_texto_pendencia = rsMy.RecordCount + 1
		data_aux = 	rsMy.Fields.Item("DT_TEXTO_DATA").Value
        nm_usuario_contato = rsMy.Fields.Item("NM_USUARIO2").Value%>
        ...
        If rsMy.Fields.Item("NM_USUARIO").Value = rsMy.Fields.Item("NM_USUARIO2").Value Then %>        
        <a href="http://www.myindaiaweb.com.br/redireciona_doc.asp?cod=<%=rsMy.Fields.Item("ARQUIVO").Value%>" target="_blank"><font color="<%= font_color%>"><%= texto_aux%></font></a></div>
        <%Else%>
        ...
        <textarea name="nm_texto" style="width:680px;height:58px;overflow:auto;top:270px;position:absolute;" wrap="virtual" class="editbox"></textarea><input name="btn_enviar" type="button" class="button" value="Enviar" style="top:270px;right:10px;position:absolute;width:50px;height:80px;background-color:#b4c7db;" onclick="EnviaMensagem_alert(document.getElementById('nm_texto'),'<%= Trim(Request("cd_pendencia"))%>','<%= Trim(Request("nr_processo"))%>', '<%= Trim(Request("nr_referencia"))%>', '<%= Trim(Request("nr_item_cliente"))%>', '<%= Trim(Request("nr_parcial"))%>', '<%= Trim(Request("cd_empresa"))%>', '<%= Trim(Request("nm_pendencia"))%>');"/>
        <form action="/arquivo/upsalvar.asp" method="post" enctype="multipart/form-data">
          <input type="hidden" name="vInTipo" value="4">
          <input type="hidden" name="vNrProcesso" value="<%= Request("nr_processo") %>">
          <input type="hidden" name="cd_pendencia" value="<%= Trim(Request("cd_pendencia")) %>">
          <input type="hidden" name="nr_referencia" value="<%= Trim(Request("nr_referencia")) %>">
          <input type="hidden" name="nr_item_cliente" value="<%= Trim(Request("nr_item_cliente")) %>">
          <input type="hidden" name="nr_parcial" value="<%= Trim(Request("nr_parcial")) %>">
          <input type="hidden" name="cd_empresa" value="<%= Trim(Request("cd_empresa")) %>">
          <input type="hidden" name="nm_contato" value="<%= nm_usuario_contato %>">
          <input type="hidden" name="cd_contato" value="<%=Session("cd_usuario")%>">
          <input type="hidden" name="cd_texto" value="<%=cd_texto_pendencia%>">
          <input type="hidden" name="nm_pendencia" value="<%=Trim(Request("nm_pendencia"))%>">
          <input type="hidden" name="vScriptPend" id="vScriptPend">
          <input type="file" id="arq_pendencia" name="arq_pendencia" class="editbox" style="width:370px;height:19px;top:331px;left:232px;position:absolute;"><input name="btn_enviar" type="submit" class="button" value="Enviar arquivo" style="top:331px;left:605px;position:absolute;width:85px;height:19px;background-color:#d5d1ca;font-weight:normal;border-color:#3c6d9d;"/>
        </form>
        <script>
            document.getElementById('vScriptPend').value = String(window.location);
        </script>
...
<select id="nm_contatos" name="nm_contatos" class="select" style="width:326px;border-style:0px;">
	...
	Case 5
		...
		if rsMy.Fields.Item("CD_STATUS").Value = "2" Then
			...
			'Else
		    sql = "UPDATE TPROCESSO_PENDENCIAS SET CD_USUARIO_CONTATO = '" & Trim(Request("cd_contato")) & "' "&_
			"  WHERE CD_PENDENCIA =" & Trim(Request("cd_pendencia")) &_
		    " AND NR_PROCESSO = '" & Trim(Request("nr_processo")) & "' "&_
		    " AND NR_REFERENCIA = '" & Trim(Request("nr_referencia")) & "' "&_
		    " AND NR_ITEM_CLIENTE = '" & Trim(Request("nr_item_cliente")) & "' "&_
