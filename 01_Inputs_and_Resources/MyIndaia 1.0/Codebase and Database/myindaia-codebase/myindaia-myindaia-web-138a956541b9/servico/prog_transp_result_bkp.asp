			...
			<div id="dv_corpo" class="divMainMid">
				<table width="2000px" border="0" cellspacing="1" cellpadding="0">
					<tr height="35" class="gridtit">
                        ...
                        <td><nobr>&nbsp;Frota/Motorista&nbsp;</nobr></td>
...
sql = "SELECT PR.CD_TRANSP_NAC, R.NR_PROCESSO, R.CD_GRUPO, R.CD_CLIENTE, R.CD_AREA, R.CD_VIA_TRANSPORTE, "&_
...
"	  CONVERT(VARCHAR,PC.DT_SAIDA_ARM,103) AS DT_SAIDA_ARM, LEFT(CONVERT(VARCHAR,PC.DT_SAIDA_ARM,108),5) AS HR_SAIDA_ARM, NM_MOTORISTA, CONVERT(VARCHAR, PC.DT_PROGRAMACAO,103) AS DT_PROGRAMACAO, AZM.NM_ARMAZEM " &_
					...
					<tr height="15" bgcolor="<%= bgcolor%>" onMouseDown="setPointer(this, <%= objrs.AbsolutePosition%>, 'click', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');" onMouseOver="setPointer(this, <%= objrs.AbsolutePosition%>, 'over', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');" onMouseOut="setPointer(this, <%= objrs.AbsolutePosition%>, 'out', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');">
						<form name="frmgrava" action="<%= url%>" method="post">
                            ...
                            <%If Session("cd_cargo")="055" and cd_transp_usuario = objrs.Fields.Item("CD_TRANSP_NAC").Value Then%> 
                            	<td align="center"><input class="editbox" type="text" id="dt_programacao" name="dt_programacao" value="<%= fnc_Mascara(objrs.Fields.Item("DT_PROGRAMACAO").Value, 5)%>" style="width:70px;margin-left:30px;" maxlength="10" onBlur="fnc_ValidaData(this);" onKeyUp="mascaraData(this);" onkeypress="if (event.keyCode == 47){return false;}"></td>
                            <%Else%>
							...
							<td><nobr>&nbsp;<%= objrs.Fields.Item("NM_ARMAZEM").Value%>&nbsp;</nobr></td>
                             <%If Session("cd_cargo")="055" and cd_transp_usuario = objrs.Fields.Item("CD_TRANSP_NAC").Value Then%> 
                            <td><nobr>Data: <input class="editbox" type="text" name="dt_saida_terminal" value="<%=objrs.Fields.Item("DT_SAIDA_ARM").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);" onKeyUp="mascaraData(this);" onkeypress="if (event.keyCode == 47){return false;}"><br>Hora: <input class="editbox" type="text" name="hr_saida_terminal" value="<%=objrs.Fields.Item("HR_SAIDA_ARM").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" onKeyUp="mascaraHora(this);"></td>
                            ...
                            <td><nobr>&nbsp;<input class="editbox" type="text" name="dt_devolucao" value="<%= objrs.Fields.Item("DT_DEVOLUCAO").Value%>" style="width:70px;margin-left:60px;" maxlength="10" onBlur="fnc_ValidaData(this);" onKeyUp="mascaraData(this);" onkeypress="if (event.keyCode == 47){return false;}">&nbsp;</nobr></td>
...
%>
					<tr class="gridtit" height="15">
						<td colspan="<%If Session("cd_cargo")="055" Then%> 19 <%Else%>18<%End If%>" align="left"><nobr><%= tx_lang_A00030(cd_lang) &" "& objrs.Fields.Item(0).Value &" "& tx_lang_A00031(cd_lang) &"."%></nobr></td>
					</tr>
					<tr height="25">
						<td colspan="19" align="center"><%= "<i>"& tx_lang_A00002(cd_lang) &": "& tx_lang_A00026(cd_lang) &" DD/MM/AAAA.</i>"%></td>
					</tr>
				</table>
</div>
