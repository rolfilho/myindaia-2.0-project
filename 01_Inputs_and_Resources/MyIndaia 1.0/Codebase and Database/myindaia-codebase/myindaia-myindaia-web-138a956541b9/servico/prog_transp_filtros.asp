...
cd_view = "1801-0803-01"
in_view_filtro = 1
%>
<!--#include virtual="/includes/inc_view.asp"-->
<%
'nome da tela
nm_tela = nm_menu &" &gt;&gt; "& nm_subm &" &gt;&gt; "& fnc_ConsultaLookup("M10_DADOS","M10_CD_BROKER",cd_evento,tx_lang_dado)
		...
		<td width="100%" valign="top"><!-- :::::::::: INICIO DO CORPO CENTRAL :::::::::: -->
			<div id="dv_corpo" class="divMainTop">
				<table width="<%= CStr(vl_width)%>" border="0" align="center" cellpadding="0" cellspacing="0">
					<form name="frmbusca" method="post" action="/servico/prog_transp_busca.asp" onSubmit="return validSubmit()">
						<tr>
							<td height="50" <%If cd_lang = 0 Then%><%Else%><%End If%>colspan="<%= CStr(vl_colspan) + 2%>" align="left">
                            <font size="2"><br><b><%= tx_lang_A00020(cd_lang)%></b><font color="<%=color_texto %>"><%If cd_lang = 0 or cd_lang = 2 Then%><% If Session("cd_cargo") <> "088" Then%><span style="margin-left:20px"><%Else%><span style="margin-left:150px"><%End If%><%Else%><span style="margin-left:100px"><%End If%><b><a href="#" onClick="javascript:ExibeDetalhes('divInfo','divInfoCorpo', 'divInfoRodape',2);" style="color:<%=color_texto %>;"><%= tx_lang_TF001(cd_lang)%> </a>/ <a href="#" onClick="javascript:carregaPesquisa('<%=cd_view %>','prog_transp_filtros');" style="color:<%=color_texto %>;"><%= tx_lang_TF043(cd_lang)%></a></b></font></font></span><br>
...
%>
	...
	'aplica a personalização sobre os filtros a exibir
...
<option value="1"><%=tx_lang_TF002(cd_lang)%></option>
            ...
            If InStr("040;060", Session("cd_cargo")) > 0 Then
              Select Case objrsv.Fields.Item("M10_CD_DADO").Value
                Case "0058"
                  sql = "SELECT DISTINCT T.CD_TRANSP_ROD AS CODIGO, T.NM_TRANSP_ROD AS DESCRICAO " &_
                   " FROM BROKER.DBO.TTRANSP_ROD T " &_
                   " INNER JOIN BROKER.DBO.TCLIENTE_TRANSPORTADOR C ON (C.CD_TRANSP  = T.CD_TRANSP_ROD AND T.IN_ATIVO = '1') " &_
                   " INNER JOIN BROKER.DBO.TEMPRESA_NAC E ON (E.CD_EMPRESA = C.CD_CLIENTE AND E.IN_ATIVO = '1') " &_
                   " INNER JOIN BROKER.DBO.TPERMISSAO_GRUPO P       ON (P.CD_GRUPO = E.CD_GRUPO AND P.IN_ATIVO = '1') "&_ 
                   " WHERE C.IN_ATIVO = '1' " &_
                   "   AND P.CD_USUARIO = '" & Session("cd_usuario") & "' " &_
                   "   AND C.CD_PRODUTO = '01' " &_
                   " ORDER BY DESCRICAO "
              End Select
              objrsx.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
              Do While Not objrsx.Eof
