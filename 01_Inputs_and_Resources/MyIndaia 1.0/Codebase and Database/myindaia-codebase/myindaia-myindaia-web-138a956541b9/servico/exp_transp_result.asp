...
Dim hr_chegada_fabr, hr_entr_terminal, hr_pre_stacking, hr_carregamento, hr_solic_ret_vazio, hr_ret_vazio 
...
tx_path_excel = "/servico/exp_transp_excel"
...
If Request.TotalBytes > 0 Then
	...
	If fnc_TestaVar(nr_processo) and fnc_TestaVar(nr_cntr) Then
		...
		sql = sql &" DT_SOLIC_RET_VAZIO = "
		If IsDate(Request.Form("dt_solic_ret_vazio")) Then
			If Request.Form("hr_solic_ret_vazio") = "" Then
			  hr_solic_ret_vazio = "00:00:00" 
			Else
			  hr_solic_ret_vazio = Request.Form("hr_solic_ret_vazio") & ":00"
			End If		
		 
			sql = sql &"'"& Request.Form("dt_solic_ret_vazio") & " " & hr_solic_ret_vazio & "', " 
		Else 
			sql = sql &"NULL, "
		End If
		
		sql = sql &" DT_RET_VAZIO = "
		If IsDate(Request.Form("dt_ret_vazio")) Then
			If Request.Form("hr_ret_vazio") = "" Then
			  hr_ret_vazio = "00:00:00" 
			Else
			  hr_ret_vazio = Request.Form("hr_ret_vazio") & ":00"
			End If		
		 
			sql = sql &"'"& Request.Form("dt_ret_vazio") & " " & hr_ret_vazio & "', " 
		Else 
			sql = sql &"NULL, "
		End If
		
		
		sql = sql &" DT_CHEGADA_FABR = "
		If IsDate(Request.Form("dt_chegada_fabr")) Then 
			If Request.Form("hr_chegada_fabr") = "" Then
			  hr_chegada_fabr = "00:00:00" 
			Else
			  hr_chegada_fabr = Request.Form("hr_chegada_fabr") & ":00"
			End If
			
			sql = sql & "'"& Request.Form("dt_chegada_fabr") & " " & hr_chegada_fabr &  "', " 
		Else 
			sql = sql &"NULL, "
		End If
		
		sql = sql &" DT_ENTR_TERMINAL = "
		If IsDate(Request.Form("dt_entr_terminal")) Then
			If Request.Form("hr_entr_terminal") = "" Then
			  hr_entr_terminal = "00:00:00" 
			Else
			  hr_entr_terminal = Request.Form("hr_entr_terminal") & ":00"
			End If		
		 
			sql = sql &"'"& Request.Form("dt_entr_terminal") & " " & hr_entr_terminal & "', " 
		Else 
			sql = sql &"NULL, "
		End If
		
		sql = sql &" DT_PRE_STACKING = "
		If IsDate(Request.Form("dt_pre_stacking")) Then 
			If Request.Form("hr_pre_stacking") = "" Then
			  hr_pre_stacking = "00:00:00" 
			Else
			  hr_pre_stacking = Request.Form("hr_pre_stacking") & ":00"
			End If		
		
			sql = sql &"'"& Request.Form("dt_pre_stacking") & " " & hr_pre_stacking & "', " 
		Else 
			sql = sql &"NULL, "
		End If
		
		sql = sql &" DT_SAIDA_FABR = "
		If IsDate(Request.Form("dt_carregamento")) Then 
			If Request.Form("hr_carregamento") = "" Then
			  hr_carregamento = "00:00:00" 
			Else
			  hr_carregamento = Request.Form("hr_carregamento") & ":00"
			End If		
		
			sql = sql &"'"& Request.Form("dt_carregamento") & " " & hr_carregamento & "' " 
		Else 
			sql = sql &"NULL "
		End If
			...
			<div id="dv_corpo" class="divMainMid">
				<table width="2000px" border="0" cellspacing="1" cellpadding="0">
					<tr height="35" class="gridtit">
						...
						<td><nobr>Nr. Conhecimento</nobr></td>
						<td><nobr>Cliente</nobr></td>
						<td><nobr>Tara</nobr></td>
						<td><nobr>Lacre</nobr></td>
                        <td><nobr>Solicitação de retirada do vazio</nobr></td>
                        <td><nobr>Retirada do vazio</nobr></td>
                        <td><nobr>Chegada na Fábrica</nobr></td>
						<td><nobr>Saída da Fábrica</nobr></td>                  
                        <td><nobr>Entrega no Terminal</nobr></td>
                		<td><nobr>Pre-Stacking</nobr></td>	      
						<td>&nbsp;OK&nbsp;</td>
					</tr>
					<%
...
%>
					<tr height="15" bgcolor="<%= bgcolor%>" onMouseDown="setPointer(this, <%= objrs.AbsolutePosition%>, 'click', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');" onMouseOver="setPointer(this, <%= objrs.AbsolutePosition%>, 'over', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');" onMouseOut="setPointer(this, <%= objrs.AbsolutePosition%>, 'out', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');">
						<form name="frmgrava" action="<%= url%>" method="post">
							...
							<td><nobr><%= objrs.Fields.Item("TARA_CNTR").Value%></nobr></td>
							<td><nobr><%= objrs.Fields.Item("NR_LACRE").Value%></nobr></td>
                            <td align="center">Data: <input class="editbox" type="text" name="dt_solic_ret_vazio" value="<%= objrs.Fields.Item("DT_SOLIC_RET_VAZIO").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);"><br>Hora: <input class="editbox" type="text" name="hr_solic_ret_vazio" value="<%=objrs.Fields.Item("HR_SOLIC_RET_VAZIO").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" ></td>
                            <td align="center">Data: <input class="editbox" type="text" name="dt_ret_vazio" value="<%= objrs.Fields.Item("DT_RET_VAZIO").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);"><br>Hora: <input class="editbox" type="text" name="hr_ret_vazio" value="<%=objrs.Fields.Item("HR_RET_VAZIO").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" ></td>
							<td align="center">Data: <input class="editbox" type="text" name="dt_chegada_fabr" value="<%=objrs.Fields.Item("DT_CHEGADA_FABR").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);"><br>Hora: <input class="editbox" type="text" name="hr_chegada_fabr" value="<%=objrs.Fields.Item("HR_CHEGADA_FABR").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" ></td>
                            <td align="center">Data: <input class="editbox" type="text" name="dt_carregamento" value="<%=objrs.Fields.Item("DT_SAIDA_FABR").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);"><br>Hora: <input class="editbox" type="text" name="hr_carregamento" value="<%=objrs.Fields.Item("HR_SAIDA_FABR").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" ></td>
                             <td align="center">Data: <input class="editbox" type="text" name="dt_entr_terminal" value="<%=objrs.Fields.Item("DT_ENTR_TERMINAL").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);"><br>Hora: <input class="editbox" type="text" name="hr_entr_terminal" value="<%=objrs.Fields.Item("HR_ENTR_TERMINAL").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" ></td>
							<td align="center">Data: <input class="editbox" type="text" name="dt_pre_stacking" value="<%=objrs.Fields.Item("DT_PRE_STACKING").Value%>" style="width: 70px;" maxlength="10" onBlur="fnc_ValidaData(this);"><br>Hora: <input class="editbox" type="text" name="hr_pre_stacking" value="<%= objrs.Fields.Item("HR_PRE_STACKING").Value%>" style="width: 70px;" maxlength="5" onBlur="fnc_ValidaHora(this);" ></td>
...
%>
					<tr class="gridtit" height="15">
						<td colspan="14" align="left"><nobr><%= tx_lang_A00030(cd_lang) &" "& objrs.Fields.Item(0).Value &" "& tx_lang_A00031(cd_lang) &"."%></nobr></td>
					</tr>
					<tr height="25">
						<td colspan="14" align="center"><%= "<i>"& tx_lang_A00002(cd_lang) &": "& tx_lang_A00026(cd_lang) &" DD/MM/AAAA.</i>"%></td>
					</tr>
				</table>
</div>
