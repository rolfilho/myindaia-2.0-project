...
Response.Redirect("track_filtros.asp?cd_menu="& cd_menu & "&cd_subm="& cd_subm& "&cd_view="& cd_view & "&in_pesquisa=1")
End If
...
Do While Not objrsx.Eof
		...
		If (fnc_TestaVar(Request(ap_filtro)) and flag_dif <> 1) or (flag_dif = 1)  Then
			If objrsx.Fields.Item("M11_IN_IGNORA_MASK").Value Then
				If objrsx.Fields.Item("M11_IN_VARIOS").Value Then
					...
					If InStr("0673", objrsx.Fields.Item("M11_CD_DADO").Value) > 0 then 
						Session("ncm") = "'" & fnc_verificaMask(Request(ap_filtro))
					End If
					For cont_filtro = 0 to total_filtro-1
						If not IsEmpty(Request(ap_filtro & "_" & cont_filtro)) Then
							...
							If InStr("0673", objrsx.Fields.Item("M11_CD_DADO").Value) > 0 then 
								Session("ncm") = Session("ncm") & "','" & fnc_verificaMask(Request(ap_filtro & "_" & cont_filtro)) 
							End If
						End If
					Next
	...
	Else			
	  If IsDate(Request(ap_filtro&"i")) or IsDate(Request(ap_filtro&"f")) Then	
			...
			Response.Cookies("datas")("dt_inicial") = ""
   			Response.Cookies("datas")("dt_final") = ""	
			Response.Cookies("datas")("flag_0611") = ""
			Response.Cookies("datas")("flag_0666") = ""	
			Response.Cookies("datas")("flag_0668") = ""		
			Response.Cookies("datas")("flag_0048") = ""	
			
			if InStr("0611;0666;0668;0048", objrsx.Fields.Item("M11_CD_DADO").Value) > 0 then
				dt_inicial  = mid(tx_filtros, 1, inStr(1,tx_filtros,"@")-1)  
				dt_inicial  = Left(dt_inicial,len(dt_inicial))
				dt_final    = mid(tx_filtros, inStr(1,tx_filtros,"@")+1, LEN(tx_filtros))
				dt_final    = Left(dt_final,len(dt_final)-1)			   
				Response.Cookies("datas")("dt_inicial") = dt_inicial
				Response.Cookies("datas")("dt_final") = dt_final
				If 	objrsx.Fields.Item("M11_CD_DADO").Value = "0611" Then
					Response.Cookies("datas")("flag_0611") = "1"
				Else
					If 	objrsx.Fields.Item("M11_CD_DADO").Value = "0666" Then
						Response.Cookies("datas")("flag_0666") = "1"
					Else
						If objrsx.Fields.Item("M11_CD_DADO").Value = "0668" Then
							Response.Cookies("datas")("flag_0668") = "1"
						Else
							Response.Cookies("datas")("flag_0048") = "1"
						End If	
					End If
				End if 
			End if
...
if Session("ncm") <> "" Then
Session("ncm") = Session("ncm") & "'"
End If
'response.write Session("ncm")
'Response.end
