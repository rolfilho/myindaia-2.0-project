...
<%
Session("sql_imp_lead") = Null
If Not fnc_TestaVar(Session("sql_imp_lead")) Then
	'dados básicos do relatório
	sql_s = "SELECT CD_GRUPO, CD_SERVICO, "
	'monta o select de acordo com os dados selecionados na personalização
  ...
  Loop
	'tabelas da busca
	sql_f = "FROM TREL_IMP_LEAD R ( NOLOCK ) "
...
tx_path_file  = "/import/lead_print"
tx_path_excel = "/excel/imp_lead"
...
<Table id="TabelaPrincipal" border="0" width="100%" cellspacing="0" cellpadding="0" valign="middle" style="padding: 0px 1px 0px 1px;" bordercolor="#FFFFFF"  bgcolor="#336699">
...
Do While Not objrsv.Eof
%>
						<th align="center"><%= objrsv.Fields.Item("M10_NM_DADO").Value%></td>
						<%
	objrsv.MoveNext
...
%>
					<tr class="detcampo">
						...
						<td align="right"><%= Round(qtd_grupo, 2)%></td>
						<td align="right"><%= Round(qtd_abaixo, 2)%></td>
						<td></td>
						<td align="right"><%= Round(qtd_acima, 2)%></td>
						<td></td>
						<td align="right"><%= Round((soma_grupo / nr_grupo), 2)%></td>
						<td></td>
					</tr>
					<%
...
%>
					<tr>
						<td height="20" colspan="<%= col%>" class="gridtop">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%= fnc_ConsultaLookup("TGRUPO_SERVICO","CD_GRUPO",objrs.Fields.Item("CD_GRUPO").Value,"NM_GRUPO")%></td>
					</tr>
					<%
	End If
...
%>
					...
					<tr class="gridleadtit">
						<td align="right">Total</td>
						<td align="right"><%= Round(qtd_geral, 2)%></td>
						<td align="right"><%= Round(qtd_ab_geral, 2)%></td>
						<td></td>
						<td align="right"><%= Round(qtd_ac_geral, 2)%></td>
						<td></td>
						<td align="right"><%= Round((soma_geral / nr_geral), 2)%></td>
						<td></td>
					</tr>
        </table>
      </div>
