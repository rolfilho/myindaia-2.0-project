...
If Not fnc_TestaVar(Session("sql_imp_custo")) Then
	...
	sql_f = "FROM TREL_IMP_CUSTO R ( NOLOCK ) "&_
					...
					"  INNER JOIN TFLP_IMPORT FI ( NOLOCK ) ON ( FI.NR_REFERENCIA = R.NR_REFERENCIA AND FI.NR_ITEM_CLIENTE = R.NR_ITEM_CLIENTE AND FI.NR_PARCIAL = R.NR_PARCIAL AND FI.NR_PROCESSO_DES = R.NR_PROCESSO AND FI.CD_EMPRESA = R.CD_EMPRESA)  "	&_			
					"  INNER JOIN TFAT_CALCULO_RATEIO_DESP FR ( NOLOCK ) ON ( FR.CD_REFERENCIA = RTRIM(R.NR_REFERENCIA_COMP) ) "&_
