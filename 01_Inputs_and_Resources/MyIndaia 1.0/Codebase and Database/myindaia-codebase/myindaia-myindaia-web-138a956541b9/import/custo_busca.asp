...
<%
'---------------------------------------------------------------------------
'Sistema MyIndaiá WEB - Analise de Custo Importação
...
If tx_erro <> "" Then
  ...
  Response.Redirect("custo_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If

'para os cargos abaixo é obrigatório ter a pesquisa por empresa e área
'Response.Redirect("teste.asp?var1="&cd_filtros&"")
...
If nr_result > 0 Then
  ...
  'escreve a url para redirecionamento
   str = "/import/custo_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = tx_lang_A00002(cd_lang) &": "& tx_lang_A00029(cd_lang) &".\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/import/custo_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
