...
<html xmlns="http://www.w3.org/1999/xhtml"><body>
    <table style="cursor:pointer;font-size:8pt;font-family:verdana;color:#000000;height:19px;BORDER-COLLAPSE:collapse;width: 100%;border-width:0px 0px 0px 0px;border-style:solid;border-color:#374BAE;" bgcolor="#374BAE" cellspacing="0" cellpadding="4">
        <%  Dim Cnn As String = "Data Source=INDAIA10;Initial Catalog=BROKER;User ID=sa;Password=123"
            ...
            Dim codigo, nome, tabela, usuario, color, SqlW, SqlE, campo, cargo As String
                ...
                Else
                    ...
                    Else
                        If InStr("TMERCADORIA", tabela) Then
                            ...
                            " OR ( CD_GRUPO = '" & Request("cd_grupo") & "' AND '" & Request("cd_grupo") & "' <> '' ) )  ORDER BY CODIGO"
                        Else
            ...
            Dim SqlAux As String = "SELECT " & codigo & " AS CODIGO, " & SqlE & " UPPER(" & nome & ") AS DESCRICAO FROM " & tabela & SqlW & " "
            ...
            Else
                Do While RsAux.Read()
                    ...
                    End If
                %>
                <tr bgcolor="<%=color%>" onclick="enviaValor('<%=Trim(RsAux("CODIGO").ToString)%>','<%=tabela %>','<%=campo %>');"><td><nobr><%=Trim(RsAux("CODIGO").ToString)%></nobr></td><%If SqlE <> "" Then%><td><nobr><%=Trim(RsAux("CODIGO_EXP").ToString)%></nobr></td><%End If%><td><nobr><%=Trim(RsAux("DESCRICAO").ToString)%></nobr></td></tr>
                <%  
                Loop
            End If
            RsAux.Close()
