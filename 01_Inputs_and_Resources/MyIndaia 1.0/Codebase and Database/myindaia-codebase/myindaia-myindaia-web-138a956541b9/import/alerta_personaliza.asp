...
If in_acao = "incluir" Then
    ...
    cd_local_des_view_aux = Request.Form("cd_local_des_view_aux")
    cd_transp_view_aux = Request.Form("cd_transp_view_aux")
    cd_status_croda_view_aux = Request.Form("cd_status_croda_view_aux")
    ...
    If fnc_TestaVar(cd_local_des_view)    Then 
      in_cd_local_des_view    = 1 
      quantidade_campos = quantidade_campos + 2
      quantidade_file   = quantidade_file + 1
    Else 
      in_cd_local_des_view = 0
    End If
    ...
    If fnc_TestaVar(cd_status_croda_view)    Then 
      in_cd_status_croda_view    = 1 
      quantidade_campos = quantidade_campos + 2
      quantidade_file   = quantidade_file + 1
    Else 
      in_cd_status_croda_view = 0
    End If
...
'	End If
	...
	Else
        ...
        If Not fnc_TestaVar(cd_local_des_view) Then cd_local_des_view = Null
        If Not fnc_TestaVar(cd_transp_view) Then cd_transp_view = Null
        If Not fnc_TestaVar(cd_status_croda_view) Then cd_status_croda_view = Null
        ...
        If CInt(contador_outros) + CInt(contador_relatorio) > 10 Then
            tx_erro = "ATENÇÃO: Máximo de 10 relatórios para o mesmo dia!"
		...
		sql = "INSERT INTO M06_VIEW ( " &_
                    ...
                    "  M06_CD_GRUPO, M06_CD_CLIENTE, M06_CD_MODAL, M06_CD_UNIDADE, M06_CD_AREA, M06_CD_CELULA, M06_CD_LOCAL_DESEMBARQUE, M06_CD_TRANSP, M06_CD_STATUS_CRODA "&_	                     
					") " &_
					"VALUES ("&_
                        ...
                        fnc_QuotedSQL(cd_grupo_view) &", "& fnc_QuotedSQL(cd_cliente_view) &", "& fnc_QuotedSQL(cd_modal_view) &","& fnc_QuotedSQL(cd_unid_view) &","& fnc_QuotedSQL(cd_area_view) &","& fnc_QuotedSQL(cd_celula_view) &","& fnc_QuotedSQL(cd_local_des_view) &","& fnc_QuotedSQL(cd_transp_view) &","& fnc_QuotedSQL(cd_status_croda_view) &") "
		Session("sql_pagina") = Session("sql_pagina") &"<br>"& sql &"<br>"
        objcnn.Execute(sql)
...
If in_acao = "editar" Then
	...
	If objrs.RecordCount > 0 Then
        ...
        cd_local_des_view = objrs.Fields.Item("M06_CD_LOCAL_DESEMBARQUE").Value
        cd_transp_view = objrs.Fields.Item("M06_CD_TRANSP").Value
        cd_status_croda_view = objrs.Fields.Item("M06_CD_STATUS_CRODA").Value
        ...
        cd_local_des_view_aux = cd_local_des_view
        cd_transp_view_aux = cd_transp_view
        cd_status_croda_view_aux = cd_status_croda_view
...
If in_acao = "alterar" Then
    ...
    cd_area_view    = Request.Form("cd_area_view")
    cd_celula_view    = Request.Form("cd_celula_view")
    cd_local_des_view = Request.Form("cd_local_des_view")
    cd_transp_view = Request.Form("cd_transp_view")
    cd_status_croda_view = Request.Form("cd_status_croda_view")
    cd_ordenacao_view = Request.Form("cd_ordenacao_view")
    tp_ordenacao_view = Request.Form("tp_ordenacao_view")
    cd_grupo_view_aux = cd_grupo_view
    cd_cliente_view_aux = cd_cliente_view
	...
	Else
		...
		sql = "UPDATE M06_VIEW SET " &_
                    ...
                    "  M06_CD_LOCAL_DESEMBARQUE  = "& fnc_QuotedSQL(cd_local_des_view)    &",  "&_
                    "  M06_CD_TRANSP  = "& fnc_QuotedSQL(cd_transp_view)    &",  "&_
                    "  M06_CD_STATUS_CRODA  = "& fnc_QuotedSQL(cd_status_croda_view)    &"  "&_
					" WHERE M06_CD_VIEW = "& fnc_QuotedSQL(cd_view) 
...
If in_acao = "excluir" Then
	...
	Else		
		...
		sql = "DELETE FROM M19_VIEW_HORARIO " &_
					"WHERE M19_CD_VIEW = "& fnc_QuotedSQL(cd_view)
		Session("sql_pagina") = Session("sql_pagina") &"<br>"& sql &"<br>"
		objcnn.Execute(sql)
...
If ex_evento Then
	...
	sql = " SELECT RTRIM(M10_CD_BROKER) AS CD_EVENTO, RTRIM(M10_NM_DADO) AS NM_EVENTO "&_
        ...
        " AND M10_IN_ATIVO = 1 "&_
        " AND M10_TP_DADO = '1' "&_
        " AND M10.M10_CD_BROKER IS NOT NULL "&_
        " ORDER BY NM_EVENTO "
...
End If%>
            <tr>
              <td align="right" class="label" style="color:<%=color_texto%>;">&nbsp;</td>
              <td colspan="2">&nbsp;&nbsp;
                <select name="cd_local_des_view" class="select" id="cd_local_des_view" style="width:250px;" onchange="alteraQuantCampos(this);">
                  <option value="" selected><%= tx_lang_004092(cd_lang)%></option>                  
                  <%
                  'seleciona todos os eventos referentes a sub-&aacute;rea
	            sql = "SELECT CD_LOCAL_EMB AS CODIGO, UPPER(NM_LOCAL_EMB) AS DESCRICAO FROM TLOCAL_EMBARQUE ORDER BY NM_LOCAL_EMB" 
	            objrs.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
                Do While Not objrs.Eof
                if cd_local_des_view = objrs("CODIGO") Then selected = "selected" Else selected = ""
%>
									<option value="<%= objrs("CODIGO")%>" <%=selected %>><%= Trim(objrs("DESCRICAO"))%></option>
								<%
					objrs.MoveNext
...
%>
            ...
            <tr>
              <td align="right" class="label" style="color:<%=color_texto%>;">&nbsp;</td>
              <td colspan="2">&nbsp;&nbsp;
                <select name="cd_status_croda_view" class="select" id="cd_status_croda_view" style="width:250px;" onchange="alteraQuantCampos(this);">
                  <option value="" selected><%= tx_lang_004094(cd_lang)%></option>                  
                  <%
                  'seleciona todos os eventos referentes a sub-&aacute;rea
	            ...
	            objrs.Open sql, objcnn, adOpenStatic, adLockReadOnly, adCmdText
                Do While Not objrs.Eof
                if cd_status_croda_view = objrs("CODIGO") Then selected = "selected" Else selected = ""
%>
									<option value="<%= objrs("CODIGO")%>" <%=selected %>><%= Trim(objrs("DESCRICAO"))%></option>
								<%
					objrs.MoveNext
...
<tr>
            ...
            <tr align="center">
              ...
              <input class="editbox" type="text" name="hr_freq_ocorre" id="hr_freq_ocorre" style="width: 150px;" maxlength="23" <%If fnc_TestaVar(hr_freq_ocorre) Then %>value="<%=hr_freq_ocorre%>"<%Else%>value="00:01"<%End If %>>&nbsp;&nbsp;(separar os horários por <strong>";"</strong>) <br /><span style="margin-left:-40px;">(formato: <strong>HH:MM</strong>)</span>
                </td>
              </tr>
              </table>
              </td>
...
%>
          ...
          <input type="hidden" name="in_cd_celula_view" id="in_cd_celula_view" value="<%=in_cd_area_view %>">
          <input type="hidden" name="in_cd_local_des_view" id="in_cd_local_des_view" value="<%=in_cd_local_des_view %>">
          <input type="hidden" name="in_cd_transp_view" id="in_cd_transp_view" value="<%=in_cd_transp_view %>">
          <input type="hidden" name="in_cd_status_croda_view" id="in_cd_status_croda_view" value="<%=in_cd_status_croda_view %>">
          ...
          <input type="hidden" name="cd_local_des_view_aux" id="cd_local_des_view_aux" value="<%=cd_local_des_view_aux %>">   
          <input type="hidden" name="cd_transp_view_aux" id="cd_transp_view_aux" value="<%=cd_transp_view_aux %>">  
          <input type="hidden" name="cd_status_croda_view_aux" id="cd_status_croda_view_aux" value="<%=cd_status_croda_view_aux %>">    
