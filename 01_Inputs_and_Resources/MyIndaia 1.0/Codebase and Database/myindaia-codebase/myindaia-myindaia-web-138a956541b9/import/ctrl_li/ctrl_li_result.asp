...
<%
Session("sql_ctrl_li") = Null
If Not fnc_TestaVar(Session("sql_ctrl_li")) Then

sql_s = " SELECT R.*, " &_
"(SELECT TOP 1 MASTER.DBO.FN_CRIPROGRAFA(RTRIM(R.NR_PROCESSO) + ';' + CONVERT(VARCHAR, PD.NM_ARQUIVO)) "&_
                "  FROM BROKER.DBO.TPROCESSO_DOCTOS PD "&_
                "   WHERE PD.NR_PROCESSO = R.NR_PROCESSO "&_
                "     AND PD.IN_ATIVO = '1' "&_
                "     AND PD.CD_TIPO_DOCTO = 8) AS CRIPTOGRAFADO "
sql_f = sql_f & " FROM TREL_IMP_CTRL_LI_NOVO R " 
...
'Response.Redirect("teste.asp?var1="&sql_s&sql_f&"")
	...
	Session("sql_ctrl_li") = sql_s & sql_f & sql_w & sql_o
	'response.Write(Session("sql_ctrl_li"))
	'response.end
End If
Session("sql_pagina") = Session("sql_pagina") &"<br>"& Session("sql_pendencias") &"<br>"
With objcmd
  .CommandTimeOut = 9999
  .CommandText    = Session("sql_ctrl_li")
...
tx_path_file  = "/import/ctrl_li/ctrl_li_excel"
tx_path_excel = "/import/ctrl_li/ctrl_li_excel"
...
%>
	...
	If objrsv.Eof Then objrsv.MoveFirst
        str_docto= "<a href='/popup/ver_arquivos.asp?in_nota=1&cod=" & objrs.Fields.Item("CRIPTOGRAFADO").Value & "' target='_blank'><img src='/imagens/30/open.gif' border=0 alt='Ver Extrato(s) de LI'></a>"  
	'verifica se o relatório vai ser quebrado por grupo, cliente, area ou modalidade	
%>
					<tr height="15" bgcolor="<%= bgcolor%>" onMouseDown="setPointer(this, <%= nrec%>, 'click', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');" onMouseOver="setPointer(this, <%= nrec%>, 'over', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');" onMouseOut="setPointer(this, <%= nrec%>, 'out', '<%= bgcolor%>', '#CCFFCC', '#FFCC99');">
