               ...
               <%
				 ...
				 If Not Session("cargo_interno") = "1" Then
				   sql = " SELECT DISTINCT U.CD_USUARIO AS CODIGO, U.NM_USUARIO AS DESCRICAO "&_
						 ...
						 " INNER JOIN TPERMISSAO P (NOLOCK) ON P.CD_USUARIO = U.CD_USUARIO "&_
			...
			Select Case objrsv.Fields.Item("M10_CD_DADO").Value
				...
				If Session("cargo_interno") = "1" Then
					   Server.Execute("/selects/option_imp_fornecedor.txt")
						...
						If objrsx.State = adStateOpen Then objrsx.Close()
						sql="SELECT  DISTINCT TG.CD_GRUPO AS GRUPO, (SELECT COUNT(CD_EMPRESA) FROM TEMPRESA_EST_GRUPO WHERE CD_GRUPO = TG.CD_GRUPO) AS CONT_GRUPO "&_
				...
				If Session("cargo_interno") = "1" Then
					   Server.Execute("/selects/option_planejadores.txt")
						...
						If objrsx.State = adStateOpen Then objrsx.Close()
						sql="SELECT  DISTINCT TG.CD_GRUPO AS GRUPO, (SELECT COUNT(CD_PLANEJADOR) FROM TPLANEJADORES WHERE CD_GRUPO = TG.CD_GRUPO) AS CONT_GRUPO "&_
							"from       TPERMISSAO     TP "&_
							"INNER JOIN TEMPRESA_NAC   TE (NOLOCK) ON(TP.CD_CLIENTE = TE.CD_EMPRESA) "&_
							"INNER JOIN TGRUPO         TG (NOLOCK) ON(TE.CD_GRUPO=TG.CD_GRUPO) "&_
							"WHERE  TP.CD_USUARIO='"& Session("cd_usuario") &"' "
				...
				case "0126","0297" 'IMPORTADOR
					...
					If objrsx.State = adStateOpen Then objrsx.Close()
						sql="SELECT  DISTINCT TG.CD_GRUPO AS GRUPO "&_
							"from       TPERMISSAO     TP "&_
							"INNER JOIN TEMPRESA_NAC   TE (NOLOCK) ON(TP.CD_CLIENTE = TE.CD_EMPRESA) "&_
							"INNER JOIN TGRUPO         TG (NOLOCK) ON(TE.CD_GRUPO=TG.CD_GRUPO) "&_
							"WHERE  TP.CD_USUARIO='"& Session("cd_usuario") &"' "
