...
<%
'---------------------------------------------------------------------------
'Sistema MyIndaiá WEB - Controle de Licensa de Importação
'Arquivo de Script e HTML: Busca de processos com utilização de filtros
'
...
If tx_erro <> "" Then
  ...
  Response.Redirect("liantes_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If
...
If in_dt_null or fnc_TestaVar(cd_evento) Then
  ...
  'If Not fnc_TestaVar(cd_evento) Then cd_evento = "088"
...
With objcmd
  .CommandTimeOut = 9999
  .CommandText = "sp_rel_imp_liantes"
...
If nr_result > 0 Then
  ...
  str = "/import/liantes_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = tx_lang_A00002(cd_lang) &": "& tx_lang_A00029(cd_lang) &".\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/import/liantes_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
