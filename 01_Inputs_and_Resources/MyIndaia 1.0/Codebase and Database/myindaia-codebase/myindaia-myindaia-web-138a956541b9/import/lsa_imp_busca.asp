	...
	Select Case objrsx.Fields.Item("M11_TP_FROM").Value 
		Case "0","2"
			If fnc_TestaVar(Request(ap_filtro)) Then
                If Not objrsx.Fields.Item("M11_IN_IGNORA_MASK").Value Then
...
If tx_erro <> "" Then
  ...
  Response.Redirect("lsa_imp_filtros.asp?cd_menu="&cd_menu&"&cd_subm="&cd_subm&"&cd_view="&cd_view&"&erro="&Server.URLEncode(tx_erro))
  Response.End
End If
...
With objcmd
  .CommandTimeOut = 9999
  .CommandText = "sp_rel_imp_ctrl_li"
...
If nr_result > 0 Then
  ...
  str = "/import/lsa_imp_result.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&nr_ident="& nr_ident &"&"& tx_usado
Else
  tx_erro = tx_lang_A00002(cd_lang) &": "& tx_lang_A00029(cd_lang) &".\n"& tx_usado
  'escreve a url para redirecionamento
  str = "/import/lsa_imp_filtros.asp?cd_menu="& cd_menu &"&cd_subm="& cd_subm &"&cd_view="& cd_view &"&tx_erro="& Server.URLEncode(tx_erro)
End If
