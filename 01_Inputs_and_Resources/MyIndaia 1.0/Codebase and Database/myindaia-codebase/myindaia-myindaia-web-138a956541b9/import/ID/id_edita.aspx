...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        "SUBSTRING(ID.NR_PROCESSO,5,15) AS NR_PROCESSO, ID.NR_PROCESSO AS NR_PROCESSO2, E.NM_EMBARCACAO, ID.NR_CONHECIMENTO, ID.ID_CLIENTE, ID.NR_PLANTA " & _
        ...
        If Request("acao") <> "lista" Then
           ...
           " CONVERT(VARCHAR,DT_AUTORIZACAO,103) AS DT_AUTORIZACAO, CD_USUARIO_AUTORIZACAO, U.NM_USUARIO AS NM_USUARIO_AUTORIZACAO, CD_RESPONSAVEL, CD_EMBARCACAO, NR_CONHECIMENTO, ID.CD_EMPRESA, ID.NR_PLANTA, TX_OBSERVACOES, ISNULL(CD_MODAL,'') AS CD_MODAL " & _
        ...
        End If
        If Request.QueryString("erro") = "2" Then
            Page.ClientScript.RegisterClientScriptBlock(Me.GetType(), "script", "alert('ATENÇÃO: LI integrada com sucesso!');location.href='id_edita.aspx?acao=lista';", True)
        End If
    ...
    Sub SalvarID(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        Dim txtNrPlanta As TextBox = frmEditaID.FindControl("txtNrPlanta")
        Dim dt_autorizacao, cd_autorizacao, in_autorizacao, cd_importador, cd_embarcacao, cd_usuario, nr_processo_des, nr_conhecimento, nr_planta As String
        ...
        If Trim(txtNrPlanta.Text) <> "" Then nr_planta = "'" & Trim(txtNrPlanta.Text) & "'" Else nr_planta = "NULL"
    ...
    Sub InserirID(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        ...
        Dim txtCdModal As TextBox = frmEditaID.FindControl("txtCdModal")
        Dim txtNrPlanta As TextBox = frmEditaID.FindControl("txtNrPlanta")
        Dim dt_autorizacao, cd_usuario, cd_importador, cd_embarcacao, nr_conhecimento, cd_modal, nr_planta As String
      ...
      " FROM TINSTRUCAO_DESEMBARACO (NOLOCK) WHERE YEAR(DT_CRIACAO) = YEAR(GETDATE())"
        ...
        sqlID.InsertCommand = "INSERT INTO TINSTRUCAO_DESEMBARACO (ID_CODIGO, CD_USUARIO, DT_CRIACAO, IN_AUTORIZADO, CD_CLIENTE, NR_PROCESSO, DT_AUTORIZACAO, CD_USUARIO_AUTORIZACAO, CD_RESPONSAVEL, CD_EMBARCACAO, NR_CONHECIMENTO, CD_EMPRESA, CD_MODAL, NR_PLANTA, TX_OBSERVACOES, IN_WEB)" & _
    ...
    Sub EnviarArquivoID(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim NomeArquivo, PrefixoArquivo, diretorio_docs, diretorio_docs2 As String
        diretorio_docs2 = "digitalizacao_novo"
        diretorio_docs = ""
        SqlAux = " SELECT BROKER.DBO.FN_PATH_DIGITALIZACAO('" + txtIDCodigo.Text + "',0) AS RESULTADO "
        Dim CnnAux As SqlConnection = New SqlConnection(Cnn)
        Dim CmdAux As SqlCommand = New SqlCommand(SqlAux, CnnAux)
        CnnAux.Open()
        Dim RsAux As SqlDataReader = CmdAux.ExecuteReader
        If RsAux.HasRows Then
            ...
            If diretorio_docs = "docs_old" Then
                diretorio_docs2 = "digitalizacao"
            End If
        End If
        RsAux.Close()
        CnnAux.Close()
        
        Dim Dir As DirectoryInfo = New DirectoryInfo("\\indaiaweb\" + diretorio_docs2 + "\TPROCESSO\" + txtIDCodigo.Text + "\")
        ...
        If PrefixoArquivo <> "" Then
            ...
            Dim Arq As FileInfo = New FileInfo("\\indaiaweb\" + diretorio_docs2 + "\TPROCESSO\" + txtIDCodigo.Text + "\" + NomeArquivo)
            If Arq.Exists Then
    ...
    Sub EnviarArquivoIDExterna(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim NomeArquivo, ExtensaoArquivo As String
        Dim prefixo As String
        
        If chkIDExterna.Checked Then
            prefixo = "LI_" + txtIDProcesso.Text + "_"
        Else
            prefixo = "DI_"
        End If
        
        NomeArquivo = fuIDExterna.FileName
        ExtensaoArquivo = GetFileExt(NomeArquivo)       
        If NomeArquivo <> "" Then
            If Trim(ExtensaoArquivo) <> "xlsx" Then
                Try
                    If Not Dir.Exists Then
                        Dir.Create()
                    End If
                    fuIDExterna.SaveAs(Dir.ToString + prefixo + Replace(NomeArquivo, " ", ""))
                    Response.Redirect("id_externa.asp?arquivo=" + prefixo + Replace(NomeArquivo, " ", "") + "&usuario=" + Session("usuario"))
...
<script type="text/javascript">
    ...
    function carregaDados() {
        ...
        if ('<%=Session("in_autorizado") %>' == '1') {
            document.getElementById('frmEditaID_txtObservacoes').readOnly = true;
        }
    ...
    function alteraCheck(objeto) {
        if (objeto.checked) {
            if (Trim(document.getElementById('frmEditaID_txtCdModal').value) != '' || 
				Trim(document.getElementById('frmEditaID_txtCdImportador').value) == '00500' ||  
				Trim(document.getElementById('frmEditaID_txtCdImportador').value) == '03185'
				) {
    ...
    function alteraGrid(ult_30, autorizados, minhas, id_codigo, resp, embarque) {
        var ult_30_valor, autorizados_valor, minhas_valor;
        if (id_codigo == 'ID') { id_codigo = ''; }
        if (resp == 'Responsável') { resp = ''; }
        if (embarque == 'Embarque') { embarque = ''; }
        if (ult_30.checked) {ult_30_valor = '1'; } else {ult_30_valor = '0'; }
        if (autorizados.checked) {autorizados_valor = '1'; } else {autorizados_valor = '0'; }
        if (minhas.checked) {minhas_valor = '1'; } else {minhas_valor = '0'; }
        var vCorpo = document.getElementById('grdPrincipal');
        var ajax = openAjax();
        ajax.open('POST', 'id_principal.aspx?ult_30=' + ult_30_valor + '&autorizados=' + autorizados_valor + '&minhas=' + minhas_valor + '&usuario=<%=Session("usuario")%>&id_codigo=' + id_codigo + '&resp=' + resp + '&embarque=' + embarque, true);
        ajax.onreadystatechange = function() {
            if (ajax.readyState == 1) {
                vCorpo.innerHTML = "<div style='position:absolute;top:40%;left:45%;'><center><img src='/imagens/icones/32x32/loading.gif'></center></div>";
            }
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4) {
                if (ajax.status == 200) {
                    var vResult = ajax.responseText;
                    if (vResult == "OK") {
                        window.open('http://www.myindaiaweb.com.br/arquivo/upload/InstrucaoDesembaraco<%=Request.QueryString("id_codigo")%>.xls')
                    } else {
    ...
    function mostraProcesso() {
        document.getElementById('txtIDProcesso').style.display = 'none';
        document.getElementById('txtIDProcesso').value = '';
        if (document.getElementById('chkIDExterna').checked) {
            document.getElementById('txtIDProcesso').style.display = 'inline';
            document.getElementById('txtIDProcesso').value = 'Processo';
            document.getElementById('txtIDProcesso').select();
        }     
    }
</script> 
...
<body  class="body" style="text-align:center;background-color:#f0f1f6;margin:0px 0px 0px 0px;width:100%;" onload="<% If Request("acao") = "lista" Then %>carregaLista();<%Else %>carregaDados();<%End If %>">
    ...
    <form id="frmID" runat="server" style="background-color:#f0f1f6;text-align:left;width:100%;text-align:center;margin-left:200px">
    ...
    <asp:Button ID="btnIDExterna" runat="server" Text="Enviar" CssClass="textbox" OnClick="EnviarArquivoIDExterna"  Width="55px" Height="20px" style="text-transform:none;font-size:9pt;font-weight:normal;background-color:#374BAE;color:#FFFFFF;font-family:Verdana;cursor:pointer;"/><br />
    <div style="width:376px;text-align:left;"><asp:CheckBox ID="chkIDExterna" runat="server" onclick="mostraProcesso();"/><asp:Label ID="lblIDLI" runat="server" Text="LI" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" CssClass="lbl" style="vertical-align:middle;"/>&nbsp;&nbsp;<asp:TextBox ID="txtIDProcesso" runat="server" CssClass="textbox" Width="140px" BackColor="#FFFFFF" style="display:none;" /></div>
    </div>
 ...
 <div id="divInfo" class="divInfo" style="overflow:hidden;background-color:#374BAE;height:10px;">
    ...
    <asp:TextBox ID="txtIDBusca" runat="server" CssClass="textbox" Width="120px" Text="ID" BackColor="#FFFFFF"  onfocus="this.select();" style="color:#979090;" onKeyDown="this.style.color='#000000';" onKeyUp="alteraGrid(document.getElementById('frmEditaID_cbUltimos'),document.getElementById('frmEditaID_cbAutorizados'),document.getElementById('frmEditaID_cbMinhasIds'), this.value, document.getElementById('frmEditaID_txtCdResponsavel').value, document.getElementById('frmEditaID_txtEmbarque').value);" onBlur="if(this.value == ''){ this.style.color = '#979090';this.value = 'ID';}"/>&nbsp;
    <asp:TextBox ID="txtCdResponsavel" runat="server" CssClass="textbox" Width="290px" Text="Responsável" BackColor="#FFFFFF"  onfocus="this.select();" style="color:#979090;"  onKeyDown="this.style.color='#000000';" onKeyUp="alteraGrid(document.getElementById('frmEditaID_cbUltimos'),document.getElementById('frmEditaID_cbAutorizados'), document.getElementById('frmEditaID_cbMinhasIds'), document.getElementById('frmEditaID_txtIDBusca').value, document.getElementById('frmEditaID_txtCdResponsavel').value, document.getElementById('frmEditaID_txtEmbarque').value);" onBlur="if(this.value == ''){this.style.color = '#979090';this.value = 'Responsável';}"/>&nbsp;
...
<AlternatingRowStyle BackColor="#DEE6EF"></AlternatingRowStyle>
    ...
    <asp:CheckBox ID="cbAutorizados" runat="server" Text="Exibir autorizados"  onclick="alteraGrid(document.getElementById('frmEditaID_cbUltimos'),this,document.getElementById('frmEditaID_cbMinhasIds'),document.getElementById('frmEditaID_txtIDBusca').value, document.getElementById('frmEditaID_txtCdResponsavel').value, document.getElementById('frmEditaID_txtEmbarque').value);"/>&nbsp;&nbsp;
    <asp:CheckBox ID="cbMinhasIds" runat="server" Text="Apenas Minhas IDs" onclick="alteraGrid(document.getElementById('frmEditaID_cbUltimos'),document.getElementById('frmEditaID_cbAutorizados'),document.getElementById('frmEditaID_cbMinhasIds'),document.getElementById('frmEditaID_txtIDBusca').value, document.getElementById('frmEditaID_txtCdResponsavel').value, document.getElementById('frmEditaID_txtEmbarque').value);"/>
    </div>
    </div>
    <div id="divDadosID" style="display:none;text-align:left;">
        ...
        <asp:Label ID="lblUsuario" runat="server" Text="Responsável" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="195px" CssClass="lbl"/>&nbsp;
        ...
        <asp:TextBox ID="txtNrConhecimento" runat="server" CssClass="textbox" Width="180px" Text='<%# Bind("NR_CONHECIMENTO") %>'/>&nbsp;&nbsp;&nbsp;
        <asp:Label ID="lblPlanta" runat="server" Text="Planta" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="55px" CssClass="lbl"/>&nbsp;
        <asp:TextBox ID="txtNrPlanta" runat="server" CssClass="textbox" Width="80px" Text='<%# Bind("NR_PLANTA") %>'/>&nbsp;&nbsp;&nbsp;
        <asp:CheckBox ID="cbAutorizado" runat="server" Font-Size="9pt" CssClass="lbl" onClick="alteraCheck(this);"/><asp:Label ID="lblAutorizado" runat="server" Text="Autorizado" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="70px" CssClass="lbl"/>&nbsp;<br /><br />
    ...
    <div id="divDadosID" style="display:none;text-align:left;">
        ...
        <asp:Label ID="lblPlanta" runat="server" Text="Planta" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="55px" CssClass="lbl"/>&nbsp;
        <asp:TextBox ID="txtNrPlanta" runat="server" CssClass="textbox" Width="80px" Text='<%# Bind("NR_PLANTA") %>'/>&nbsp;&nbsp;&nbsp;
        <asp:CheckBox ID="cbAutorizado" runat="server" Font-Size="9pt" CssClass="lbl" onClick="alteraCheck(this);" Checked='<%# Container.DataItem("IN_AUTORIZADO") %>'/><asp:Label ID="lblAutorizado" runat="server" Text="Autorizado" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="70px" CssClass="lbl"/>&nbsp;<br /><br />
        <asp:Label ID="lblDtAutorizacao" runat="server" Text="Data Autorização" Font-Names="Verdana" Font-Size="9pt" Font-Bold="false" Width="195px" CssClass="lbl"/>&nbsp;
...
<AlternatingRowStyle BackColor="#DEE6EF"></AlternatingRowStyle>
            ...
            <Columns>
                ...
                <ItemTemplate>
                        <asp:HyperLink ID="lb_nm_arquivo" runat="server" EnableTheming="True" Target="_blank"
                            ...
                            NavigateUrl='<%# Eval("PROCESSO", "/"&Trim(Eval("RESULTADO"))&"/{0}/"&Trim(Eval("ARQUIVO"))) %>' class="nounderline" ItemStyle-Width="130px"></asp:HyperLink>
                    </ItemTemplate>
                </asp:TemplateField>
    ...
    ConnectionString="Data Source=INDAIA10;Initial Catalog=BROKER;Persist Security Info=True;User ID=sa;Password=123" 
    SelectCommand="SELECT PD.NM_ARQUIVO AS ARQUIVO, PD.NM_DESCRICAO AS DESCRICAO, TDD.NM_TIPO_DOCTO AS TIPO, CONVERT(VARCHAR,PD.DT_CRIACAO,103) + ' ' + CONVERT(VARCHAR,PD.DT_CRIACAO,108)  AS CRIACAO, DBO.FN_PATH_DIGITALIZACAO(@ID_CODIGO,'0') AS RESULTADO, PD.NR_PROCESSO AS PROCESSO 
