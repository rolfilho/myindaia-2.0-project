if ((window.location.toString().indexOf("index") >= 0) && (localStorage._cs) ) {
	...
	function BuscaQtdProcessosEmAndamento(){

		var url = servidor+':'+porta+'/indaia/api/Service/ProcessosEmAndamento';
		...
		request.onreadystatechange = function() {
		  if (request.readyState === XMLHttpRequest.DONE) {
			if (request.status === 200) {
			  ...
			  carregaQtdProcessosEmAndamento(results);
			} else {
	...
	function carregaQtdProcessosEmAndamento(data) {
