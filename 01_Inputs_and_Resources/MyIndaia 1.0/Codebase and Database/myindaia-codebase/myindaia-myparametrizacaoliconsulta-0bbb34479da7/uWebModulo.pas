...
begin
  if ComponentCount > 0 then
  begin
     DownloadArquivos := FindWindow(nil, 'Download de Arquivos');

     if DownloadArquivos = 0 then
     begin
       SalvarComo := FindWindow(nil, 'Salvar como');
       if SalvarComo = 0 then
        SalvarComo := FindWindow(nil, 'Save As');
     end;

     if DownloadArquivos <> 0 then
     begin
       SetForegroundWindow(DownloadArquivos);
       ListarComponentes(DownloadArquivos);
       Sleep(2000);
     ...
     if SalvarComo <> 0 then
     begin
        FFileName := StringReplace(ExtractFilePath(Application.ExeName)  + 'Downloads\ExtratoLI.pdf', '\\', '\', [rfReplaceAll]);
        ...
        SetForegroundWindow(SalvarComo);
        ListarComponentes(SalvarComo);
        Sleep(1000);
        SendText(LocalizarComponente('Edit', '').Hnd, FFileName);
        SendText(LocalizarComponente('Edit', '').Hnd, FFileName);
        Sleep(1000);
        SendClick(LocalizarComponente('', 'Salvar').Hnd);
        SendClick(LocalizarComponente('', 'Salvar').Hnd);
        SendClick(LocalizarComponente('Edit', '').Hnd);
        Sleep(2000);
...
procedure Tfrm_Web.ScrollFrame;
var
  IEHandle: HWND;
begin
