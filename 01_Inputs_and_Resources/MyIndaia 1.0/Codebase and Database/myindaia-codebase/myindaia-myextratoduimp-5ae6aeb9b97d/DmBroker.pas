...
type
  ...
  TdmdBroker = class(TDataModule)
    ...
    qryBroker: TFDQuery;
    qryDuimpRegistro: TFDQuery;
    cdsDuimpRegistro: TClientDataSet;
    cdsDuimpRegistroNR_PROCESSO: TStringField;
    cdsDuimpRegistroCD_DUIMP: TStringField;
    cdsDuimpRegistroQT_TOTAL: TIntegerField;
    dspDuimpRegistro: TDataSetProvider;
    qryDuimpCanal: TFDQuery;
    qryDuimpCanalNR_PROCESSO: TStringField;
    qryDuimpCanalCD_DUIMP: TStringField;
    qryDuimpCanalQT_TOTAL: TIntegerField;
    cdsDuimpCanal: TClientDataSet;
    cdsDuimpCanalNR_PROCESSO: TStringField;
    cdsDuimpCanalCD_DUIMP: TStringField;
    cdsDuimpCanalQT_TOTAL: TIntegerField;
    dspDuimpCanal: TDataSetProvider;
    qryDuimpRegistroNR_PROCESSO: TStringField;
    qryDuimpRegistroCD_DUIMP: TStringField;
    qryDuimpRegistroQT_TOTAL: TIntegerField;
  private
    ...
    procedure AtualizarEvento(const NumeroProcesso, CodigoEvento: string);
  public
...
begin
  try
    ...
    //  dbBroker.Connected := true;
      ...
      TabelaId := 1652005462;
      NovoNomeArquivo := GetNomeArquivo(NR_Processo, 9, TabelaId, DigPath) + ExtractFileExt(NomeArquivo);
      ...
      if Incluir then
      begin
        IncluirArquivo(NR_Processo, NovoNomeArquivo, Descricao, 9, TabelaId);
        AtualizarEvento(NR_Processo, '636');
        Log('Arquivo enviado para o Digitalizar: ' + NovoNomeArquivo, nil);
        if FileExists(DigPath + 'ExtratoDuimp.pdf') then
          DeleteFile(PWideChar(DigPath + 'ExtratoDuimp.pdf'));
      end
      else
...
procedure TdmdBroker.AtualizarEvento(const NumeroProcesso, CodigoEvento: string);
begin
  qryBroker.Close;
  qryBroker.SQL.Text :=
      'UPDATE TFOLLOWUP ' +
  ...
  qryBroker.ParamByName('NR_PROCESSO').AsString := NumeroProcesso;
  qryBroker.ParamByName('CodigoEvento').AsString := CodigoEvento;
  qryBroker.ExecSQL;
end;

procedure TdmdBroker.MontaLista;
begin
  cdsDuimpRegistro.Close;
  qryDuimpRegistro.Close;
  cdsDuimpRegistro.Open;
end;
