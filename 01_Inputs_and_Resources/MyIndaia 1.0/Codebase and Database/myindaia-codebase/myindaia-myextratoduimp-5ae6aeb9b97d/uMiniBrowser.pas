...
uses
  ...
  uCEFConstants, uCEFWinControl, uCEFChromiumCore, uCEFFileDialogInfo, System.DateUtils,
  Vcl.Buttons;
...
const
  ...
  MINIBROWSER_HOMEPAGE = 'https://portalunico.siscomex.gov.br/portal/';
...
type
  ...
  TMiniBrowserFrm = class(TForm)
    ...
    btnSettings: TButton;
    Button1: TButton;
    tmrClickExtrato: TTimer;
    tmrExtratoDialog: TTimer;
    ...
    procedure tmrClickGridTimer(Sender: TObject);
    procedure tmrIntegracaoTimer(Sender: TObject);
    procedure btnProcessarClick(Sender: TObject);
    procedure btnSettingsClick(Sender: TObject);
    procedure Button1Click(Sender: TObject);
    procedure tmrClickExtratoTimer(Sender: TObject);
    procedure tmrExtratoDialogTimer(Sender: TObject);
  private
...
uses
  ...
  uCEFWindowInfoWrapper, uCEFTaskManager, DmBroker, uPreferencias, Settings, Utils,
  System.AnsiStrings, System.Masks;
...
procedure TMiniBrowserFrm.btnProcessarClick(Sender: TObject);
begin
  LimpaPastaExtrato;
  LimpaLog;
  ...
  dmdBroker.MontaLista;
  if not dmdBroker.cdsDuimpRegistro.IsEmpty then
  begin
    log(InttoStr(dmdBroker.cdsDuimpRegistro.RecNo) +'/' + dmdBroker.cdsDuimpRegistroQT_TOTAL.AsString, logar);
    log('Processo : ' + dmdBroker.cdsDuimpRegistroNR_PROCESSO.AsString + ' DUIMP : ' + dmdBroker.cdsDuimpRegistroCD_DUIMP.AsString, logar);
    dmdBroker.cdsDuimpRegistro.first;
    Chromium1.LoadUrl(MINIBROWSER_HOMEPAGE);
  end
  else
  begin
    log('Lista vazia', logar);
...
procedure TMiniBrowserFrm.btnSettingsClick(Sender: TObject);
var
...
procedure Esperar(segundos : integer = 1);
begin
...
begin
  ...
  if (ansicomparestr(browser.MainFrame.url,'https://portalunico.siscomex.gov.br/portal/#/') = 0 ) then
  begin
    Chromium1.LoadURL('https://portalunico.siscomex.gov.br/duimp/#/consultar-duimp');
  end;

//  if (ansicomparestr(browser.MainFrame.url,'https://portalunico.siscomex.gov.br/due/x/#/consulta/consulta-filtro') = 0 ) then
//  begin
//    Application.ProcessMessages;
//    sleep(2000);
//    Application.ProcessMessages;
//
//    browser.MainFrame.ExecuteJavaScript('var vnmdue = document.getElementById("nrDue");' +
//                                        'console.log("PGConsDUE|" + vnmdue)', 'about:blank', 0);
//  end;

  if ContainsText(browser.MainFrame.Url,
                  'https://portalunico.siscomex.gov.br/duimp/v2/#/detalhar-duimp') then
  begin
    tmrClickExtrato.Enabled := true;
  end;
...
procedure TMiniBrowserFrm.tmrIntegracaoTimer(Sender: TObject);
var
  Horas: Integer;
  Minutos: Integer;
  Segundos: Integer;
...
begin
  ...
  StatusBar1.Panels.Items[4].Text :=  FormatFloat('00', Horas) + ':' +
  ...
  begin
    ...
    if (LocalSettings.InicioIntegracao <> '  :  ') and (LocalSettings.InicioIntegracao <>'')  then
      InicioHora := StrToTime(LocalSettings.InicioIntegracao)
    else
      InicioHora := StrToTime('08:10');

    if (LocalSettings.FimIntegracao <> '  :  ') and (LocalSettings.FimIntegracao <>'')  then
      FimHora := StrToTime(LocalSettings.FimIntegracao)
    else
      FimHora := StrToTime('18:15');
...
begin
  ...
  TempName     := dmdBroker.cdsDuimpRegistroNR_PROCESSO.AsString +  '.pdf';
  TempFullPath := IncludeTrailingPathDelimiter(GetLocalPath+'download') + TempName;
  log('Baixando arquivo : ' + TempFullPath , logar);
...
procedure TMiniBrowserFrm.Chromium1ConsoleMessage(Sender: TObject;
  ...
  procedure executar(script : string);
  begin
    browser.MainFrame.ExecuteJavaScript(script, 'about:blank', 0);
  end;

  procedure PreencheNumeroDuimp;
  var
    Js   : string;
    Duimp: string;
  begin
    Duimp := dmdBroker.cdsDuimpRegistroCD_DUIMP.AsString;

    Js :=
      '(function(){' + sLineBreak +
      '  function trigger(el, name, opts){' + sLineBreak +
      '    var ev;' + sLineBreak +
      '    opts = opts || {bubbles:true, cancelable:true};' + sLineBreak +
      '    try {' + sLineBreak +
      '      ev = new Event(name, opts);' + sLineBreak +
      '    } catch(e) {' + sLineBreak +
      '      ev = document.createEvent("Event"); ev.initEvent(name, true, true);' + sLineBreak +
      '    }' + sLineBreak +
      '    el.dispatchEvent(ev);' + sLineBreak +
      '  }' + sLineBreak +
      '' + sLineBreak +
      '  function setNativeValue(el, value){' + sLineBreak +
      '    var setter = Object.getOwnPropertyDescriptor(el.__proto__, "value")' + sLineBreak +
      '               || Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value");' + sLineBreak +
      '    setter.set.call(el, value);' + sLineBreak +
      '  }' + sLineBreak +
      '' + sLineBreak +
      '  var el = document.getElementById("inputDuimp") || document.querySelector("input#inputDuimp, input[name=inputDuimp]");' + sLineBreak +
      '  if(!el){ console.warn("inputDuimp nao encontrado"); return; }' + sLineBreak +
      '' + sLineBreak +
      '  // 1) Simula clique/foco do usuario (muitos mascaradores s� ativam aqui)' + sLineBreak +
      '  el.scrollIntoView({block:"center"});' + sLineBreak +
      '  el.click();' + sLineBreak +
      '  el.focus();' + sLineBreak +
      '  trigger(el, "focus");' + sLineBreak +
      '' + sLineBreak +
      '  // 2) Limpa e digita via setter nativo + eventos' + sLineBreak +
      '  setNativeValue(el, "");' + sLineBreak +
      '  trigger(el, "input");' + sLineBreak +
      '' + sLineBreak +
      '  trigger(el, "keydown");' + sLineBreak +
      '  setNativeValue(el, ' + QuotedStr(Duimp) + ');' + sLineBreak +
      '  trigger(el, "input");' + sLineBreak +
      '  trigger(el, "keyup");' + sLineBreak +
      '  trigger(el, "change");' + sLineBreak +
      '' + sLineBreak +
      '  // 3) Alguns validadores/mascaras s� �fixam� no blur' + sLineBreak +
      '  el.blur();' + sLineBreak +
      '  trigger(el, "blur");' + sLineBreak +
      '' + sLineBreak +
      '  // 4) Clique no botao de consultar (id pucx-consultar) (ajuste o texto se necess�rio)' + sLineBreak +
      '  // d� um pequeno tempo pro framework reagir e ent�o clica' + sLineBreak +
      '  setTimeout(function(){' + sLineBreak +
      '    var btn = document.getElementById("pucx-consultar");' + sLineBreak +
      '    if(!btn){ console.warn("botao pucx-consultar nao encontrado"); return; }' + sLineBreak +
      '    btn.scrollIntoView({block:"center"});' + sLineBreak +
  ...
  var
    Chave, Valor, Valor2: string;
begin
  //Inteceptar as mensagens de log no console
  if AnsiContainsText(message_, '|') then
  begin
    Chave  := TokenN(message_, 1);
    Valor  := TokenN(message_, 2);
    Valor2 := TokenN(message_, 3);

    if Chave.Equals('PgIni') then
    begin
      {est� na pagina que contem o bot�o importador exportador, mas o captcha nao esta aparecendo
       clicar nos bot�es para mostrar o captcha
      }
      if   (Valor <> 'null')  // grp_impexp = bot�o importador exportador
       and (Valor2 = 'null')  // recaptcha-accessible-status = captcha
      then
      begin
        Esperar;
        executar('document.getElementById(''grp_impexp'').click()');
        Esperar;
        executar('document.getElementById(''prf_impexp'').click()');
        Esperar;
        executar('document.body.scrollTop = document.body.offsetHeight');
      end;
    end
    else
    if Chave.Equals('PGConsDUIMP') then
    begin
      if (Valor <> 'null') then
      begin
        Esperar;
        PreencheNumeroDuimp;
        esperar(3);
        tmrClickGrid.Enabled := true;
      end;
    end
    else
    if Chave.Equals('PgGrdDuimp') then
    begin
      if Valor = '1' then
      begin
        Esperar(3);
        executar(
          '(function(){' + sLineBreak +
          '  var divWithClick = document.querySelector(''div[ng-click*="abrirDuimp"]'');' + sLineBreak +
          '  if(!divWithClick){ console.warn("Link DUIMP nao encontrado"); return; }' + sLineBreak +
          '  divWithClick.scrollIntoView({block:"center"});' + sLineBreak +
          '  divWithClick.focus();' + sLineBreak +
          '  var ev = new MouseEvent("click", {bubbles:true, cancelable:true, view:window});' + sLineBreak +
          '  divWithClick.dispatchEvent(ev);' + sLineBreak +
          '})();'
        );

        tmrClickGrid.Enabled := False;
//        tmrClickExtrato.Enabled := true;
      end;
    end
    else
    if Chave.Equals('BtnExtratoDuimp') then
    begin
      if Valor = '1' then
      begin
        executar(
          '(function(){' + sLineBreak +
          '  var btn = document.querySelector(''p-button[label="Gerar Extrato"] button'') || ' + sLineBreak +
          '    Array.from(document.querySelectorAll("button, a, [role=''button''], span, p-button")).find(function(el){' + sLineBreak +
          '      var t = (el.innerText || el.textContent || "").trim().toLowerCase();' + sLineBreak +
          '      return t.indexOf("gerar extrato") >= 0;' + sLineBreak +
          '    });' + sLineBreak +
          '  if(!btn){ console.warn("Botao Gerar Extrato nao encontrado"); return; }' + sLineBreak +
          '  btn.scrollIntoView({block:"center"});' + sLineBreak +
          '  btn.click();' + sLineBreak +
          '})();'
        );
        tmrClickExtrato.Enabled := False;
        tmrExtratoDialog.Enabled := True;
      end;
    end
    else
    if Chave.Equals('FaixaDuimp') then
    begin
      log('Faixa de itens DUIMP: ' + Valor, logar);
    end;
  end;
end;
...
begin
  ...
  if downloadItem.IsComplete then
  begin
    dmdBroker.GravaArquivo(IncludeTrailingPathDelimiter(GetLocalPath + 'download'),
                           ExtractFileName(downloadItem.FullPath),
                           dmdBroker.cdsDuimpRegistroNR_PROCESSO.AsString, 'Extrato Siscomex');

    dmdBroker.cdsDuimpRegistro.Next;
    if not dmdBroker.cdsDuimpRegistro.Eof then
    begin
      sleep(2000);
      log(InttoStr(dmdBroker.cdsDuimpRegistro.RecNo) +'/' + dmdBroker.cdsDuimpRegistroQT_TOTAL.AsString, logar);
      log('Processo : ' + dmdBroker.cdsDuimpRegistroNR_PROCESSO.AsString + ' DUIMP : ' + dmdBroker.cdsDuimpRegistroCD_DUIMP.AsString, logar);
...
begin
  ...
  if frame.IsMain then
  begin
    // For�a qualquer window.open a abrir na mesma aba dentro do app
    ...
    if ansicomparestr(browser.MainFrame.url,'https://portalunico.siscomex.gov.br/duimp/#/consultar-duimp') = 0 then
    begin
      Application.ProcessMessages;
      sleep(2000);
      Application.ProcessMessages;

      browser.MainFrame.ExecuteJavaScript('var vnmduimp = document.getElementById("inputDuimp");' +
                                          'console.log("PGConsDUIMP|" + vnmduimp)', 'about:blank', 0);
    end;
  end;
end;
...
begin
  ...
  if (title <> '') then
    caption := 'Extrato Duimp - ' + title
   else
    caption := 'Extrato Duimp';
end;
...
begin
  ...
  dmdBroker := TdmdBroker.Create(Application);
...
//  Chromium1.DefaultURL       := MINIBROWSER_HOMEPAGE;
  ...
  if not DirectoryExists(GetLocalPath+'download') then
    ForceDirectories(GetLocalPath+'download');
  if not DirectoryExists(GetLocalPath+'log') then
    ForceDirectories(GetLocalPath+'log');

  Log('Iniciado.', Logar);
  FTempoRestante := 1 *  60000;
...
procedure TMiniBrowserFrm.tmrClickExtratoTimer(Sender: TObject);
begin
    Application.ProcessMessages;
    sleep(1000);
    Application.ProcessMessages;
    ...
    Chromium1.Browser.MainFrame.ExecuteJavaScript(
      '(function(){' + sLineBreak +
      '  var btnDuimp = document.querySelector(''p-button[label="Gerar Extrato"] button'') || ' + sLineBreak +
      '    Array.from(document.querySelectorAll(''button, a, [role="button"], span, p-button'')).find(function(el){' + sLineBreak +
      '      var t = (el.innerText || el.textContent || "").trim().toLowerCase();' + sLineBreak +
      '      return t.indexOf("gerar extrato") >= 0;' + sLineBreak +
      '    });' + sLineBreak +
      '  if(btnDuimp){ console.log("BtnExtratoDuimp|1"); return; }' + sLineBreak +
      '  var vBtnExtrato = document.querySelector(''p-button[label="Gerar PDF Simplificado"] button'');' + sLineBreak +
      '  console.log("BtnExtrato|" + (vBtnExtrato ? "1" : "0"));' + sLineBreak +
      '})();',
      'about:blank',
      0
    );
...
procedure TMiniBrowserFrm.tmrExtratoDialogTimer(Sender: TObject);
begin
  Chromium1.Browser.MainFrame.ExecuteJavaScript(
    '(function(){' + sLineBreak +
    '  var dlg   = document.querySelector(''div[role="dialog"] .pucx-modal-title'');' + sLineBreak +
    '  var faixa = document.getElementById("faixa");' + sLineBreak +
    '  var btnOk = document.querySelector(''div[role="dialog"] p-button[type="submit"][label="Gerar Extrato"] button[type="submit"]'');' + sLineBreak +
    '  if(!(dlg && faixa && btnOk)) return;' + sLineBreak +
    '' + sLineBreak +
    '  // 1) Tentar obter o total de itens pela paginação (ex: "1 - 10 de 14 itens")' + sLineBreak +
    '  var spanPag = document.querySelector(''span.p-paginator-current'');' + sLineBreak +
    '  var total = null;' + sLineBreak +
    '' + sLineBreak +
    '  if(spanPag && spanPag.textContent){' + sLineBreak +
    '    var txt = spanPag.textContent.replace(/\s+/g, '' '').trim();' + sLineBreak +
    '    var m = txt.match(/de\s+(\d+)\s+itens/i);' + sLineBreak +
    '    if(m) total = parseInt(m[1], 10);' + sLineBreak +
    '  }' + sLineBreak +
    '' + sLineBreak +
    '  // 2) Se não tiver paginação, contar as linhas da grid' + sLineBreak +
    '  if(total === null){' + sLineBreak +
    '    var tbody = document.querySelector(''table.p-datatable-table tbody.p-datatable-tbody'');' + sLineBreak +
    '    if(tbody){' + sLineBreak +
    '      var linhas = Array.from(tbody.querySelectorAll(''tr.p-selectable-row''));' + sLineBreak +
    '      total = linhas.length;' + sLineBreak +
    '    }' + sLineBreak +
    '  }' + sLineBreak +
    '' + sLineBreak +
    '  if(!total || isNaN(total) || total <= 0){' + sLineBreak +
    '    total = 1;' + sLineBreak +
    '  }' + sLineBreak +
    '' + sLineBreak +
    '  var faixaStr = ''1-'' + total;' + sLineBreak +
    '  faixa.value = faixaStr;' + sLineBreak +
    '  faixa.dispatchEvent(new Event("input", {bubbles:true}));' + sLineBreak +
    '  faixa.dispatchEvent(new Event("change", {bubbles:true}));' + sLineBreak +
    '  faixa.dispatchEvent(new Event("blur", {bubbles:true}));' + sLineBreak +
    '' + sLineBreak +
    '  // Logar no console para o Delphi registrar no arquivo de log' + sLineBreak +
    '  console.log("FaixaDuimp|" + faixaStr);' + sLineBreak +
    '' + sLineBreak +
    '  btnOk.scrollIntoView({block:"center"});' + sLineBreak +
    '  btnOk.click();' + sLineBreak +
    '})();',
    'about:blank',
    0
  );

  tmrExtratoDialog.Enabled := False;
end;

procedure TMiniBrowserFrm.tmrClickGridTimer(Sender: TObject);
begin
  Chromium1.Browser.MainFrame.ExecuteJavaScript(
    '(function(){' + sLineBreak +
    '  var lnk = document.querySelector(''.ui-grid-cell div[ng-click*="abrirDuimp"] a.ng-binding'') || document.querySelector(''.ui-grid-cell a.ng-binding'');' + sLineBreak +
    '  if(lnk){ console.log("PgGrdDuimp|1"); return; }' + sLineBreak +
    '})();',
    'about:blank',
    0
  );
...
procedure TMiniBrowserFrm.LimpaPastaExtrato;
var
  pesquisa: TSearchRec;
...
begin
  ...
  try
    if FindFirst(IncludeTrailingPathDelimiter(GetLocalPath+'download')+'*.pdf', faAnyFile-faDirectory, pesquisa) = 0 then
    begin
      repeat
    ...
    begin
      Deletefile(IncludeTrailingPathDelimiter(GetLocalPath+'download')+ ListaArquivos[i]);
    end;
  finally
    ListaArquivos.free;
  end;
...
procedure TMiniBrowserFrm.LimpaLog;
var
  pesquisa: TSearchRec;
  NomeArquivo : String;
...
begin
  ...
  try
    if FindFirst(IncludeTrailingPathDelimiter(GetLocalPath) + 'log\'+ '*.log', faAnyFile-faDirectory, pesquisa) = 0 then
    begin
      repeat
        if (pesquisa.Attr <> faDirectory) then
        begin
    ...
    begin
      ...
      //se for mais antigo que 4 dias, exclui
      if sDataArquivo < sDataLimite then
      begin
        Deletefile(IncludeTrailingPathDelimiter(GetLocalPath) + 'log\'+ ListaArquivosAntigos[i]);
      end;
    end;
...
Procedure TMiniBrowserFrm.Logar;
begin
