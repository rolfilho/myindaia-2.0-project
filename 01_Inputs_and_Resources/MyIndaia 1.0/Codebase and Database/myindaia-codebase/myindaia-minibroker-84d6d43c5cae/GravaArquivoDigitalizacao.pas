...
uses
  ...
  Dialogs, System.Generics.Collections , DateUtils,  Db,
  ...
  function GravaArquivo(var Conexao : TFDConnection;
  const Arquivo, NR_Processo, Descricao: string; TipoDocumento : integer = 56; Substituir : Boolean=false) :string;
  ...
  function VerificaArquivoJaDigitalizado(AProcesso : string; TipoDocumento : Integer; Descricao : String): String;
...
begin
  try
    ...
    NomeArquivo := '';
    if Substituir then
      NomeArquivo := VerificaArquivoJaDigitalizado(NR_Processo, TipoDocumento , Descricao);

    if trim(NomeArquivo) = '' then
      NomeArquivo := GetNomeArquivo(NR_Processo, TipoDocumento, TabelaId,
                                    DigPath, ExtractFileExt(Arquivo)) + ExtractFileExt(Arquivo);
    ...
    begin
      ...
      result:= 'Arquivo enviado para a Digitalização: ' + NomeArquivo;
    end
    else
      result:= 'Arquivo substituí na Digitalização ' + NomeArquivo;
...
begin
  ...
  try
    qryBroker.Connection := dbBroker;
    qryBroker.Close;
    qryBroker.SQL.Text := 'INSERT INTO BROKER.DBO.TPROCESSO_DOCTOS '+
...
begin
  ...
  try
    ...
    qryBroker.sql.text := ' SELECT NM_PREFIXO_ARQ                  '+
                          '   FROM BROKER.DBO.TTP_DOCTO_DIGITALIZADO (NOLOCK) '+
    ...
    qryBroker.sql.text := ' SELECT ISNULL(MAX(BROKER.DBO.FN_DOCTO_NUMERO(NM_ARQUIVO, :NM_PREFIXO_1 )), 0) AS ORDEM ' +
                          '   FROM BROKER.DBO.TPROCESSO_DOCTOS                                                         ' +
...
begin
  ...
  try
    ...
    qryBroker.Close;
    qryBroker.sql.text := '  SELECT TOP 1 NR_PROCESSO, NM_ARQUIVO    '+
                          '    FROM BROKER.DBO.TPROCESSO_DOCTOS (NOLOCK)        '+
                          '   WHERE NR_PROCESSO   = :NR_PROCESSO     '+
                          '     AND CD_TIPO_DOCTO = :CD_TIPO_DOCTO   '+
                          '     AND NM_DESCRICAO  = :NM_DESCRICAO    ';
    qryBroker.ParamByName('NR_PROCESSO').asString    := AProcesso;
    qryBroker.ParamByName('NM_DESCRICAO').asString   := Descricao;
    qryBroker.ParamByName('CD_TIPO_DOCTO').asInteger := TipoDocumento;
    qryBroker.open;
    if not qryBroker.IsEmpty then
      result := qryBroker.FieldByName('NM_ARQUIVO').AsString;
