...
begin
  ...
  begin
    log(InttoStr(dmdBroker.cdsStatus.RecNo) +'/' + dmdBroker.cdsStatusQT_TOTAL.AsString, logar);
    log('Processo : ' + dmdBroker.cdsStatusNR_PROCESSO.AsString + ' DUE : ' + dmdBroker.cdsStatusCD_DUE.AsString, logar);
    dmdBroker.cdsStatus.first;
...
begin
  ...
  begin
    Chromium1.LoadURL('https://portalunico.siscomex.gov.br/due/x/#/consulta/consulta-filtro');
  end;
  ...
  if (ContainsText(browser.MainFrame.Url, 'https://portalunico.siscomex.gov.br/due/x/#/dueCompleta')) then
  begin
...
begin
  ...
  TempName     := dmdBroker.cdsStatusNR_PROCESSO.AsString +  '.pdf';
...
procedure TMiniBrowserFrm.Chromium1ConsoleMessage(Sender: TObject;
  ...
  procedure PreencheNumeroDue;
  var
    Js: string;
    Due: string;
  begin
    Due := dmdbroker.cdsStatusCD_DUE.AsString;
    ...
    Js :=
      ...
      '  }' + sLineBreak +
      '' + sLineBreak +
      '  var el = document.getElementById("nrDue") || document.querySelector("input#nrDue, input[name=nrDue]");' + sLineBreak +
      '  if(!el){ console.warn("nrDue nao encontrado"); return; }' + sLineBreak +
      '' + sLineBreak +
      '  // 1) Simula clique/foco do usuario (muitos mascaradores só ativam aqui)' + sLineBreak +
      ...
      '  setNativeValue(el, ' + QuotedStr(Due) + ');' + sLineBreak +
      '  trigger(el, "input");' + sLineBreak +
      '  trigger(el, "keyup");' + sLineBreak +
      '  trigger(el, "change");' + sLineBreak +
      '' + sLineBreak +
      '  // 3) Alguns validadores/mascaras só “fixam” no blur' + sLineBreak +
      '  el.blur();' + sLineBreak +
      '  trigger(el, "blur");' + sLineBreak +
      '' + sLineBreak +
      '  // 4) Clique no botao (seletor mais robusto)' + sLineBreak +
      '  function findButton(){' + sLineBreak +
      '    // tenta por tipo submit dentro do mesmo form' + sLineBreak +
      '    var f = el.closest("form");' + sLineBreak +
      '    if(f){' + sLineBreak +
      '      var b = f.querySelector("button[type=submit], button.p-button, p-button button");' + sLineBreak +
      '      if(b) return b;' + sLineBreak +
      '    }' + sLineBreak +
      '    // fallback: procurar botao por texto (ajuste o texto se necessário)' + sLineBreak +
      '    var buttons = Array.from(document.querySelectorAll("button"));' + sLineBreak +
      '    return buttons.find(x => (x.innerText||"").trim().toLowerCase() === "consultar"' + sLineBreak +
      '                       || (x.innerText||"").trim().toLowerCase() === "pesquisar");' + sLineBreak +
      '  }' + sLineBreak +
      '' + sLineBreak +
      '  // dá um pequeno tempo pro framework reagir e então clica' + sLineBreak +
      '  setTimeout(function(){' + sLineBreak +
      '    var btn = findButton();' + sLineBreak +
      '    if(!btn){ console.warn("botao nao encontrado"); return; }' + sLineBreak +
...
begin
  ...
  begin
    ...
    begin
      {está na pagina que contem o botão importador exportador, mas o captcha nao esta aparecendo
       clicar nos botões para mostrar o captcha
      }
      if   (Valor <> 'null')  // grp_impexp = botão importador exportador
    ...
    if Chave.Equals('PgPortal') then
    begin
      if (Valor <> 'null') then
        Chromium1.LoadURL('https://portalunico.siscomex.gov.br/due/#/consultadue')
    end
    else
    if Chave.Equals('PGConsDUE') then
    begin
      if (Valor <> 'null') then
      begin
        Esperar;
    ...
    if Chave.Equals('PgGrdDue') then
    begin
      if (Valor <> 'null') and (Valor <> 'undefined') then
      begin
        executar(
          '(function(){' + sLineBreak +
          '  var a = document.querySelector(''a[data-test="DU-E"]'');' + sLineBreak +
          '  if(!a){ console.log("PgGrdDue|notfound"); return; }' + sLineBreak +
          '  a.scrollIntoView({block:"center"});' + sLineBreak +
          '  a.click();' + sLineBreak +
          '  console.log("PgGrdDue|clicked|" + (a.getAttribute("href")||"") + "|" + (a.innerText||"").trim());' + sLineBreak +
          '})();'
        );
    ...
    if Chave.Equals('BtnExtrato') then
    begin
      if Valor = '1' then
      begin
        executar(
          '(function(){' + sLineBreak +
          '  var btn = document.querySelector(''p-button[label="Gerar PDF Simplificado"] button'');' + sLineBreak +
          '  if(!btn){ console.warn("Botao Gerar PDF Simplificado nao encontrado"); return; }' + sLineBreak +
          '  btn.scrollIntoView({block:"center"});' + sLineBreak +
          '  btn.click();' + sLineBreak +
          '})();'
        );
...
begin
  ...
  begin
    dmdBroker.GravaArquivo(IncludeTrailingPathDelimiter(GetLocalPath + 'download'),
                           ...
                           Copy(ExtractFileName(downloadItem.FullPath), 1, 16), 'Extrato Siscomex');

    dmdBroker.cdsStatus.Next;
    if not dmdBroker.cdsStatus.Eof then
    begin
      sleep(2000);
      log(InttoStr(dmdBroker.cdsStatus.RecNo) +'/' + dmdBroker.cdsStatusQT_TOTAL.AsString, logar);
      log('Processo : ' + dmdBroker.cdsStatusNR_PROCESSO.AsString + ' DUE : ' + dmdBroker.cdsStatusCD_DUE.AsString, logar);
...
begin
    ...
    Chromium1.Browser.MainFrame.ExecuteJavaScript(
      ' var vBtnExtrato = document.querySelector(' +
      '   ''p-button[label="Gerar PDF Simplificado"] button''' +
      ' ); ' + sLineBreak +
      ' console.log("BtnExtrato|" + (vBtnExtrato ? "1" : "0")); ',
      'about:blank',
      0
    );
end;
...
begin
  Chromium1.Browser.MainFrame.ExecuteJavaScript(
    '(function(){' + sLineBreak +
    '  var a = document.querySelector(''a[data-test="DU-E"]'');' + sLineBreak +
    '  console.log("PgGrdDue|" + (a ? "1" : "null"));' + sLineBreak +
    '})();',
    'about:blank',
    0
  );
